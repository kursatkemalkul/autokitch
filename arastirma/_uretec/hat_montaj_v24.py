...
ÇIKTI: otonom/hat3d/hat_v24.glb + .usdz  ·  otonom/hat3d/durum.json (sayfa oradan okur)  ·  FULL_MAKINE/HAT_v24_YERLESIM.step (SolidWorks)
...
def TC_AG(wp, kaba=False):
    ...
    if kaba:
        ...
        return _K.ag(cq.Workplane(obj=sh), 1.2, 1.0)
...
if __name__ == "__main__":
    ...
    b1 = glb_yaz(os.path.join(OUT, "hat_v24.glb"), parcalar, dokular)
    print("hat_v24.glb · %d birim · %.0f KB" % (len(parcalar), b1 / 1024.0))
    b2, prim, sorun, _u = usdz_yaz([os.path.join(OUT, "hat_v24.usdz")], "hat_v24", parcalar, dokular)
    print("hat_v24.usdz · %.0f KB · %d prim · USD denetimi: %s" % (b2 / 1024.0, prim, "GECTI" if not sorun else "KALDI"))
    for x in sorun: print("   HATA:", x)
    os.makedirs(STEP, exist_ok=True); asm.save(os.path.join(STEP, "HAT_v24_YERLESIM.step"))
    print("HAT_v24_YERLESIM.step · SolidWorks'te montaj olarak acilir · %s" % STEP)
