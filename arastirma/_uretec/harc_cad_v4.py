...
URETIM = os.path.join(KOK, "arastirma", "3_TOPPING", "harc_kaseti_v4")
...
BORU_D, BORU_ET, BORU_ALT, BORU_GECIS = 16.0, 3.0, -8.0, 10.0      # yuvarlak çıkış borusu: iç çap · et · alt uç · tüpten boruya konik geçiş boyu
...
if __name__ == "__main__":
    ...
    b1 = glb_yaz(os.path.join(OUT, "harc_v4.glb"), kap_ag, dokular)
    print("harc_v4.glb · %d parca · %d ucgen · %.0f KB" % (len(kap_ag), sum(len(m.I) // 3 for _, m, _, _ in kap_ag), b1 / 1024.0))
    b2, prim, sorun, uyari = usdz_yaz([os.path.join(OUT, "harc_v4.usdz")], "harc_v4", [(a, m, mal) for a, m, mal, _ in kap_ag], dokular)
    ...
    b3 = glb_yaz(os.path.join(OUT, "harc_v4_dozaj.glb"), doz, dokular)
    print("harc_v4_dozaj.glb · %d parca · %.0f KB · toplam %.0f sn" % (len(doz), b3 / 1024.0, time.time() - t0))
