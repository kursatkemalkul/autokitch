# -*- coding: utf-8 -*-
"""AUTOKITCH · KAŞAR KABI v8 — ÜRETİM MODELİ (21 Eyl 2026)
v7'den FARK: KANAT ARTIK SÜREKLİ DEĞİŞEN HATVELİ. Kemal: "onu modelle, önemli olan o parça." DÜZELTME: v5'te de hatve basamaklıydı
  (22/26/30 üç bölge) — "v5'te sürekliydi" demem yanlıştı. v8'de gerçekten sürekli: hatve z ile DOĞRUSAL 22,00 → 28,99 mm.
  Kanat üçgen ağ değil, TAM YÜZEY: dört kenar eğrisi (iç/dış × üst/alt) BSpline, aralarını BRepFill ruled yüzeyler dolduruyor,
  dikilip katıya çevriliyor → 6 yüzlü tek katı. Süpürme kullanılmadığı için OpenCascade'in kanat düşürme/çökertme hatası da yok.
  Toplam dönüş TAM 12,0000 tur seçildi (hatve sonu bunun için çözüldü): bölme yerleri çeyrek turun katına denk gelsin, parçalar
  kare çubuğa tek doğru açıda otursun. Bölme z'leri de 90°'nin katından seçildi. Tek parça hâli ayrıca export ediliyor.
 · CadQuery (OpenCascade) ile GERÇEK KATILAR
...
ÇIKTI: otonom/kaset3d/kasar_v8.glb/.usdz + kasar_v8_dozaj.glb  ·  arastirma/3_TOPPING/kasar_kabi_v8/{step,stl}/ + MONTAJ.step + BOM.csv
"""
...
URETIM = os.path.join(KOK, "arastirma", "3_TOPPING", "kasar_kabi_v8")
...
if __name__ == "__main__":
 ...
 ("orumcek_arka", "gobek_karistirici", "örümcek ↔ göbek flanşı (1,0)"), ("orumcek_on", "on_kovan", "örümcek ↔ kovan flanşı (1,0)"),
    ...
    asm = cq.Assembly(name="KASAR_KABI_v8"); RENK = dict(cam=(0.75, 0.85, 0.92, 0.45), pom=(0.95, 0.95, 0.92, 1), celik=(0.75, 0.77, 0.8, 1), silikon=(0.16, 0.5, 0.95, 1), koyu=(0.15, 0.15, 0.17, 1))
    bom = []
    for p in PARCALAR:
    ...
    asm.save(os.path.join(URETIM, "KASAR_KABI_v8_MONTAJ.step"))
    ...
    b1 = glb_yaz(os.path.join(OUT, "kasar_v8.glb"), kap_ag, dokular)
    print("kasar_v8.glb · %d parca · %d ucgen · %.0f KB" % (len(kap_ag), sum(len(m.I) // 3 for _, m, _, _ in kap_ag), b1 / 1024.0))
    b2, prim, sorun, uyari = usdz_yaz([os.path.join(OUT, "kasar_v8.usdz")], "kasar_v8", [(a, m, mal) for a, m, mal, _ in kap_ag], dokular)
    ...
    b3 = glb_yaz(os.path.join(OUT, "kasar_v8_dozaj.glb"), doz, dokular)
    print("kasar_v8_dozaj.glb · %d parca · %.0f KB · toplam %.0f sn" % (len(doz), b3 / 1024.0, time.time() - t0))
