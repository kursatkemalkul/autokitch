# -*- coding: utf-8 -*-
"""AKTARMA İTİCİSİ v1 · itici_cad_v1 · 26 Eyl 2026 gece (Kemal: "itici tasarımını yap")
...
MAKARA = dict(s=-7.5, y=1214.0, r=4.0, w=37.0)   # POM makara Ø8 × 8: link'in −s yüzünde, +w'de (w = wa+37 = −5: kızak gövdesinin/tablanın dışında); altı 1210
...
def kur(s_araba=S_HOME, kalkik=True):
    ...
    ekle("pim_braketi", yerel(kut(PIM_KALDIRMA_S - 12.0, PIM_KALDIRMA_S + 12.0, TAVAN - 6.0, TAVAN, wa + MY["NW"] / 2.0, wa + MAKARA["w"] + 10.0)), "paslanmaz", "C_ITICI",
         bom=("Kaldırma pimi braketi 6 mm", 1, "304 · 24 × %.0f" % (MAKARA["w"] + 10.0 - MY["NW"] / 2.0 + 0.5), "kirişin +w kenarına kaynak · pim M8 dişle · ev ucunda (her iki borudan uzak)"))
    ...
    ekle("pivot_braketi", yerel(kut(sa - 30.0, sa + 30.0, Y_TABLA_YUZ - 3.0, Y_TABLA_YUZ, wa - 15.0, wa + 19.0)
    ...
    ekle("pivot_pimi", yerel(silz(sa, PIVOT_Y, 2.0, wa - 15.0, wa + 20.0)), "celik", "C_ITICI", "ARABA", kaynak="K",
         bom=("Pim ISO 8734 4 m6 × 35 A1", 1, "sertleştirilmiş paslanmaz", "kulaklara sıkı, link POM burçta döner"))
    ekle("dayama_pimi", yerel(silz(sa + STOP_S, PIVOT_Y - 4.0, 1.5, wa - 13.0, wa + 19.0)), "celik", "C_ITICI", "ARABA", kaynak="K",
    ...
    link = kut(sa - 3.0, sa, BAR_Y1, PIVOT_Y + 4.0, wa - 10.0, wa + 16.0)                                    # ana plaka 26 geniş (kulakların arasında)
    ...
    mak_kulak = kut(sa - 12.0, sa - 3.0, MAKARA["y"] - 6.0, MAKARA["y"] + 4.0, mw - 7.0, mw - 4.0)         # makara kulağı 3 mm (−s yüzünden çıkar)
    kol = link.union(bar).union(mak_kulak)
    burc = silz(sa - 1.5, PIVOT_Y, 2.75, wa - 10.0, wa + 16.0).cut(silz(sa - 1.5, PIVOT_Y, 2.05, wa - 11.0, wa + 17.0))
    mak = silz(sa + MAKARA["s"], MAKARA["y"], MAKARA["r"], mw - 4.0, mw + 4.0)
    mak_pim = silz(sa + MAKARA["s"], MAKARA["y"], 1.5, mw - 8.0, mw + 4.0)
