...
ÇIKTI: otonom/hat3d/hat_v39.glb + .usdz  ·  otonom/hat3d/durum.json (sayfa oradan okur)  ·  (v38: SolidWorks STEP'i YAZILMAZ — Kemal 25 Eyl)
...
import store_cad_v3 as SC                                  # v39: 3 elemanlı ray
...
if __name__ == "__main__":
    ...
    print("B CEKMECE MODULU (store_cad_v3 · 3 elemanli ray): %d birim · %d parca · %d cekmece" % (len(_bs), sum(b.get("parca", 0) for b in _bs), len(SC.CEK)))
    ...
    b1 = glb_yaz(os.path.join(OUT, "hat_v39.glb"), parcalar, dokular)
    print("hat_v39.glb · %d birim · %.0f KB" % (len(parcalar), b1 / 1024.0))
    b2, prim, sorun, _u = usdz_yaz([os.path.join(OUT, "hat_v39.usdz")], "hat_v39", parcalar + E_USDZ, dokular)
    print("hat_v39.usdz · %.0f KB · %d prim · USD denetimi: %s" % (b2 / 1024.0, prim, "GECTI" if not sorun else "KALDI"))
    ...
    with io.open(os.path.join(OUT, "durum.json"), "w", encoding="utf-8") as f:
        json.dump(dict(hat=dict(w=HAT_W, h=H_MAK, d=DZ, pafta="HAT_ATOSA_TABLALI_v7 (+ E 830: pafta v8 bekliyor) · v39 · E = kutu_cad_v2 · B = store_cad_v3 (3 elemanli ray) · taban hizasi · surec 1168"), sayac=sayac, modul=[dict(kod=k, ad=a, x=[x, x + w], y=list(MODUL_Y[k])) for k, a, x, w in MODUL],
