# -*- coding: utf-8 -*-
"""AUTOKITCH - BASE HAT (ROBOT KOLLU) TEKNIK RESIM v9 · TAM 3 GUN (14 Eyl 2026): ON + UST (PLAN KESITI) + YAN (STORE KESITI). Olculer mm.
Kemal (hat secenekleri karsilastirmasi): "tablali 1 gunluk disindaki 3 secenekte hamur, icecek vs ayni olmali".
v8'den FARKI: 14 Eyl kurali (her sey TAM 3 gun) + cekmece v2 (conta alin 33: pide ic 75 -> adim 108 · lahmacun ic 60, 35'lik -> 93 · icecek 132 -> 165).
  pide 240 = 12 cekmece (hepsi +3, donmus bolme yok - MODULER v1/v7 ile ayni) · lahmacun 600 = 18 cekmece (630 yer) · icecek 210 = 4 cekmece (224 yer)
  kaset 12,5 L, gunluk set 6 (harc 2 · kiyma · kusbasi · kasar · sucuk) -> 3 gun = 18: TOPPING 14 yuva (dozaj 6 + depo 8) + STORE yedek kaset rafi 4
  vakum paket cekmeceleri kalkti (kasar/sucuk da 3 gun kasette).
  STORE v7: 3 kolon (K1 12 pide + 2 lahm 1482 · K2 16 lahm 1488 · K3 4 icecek + raf 960) -> 2055; HAT 5540 -> 4855.
  PACK sarjor 506 = 1,8 gun (3 gun 840 blank; karar Kemal'de).
Kural: paftada yalniz gorunus + olcu + parca adi; aciklama mesajda.
"""
import os, math
from PIL import Image, ImageDraw, ImageFont

OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\HAT_BASE_v13_2gun_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 4900, 2800
S = 0.62
...
XI, WO, BOLME = 62.5, 620.0, (35.0, 35.0)
CELL0, CELL1, BIND, FUGA, TEK0 = 182.5, 1660.0, 15.0, 3.0, 1720.0
YUZ0 = CELL0 - BIND
HH = {"hamur": 75.0, "lahm": 60.0, "icecek": 132.0, "tatli": 132.0, "raf": 267.0}
AD = {"hamur": "TAZE PİDE", "lahm": "LAHMACUN", "icecek": "İÇECEK 330 ml", "tatli": "TATLI", "raf": "TEMİZ KASET RAFI"}
CAP = {"hamur": (20, "top"), "lahm": (35, "top"), "icecek": (56, "kutu"), "tatli": (14, "adet"), "raf": (4, "kaset")}
KOLON = [[(8, "hamur", "+3"), (6, "lahm", "+3")],
         [(6, "lahm", "+3"), (3, "icecek", "+3"), (1, "tatli", "+3")],
         [(2, "raf", "+3")]]
KOLON_AD = ("K1 · +3", "K2 · +3", "K3 · +3")
...
ISTASYON = [("STORE v8", WS, DZ), ("PRESS v6", 700.0, DZ), ("TOPPING v7", 700.0, DZ),
            ("OVEN v6", 700.0, DZ), ("PACK v4", 700.0, DZ), ("PICKUP v1", 860.0, DZ)]
...
CEPHE = {
    ...
    "TOPPING v7": [("k", 33.0, 667.0, 210.0, 489.0, "", "alt2", None),
    ...
    "OVEN v6": [("k", 33.0, 667.0, 123.0, 370.0, "YAĞ KABI · PANO", None, None),
                ...
                ("k", 33.0, 667.0, 1382.0, 1657.0, "FIRIN HAZNESİ 3|PİDE · 240 s · ayrı sıcaklık", None, None),
    ...
    "PICKUP v1": [("k", 30.0, 420.0, 600.0, 790.0, "", None, None), ("k", 440.0, 830.0, 600.0, 790.0, "", None, None), ("k", 30.0, 420.0, 800.0, 990.0, "", None, None), ("k", 440.0, 830.0, 800.0, 990.0, "", None, None), ("k", 30.0, 420.0, 1000.0, 1190.0, "", None, None), ("k", 440.0, 830.0, 1000.0, 1190.0, "", None, None), ("k", 30.0, 420.0, 1200.0, 1390.0, "", None, None), ("k", 440.0, 830.0, 1200.0, 1390.0, "", None, None), ("k", 30.0, 420.0, 1400.0, 1590.0, "", None, None), ("k", 440.0, 830.0, 1400.0, 1590.0, "", None, None), ("k", 30.0, 420.0, 1600.0, 1790.0, "QR GÖZÜ", None, None), ("k", 440.0, 830.0, 1600.0, 1790.0, "QR GÖZÜ", None, None)],
}
YUVA = {"kat1": ("KAŞAR KABI", "SUCUK DİLİMLEYİCİ"), "kat2": ("HARÇ", "HARÇ"), "kat3": ("HARÇ", "HARÇ"),
        "alt1": ("KIYMA", "KUŞBAŞI", None, None), "alt2": (None, None, None, None)}
...
# ======================= BASLIK =======================
txt(OX, 70, "AUTOKITCH  ·  BASE HAT  ·  TEKNİK RESİM  v13  ·  2 GÜNLÜK STOK  ·  YER RAYI + TEK ROBOT", f38, INK)
txt(OX, 138, "ön · üst · yan görünüş  ·  günde 80 pide + 200 lahmacun  ·  2 gün stok  ·  3 gözlü fırın  ·  tüm istasyonlar 830 derin  ·  ölçüler mm  ·  16 Eylül 2026", f13, GRAY)
...
def istasyon_on(x0mm, ad, w=700.0):
    kabin_on(x0mm, w, ad)
    ...
    for ki, gruplar in enumerate(KOLON):
        ...
        for g in gruplar:
            ...
            for _ in range(adet):
                ...
                if tip == "raf":
                    for kk in range(4):
                        drect(fx(cx + 25 + kk * 150), fy(y + 255), fx(cx + 165 + kk * 150), fy(y + 15), INK, 1)
                y += h + FUGA
            cap, br = CAP[tip]
...
# --- YER RAYI + ROBOT (koridorda)
RY0, RY1 = 250.0, HAT - 250.0
RB1, RB2 = 1400.0, 4300.0
d.rectangle([fx(RY0), py(40.0) + 52, fx(RY1), py(40.0) + 104], fill=ACC, outline=INK, width=2)
txt(fx(HAT / 2), py(40.0) + 150, "YER RAYI  ·  strok %s  ·  robot koridoru 800" % sayi(RY1 - RY0), f9, ACC, "mm")
d.ellipse([fx(RB1) - 26, py(40.0) + 52, fx(RB1) + 26, py(40.0) + 104], fill=BG, outline=INK, width=3)
txt(fx(RB1), py(40.0) + 150, "ROBOT", f9, ACC, "mm")
d.ellipse([fx(RB2) - 24, py(40.0) + 56, fx(RB2) + 24, py(40.0) + 100], fill=BG, outline=BOSL, width=2)
txt(fx(RB2), py(40.0) + 150, "2. ROBOT YERİ", f9, GRAY, "mm")
txt(fx(HAT / 2), py(40.0) + 196, "ÖN  ·  robot tarafı", f9, GRAY, "mm")
...
y0 = CELL0
for i in range(16):
    d.rectangle([sz(-680.0), fy(y0 + 60.0), sz(0.0), fy(y0)], fill=BG, outline=DOLAP, width=1)
    d.rectangle([sz(0.0), fy(y0 + 60.0 + BIND), sz(40.0), fy(y0 - BIND)], fill=BG, outline=DOLAP, width=1)
    y0 += 60.0 + 2 * BIND + FUGA
txt(sz(-340), fy(915.0), "ÇEKMECE 680", f9, INK, "mm")
txt(sz(-340), fy(915.0) + 26, "LAHMACUN × 16", f7, DOLAP, "mm")
...
# --- koridor + yer rayi + robot (yan gorunus)
d.line([(sz(40), fy(0)), (sz(900.0), fy(0))], fill=INK, width=3)
d.rectangle([sz(330.0), fy(150.0), sz(650.0), fy(0)], fill=ACC, outline=INK, width=2)
txt(sz(490.0), fy(75.0), "RAY", f7, BG, "mm")
d.rectangle([sz(300.0), fy(360.0), sz(680.0), fy(150.0)], fill=SOFT, outline=INK, width=2)
txt(sz(490.0), fy(255.0), "ARABA", f7, GRAY, "mm")
d.rectangle([sz(410.0), fy(1340.0), sz(570.0), fy(360.0)], fill=SOFT, outline=INK, width=2)
txt(sz(700.0), fy(860.0), "KAİDE 1040", f7, GRAY, "lm")
d.ellipse([sz(490.0) - 18, fy(1400.0) - 18, sz(490.0) + 18, fy(1400.0) + 18], fill=ACC, outline=INK, width=2)
d.line([(sz(490.0), fy(1400.0)), (sz(240.0), fy(1730.0))], fill=ACC, width=8)
d.line([(sz(240.0), fy(1730.0)), (sz(-60.0), fy(1280.0))], fill=ACC, width=8)
txt(sz(490.0), fy(1400.0) - 34, "ROBOT · FR5", f8, ACC, "mm")
olcu_h(sz(40), sz(840.0), fy(0) + 46, "KORİDOR 800", f8, INK)
olcu_v(sz(900.0) + 30, fy(1400.0), fy(0), "OMUZ 1400", f8, INK, "r")
...
ly = H_PX - 78
LEJ = [(LINE, BG, "kapak / panel", False), (RED, AGZ, "açık ağız", False),
       (DOLAP, BG, "çekmece +3 °C", False),
...
cek = sum(g[0] for K in KOLON for g in K if g != "ayirici" and g[1] != "raf")
txt(W_PX - 170, ly, "HAT %s × 1970 × 830  ·  2 gün  ·  STORE %d çekmece (8 pide · 12 lahmacun · 3 içecek · 1 tatlı)  ·  6 kaset + kaşar kabı + sucuk dilimleyici  ·  FIRIN 3 göz  ·  QR 12 göz" % (sayi(HAT), cek), f9, GRAY, "rm")
