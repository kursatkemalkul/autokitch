# -*- coding: utf-8 -*-
"""KAPAK YOK (Kemal, 22 Eyl: "kapak olmasın, TOPPING'de de yok") → gövde ağzında İÇE 8 mm bükme dudak · ROTOR KAYNAKLI (lama-kol kaynak yakaları) · ön kovanda SAVURMA DİSKİ. Üreteç: yap_kapaksiz_v1.py · önceki sürüm: kiyma_cad_v4.py
...
KLİKLİ bayonet (tırnak + tümsek + cep + O-ring yayı) · gövde contası · muylu O-ringleri · tüp O-ringi + M4 insert. Önceki sürüm: kiyma_cad_v3.py
AUTOKITCH · KIYMA KASETİ v3 — ÜRETİM MODELİ (22 Eyl 2026) · ORTAK GÖVDE · kasar_cad_v8 mimarisinden türetildi (yap_kiyma_v6.py)
...
ÇIKTI: otonom/kaset3d/kiyma_v6.glb/.usdz + kiyma_v6_dozaj.glb  ·  arastirma/3_TOPPING/kiyma_kaseti_v6/{step,stl}/ + MONTAJ.step + BOM.csv
SURUM: kare agiz YUVARLAK BORU oldu (ic D 32, kopruleme kurali D >= 3 x en buyuk parca) ve
...
URETIM = os.path.join(KOK, "arastirma", "3_TOPPING", "kiyma_kaseti_v6")
...
R_MIL, R_KANAT, KARE = 9.0, 34.0, 8.0                                # helezon mili Ø16 · kanat Ø40 · kare çekirdek 8 × 8
KOK_Z0, KOK_Z1, R_KOK = -91.0, -151.0, 15.0                          # v7: ARKADA 60 mm KONİK MİL KÖKÜ Ø16 → Ø26 (v5'te vardı, v6'da düşmüştü)
MUYLU, MUYLU_D = 22.0, 22.4                                          # göbek muylusu / plaka deliği
KAN_Z0, KAN_Z1, HATVE0, TUR = -148.0, 156.0, 36.0, 7.25             # kanat z aralığı · baştaki hatve · toplam tur (ÇEYREK turun katı olmalı:
#   47 çeyrek tur. Böylece hatve sonu ≈30 çıkıyor, yani v7'deki son hatve — dozaj sayıları (7,9 g/tur, 42/35 dev/dk) değişmiyor.)
UC_HATVE, UC_Z0, UC_Z1 = None, 156.0, 228.0      # tüp içi: ana hatvenin sonu sabit sürer, ağzın sonuna kadar                          # tüp içi çift ağızlı bölüm
AG_Z0, AG_Z1, AG_X = 198.0, 228.0, 15.0
Y_AGIZ = 8.0                                                         # meme dudağı kotu (ayak düzlemi 0 — tapayla birlikte masada durabilsin)                              # alt ağız 36 × 38
TUP_Z1 = 246.5
BORU_D, BORU_ET, BORU_ALT = 32.0, 3.0, -100.0      # yuvarlak çıkış borusu: iç çap · et · alt uç (kaset tabanının altında)
...
def kap():
    ...
    ZC1, ZC2 = bolme_z(10), bolme_z(20)      # bölme yerleri: θ = 19 ve 35 çeyrek tur → parçalar kare çubuğa doğru açıda oturur
    SEG = [("helezon_A", -151.0, ZC1, "hatve %.2f → %.2f · arkada 60 mm KONİK KÖK Ø30 → Ø18" % (hatve(KAN_Z0), hatve(ZC1))),
           ("helezon_B", ZC1, ZC2, "hatve %.2f → %.2f" % (hatve(ZC1), hatve(ZC2))),
           ("helezon_C", ZC2, UC_Z0, "hatve %.2f → %.2f" % (hatve(ZC2), hatve(KAN_Z1))),
           ("helezon_D", UC_Z0, 253.0, "tüp içi TEK ağız sabit hatve, ağzın üstüne kadar · pim YOK (eti sıvar) · ön muylu Ø12")]
    for k_, (ad, zA, zB, acik) in enumerate(SEG):
        ...
        ekle(ad, tamam, "pom", G, bom=("Helezon segmenti " + ad[-1], 1, "DENEME: PETG / PA12 baskı · ÜRETİM: POM-C talaşlı, Ra ≤ 0,8 (çiğ et teması)", acik + " · boy %.0f mm · kare delik 8,3" % (zB - zA)))
    ...
    ekle("cikis_tupu", tup, "cam", bom=("Çıkış tüpü + meme", 1, "PC / PETG şeffaf · baskı ya da torna + freze", "iç Ø72 · ön plakaya faturalı + 2 × M4 · ilk 35 mm KAPALI (macun tıkacı) · alt meme 30 × 30 · 3 bayonet TIRNAĞI 8 × 4 × 3,2 (tüple tek parça) · plakaya geçme 1,3 + O-ring"))
    ...
    for ad, z0, z1 in (("arka", -150.5, -133.0), ("orta", -7.0, 7.0), ("on", 133.0, 150.5)):
        ekle("orumcek_" + ad, (orumcek(z0, z1).union(silz(0, YC, 14.0, z1, 153.2).cut(karez(0, YC, KARE + 0.3, z1 - 1, 154.0))) if ad == "on" else orumcek(z0, z1)), "pom", Kg, bom=("Rotor göbeği · 2 kollu", 3, "DENEME: PETG / PA12 baskı (yakalarla tek parça çıkar) · ÜRETİM: 304, lamalarla KAYNAKLI", "2 kol: lamaları r 57'de 20° eğik tutar · her birleşimde 3 mm KAYNAK YAKASI · kare delik 8,3") if ad == "arka" else None)
...
GRUP = {"helezon": dict(pivot=(0, CY * MM, 0), eksen="z", aci=lambda t: -2.4 * min(t, T_DOK) / T_DOK),
...
def glb_yaz(yol, parcalar, dokular):
    ...
    while off[0] % 4: blob.append(b"\x00"); off[0] += 1
    bb = b"".join(blob)
    g = {"asset": {"version": "2.0", "generator": "AUTOKITCH kiyma_cad_v3"}, "scene": 0, "scenes": [{"nodes": kok}], "nodes": nodes, "meshes": meshes, "materials": mats, "accessors": accs,
...
if __name__ == "__main__":
 ...
 ("orumcek_arka", "gobek_karistirici", "örümcek ↔ göbek flanşı (1,0)"), ("orumcek_on", "on_kovan", "örümcek ↔ kovan flanşı (1,0)"),
    ...
    asm = cq.Assembly(name="KIYMA_KASETI_v5"); RENK = dict(cam=(0.75, 0.85, 0.92, 0.45), pom=(0.95, 0.95, 0.92, 1), celik=(0.75, 0.77, 0.8, 1), silikon=(0.16, 0.5, 0.95, 1), koyu=(0.15, 0.15, 0.17, 1))
    bom = []
    for p in PARCALAR:
    ...
    asm.save(os.path.join(URETIM, "KIYMA_KASETI_v5_MONTAJ.step"))
    ...
    b1 = glb_yaz(os.path.join(OUT, "kiyma_v6.glb"), kap_ag, dokular)
    print("kiyma_v6.glb · %d parca · %d ucgen · %.0f KB" % (len(kap_ag), sum(len(m.I) // 3 for _, m, _, _ in kap_ag), b1 / 1024.0))
    b2, prim, sorun, uyari = usdz_yaz([os.path.join(OUT, "kiyma_v6.usdz")], "kiyma_v6", [(a, m, mal) for a, m, mal, _ in kap_ag], dokular)
    ...
    b3 = glb_yaz(os.path.join(OUT, "kiyma_v6_dozaj.glb"), doz, dokular)
    print("kiyma_v6_dozaj.glb · %d parca · %.0f KB · toplam %.0f sn" % (len(doz), b3 / 1024.0, time.time() - t0))
