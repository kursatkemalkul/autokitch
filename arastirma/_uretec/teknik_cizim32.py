# -*- coding: utf-8 -*-
# v32 — KARSILIKLI YERLESIM PLANI (ustten): iki sira kabin, ortada robot koridoru
import io, math
...
tx(30,36,'v32 — KARŞILIKLI YERLEŞİM PLANI (üstten görünüş)',21,'#1a1a1a','bold')
tx(30,58,'İki sıra kabin karşı karşıya, robot TAM ORTADA — düz hattın (485+70) yerine kompakt blok. Kabinler ve iç düzen v31 ile AYNI, sadece diziliş değişti.',11.5,'#666')
...
xS0, xS1 = xQ1, xQ1+px(2100)   # soguk depo
xP1 = xS1+px(700)              # pres
# alt sira sagdan: DOZAJ 2650-3350, FIRIN 2000-2650, KESIM 1300-2000, (bosluk/servis 550-1250)
xD0 = xQ1+px(2100); xD1 = xQ1+px(2800)
xF0 = xQ1+px(1450); xF1 = xD0
xC0 = xQ1+px(750);  xC1 = xF0

# UST SIRA
rc(xS0,yU0,px(2100),px(840),2.4)
tx((xS0+xS1)/2,yU0+px(390),'1 · SOĞUK DEPO',14,'#1a1a1a','bold','middle')
tx((xS0+xS1)/2,yU0+px(390)+18,'210 (3 kapı — v31 iç düzeni aynı)',11,'#555','normal','middle')
...
# servis istasyonu (soluk, opsiyonel konum)
rc(xQ1,yA0,px(700),px(840),1.4,'#999',dash='6,4')
tx((xQ1+xC0)/2,yA0+px(380),'SERVİS İST.',11.5,'#999','bold','middle')
tx((xQ1+xC0)/2,yA0+px(380)+15,'70 — yeri esnek,',10,'#999','normal','middle')
tx((xQ1+xC0)/2,yA0+px(380)+29,'ayrı duvarda da olur',10,'#999','normal','middle')
...
ry = (yK0+yK1)/2
rx0, rx1 = xQ1+px(250), xQ1+px(2050)
ln(rx0,ry,rx1,ry,3.5,'#8a2be2')
ci((rx0+rx1)/2,ry,9,2,'#8a2be2','#ffffff')
tx((rx0+rx1)/2,ry-16,'ROBOT — kısa ray 180 (eski 300-400)',11.5,'#8a2be2','bold','middle')
for cx_ in (rx0,rx1):
...
tx(30,ny+6,'AYAK İZİ: ≈ 3,35 × 2,65 m (düz hat: 4,85 m + ayrı 0,70 duvar isterdi) — küçük dükkâna köşeye oturur',12.5,'#1a1a1a','bold')
tx(30,ny+26,'· Robot ortada, ray 300-400 → 180 cm: daha ucuz ray, daha kısa yol, tüm cepheler R130 erişim içinde',11.5,'#333')
tx(30,ny+44,'· Akış saat yönünde: depo → pres → dozaj → fırın → kesim → QR teslim (sol uç müşteriye bakar, sağ uç CAM izleme)',11.5,'#333')
tx(30,ny+62,'· Koridor 95 — robot süpürge girer; kabinler modüler, sırtlar duvara; SERVİS İST. istenirse blok içi sol boşluğa',11.5,'#333')
tx(W-30,H-14,'AUTOKITCH · hat_plan_karsilikli_v32',10.5,'#999','normal','end')
...
out = r"C:\Users\Kemal\Desktop\Kemal\WEBSİTE\AUTOKITCH\arastirma\hat_plan_karsilikli_v32.svg"
