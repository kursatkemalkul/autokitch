...
BEDELI: cekmece 700 -> 680 · icecek 63 -> 56 kutu.

v3 (Kemal, 12 Eyl): 1 LITRELIK ICECEK MENUDEN KALKTI.
  -> 3 cekmece bosaldi, STORE 4 kolon 2740 -> 3 KOLON 2085 mm, hat 5540 -> 4885 mm.
  32 cekmece: donmus pide 7 · taze pide 4 · lahmacun 15 · icecek 6.
  Oncelik hamurda: pide 3,14 gun · lahmacun 3,19 gun (kural 3 gun) — ikisi de tam.
  Icecek 336 kutu: 1 L musterisi kutuya kaymazsa 7,5 gun, kayarsa 5,9 gun.
...
OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\STORE_TOPPING_v4_ic_gorunum.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 3100, 3300
S = 0.80
...
CAP = {"donmus":20,"hamur":20,"lahm":30,"icecek":56}             # 680 derin cekmece izgarasi
BR  = {"donmus":"top","hamur":"top","lahm":"top","icecek":"kutu","1L":"sise"}
IHT = {"donmus":140,"hamur":70,"lahm":423,"icecek":315}   # 3 gun hamur · icecek 45 kutu/gun
GUN = {"donmus":"2 gun","hamur":"1 gun","lahm":"3 gun","icecek":"7 gun"}
...
KG = {"donmus":(17.8, 6.00), "hamur":(17.0, 6.00), "lahm":(17.0, 4.90), "icecek":(21.0, 19.88)}
ST = dict(kolon=[[(7,"donmus","-18"),"ayirici",(3,"lahm","+3")],
                 [(12,"lahm","+3")],
                 [(4,"hamur","+3"),(6,"icecek","+3")]],
          bolme=[65,35])
WS = 2*XI + len(ST["kolon"])*WO + sum(ST["bolme"])          # 2085
# --- TOPPING ---
...
def dline(p0, p1, c, w=1, dash=7, gap=4):
    (ax,ay),(bx,by) = p0,p1; L = math.hypot(bx-ax, by-ay)
    if L < 1: return
    n = int(L//(dash+gap))+1
    for i in range(n):
...
def dcirc(cxx,cyy,r,c,w=1,n=20):
    p=[(cxx+r*math.cos(2*math.pi*i/n), cyy+r*math.sin(2*math.pi*i/n)) for i in range(n+1)]
    for i in range(0,n,2): d.line([p[i],p[i+1]],fill=c,width=w)

# cekmece ici: (yatay adet, derinlik sirasi, urun capi mm, urun adi, birim)
ICERIK = {"donmus": (4, 5, 95.0, "DONMUS PIDE", "top"),
          "hamur":  (4, 5, 95.0, "TAZE PIDE",   "top"),
          "lahm":   (5, 6, 75.0, "LAHMACUN",    "top"),
          "icecek": (7, 8, 66.0, "ICECEK 330",  "kutu")}

def _zon_serit(cx, ybas, yson, zon):
    """kolonun SOLUNDAKI duvar/bolme bandinda ince bolge seridi (cekmecenin icine girmez)"""
    col = BUZ if zon == "-18" else DOLAP
    d.rectangle([cx-11, yson, cx-4, ybas], fill=col)

def icerik_ciz(ix0, iy0, ix1, iy1, tip, zon, no, adet):
    """cekmece on yuzunun ARKASINDAKI kutuyu ve ilk sira urunu KESIKLI ciz + etiket"""
    nx, nz, dcap, ad, br = ICERIK[tip]
    col = BUZ if zon=="-18" else DOLAP
    # kutu (on yuzun arkasinda, 16 mm icerde)
    drect(ix0+10, iy0+5, ix1-10, iy1-4, (170,170,178), 1)
    # ilk sira urun — yuksekligi asmayacak sekilde olceklenir
    r = min(dcap/2.0*S, (iy1-iy0)*0.34)
    pit = (ix1-ix0-24) / nx
    cyy = (iy0+iy1)/2.0
    for i in range(nx):
        dcirc(ix0+12+pit*(i+0.5), cyy, r, (150,150,158), 1)
    # etiket
    et = "%s  %d/%d  ·  %d %s  (%dx%d)" % (ad, no, adet, nx*nz, br, nx, nz)
    tw = d.textlength(et, font=f7)
    d.rectangle([ix0+14, cyy-10, ix0+14+tw+10, cyy+10], fill=(255,255,255), outline=col, width=1)
    txt(ix0+19, cyy, et, f7, INK, "lm")
...
def on_store(ox, oy):
    ...
    for ki, gruplar in enumerate(ST["kolon"]):
        ...
        for g in gruplar:
            ...
            for i in range(adet):
                yt, yb2 = oy-(y+h)*S, oy-y*S
                col = BUZ if zon=="-18" else DOLAP
                d.rectangle([cx+cv,yt,cx+cw-cv,yb2],fill=(255,255,255),outline=col,width=1)
                icerik_ciz(cx+cv, yt, cx+cw-cv, yb2, tip, zon, i+1, adet)
                y += h+FUGA_C
            zt = "-18 C" if zon=="-18" else "+3 C"
            _zon_serit(cx, oy-ybas*S, oy-(y-FUGA_C)*S, zon)
        bos = CELL1-(y-FUGA_C)
        if bos > 20:
        ...
        zonlar = sorted({g[2] for g in gruplar if g != "ayirici"}, reverse=True)
        zt2 = "alt -18 / ust +3" if len(zonlar) > 1 else ("-18 C" if zonlar[0]=="-18" else "+3 C")
        txt(cx+cw/2, oy-1660*S-13, "K%d  ·  %s  ·  %%%d dolu" % (ki+1, zt2, round(100*(y-FUGA_C-YUZ0)/(CELL1-YUZ0))), f8, GRAY, "md")
        cx += cw + (ST["bolme"][ki]*S if ki < len(ST["bolme"]) else 0)
    txt((x0+x1)/2,y1-46,"STORE v5  ·  %d x 830" % WS,f16,ACC,"md")
    olcu(x0,x1,y0+40,str(int(WS)),INK,f11)
...
def plan_store(ox, oy):
    ...
    for ki in range(len(ST["kolon"])):
        ...
        cx += WO + (ST["bolme"][ki] if ki < len(ST["bolme"]) else 0)
    
    olcu(x0-40,y0,y1,"830",INK,f11,dik=True)
    txt(*P(WS/2, 40+70),"STORE v5 · PLAN KESITI (+3 kolon) · DERINLIK 830 = PU 60 + PLENUM 47,5 (evap 45 · fan 40 · motor 36) + CEKMECE 680 + CEPHE 40",f11,ACC,"mm")
    return x1
...
sat = []; tb=tk=tg=0
for t in ("donmus","hamur","lahm","icecek"):
...
for i,(c,a) in enumerate(LEJ):
    xx=240+i*320; d.rectangle([xx,ly+8,xx+26,ly+26],fill=(c if c in (INK,PUC) else (255,255,255)),outline=c,width=2); txt(xx+34,ly+17,a,f9,INK,"lm")
txt(W_PX-240,ly+40,"kabin 1970 x 830 (Oztiryakiler std) · plint 120 · cekmece 620 x 680 · alin 33 · 3 kolon · 1 L YOK",f8,GRAY,"rm")
