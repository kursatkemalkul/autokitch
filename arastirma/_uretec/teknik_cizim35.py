...
tx(X0+px(T)/2,YT2+px(1815),"KAPALI PANEL — CAM YOK, mutfak müşteriden GİZLİ · tek açıklık: QR teslim dolabı + kiosk",10,"middle","","#555")
...
tx(sv+sw/2,YZ+42,"(ayrı duvarda — makineye bağlı değil)",9.5,"middle","","#555")
...
tx(X0,Y0-70,"AUTOKITCH — HAT v35 · 3 GÜNLÜK DÜZ HAT — ÖN + ÜST + YAN GÖRÜNÜŞ · KAPALI MUTFAK (cam yok)",15,"start","bold")
...
pnl=k1
rc(pnl,Y0,px(40),px(HG)+px(HA),1.8)                               # kapali panel tam boy
tx(pnl+px(80),Y0+px(480),"KAPALI PANEL",11,"start","bold")
tx(pnl+px(80),Y0+px(570),"(sac/alçıpan — CAM YOK,",9.5,"start","","#555")
tx(pnl+px(80),Y0+px(650),"mutfak müşteriden gizli)",9.5,"start","","#555")
tx(pnl+px(80),Y0+px(800),"müşteri tarafı →",9.5,"start","","#555")
oy(sv2,k0,YZ+56,"84"); oy(k0,k1,YZ+56,"95")
ox(sv2-40,Y0,YZ,"197")
tx(sv2+px(1265),YZ+92,"tek açıklık: QR TESLİM DOLABI (hat ucunda, panele gömülü) — gerisi tamamen kapalı",9.5,"middle","","#555")

W=int(X0+px(T)+px(3950)); H=int(YT2+px(1860))
svg=(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}">'
     f'<rect width="{W}" height="{H}" fill="#ffffff"/>'+''.join(E)+'</svg>')
OUT=r"C:\Users\Kemal\Desktop\Kemal\WEBSİTE\AUTOKITCH\arastirma\hat_teknik_3gun_v35.svg"
