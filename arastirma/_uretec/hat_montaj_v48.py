...
birim("TOPPING_MODUL", "TOPPING v2 (UNO'lu): 4 UNO çekirdeği (sos · harç · kıyma · kuşbaşı) + kaşar ve küp sucuk kaseti + hava tesisatı · tabla mekanizması v1'den", "C", "GERCEK_MODUL",
      (X_BC, X_BC + W_BC), (H_B, H_B + TC.Y), (-DZ, 0.0), "sac", "topping_uno_cad_v6.py + topping_cad_v24.py", "hat/topping_v2.html")
...
_sp = _ilu.spec_from_file_location("TU6", os.path.join(os.path.dirname(os.path.abspath(__file__)), "topping_uno_cad_v6.py"))   # v48: kaset yuvaları + yalıtım bloğu + fitil   # v45: kama yarık + dönen gruplar
...
if __name__ == "__main__":
    ...
    print("ANA MONTAJ ANIMASYONU (v48): tek tam dongu %.0f sn · %d kanal · %d adim · %d eksenli doner dugum" % (T_J, len(ANIM_HAT), len(ADIM_HAT), len(OZEL_T)))
    ...
    b1 = glb_yaz(os.path.join(OUT, "hat_v48.glb"), [x for x in parcalar if x[0] not in HARIC_T], dokular, ozel=OZEL + OZEL_T, liste=ANIM_HAT)   # v46: donenler kendi ekseninde
    ...
    print("hat_v48.glb · %d dugum · %.0f KB · animasyon %.0f sn" % (len(parcalar), b1 / 1024.0, T_J))
    _usd = [x for x in parcalar if not x[0].startswith("URUN__")]                         # ürün düğümleri yalnız animasyonda (USDZ durağan)
    b2, prim, sorun, _u = usdz_yaz([os.path.join(OUT, "hat_v48.usdz")], "hat_v48", _usd + E_USDZ, dokular)
    print("hat_v48.usdz · %.0f KB · %d prim · USD denetimi: %s" % (b2 / 1024.0, prim, "GECTI" if not sorun else "KALDI"))
