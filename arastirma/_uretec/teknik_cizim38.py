...
for (g0,g1) in ((a0,am),(am,a1)):
    ...
    for j in range(5):
        for i in range(4):
            ...
            ci(g0+px(162)+i*px(125),YT2+px(160)+j*px(125),px(60),.7)
...
not_((a0+a1)/2,YT2+px(890),"MOTORLU SERVİS KAPAĞI — BEYİN, robot yola çıkarken AÇAR (kol kapağa dokunmaz) · switch teyidi")
...
tx(X0+px(T)/2,YT2+px(1815),"KAPALI PANEL — CAM YOK, mutfak müşteriden GİZLİ · tek açıklık: kiosk + QR teslim dolabı",10,"middle","","#555")
...
tx(X0,Y0-70,"AUTOKITCH — HAT v38 · İST.1 KARAR: 2 KAPILI 140 — TAZE 2G + DONMUŞ 1G + İÇECEK HAFTALIK · fırıncı 3 günde bir",15,"start","bold")
...
tx(X0,Y0-50,"ölçüler cm · ALGORİTMİK YERLEŞİM: aynı aile aynı kolonda — SOL: donmuş 80 üst + TÜM içecek/tatlı (4×7 kanal + 1L altında) · SAĞ: taze 160 + boşluk TEK blok altta",10.5,"start")

W=int(X0+px(T)+px(1030)); H=int(YT2+px(1860))
svg=(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}">'
     f'<rect width="{W}" height="{H}" fill="#ffffff"/>'+''.join(E)+'</svg>')
OUT=r"C:\Users\Kemal\Desktop\Kemal\WEBSİTE\AUTOKITCH\arastirma\hat_on_gorunus_teknik_v38.svg"
