# -*- coding: utf-8 -*-
"""AUTOKITCH - SERVIS + TESLIM PAFTASI v3 (17 Eyl 2026). v2 Kemal: servis duvari mutfak gibi (alt dolap + tezgah + ust dolap, solda uzun dolap yok); stok 4 gun (hat 2 gun tasir). v1 Kemal: QR dolabinda icecek/sogutucu YOK, dolap kisa ve en kucuk;
...
OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\SERVIS_TESLIM_v3_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 5300, 3850
S = 0.62
...
SV_W = 1800.0                                     # tezgah 1800 x 800 x 850 · ust dolap 1800 x 350 x 700 (1400-2100)
...
# ======================= BASLIK =======================
txt(OX, 70, "AUTOKITCH  ·  SERVİS + TESLİM PAFTASI  v3  ·  QR DOLABI 860 × 520 × 1650  ·  SERVİS TEZGÂHI 1800 × 800 · ALT + ÜST DOLAP  ·  4 GÜNLÜK STOK", f38, INK)
txt(OX, 138, "QR / PIN ile açma  ·  bulaşık makinesi yok, evyede elde yıkama  ·  mutfak düzeni: alt dolap + tezgâh + üst dolap  ·  hat 2 gün taşır, burada 4 gün: karton 1.120 · içecek 280  ·  yangın · kamera · UPS  ·  ölçüler mm  ·  17 Eylül 2026", f13, GRAY)
...
# ======================= 5 · SERVIS DUVARI · on gorunus (mutfak duzeni) =======================
SX0 = KX0 + Q_D * S + 420.0
...
H_S = 2100.0
txt(SX0, FY_TOP - 190, "5 · SERVİS TEZGÂHI · ön görünüş", f16, ACC)
txt(SX0, FY_TOP - 152, "eleman tarafı · alt dolaplar + tezgâh + üst dolaplar · 4 günlük stok (hat 2 gün taşır) · elde yıkama · kaset doldurma", f9, GRAY)
# alt dolaplar 0-850 (plint 100), tezgah 850-890
d.rectangle([Xs(0), Y(850.0), Xs(SV_W), Y(0)], fill=FILL, outline=LINE, width=3)
d.rectangle([Xs(0), Y(100.0), Xs(SV_W), Y(0)], fill=SOFT, outline=LINE, width=2)
d.rectangle([Xs(-20), Y(890.0), Xs(SV_W + 20), Y(850.0)], fill=SOFT, outline=LINE, width=2)
for x0, x1 in ((20.0, 420.0), (430.0, 830.0), (840.0, 1240.0)):
    d.rectangle([Xs(x0), Y(830.0), Xs(x1), Y(110.0)], fill=BG, outline=LINE, width=2)
    d.rectangle([Xs(x1 - 40), Y(520.0), Xs(x1 - 28), Y(420.0)], fill=INK)
    kesik(Xs, x0 + 30.0, x1 - 60.0, 140.0, 800.0, "", INK)
    for i in range(7):
        d.line([(Xs(x0 + 40), Y(170.0 + i * 90.0)), (Xs(x1 - 70), Y(170.0 + i * 90.0))], fill=BOSL, width=1)
    txt(Xs((x0 + x1) / 2), Y(470.0) - 10, "KARTON BLANK", f7, INK, "mm")
    txt(Xs((x0 + x1) / 2), Y(470.0) + 10, "400 × 760 · yığın 650 = 360–430", f7, GRAY, "mm")
txt(Xs(630), Y(60.0), "3 yığın = 1.080–1.300 kutu → 4 gün (1.120) ✓ · blank yatay, 760 derine sığar", f7, GRAY, "mm")
d.rectangle([Xs(1250), Y(830.0), Xs(1780), Y(110.0)], fill=BG, outline=LINE, width=2)
d.rectangle([Xs(1262), Y(520.0), Xs(1274), Y(420.0)], fill=INK)
kesik(Xs, 1280.0, 1500.0, 140.0, 440.0, "UPS 3 kVA", INK, "440 × 600 × 250 yatık")
kesik(Xs, 1520.0, 1760.0, 140.0, 300.0, "NVR + modem", INK)
kesik(Xs, 1520.0, 1760.0, 320.0, 440.0, "sifon", GRAY)
kesik(Xs, 1280.0, 1760.0, 460.0, 800.0, "ÇÖP 60 L · evye altı", GRAY)
# tezgah ustu: doldurma yuzeyi + evye + el lavabosu
d.rectangle([Xs(40), Y(910.0), Xs(1200), Y(890.0)], fill=BG, outline=INK, width=1)
txt(Xs(620), Y(1000.0) - 8, "KASET DOLDURMA YÜZEYİ 1160 × 800", f8, INK, "mm"); txt(Xs(620), Y(1000.0) + 12, "kasap paketi açılır, kaset dolar → hatta takılır · tepsi/kaset kurutma", f7, GRAY, "mm")
d.rectangle([Xs(1250), Y(890.0), Xs(1650), Y(600.0)], fill=SU, outline=BUZ, width=2)
txt(Xs(1450), Y(745.0) - 8, "EVYE 400 × 400 × 250", f8, BUZ, "mm"); txt(Xs(1450), Y(745.0) + 12, "kaset · tepsi · kap · sıcak-soğuk", f7, GRAY, "mm")
d.rectangle([Xs(1850), Y(1150.0), Xs(2200), Y(900.0)], fill=SU, outline=BUZ, width=2)
txt(Xs(2025), Y(1025.0) - 8, "EL LAVABOSU", f8, BUZ, "mm"); txt(Xs(2025), Y(1025.0) + 12, "350 × 300 duvar tipi · ayrı", f7, GRAY, "mm")
d.rectangle([Xs(1900), Y(1400.0), Xs(2150), Y(1200.0)], fill=BG, outline=GRAY, width=1)
txt(Xs(2025), Y(1300.0) - 8, "sabun · kâğıt havlu", f7, GRAY, "mm"); txt(Xs(2025), Y(1300.0) + 8, "hijyen listesi", f7, GRAY, "mm")
# ust dolaplar 1400-2100, 350 derin
d.rectangle([Xs(0), Y(H_S), Xs(SV_W), Y(1400.0)], fill=FILL, outline=LINE, width=3)
for x0, x1, et, alt in ((20.0, 620.0, "İÇECEK 4 GÜN", "280 kutu = 12 shrink 24'lü · oda sıcaklığı"), (630.0, 930.0, "TEMİZLİK", "kimyasal · bez · fırça"),
                        (940.0, 1240.0, "SARF", "eldiven · poşet · sachet"), (1250.0, 1780.0, "YEDEK", "tepsi × 6 · pençe · bıçak · filtre")):
    d.rectangle([Xs(x0), Y(H_S - 20.0), Xs(x1), Y(1420.0)], fill=BG, outline=LINE, width=2)
    d.rectangle([Xs(x1 - 40), Y(1560.0), Xs(x1 - 28), Y(1460.0)], fill=INK)
    kesik(Xs, x0 + 30.0, x1 - 60.0, 1450.0, H_S - 50.0, et, INK, alt)
txt(Xs(SV_W / 2), Y(H_S) + 34, "ÜST DOLAP 1800 × 350 × 700 · kot 1400–2100", f8, GRAY, "mm")
# duvar: yangin + kamera + dedektor (tezgahin saginda, lavabo yaninda)
d.rectangle([Xs(2320), Y(1500.0), Xs(2440), Y(1100.0)], fill=RED, outline=INK, width=1)
txt(Xs(2380), Y(1560.0), "6 kg ABC", f7, RED, "mm")
d.rectangle([Xs(2480), Y(1500.0), Xs(2600), Y(1100.0)], fill=(140, 140, 150), outline=INK, width=1)
txt(Xs(2540), Y(1560.0), "5 kg CO₂", f7, INK, "mm")
txt(Xs(2460), Y(1050.0), "askı kotu 900", f7, GRAY, "mm")
d.ellipse([Xs(2380) - 14, Y(2300.0) - 14, Xs(2380) + 14, Y(2300.0) + 14], fill=BG, outline=INK, width=2)
txt(Xs(2380), Y(2360.0), "duman dedektörü", f7, INK, "mm")
d.rectangle([Xs(2500), Y(2320.0), Xs(2580), Y(2280.0)], fill=INK)
txt(Xs(2540), Y(2360.0), "kamera", f7, INK, "mm")
olcu_h(Xs(0), Xs(SV_W), Y(0) + 44, "SERVİS TEZGÂHI  %s" % sayi(SV_W), f13, INK)
olcu_h(Xs(1850), Xs(2200), Y(0) + 44, "el lavabosu 350", f7, GRAY)
olcu_v(Xs(0) - 44, Y(H_S), Y(0), sayi(H_S), f11, INK, "l")
for yy in (100.0, 850.0, 890.0, 1400.0, H_S):
    d.line([(Xs(SV_W) + 6, Y(yy)), (Xs(SV_W) + 24, Y(yy))], fill=INK, width=2)
    txt(Xs(SV_W) + 30, Y(yy), sayi(yy), f8, INK, "lm")

# ======================= 6 · SERVIS TEZGAHI · plan =======================
txt(SX0, PY_TOP - 150, "6 · SERVİS TEZGÂHI · plan", f16, ACC)
...
d.rectangle([Xs(0), Zs(0), Xs(SV_W), Zs(350)], fill=SOFT, outline=GRAY, width=1)
txt(Xs(SV_W / 2), Zs(175), "ÜST DOLAP 1800 × 350 (kesik: tezgâh üstünde)", f7, GRAY, "mm")
d.rectangle([Xs(-20), Zs(0), Xs(SV_W + 20), Zs(800)], fill=FILL, outline=LINE, width=3)
for x0, x1 in ((20.0, 420.0), (430.0, 830.0), (840.0, 1240.0)):
    drect(Xs(x0 + 20), Zs(20), Xs(x1 - 20), Zs(780), INK, 1)
    txt(Xs((x0 + x1) / 2), Zs(400) - 8, "BLANK", f7, INK, "mm"); txt(Xs((x0 + x1) / 2), Zs(400) + 8, "400 × 760", f7, GRAY, "mm")
txt(Xs(630), Zs(700), "KASET DOLDURMA YÜZEYİ (tezgâh üstü, 1160 × 800)", f7, INK, "mm")
d.rectangle([Xs(1250), Zs(200), Xs(1650), Zs(600)], fill=SU, outline=BUZ, width=2)
txt(Xs(1450), Zs(400), "EVYE 400 × 400", f7, BUZ, "mm")
drect(Xs(1280), Zs(20), Xs(1760), Zs(180), INK, 1)
txt(Xs(1520), Zs(100), "altta UPS · NVR", f7, GRAY, "mm")
d.rectangle([Xs(1850), Zs(0), Xs(2200), Zs(300)], fill=SU, outline=BUZ, width=2)
txt(Xs(2025), Zs(150), "EL LAVABOSU 350 × 300", f7, BUZ, "mm")
d.line([(Xs(-60), Zs(0)), (Xs(2700), Zs(0))], fill=INK, width=4)
txt(Xs(900), Zs(0) - 22, "DUVAR", f9, GRAY, "mm")
d.line([(Xs(-60), Zs(800 + 900)), (Xs(2700), Zs(800 + 900))], fill=GRAY, width=1)
txt(Xs(900), Zs(800 + 900) + 22, "eleman geçişi 900", f8, GRAY, "mm")
olcu_v(Xs(2700) + 60, Zs(0), Zs(800), "800", f8, INK, "r")
olcu_v(Xs(2700) + 60, Zs(800), Zs(1700), "900", f8, INK, "r")
olcu_h(Xs(0), Xs(SV_W), Zs(1700) + 60, sayi(SV_W), f8, INK)
olcu_h(Xs(1850), Xs(2200), Zs(1700) + 60, "350", f8, INK)
txt(Xs(900), Zs(0) - 60, "KAMERA 4: koridor · servis · teslim cephesi · giriş → NVR (tezgâh altı)", f7, GRAY, "mm")
...
PARCA = [
    ...
    ("SERVİS", "alt dolap 1800 × 800 × 850 · 4 kapak · 3 karton yığını + UPS/NVR/çöp", "1", "karton 4 gün 1.120 blank (3 × 360–430)"),
    ("SERVİS", "tezgâh paslanmaz 1800 × 800 · evye 400 × 400 × 250 · doldurma yüzeyi 1160", "1", "elde yıkama + kaset doldurma"),
    ("SERVİS", "üst dolap 1800 × 350 × 700 · 4 kapak", "1", "içecek 4 gün 280 · temizlik · sarf · yedek tepsi/pençe"),
    ...
    ("KALDIRILAN", "bulaşık makinesi · içecek soğutucu · uzun servis dolabı", "—", "yıkama elde · içecek soğuk stoğu kararı açık"),
]
kol = (0, 130, 900, 990)
th = 30
...
txt(W_PX - 170, ly, "QR dolabı 860 × 520 × 1650 · 12 göz · QR/PIN  ·  servis tezgâhı 1800 × 800 alt + üst dolap + duvar lavabosu 350  ·  4 gün stok (karton 1.120 · içecek 280)  ·  bulaşık makinesi yok  ·  UPS 3 kVA  ·  4 kamera  ·  2 söndürücü + aerosol", f9, GRAY, "rm")
