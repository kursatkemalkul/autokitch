# -*- coding: utf-8 -*-
"""AUTOKITCH · KAŞAR KABI v7 — ÜRETİM MODELİ (21 Eyl 2026) · CadQuery (OpenCascade) ile GERÇEK KATILAR
v6'dan FARK — TEK DEĞİŞİKLİK: Kemal "vidayı dümdüz yapmışsın, çizdiğinde yukarıdan açılı geliyordu, simülasyondaki gibi yap" dedi. HAKLI:
  v5'te helezonun arkasında 60 mm KONİK KÖK vardı (Ø26 → Ø16), v6 CadQuery yeniden kurulumunda düşmüştü. v7'de geri kondu.
  Ne işe yarıyor: arka kanat boşluğunu daraltır → Roberts kapasitesi arkada %69, öne doğru %100. Sabit kesitli helezon kaşarı YALNIZ
  arkadan çeker (KWS); konik kök + artan hatve (22/26/30/36) birlikte yükü boyuna yayar, arkada çukur açılmasını ve köprüyü geciktirir.

Kemal (AR'de yakından bakınca): "parçalar iç içe geçiyor, her parçanın deliği/boşluğu doğru olmalı, her parçanın bir anlamı olmalı,
üretime gidip basılınca tam çalışmalı. Ağzın altındaki kapak ne, gerek var mı?"

v5'ten FARK: v5 görselleştirme ağıydı (delik yok, parçalar iç içe). v6'da her parça ayrı katı; delikler, geçme boşlukları, montaj sırası gerçek.
  · uç plakaları 8 mm: gövde için 4 mm derin KANAL, yatak delikleri, saplama delikleri, tüp faturası, konum pimi delikleri
  · gövde: kadeh kesit 3 mm cidar, iki plakanın kanalına oturur, 4 saplama ile sıkılır (üst 2 saplama dolum çizgisinin üstünde)
  · helezon: 8 × 8 paslanmaz KARE ÇUBUK çekirdek + 4 baskı segment (A/B/C/D — her biri TEK sabit hatve: 22 / 26 / 30 / çift ağız 36) · mil Ø16 · kanat Ø40
  · tahrik göbeği ×2 (helezon + karıştırıcı aynı parça): iç flanş + Ø22 muylu + kare yuva; dışına HAÇ KAVRAMA yaylı pimle
  · karıştırıcı: 8 × 8 kare mil + 3 örümcek göbek + 4 çubuk Ø6; önde KOVAN + TOPUZ + teğet KİLİT PİMİ (çek → mil öne çıkar → kafes üstten alınır)
  · çıkış tüpü kulaklı flanşla ön plakaya 2 × M4; ağız 36 × 38 + bilezik; bayonet YATAK KAPAĞI
  · MENTEŞELİ KAPAK KALDIRILDI (yay + menteşe + kol + makinede pim = 5 parça, peynir tuzağı). Kapalı tüp bölümü kaşarı zaten tutuyor;
    taşıma/depolama için tak-çıkar TPU TAPA yeterli.
  · anlamsız süsler kaldırıldı: uyarı etiketleri, asma deliği (kabın o bölgesi zaten hazne), ipli halka yerine pimin kendi halkası
GEÇME KURALLARI: dönen baskı parça / delik +0,4 çap · kare çekirdek / kare delik +0,3 · pres geçme çubuk Ø6 / delik Ø6,0 · vida M4 serbest Ø4,5
ÇIKTI: otonom/kaset3d/kasar_v7.glb/.usdz + kasar_v7_dozaj.glb  ·  arastirma/3_TOPPING/kasar_kabi_v7/{step,stl}/ + MONTAJ.step + BOM.csv
"""
...
from kasar_akis_model_v2 import RHO, YASA, T_DOK, r_t

KOK = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
URETIM = os.path.join(KOK, "arastirma", "3_TOPPING", "kasar_kabi_v7")
...
KANAT = [(22.0, 5, -148.0), (26.0, 4, -38.0), (30.0, 3, 66.0)]       # hatve, tur, z başlangıç → 156'da biter (kanat gövdesi −150,5'ten başlar: göbek flanşına 1 mm)
...
def teta_z(z):
    """helis açısı (rad): her hatve bölümü tam tur attığı için bölüm başlarında faz 0"""
    for p, n, z0 in KANAT:
        if z0 <= z <= z0 + p * n: return 2 * math.pi * (z - z0) / p
    return 2 * math.pi * (z - UC_Z0) / UC_HATVE
...
def kaynat(g, f, ad=""):
    ...
    for clean in (True, False):
        ...
        if r.val().isValid() and r.val().Volume() > v0 + 0.45 * vf: return r
...
def kap():
    ...
    SEG = [("helezon_A", -151.0, KANAT[1][2], [(KANAT[0][0], KANAT[0][2], KANAT[1][2], 0.0)], "hatve 22 · 5 tur · arkada 60 mm KONİK KÖK Ø26 → Ø16"),
           ("helezon_B", KANAT[1][2], KANAT[2][2], [(KANAT[1][0], KANAT[1][2], KANAT[2][2], 0.0)], "hatve 26 · 4 tur"),
           ("helezon_C", KANAT[2][2], UC_Z0, [(KANAT[2][0], KANAT[2][2], UC_Z0, 0.0)], "hatve 30 · 3 tur"),
           ("helezon_D", UC_Z0, 253.0, [(UC_HATVE, UC_Z0, UC_Z1, 0.0), (UC_HATVE, UC_Z0, UC_Z1, 180.0)], "tüp içi ÇİFT AĞIZ hatve 36 + 6 çırpıcı pim + ön muylu Ø12")]
    for k_, (ad, zA, zB, kanatlar, acik) in enumerate(SEG):
        ...
        for pch, z0, z1, faz in kanatlar:
            zb, zs = z0 + (0 if ilk else GERI), z1 - (0 if son else GERI)
            g = kaynat(g, kanat_sabit(pch, zb, zs, faz + 360.0 * (zb - z0) / pch), ad + " kanat")
        if son:
    ...
    tup = silz(0, CY, 25.0, ZF - 2.8, TUP_Z1)
    flans = silz(0, CY, 31.0, ZF, ZF + 5.0).union(kut(-41, 41, CY - 9, CY + 9, ZF, ZF + 5.0))
    bilezik = kut(-AG_X - 3, AG_X + 3, 10.0, CY, AG_Z0 - 3, AG_Z1 + 3)
    tup = tup.union(flans).union(bilezik).cut(silz(0, CY, RT, ZF - 4, TUP_Z1 + 1)).cut(kut(-AG_X, AG_X, 5.0, CY, AG_Z0, AG_Z1))
    for s in (1, -1): tup = tup.cut(silz(s * 33.0, CY, 2.25, ZF - 1, ZF + 6))
    for a in (90.0, 210.0, 330.0):
        t = math.radians(a); tup = tup.union(cq.Workplane(obj=silx(0, 0, 2.5, 24.5, 28.0).val().rotate(cq.Vector(0, 0, 0), cq.Vector(0, 0, 1), a).translate(cq.Vector(0, CY, 240.0))))
    ekle("cikis_tupu", tup, "cam", bom=("Çıkış tüpü", 1, "PC / PETG şeffaf · baskı ya da torna + freze", "iç Ø44 · ön plakaya faturalı + 2 × M4 · alt ağız 36 × 38 + bilezik · 3 bayonet pimi"))
    ...
    kp = silz(0, CY, 29.0, 236.0, 254.5).cut(silz(0, CY, 25.3, 235.0, TUP_Z1)).cut(silz(0, CY, 6.2, TUP_Z1 - 1, 253.2))
    kp = kp.union(kut(-24, 24, CY - 4, CY + 4, 254.5, 266.0))
    for a in (90.0, 210.0, 330.0):
        giris = kut(24.8, 29.5, -2.9, 2.9, 235.0, 243.0)
        halka = cq.Workplane("XY").circle(29.5).circle(24.8).extrude(5.8).translate((0, 0, 237.1)).intersect(
            cq.Workplane("XY").polyline([(0, 0), (60, -3), (60 * math.cos(math.radians(44)), 60 * math.sin(math.radians(44)))]).close().extrude(30).translate((0, 0, 230)))
    ...
    ekle("tasima_tapasi", kut(-AG_X - 5.4, AG_X + 5.4, 0.5, 14.5, AG_Z0 - 5.4, AG_Z1 + 5.4).union(kut(AG_X + 5.4, AG_X + 22, 0.5, 2.5, 205, 220))
         .cut(kut(-AG_X - 3.2, AG_X + 3.2, 2.5, 14.6, AG_Z0 - 3.2, AG_Z1 + 3.2)), "silikon",
         bom=("Taşıma tapası", 1, "TPU 95A baskı", "kap makine dışındayken ağız bileziğine takılır · makineye sürmeden çıkarılır"))
    ...
    for ad, z0, z1 in (("arka", -150.5, -133.0), ("orta", -7.0, 7.0), ("on", 133.0, 150.5)):
        ekle("orumcek_" + ad, orumcek(z0, z1), "pom", Kg, bom=("Örümcek göbek", 3, "PETG / PA12 baskı (seri üretimde kaynaklı paslanmaz)", "4 kol: çubukları R124 / 124 / 86 / 43'te tutar · kare delik 8,3") if ad == "arka" else None)
    for i, (a, r) in enumerate(KOL):
        t = math.radians(a)
        ekle("cubuk_%d" % i, silz(r * math.cos(t), YC + r * math.sin(t), 3.0, -146.0, 146.0), "celik", Kg, bom=("Çubuk Ø6 × 292", 4, "AISI 304 · taşlanmış", "örümceklere sıkı geçer · kaşarı süpürür") if i == 0 else None)
    ...
    ekle("on_kovan", kovan, "pom", Kg, bom=("Ön kovan", 1, "POM / baskı", "içeriden takılır · plakada döner · mil içinden kayar · teğet pim deliği"))
    ...
    # ---- 8 KAPAK ----
    kapak = cq.Workplane("XZ").rect(W, D).extrude(-4.0).edges("|Y").fillet(12.0).translate((0, Y_UST, 0))
    for s in (1, -1): kapak = kapak.union(kut(min(s * 130.5, s * 133.5), max(s * 130.5, s * 133.5), Y_UST - 6, Y_UST + 0.1, -100, 100)).cut(sily(s * 60.0, 0.0, 11.0, Y_UST - 1, Y_UST + 5))
    ekle("kapak", kapak, "cam", bom=("Kapak", 1, "PC 4 mm + 2 kılavuz çıta (baskı / yapıştırma)", "çıtalar gövde ağzına oturur · 2 parmak deliği Ø22"))
...
def glb_yaz(yol, parcalar, dokular):
    ...
    while off[0] % 4: blob.append(b"\x00"); off[0] += 1
    bb = b"".join(blob)
    g = {"asset": {"version": "2.0", "generator": "AUTOKITCH kasar_cad_v7"}, "scene": 0, "scenes": [{"nodes": kok}], "nodes": nodes, "meshes": meshes, "materials": mats, "accessors": accs,
...
if __name__ == "__main__":
    ...
    for ad, zA, zB in (("helezon_A", -148.0, KANAT[1][2]), ("helezon_B", KANAT[1][2], KANAT[2][2]), ("helezon_C", KANAT[2][2], UC_Z0), ("helezon_D", UC_Z0, UC_Z1)):
 ...
 ("orumcek_arka", "gobek_karistirici", "örümcek ↔ göbek flanşı (1,0)"), ("orumcek_on", "on_kovan", "örümcek ↔ kovan flanşı (1,0)"),
            ("topuz", "plaka_on", "topuz ↔ plaka (1,0)"), ("topuz", "on_kovan", "topuz eteği ↔ kovan (0,25)"), ("kapak", "govde", "kapak çıtası ↔ gövde (0,5)"), ("saplama_2", "govde", "üst saplama ↔ gövde"),
            ("tasima_tapasi", "cikis_tupu", "tapa ↔ bilezik (0,2)"), ("kulp", "topuz", "kulp ↔ topuz")]
    ...
    asm = cq.Assembly(name="KASAR_KABI_v7"); RENK = dict(cam=(0.75, 0.85, 0.92, 0.45), pom=(0.95, 0.95, 0.92, 1), celik=(0.75, 0.77, 0.8, 1), silikon=(0.16, 0.5, 0.95, 1), koyu=(0.15, 0.15, 0.17, 1))
    bom = []
    for p in PARCALAR:
    ...
    asm.save(os.path.join(URETIM, "KASAR_KABI_v7_MONTAJ.step"))
    ...
    print("HACIM %.1f L (ic boy %.0f) · 8,8 kg @ %.2f → dolum kotu y %.0f (%%%.0f)" % (V, D - 2 * TP, RHO, yd, 100 * 8.8 / RHO / V))
    dokular = {"ad": doku_ad("KAŞAR KABI", "bu yönde tak  ·  280 × 325 × 360 mm  ·  %s L  ·  çıkış ÖNDE alttan" % ("%.1f" % V).replace(".", ","), ok_sol=True),
    ...
    b1 = glb_yaz(os.path.join(OUT, "kasar_v7.glb"), kap_ag, dokular)
    print("kasar_v7.glb · %d parca · %d ucgen · %.0f KB" % (len(kap_ag), sum(len(m.I) // 3 for _, m, _, _ in kap_ag), b1 / 1024.0))
    b2, prim, sorun, uyari = usdz_yaz([os.path.join(OUT, "kasar_v7.usdz")], "kasar_v7", [(a, m, mal) for a, m, mal, _ in kap_ag], dokular)
    ...
    doz.append(("kasar_pide_ustu", ks.duzelt(), "kasar", "tabla")); dk = Mesh()
    for i in range(40):
    ...
    doz.append(("kasar_dusen", dk, "kasar", None)); doz.append(("kasar_dolgu", v4.kasar_dolgu(yd), "kasar_dolgu", None))
    b3 = glb_yaz(os.path.join(OUT, "kasar_v7_dozaj.glb"), doz, dokular)
    print("kasar_v7_dozaj.glb · %d parca · %.0f KB · toplam %.0f sn" % (len(doz), b3 / 1024.0, time.time() - t0))
