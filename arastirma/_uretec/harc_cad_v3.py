# -*- coding: utf-8 -*-
"""AUTOKITCH · HARÇ / SOS ÜNİTESİ v2 (22 Eyl 2026) — VİDA YOK.
...
URETIM = os.path.join(KOK, "arastirma", "3_TOPPING", "harc_kaseti_v3")
...
BORU_D, BORU_ET, BORU_ALT, BORU_GECIS = 16.0, 3.0, -10.0, 10.0      # yuvarlak çıkış borusu: iç çap · et · alt uç · tüpten boruya konik geçiş boyu
...
def kap():
    ...
    SEG = [] if 1 else [("helezon_A", -151.0, ZC1, "hatve %.2f → %.2f · arkada 60 mm KONİK KÖK Ø26 → Ø16" % (hatve(KAN_Z0), hatve(ZC1))),
    ...
    ekle("helezon_cekirdek", karez(0, CY, KARE, -163.0, 152.0), "celik", G,
    ...
    # ---- 6 · HAZNE RAKORU + PERİSTALTİK POMPA + BORULU NOZZLE (v2: vida yok) ----
    ...
    RB_V = BORU_D / 2 + BORU_ET   # 1/2" duckbill (boru iç Ø16)
    ...
    ekle("duckbill_valf", vlf, "silikon",
         bom=("Duckbill valf 1/2\"", 1, "platin silikon, gıda onaylı · KATALOG PARÇASI", "borunun ucuna geçer; basınçla açılır, doz bitince kendi elastikliğiyle kapanır → damlama yok"))
...
if __name__ == "__main__":
    ...
    ADK = {p["ad"]: p["wp"].val() for p in PARCALAR}
    if "helezon_A" in ADK:                                   # v2: helezon kalkti, kanat denetimi konusuz
        print("KANAT DENETIMI (ag uzerinde, 5 mm dilimlerde en buyuk yaricap):", end=" "); eksik = []
    ...
    CIFT = []        # v2: helezon/tup/topuz kalkti, eski cift listesi gecersiz — bosluklar pompa duzeni oturunca yeniden kurulacak
    ...
    # v2: helezon yok — dA = asagi.distance(AD["helezon_B"]); BOSLUK.append(("orumcek_orta (kol aşağı)", "helezon_B", "örümcek gözü ↔ helezon kanadı (5,25)", round(dA, 2)))
    # v2: helezon yok — print("   %-16s ↔ %-18s %6.2f mm   %s" % ("orumcek (kol asagi)", "helezon_B", dA, "örümcek gözü ↔ helezon kanadı (5,25)"))
    ...
    b1 = glb_yaz(os.path.join(OUT, "harc_v3.glb"), kap_ag, dokular)
    print("harc_v3.glb · %d parca · %d ucgen · %.0f KB" % (len(kap_ag), sum(len(m.I) // 3 for _, m, _, _ in kap_ag), b1 / 1024.0))
    b2, prim, sorun, uyari = usdz_yaz([os.path.join(OUT, "harc_v3.usdz")], "harc_v3", [(a, m, mal) for a, m, mal, _ in kap_ag], dokular)
    ...
    b3 = glb_yaz(os.path.join(OUT, "harc_v3_dozaj.glb"), doz, dokular)
    print("harc_v3_dozaj.glb · %d parca · %.0f KB · toplam %.0f sn" % (len(doz), b3 / 1024.0, time.time() - t0))
