...
W, H = 1560, 1180
tx(40,44,'İSTASYON 3 — TOPPING · DETAY v8 (TEPSİ ELİ: tabla + kızak + 2 motor KALKTI · robot tepsiyi çıkış altında gezdirir · dozaj boşluğu AÇIK)',17,'start','bold')
tx(40,68,'Üstten aşağı: TEKNİK BÖLME (soğutma 25 + elektrik 14 — erişim üstten) → KASET KATI 70×42 tam dolu (kaşar 35×42 · sucuk 35×21 · kavurma 17×21 · kuşbaşı 17×21) → çark + motor ×4, HER BİRİNİN KENDİ ÇIKIŞI (serbest konum, ağız tabandan sarkar) → AÇIK DOZAJ BOŞLUĞU (robot + tepsi) → GEÇİŞ RAFI 2 kat (ayrı soğuk bölme) · buzluk YOK',10.5,'start','','#555')
...
tx(X0+px(350),Y0+px(295),'ELEKTRİK BÖLMESİ (ÜSTTE) — PLC I/O · 4 step sürücü · 24 V güç (tabla sürücüleri kalktı)',9.5,'middle','bold')
...
tx(X0+px(150),Y0+px(424),'— izoleli tavan: soğuk bölge —',8,'middle','','#888')
...
CY = KY0+355
for cx0,r,sgn in ((200,55,-1),(470,40,1)):
    cy = Y0+px(CY)
...
tx(X0+px(330),Y0+px(CY+70),'kaşar çarkı BÜYÜK',8.5,'middle','bold','#b3452b')
tx(X0+px(330),Y0+px(CY+87),'(80 g/porsiyon)',8,'middle','','#b3452b')
ln(X0+px(15),Y0+px(CY+135),X0+px(685),Y0+px(CY+135),1.6,'#111')
...
E.append('<text x="%.1f" y="%.1f" text-anchor="middle" font-size="7.5" fill="#1a49b8" font-family="Arial" transform="rotate(-90 %.1f %.1f)">soğuk hava kanalı → geçiş rafı</text>' % (X0+px(667),Y0+px(CY+268),X0+px(667),Y0+px(CY+268)))
...
def tk(kx,ky,kw,kh,ad,alt,kulp=True,fr=0.28):
    rc(X2+p2(kx)+3,Y2+p2(ky)+3,p2(kw)-6,p2(kh)-6,1.6,3)
    if kulp: rc(X2+p2(kx)+p2(kw)-p2(28),Y2+p2(ky)+p2(kh/2)-p2(30),p2(20),p2(60),1.2,2)
    tx(X2+p2(kx)+p2(kw/2)-(p2(12) if kulp else 0),Y2+p2(ky)+p2(kh)*fr-2,ad,9.5,'middle','bold')
    tx(X2+p2(kx)+p2(kw/2)-(p2(12) if kulp else 0),Y2+p2(ky)+p2(kh)*fr+12,alt,8.2)
tk(0,0,350,420,'KAŞAR','35 × 42',fr=0.1)
...
E.append('<path d="%s" fill="none" stroke="#1a49b8" stroke-width="1"/>' % pth)
...
nots = [
 ...
 ('1. Taban zaten TEPSİDE (press ten beri)','#333'),
 ('2. BEYİN reçete: kaşar 4 cep; karışık +3+3 cep','#333'),
 ('3. Kol TEPSİYİ ilgili çıkışın altında gezdirir:','#333'),
 ('   spiral / halka / merkez — desen yazılımda','#666'),
 ('4. Kol pideyi fırına götürür','#333'),
 ('','#333'),
 ('KARARLAR (v8 — TEPSİ ELİ, Kemal):','#333'),
 ('· Tabla + kızak + 2 motor + 2 sürücü KALKTI','#b3452b'),
 ('· Çıkış konumu serbest — tek sınır: tepsi merkezi','#b3452b'),
 ('  x 20-52, y ≥17 (yan/arka duvar); ön AÇIK','#666'),
 ('· Dozaj boşluğu AÇIK: soğuk kapak yok, robot','#b3452b'),
 ('  20 sn içeride kalsa da soğuk kaçmaz','#666'),
...
tx(W-24,H-14,'AUTOKITCH · ist3_topping_detay_v8',10,'end','','#999')
...
OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSİTE\AUTOKITCH\arastirma\3_TOPPING\ist3_topping_detay_v8.svg"
