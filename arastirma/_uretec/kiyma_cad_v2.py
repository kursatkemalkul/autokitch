# -*- coding: utf-8 -*-
"""AUTOKITCH · KIYMA KASETİ v2 — ÜRETİM MODELİ (22 Eyl 2026) · kasar_cad_v8 mimarisinden türetildi (yap_kiyma_v2.py)
...
ÇIKTI: otonom/kaset3d/kiyma_v2.glb/.usdz + kiyma_v2_dozaj.glb  ·  arastirma/3_TOPPING/kiyma_kaseti_v2/{step,stl}/ + MONTAJ.step + BOM.csv
"""
...
URETIM = os.path.join(KOK, "arastirma", "3_TOPPING", "kiyma_kaseti_v2")
...
CY, RT, YC, RB, RF, Y_UST, Y_DOLUM = 60.0, 30.0, 158.0, 67.0, 8.0, 352.0, 332.0      # v2: alt mil ekseni 140 sınıfında ORTAK (60) · üst eksen = 60 + 28 + 5 + 65
R_MIL, R_KANAT, KARE = 9.0, 28.0, 8.0                                # helezon mili Ø16 · kanat Ø40 · kare çekirdek 8 × 8
...
def kap():
    ...
    ekle("plaka_on", plaka(True), "cam",
         bom=("Ön uç plakası", 1, "PC / POM / HDPE 8 mm · CNC freze", "kanal · tüp geçişi Ø60 + fatura Ø66,4 × 3 · yatak Ø22,4 · 2 × M4"))
...
def glb_yaz(yol, parcalar, dokular):
    ...
    while off[0] % 4: blob.append(b"\x00"); off[0] += 1
    bb = b"".join(blob)
    g = {"asset": {"version": "2.0", "generator": "AUTOKITCH kiyma_cad_v2"}, "scene": 0, "scenes": [{"nodes": kok}], "nodes": nodes, "meshes": meshes, "materials": mats, "accessors": accs,
...
if __name__ == "__main__":
    ...
    print("→ %s" % ("TAMAM: kanat her yerde Ø56" if not eksik else "EKSIK: %s" % eksik)); assert not eksik
 ...
 ("orumcek_arka", "gobek_karistirici", "örümcek ↔ göbek flanşı (1,0)"), ("orumcek_on", "on_kovan", "örümcek ↔ kovan flanşı (1,0)"),
    ...
    asm = cq.Assembly(name="KIYMA_KASETI_v2"); RENK = dict(cam=(0.75, 0.85, 0.92, 0.45), pom=(0.95, 0.95, 0.92, 1), celik=(0.75, 0.77, 0.8, 1), silikon=(0.16, 0.5, 0.95, 1), koyu=(0.15, 0.15, 0.17, 1))
    bom = []
    for p in PARCALAR:
    ...
    asm.save(os.path.join(URETIM, "KIYMA_KASETI_v2_MONTAJ.step"))
    ...
    b1 = glb_yaz(os.path.join(OUT, "kiyma_v2.glb"), kap_ag, dokular)
    print("kiyma_v2.glb · %d parca · %d ucgen · %.0f KB" % (len(kap_ag), sum(len(m.I) // 3 for _, m, _, _ in kap_ag), b1 / 1024.0))
    b2, prim, sorun, uyari = usdz_yaz([os.path.join(OUT, "kiyma_v2.usdz")], "kiyma_v2", [(a, m, mal) for a, m, mal, _ in kap_ag], dokular)
    ...
    b3 = glb_yaz(os.path.join(OUT, "kiyma_v2_dozaj.glb"), doz, dokular)
    print("kiyma_v2_dozaj.glb · %d parca · %.0f KB · toplam %.0f sn" % (len(doz), b3 / 1024.0, time.time() - t0))
