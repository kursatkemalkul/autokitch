# -*- coding: utf-8 -*-
# OVEN v4 (6 Eyl 2026) — Kemal: pompa arka duvarda, sağda solda 2 STANDART KAP (yağ versiyonu, helezon/tarak yok), biri bitince diğeri;
...
TEPSI = (117,145)
KAP_L = 10.0; KAP_MAX = 12.0; DOZ = 4.0; PIDE = 100
...
tx(30,40,'AUTOKITCH — OVEN v4 (6 Eyl 2026) — Omake 2 kat · 2 STANDART YAĞ KABI (sağ/sol) + pompalar arka duvarda + sprey ORTADA · KESME PRESİ (PACK'+AP+'ten taşındı) · fan şart, filtre opsiyon, baca yok · zonlar hesapla küçüldü',14,'start','bold')
tx(30,62,'Kemal: "pompa arka duvarda, sağda solda standart kartuşlar (helezonsuz), biri bitince diğeri · bir kap kaç gün? · yağ sıcak mı olmalı, ısıtıcı niye? · sprey önde, az yer kaplasın · havalandırma/filtre şart mı, koku, baca, elektrikli · kesmeyi bu istasyona koy: pres + 4 bölen bıçak, tepsi zemine, motor hesabı · büyük büyük koydun"',9,'start','','#444')
...
zones=[('plint','#e9e4d6','plint 12: hava girişi ızgarası'),('pano','#e3f2fb','PANO 20: PLC · 2 menteşe + 1 pres sürücüsü · 2 SSR · 2 pompa rölesi · 24 V'),('kesme','#f4eef8',''),('yag','#fff8e0',''),('firin','#fff',''),('plenum','#fde9c9','PLENUM 12 + FAN Ø120 (şart)'),('filtre','#eef3f8','karbon filtre 5 (opsiyon)'),('ust','#f7f7f7','üst boşluk 12 (yedek)')]
...
tx(X(35),Zf(zy1-2)+2,'YAĞ + SPREY 30 · slot 15 | tepsi 34 | slot 15 · klape 4 · sıcak dolap 42 °C',4.3,'middle','bold',AMB)
...
tx(XB+14,YB+22,'B · YAN KESİT 84 × 197 — kap 68 + pompa arkada · sprey önde · pres',10,'start','bold')
...
tx(XS(55),ZS(zy0+24)+2,'YAĞ KABI 68 (yanda, kesit dışı)',3.8,'middle','bold','#8a6a3a'); tx(XS(55),ZS(zy0+20)+2,'kapaklı · dolum ağzı önde',3.4,'middle','','#8a6a3a')
rc(XS(72),ZS(zy0+6),K*3,K*3,1,0,GRN,None,'#eaf6ee'); tx(XS(73.5),ZS(zy0+2)+2,'kuru bağl.',3.1,'middle','',GRN)
rc(XS(76),ZS(zy0+18),K*6,K*10,1,1,'#333',None,'#eee'); tx(XS(79),ZS(zy0+14)+2,'POMPA',3.4,'middle','bold'); tx(XS(79),ZS(zy0+10.5)+2,'×2 · 24 V',3,'middle','','#555')
...
para(XB+14,YB+572,'Yağ zonu kapalı sıcak dolaptır (42 °C, 100 W ısıtıcı + termostat, üstteki fırın da ısıtır): kap içindeki sadeyağ akışkan kalır, hat ısıtması gerekmez. İki pompa arka duvarda, kapların kuru bağlantısının hemen arkasında; BEYİN biri boşalınca (şamandıra) diğerine geçer. Tepsi kapların arasına 40 cm girer, nozül 20 derinlikte.',80,5.2,'#333')
...
ln(XK(0.5),ZK(18),XK(67.5),ZK(18),.7,'#8a6a3a','3,2'); tx(XK(34),ZK(19.5),'dolum çizgisi %d L (max %d L)' % (KAP_L,KAP_MAX),3.8,'middle','','#8a6a3a')
...
rows=[('dolum','%d L (max %d)' % (KAP_L,KAP_MAX)),('bir kap','%d gün (max %d)' % (round(GUN_KAP),round(GUN_MAX))),('iki kap','%d gün' % round(2*GUN_KAP)),('dolu kap','%s kg' % f1(KAP_KG)),('eleman','ayda 1 kap değişimi'),('robot','dokunmaz')]
yy=YC+228
...
QA=[('YAĞ SICAK MI OLMALI, ISITICI NİYE?','Pide için sıcak olması şart değil; ısıtıcı pompa içindir. Sadeyağ 32 °C altında katıdır, oda sıcaklığında AKMAZ (bozulmaz demiştim, o doğru; ama akmaz). 42 °C sıcak dolap: kap içindeki yağ sıvı kalır, hat/nozül dolapta → ayrı hat ısıtması yok. Isıtıcısız tek yol: sıvı yağ (ayçiçek/zeytinyağı) — lezzet kararı Kemal'+AP+'in.'),
    ('BİR KAP KAÇ GÜN GİDER?','10 L dolum = 25 gün (4 ml × 100 pide = 0,4 L/gün); max 12 L = 30 gün. İki kap = 50 gün; eleman AYDA 1 kap değiştirir. 42 °C'+AP+'de 1 aydan uzun bekletme (acılaşma) → 10 L dolum, sırayla tüketim.'),
    ...
    ('SPREY ÖNDE, AZ YER — OLDU MU?','Evet: nozül 20 cm derinlikte, kapların ARASINDA (15 + 34 + 15); ayrı niş kalktı, 18 cm kazanıldı; damlalık tavası altta. Kap 68 + kuru bağlantı 4 + pompa 12 = 84 tam oturdu.'),
    ...
    ('ISITICI GÜCÜ NE, GECE AÇIK MI KALIR?','Sıcak dolap yalıtımlıdır: kayıp 20–30 W, günde ≈ 0,6 kWh (buzdolabı gibi) → gece de açık kalır, fırın gece KAPALI. Yeni takılan kap oda sıcaklığında katıdır, 4–6 saatte erir → yedek kap en az 1 gün önce takılır (zaten yedek olarak bekler).'),
    ('ELEMAN NE YAPAR?','Ayda 1: boş kabı öne çekip alır (12 kg, bel hizası z 82), doluyu kızağa sürer, kuru bağlantı arkada kendini takar. SERVICE'+AP+'te 16 kg tenekeden kaşıkla doldurur (katı sadeyağ, 5 dk), kapağı geçirir. Haftada 1 damlalık tavasını boşaltır. Kesme bıçağı: ayda 1 kontrol, 6 ayda 1 bileme.')]
yy=YE+42
for q,a in QA:
...
tx(XF+14,YF+22,'F · v3 → v4 DEĞİŞİKLİKLERİ · KONTROL · AÇIK',10,'start','bold')
notes=[(('v4: kartuş 4 L → 2 STANDART KAP (yağ versiyonu) sağ/sol, %d L = %d gün, eleman ayda 1 · pompalar (×2, 24 V dişli) arka duvarda · sprey ortada önde (niş kalktı) · sıcak dolap 42 °C (klape 4) · KESME PRESİ OVEN'+AP+'de (50) · zonlar hesapla: plenum+fan 12, filtre 5 (opsiyon), pano 20, yedek 12 · tepsi düzlemleri fırın 117/145, sprey 90, kesme 36.') % (KAP_L,round(GUN_KAP)),BLU,'bold'),
       ('KONTROL ✓ 12+20+50+30+56+12+5+12 = 197 · derinlik 68+4+12 = 84 (yağ) · 60+24 = 84 (fırın) · kap slotları 15+34+15 = 64 · tepsi Ø32 + pay 1 · bıçak 28 < tepsi 32 · kuvvet 0,85–1,1 kN < 1,5 kN · kap dolu %s kg (eleman, bel hizası z 82) · fırın kapağı açıkken yağ zonu önü kapanır (aynı anda gerekmez).' % f1(KAP_KG),GRN,'bold'),
       ('BEYİN: 2 pompa rölesi + 2 şamandıra (boşalınca diğerine geç, "yağ az" mesajı) · dolap termostatı 42 °C · pres aktüatörü (2 switch + akım limiti) · fan (fırın açıkken hep, kapandıktan 10 dk sonra durur) · kapak motorları · durumlar KAPALI / ÖN ISITMA / BEKLEME 340 / EKO 250 / PİŞİRME.','#333',''),
...
tx(W-40,H-10,'AUTOKITCH · arastirma/4_OVEN/ist4_oven_detay_v4 · 6 Eyl 2026',7,'end','',GRY)
o.append('</svg>')
svg=chr(10).join(o)
xml.dom.minidom.parseString(svg.encode('utf-8'))
out=r"C:\Users\Kemal\Desktop\Kemal\WEBSİTE\AUTOKITCH\arastirma\4_OVEN\ist4_oven_detay_v4.svg"
