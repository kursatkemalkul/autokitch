# -*- coding: utf-8 -*-
"""AUTOKITCH - ROBOT YERLESIMI v6 (16 Eyl 2026): Kemal'in iki secenegi. ON + UST GORUNUS.
...
OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\HAT_ROBOT_v6_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 5400, 3040
...
def on_gorunus(OX, ZY, S, mod, secenek):
    ...
    if secenek == 1:
        ...
        txt(px(cx) + 64, pz(1900.0), "SARKITMA 900", f8, GRAY, "lm")
        ...
        # iki sabit robot
        for cx, hedef, ad in ((1860.0, (1550.0, 1150.0), "ROBOT 1 · FR5"), (5685.0, (5420.0, 1300.0), "ROBOT 2 · FR5")):
    ...
    olcu_v(px(L) + 90, pz(H_MAK), pz(0), "1915")
    olcu_v(px(L) + 210, pz(BANT1), pz(BANT0), "BANT 1100")
...
# ======================= SAYFA =======================
txt(210, 92, "AUTOKITCH · ROBOT YERLEŞİMİ v6 · İKİ SEÇENEK", f34, INK)
txt(210, 152, "ön görünüş + üst görünüş  ·  ölçüler mm  ·  16 Eylül 2026", f13, GRAY)

SA = 0.34
txt(250, 250, "SEÇENEK 1  ·  TEK ROBOT + HAVADAN RAY", f20, ACC)
on_gorunus(420, 1180, SA, S1_MOD, 1)
ust_gorunus(420, 1930, SA, S1_MOD, 1)

txt(2810, 250, "SEÇENEK 2  ·  İKİ SABİT ROBOT (gövdeye bağlı)", f20, ACC)
on_gorunus(2980, 1180, SA, S2_MOD, 2)
ust_gorunus(2980, 1930, SA, S2_MOD, 2)
