...
ÇIKTI: otonom/hat3d/hat_v15.glb + .usdz  ·  otonom/hat3d/durum.json (sayfa oradan okur)  ·  FULL_MAKINE/HAT_v15_YERLESIM.step (SolidWorks)
...
import topping_hesap_v4 as TH, topping_cad_v9 as TC                                      # TOPPING modülü: derinlik dizilimi ve yuva konumları ORADAN okunur
...
KASET_CAD = {"KAŞAR KABI": ("kasar_cad_v13", "Kaşar kabı v13", "kaset3d/index.html?k=kasar_v13"),
             "KIYMA": ("kiyma_cad_v8", "Kıyma kaseti v8", "kaset3d/index.html?k=kiyma_v8"),
             "KUŞBAŞI": ("kusbasi_cad_v7", "Kuşbaşı kaseti v7", "kaset3d/index.html?k=kusbasi_v7"),
             "KÜP SUCUK": ("sucuk_cad_v6", "Küp sucuk kaseti v6", "kaset3d/index.html?k=sucuk_v6"),
             "HARÇ": ("harc_cad_v3", "Harç / sos ünitesi v3 · pompalı", "kaset3d/index.html?k=harc_v3")}
...
def glb_yaz(yol, parcalar, dokular):
    ...
    while off[0] % 4: blob.append(b"\x00"); off[0] += 1
    bb = b"".join(blob)
    g = {"asset": {"version": "2.0", "generator": "AUTOKITCH hat_montaj_v15"}, "scene": 0, "scenes": [{"nodes": list(range(len(nodes)))}], "nodes": nodes, "meshes": meshes,
...
if __name__ == "__main__":
    ...
    b1 = glb_yaz(os.path.join(OUT, "hat_v15.glb"), parcalar, dokular)
    print("hat_v15.glb · %d birim · %.0f KB" % (len(parcalar), b1 / 1024.0))
    b2, prim, sorun, _u = usdz_yaz([os.path.join(OUT, "hat_v15.usdz")], "hat_v15", parcalar, dokular)
    print("hat_v15.usdz · %.0f KB · %d prim · USD denetimi: %s" % (b2 / 1024.0, prim, "GECTI" if not sorun else "KALDI"))
    for x in sorun: print("   HATA:", x)
    os.makedirs(STEP, exist_ok=True); asm.save(os.path.join(STEP, "HAT_v15_YERLESIM.step"))
    print("HAT_v15_YERLESIM.step · SolidWorks'te montaj olarak acilir · %s" % STEP)
