...
W, H = 1560, 1180
tx(40,44,'İSTASYON 3 — TOPPING · DETAY v3 (Kemal krokisi — kaset hazneler + yedek kasetler + soğutma)',17,'start','bold')
tx(40,68,'Üstten aşağı: 4 KASET HAZNE (2 önde 2 arkada, sıfır boşluk) → hücreli çark + motor (×4) → nozül → DÖNER+KAYAR TABLA + motoru → 4 DOLU YEDEK KASET → SOĞUTMA GRUBU (tüm kabin +3 °C)',10.5,'start','','#555')
...
# koniler
for cx0 in (187,512):
    ln(X0+px(cx0-120),Y0+px(310),X0+px(cx0-35),Y0+px(360),1.4); ln(X0+px(cx0+120),Y0+px(310),X0+px(cx0+35),Y0+px(360),1.4)
...
for cx0 in (187,512):
    ...
    rc(X0+px(cx0+55),Y0+px(400),px(50),px(40),1.4,2)
    tx(X0+px(cx0+80),Y0+px(426),'M',10,'middle','bold')
    ln(X0+px(cx0+45),Y0+px(420),X0+px(cx0+55),Y0+px(420),2)
# arka carklar kesik
for cx0 in (222,547):
    ci(X0+px(cx0),Y0+px(400),px(45),1.1,'#999','5,4')
not_(X0+px(350),Y0+px(500),'HÜCRELİ ÇARK ×4 (her biri kendi step motoruyla) — cep cep bırakır',c='#333')
# cikis borulari merkeze
for cx0 in (187,512,222,547):
    ln(X0+px(cx0),Y0+px(465),X0+px(350),Y0+px(560),1,'#777','4,3')
rc(X0+px(335),Y0+px(555),px(30),px(35),1.4,2)
tx(X0+px(395),Y0+px(580),'nozül',9,'start','','#555')

# --- TABLA + MOTOR y 620-820
el(X0+px(350),Y0+px(640),px(140),px(13),1.1)
tx(X0+px(350),Y0+px(625),'pide Ø28',8.5)
...
not_(X0+px(350),Y0+px(850),'pide döner + kızakta kayar → spiral kaplama (dış + orta dolar)',c='#1a49b8')
...
tx(X2+p2(350),Y2+p2(880),'kesik: nozül çıkışları merkeze · altta tabla Ø40 + kızak (mavi)',9.5,'middle','','#1a49b8')
...
nots = [
 ...
 ('3. Tabla döner + kızakta kayar; ilgili','#333'),
 ('   çark cep cep bırakır → spiral örtü','#666'),
...
tx(W-24,H-14,'AUTOKITCH · ist3_topping_detay_v3',10,'end','','#999')
...
OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSİTE\AUTOKITCH\arastirma\3_TOPPING\ist3_topping_detay_v3.svg"
