# -*- coding: utf-8 -*-
"""AUTOKITCH · KUŞBAŞI KASETİ v1 — ÜRETİM MODELİ (22 Eyl 2026) · kiyma_cad_v1'den türetildi (yap_kusbasi_v1.py)
...
URETIM = os.path.join(KOK, "arastirma", "3_TOPPING", "kusbasi_kaseti_v1")
...
def kap():
    ...
    for i, a in enumerate((0.0, 180.0)):
        ekle("cubuk_%d" % i, yerine(lama(LAMA_G, LAMA_K, -146.0, 146.0), a), "celik", Kg, bom=("Sıyırıcı lama 15 × 3 × 292", 2, "AISI 304 lama · kenarları kırık", "çentiklere sıkı geçer · duvara 2,4 mm (küp giremez) · kanada 18 mm (küp sıkışmaz) · köprüyü bozar") if i == 0 else None)
...
def glb_yaz(yol, parcalar, dokular):
    ...
    while off[0] % 4: blob.append(b"\x00"); off[0] += 1
    bb = b"".join(blob)
    g = {"asset": {"version": "2.0", "generator": "AUTOKITCH kusbasi_cad_v1"}, "scene": 0, "scenes": [{"nodes": kok}], "nodes": nodes, "meshes": meshes, "materials": mats, "accessors": accs,
...
if __name__ == "__main__":
 ...
 ("orumcek_arka", "gobek_karistirici", "örümcek ↔ göbek flanşı (1,0)"), ("orumcek_on", "on_kovan", "örümcek ↔ kovan flanşı (1,0)"),
    ...
    asm = cq.Assembly(name="KUSBASI_KASETI_v1"); RENK = dict(cam=(0.75, 0.85, 0.92, 0.45), pom=(0.95, 0.95, 0.92, 1), celik=(0.75, 0.77, 0.8, 1), silikon=(0.16, 0.5, 0.95, 1), koyu=(0.15, 0.15, 0.17, 1))
    bom = []
    for p in PARCALAR:
    ...
    asm.save(os.path.join(URETIM, "KUSBASI_KASETI_v1_MONTAJ.step"))
    ...
    import kusbasi_akis_model_v1 as KM                                    # CAD ile HESAP aynı sayıları mı kullanıyor?
    ...
    b1 = glb_yaz(os.path.join(OUT, "kusbasi_v1.glb"), kap_ag, dokular)
    print("kusbasi_v1.glb · %d parca · %d ucgen · %.0f KB" % (len(kap_ag), sum(len(m.I) // 3 for _, m, _, _ in kap_ag), b1 / 1024.0))
    b2, prim, sorun, uyari = usdz_yaz([os.path.join(OUT, "kusbasi_v1.usdz")], "kusbasi_v1", [(a, m, mal) for a, m, mal, _ in kap_ag], dokular)
    ...
    b3 = glb_yaz(os.path.join(OUT, "kusbasi_v1_dozaj.glb"), doz, dokular)
    print("kusbasi_v1_dozaj.glb · %d parca · %.0f KB · toplam %.0f sn" % (len(doz), b3 / 1024.0, time.time() - t0))
