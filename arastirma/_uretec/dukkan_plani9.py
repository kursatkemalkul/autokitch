# -*- coding: utf-8 -*-
"""DUKKAN PLANI v9 (17 Eyl 2026) — Kemal: "ray olsun, FR5 erisebilecegi kadar yakin; cekmece acilirken robot saga/sola kayar, alir, cekmece kapanir, prese gider — ciz, oluyor mu?"
...
OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\dukkan_plani_v9_ray.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 3400, 2700
S = 3.0
...
# ======================= BASLIK =======================
txt(OX, 60, "AUTOKITCH  ·  DÜKKAN v9 · RAYLI  ·  TEK FR5 YER RAYINDA, HATTA 25 cm  ·  iç 390 × 263 = 10,3 m²  ·  OLUYOR MU?", f38, INK)
txt(OX, 122, "v8 kurgusu, koridor 120 → 90: robot çekmece açılırken yana kayar, alır, çekmece kapanır, prese gider · ray ekseni z 25 · FR5 922 mm (pratik 83) · omuz 100 · ölçüler cm · 17 Eylül 2026", f13, GRAY)
...
d.rectangle([X(DUV_X1), Y(Y_KOR1), X(IC_W), Y(QR[2])], fill=KOR, outline=None)
...
while a < 360.0:
    ...
    a += 4.0
for px in (35.0, 385.0):                                    # pres onu ve ray ucu
    a = 0.0
    while a < 360.0:
...
for px, et in ((35.0, "pres önünde"), (385.0, "ray ucunda")):
    d.ellipse([X(px) - 6, Y(RAY_Y) - 6, X(px) + 6, Y(RAY_Y) + 6], fill=(150, 150, 160))
    txt(X(px), Y(RAY_Y - 14), et, f7, GRAY, "mm")
txt(X(6), Y(Y_KOR1 - 10), "mavi daire: FR5 pratik erişim 83 (robot K1 yanında) · gri: pres önünde ve ray ucunda", f7, ACC, "la")
# aktarma rafi
...
d.line([(X(DUV_X1), Y(Y_KOR1)), (X(DUV_X1), Y(QR[2]))], fill=LINE, width=3)
txt(X(347), Y(Y_KOR1 + 18), "kıvrım 86 × 38", f7, ACC, "mm")
# on zon
...
txt(X(347), Y(QR[2] + 14), "QR DOLABI 86 × 52", f8, INK, "mm"); txt(X(347), Y(QR[2] + 28), "arkası y %s" % sayi(QR[2]), f7, GRAY, "mm")
...
dline((X(385.0), Y(RAY_Y)), (X(347.0), Y(QR[2])), RED, 2, 8, 5)
txt(X(372), Y((RAY_Y + QR[2]) / 2), "%d cm YOK" % round(math.hypot(385 - 347, QR[2] - RAY_Y)), f8, RED, "lm")
# olculer
...
PX0 = X(IC_W) + 260
d.rectangle([PX0, OY - 60, PX0 + 1050, OY + 800], fill=BG, outline=LINE, width=3)
txt(PX0 + 30, OY - 20, "FR5 RAYDA — ERİŞİM KONTROLÜ (omuz 100, ray z 25)", f16, INK)
TAB = [("Hedef", "dx", "dy", "dz", "mesafe", "FR5 83"),
       ...
       ("QR dolabı arkası (cephede, y 189)", "38", "5", "133", "138", "YOK!")]
yy = OY + 30
...
yy += 14
KARAR = [
    "SONUÇ: çekmece dizisi ÇALIŞIR (robot yana kayar, yandan alır) — ama FR5 kolu 4 yerde yetmiyor:",
    " 1 · alt çekmeceler: yandan alırken uzak yarı 87 > 83 → her çekmece iki yandan alınır (sol yarı sağdan, sağ yarı soldan) OK.",
    " 2 · pres: PZP-400 B'nin üstünde olunca plaka ~164'te → 92. Çözüm: tezgâh üstü pres (plaka B + 10–15) ya da omuz 120.",
    " 3 · fırın üst gözü 172 → 96. Çözüm: D kolonunda gözler ALTA (kot 40–130), kesici · yağ · pano ÜSTE.",
    " 4 · QR dolabı cephede: ray ucundan arkasına 138 → hiçbir kol yetişmez, ray düz. Çözüm: dolap koridor ucunda",
    "     hatta dik (müşteri yüzü yan duvarda) ya da ray kıvrımlı (pahalı) ya da 2. kısa ray.",
    "KAPASİTE: tek robot hamur → pres → topping → fırın → kesme → kutu → dolap ≈ 105 s/ürün → ~34/saat.",
    "  Günlük 280 OK (12 saatte 23/saat), akşam piki 55/saat YOK — pikte kuyruk. 2. robot sonra eklenir (ray hazır).",
    "FR5 yerine FR5 WML (190, 12.000 €) ya da FR10 (140, 13.590 €) rayda: 1–3 kendiliğinden çözülür, 4 çözülmez.",
    "RAY: 3,7 m strok · eksen z 25 · araba 40 × 30 · kapaklı (un/hamur düşmesin) · hız ≥ 1 m/s · fiyat Logorob'dan bekleniyor.",
]
for l in KARAR:
...
txt(W_PX - 140, H_PX - 45, "AUTOKITCH · arastirma/FULL_MAKINE/dukkan_plani_v9_ray · 17 Eyl 2026 · üretici _uretec/dukkan_plani9.py", f9, GRAY, "rm")
