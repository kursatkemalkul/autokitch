# -*- coding: utf-8 -*-
"""AUTOKITCH - BASE HAT TEKNIK RESIM v6 (13 Eyl 2026): ON + UST (PLAN KESITI) + YAN (STORE KESITI). Olculer mm.

SON KARAR DURUMU (tek pafta):
  STORE v5    2085 x 1970 x 830   3 kolon x 620 · 32 cekmece · bolme 65 (-18|+3) / 35   <- YALNIZ CIZIMDE (model 1_STORE_v4: 2800 / 40 cekmece / 860)
  PRESS v5     700 x 1970 x 860   Fersah PZP-400 640 x 950 x 800                        <- model 2_PRESS_v5
  TOPPING v4   700 x 1970 x 830   14 yuva, 11 dolu                                       <- model 3_TOPPING_v4 (modelde 860)
  OVEN v4      700 x 1970 x 860   3 hazne, ortak havuz                                   <- model 4_OVEN_v4
  PACK v3      700 x 1970 x 860   kutu 320 x 320, kalip + sarjor                         <- model 5_PACK_v3
  HAT 4885 mm.
KAYNAK (her olcu):
  STORE on / plan ...... teknik_store_topping_v4.py (ST, HH, CELL0/1, BIND, FUGA, plan_store)
  STORE dikey / kesit .. sw_store_v4.py (TEK0 1720, pu_alt 121,5..181,5, pu_tavan 1661..1719)
                         + 830 katmani: arka sac 1,5 + PU 60 + ic sac 1 + plenum 47,5 + cekmece 680 + on 40
  PRESS ................ sw_press5.py (PANEL, O1 / O2 / O3, COPK 59 L, PZP400 x 29..669 z -818..-18)
  TOPPING .............. sw_topping4.py (KLAPE, ROBOT_AGZI, RAF_X, kanat 33..667)
                         + teknik_store_topping_v4.py (YUVA_KAT, 830'da kaset z -695,5..-15,5)
  OVEN ................. sw_oven4.py (YAG, AGIZ, MEK, FKAP, EGZOZ, firin kutusu x 30..670 z -600..-20)
  PACK ................. sw_pack3.py (PANO, TAHRIK, AGIZ, SARJOR, UST, kutu x 190..510 z -440..-120, blank x 150..550 z -815,5..-55,5)
  derinlik ............. sw_lib.py ZK -820, ZF1 40 -> 860 ; 830 istasyonlarda arka yuz -790
Kural: paftada yalniz gorunus + olcu + parca adi; aciklama mesajda.
"""
import os, math
from PIL import Image, ImageDraw, ImageFont

OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\HAT_BASE_v6_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 4300, 2800
S = 0.62
...
# ======================= VERI =======================
XI, WO, BOLME = 62.5, 620.0, (65.0, 35.0)
CELL0, CELL1, BIND, FUGA, TEK0 = 182.5, 1660.0, 15.0, 3.0, 1720.0
YUZ0 = CELL0 - BIND
HH = {"donmus": 115.0, "hamur": 88.0, "lahm": 88.0, "icecek": 132.0}
AD = {"donmus": "DONMUŞ PİDE", "hamur": "TAZE PİDE", "lahm": "LAHMACUN", "icecek": "İÇECEK 330 ml"}
CAP = {"donmus": (20, "top"), "hamur": (20, "top"), "lahm": (30, "top"), "icecek": (56, "kutu")}
KOLON = [[(7, "donmus", "-18"), "ayirici", (3, "lahm", "+3")],
         [(12, "lahm", "+3")],
         [(4, "hamur", "+3"), (6, "icecek", "+3")]]
KOLON_AD = ("K1 · alt −18 / üst +3", "K2 · +3", "K3 · +3")
WS = 2 * XI + len(KOLON) * WO + sum(BOLME)            # 2085

ISTASYON = [("STORE v5", WS, 830.0), ("PRESS v5", 700.0, 860.0), ("TOPPING v4", 700.0, 830.0),
            ("OVEN v4", 700.0, 860.0), ("PACK v3", 700.0, 860.0)]
HAT = sum(w for _, w, _ in ISTASYON)                   # 4885

# cephe: (tip k=kapak / a=acik agiz, x0, x1, y0, y1, ad, kaset yuvasi, etiket kotu)
CEPHE = {
    "PRESS v5": [("k", 33.0, 667.0, 121.5, 1091.5, "FERSAH PZP-400|640 × 950 × 800", None, 440.0),
    ...
    "TOPPING v4": [("k", 33.0, 667.0, 210.0, 489.0, "", "alt2", None),
    ...
    "OVEN v4": [("k", 33.0, 667.0, 123.0, 370.0, "YAĞ KABI · PANO", None, None),
    ...
    "PACK v3": [("k", 33.0, 667.0, 123.0, 420.0, "PANO", None, None),
...
YUVA = {"kat1": ("KAŞAR", "SUCUK"), "kat2": ("HARÇ", None), "kat3": ("KIYMA", "KUŞBAŞI"),
        "alt1": ("HARÇ", "HARÇ", "HARÇ", "KAŞAR"), "alt2": ("KAŞAR", "KIYMA", None, None)}
...
PY_TOP = FY + 300.0                                     # plan: z = -820 cizgisi
...
def py(z):
    return PY_TOP + (z + 820.0) * S


# ======================= BASLIK =======================
txt(OX, 70, "AUTOKITCH  ·  BASE HAT  ·  TEKNİK RESİM  v6", f38, INK)
txt(OX, 138, "ön · üst · yan görünüş  ·  ölçüler mm  ·  13 Eylül 2026", f13, GRAY)
...
def store_on(x0mm):
    ...
    for ki, gruplar in enumerate(KOLON):
        ...
        for g in gruplar:
            ...
            etiket(fx(cx + WO / 2), ym + 13, "%d %s · %s" % (adet * cap, br, "−18 °C" if zon == "-18" else "+3 °C"), f7, col, col)
        ust = y - FUGA
        if CELL1 - ust > 20:
...
txt(fx(HAT / 2), py(-820) - 40, "ARKA", f9, GRAY, "mm")
x = 0.0
for ad, w, dz in ISTASYON:
    ...
    if ad.startswith("STORE"):
        ...
        for xc in RAF_X["kat"]:
            ...
            txt(fx(x + xc), py(-385.0), "KASET", f7, INK, "mm")
            txt(fx(x + xc), py(-355.0), "140 × 680", f7, GRAY, "mm")
    elif ad.startswith("PRESS"):
        d.rectangle([fx(x + 29), py(-818.0), fx(x + 669), py(-18.0)], fill=BG, outline=INK, width=2)
        txt(fx(x + 349), py(-433.0), "FERSAH PZP-400", f8, INK, "mm")
        txt(fx(x + 349), py(-403.0), "640 × 800", f7, GRAY, "mm")
        ...
        drect(fx(x + 150), py(-815.5), fx(x + 550), py(-55.5), GRAY, 1)
        txt(fx(x + 350), py(-640.0), "BLANK 400 × 760", f7, GRAY, "mm")
        d.rectangle([fx(x + 190), py(-440.0), fx(x + 510), py(-120.0)], fill=BG, outline=INK, width=2)
        txt(fx(x + 350), py(-295.0), "KUTU", f8, INK, "mm")
        txt(fx(x + 350), py(-265.0), "320 × 320", f7, GRAY, "mm")
    txt(fx(x + w / 2), py(40.0) + 30, "%s  ·  %s × %s" % (ad, sayi(w), sayi(dz)), f9, INK, "mm")
    x += w
olcu_v(fx(HAT) + 44, py(-820.0), py(40.0), "860", f11, INK, "r")
...
txt(W_PX - 170, ly, "HAT %s × 1970  ·  derinlik 830 / 860  ·  STORE 32 çekmece  ·  TOPPING 14 yuva  ·  OVEN 3 hazne" % sayi(HAT), f9, GRAY, "rm")

os.makedirs(os.path.dirname(OUT), exist_ok=True)
im.save(OUT)
print("yazildi:", OUT, "· hat %s mm · STORE %s" % (sayi(HAT), sayi(WS)))
