...
KLİKLİ bayonet (tırnak + tümsek + cep + O-ring yayı) · gövde contası · muylu O-ringleri · tüp O-ringi + M4 insert. Önceki sürüm: sucuk_cad_v1.py
AUTOKITCH · KÜP SUCUK KASETİ v1 — (22 Eyl 2026) · kusbasi_cad_v2'den türetildi (yap_sucuk_v2.py)
...
ÇIKTI: otonom/kaset3d/sucuk_v2.glb/.usdz + sucuk_v2_dozaj.glb · arastirma/3_TOPPING/sucuk_kaseti_v2/
"""
...
import sucuk_akis_model_v1 as AM                                     # ürünün kendi akış modeli: tabla yasası + hesap sayıları buradan
R0 = AM.r_t_k(0.0)                                                   # doz başında ağzın pide merkezine uzaklığı
RHO = 0.60                                                           # kıymalı harç g/mL · VARSAYIM (çiğ kıyma USDA ≈ 0,95; tartılacak)
KG2 = 2.8                                                            # 2 günlük (pafta HAT v19)
MALZEME.setdefault('sucuk', dict(renk=(0.66, 0.20, 0.10, 1.0), met=0.0, ruf=0.85))
MALZEME.setdefault('sucuk_dolgu', dict(renk=(0.66, 0.20, 0.10, 1.0), met=0.0, ruf=0.85))

KOK = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
URETIM = os.path.join(KOK, "arastirma", "3_TOPPING", "sucuk_kaseti_v2")
...
def kap():
    ...
    ekle("cikis_tupu", tup, "cam", bom=("Çıkış tüpü + eşik", 1, "PC / PETG şeffaf · baskı ya da torna + freze", "iç Ø72 · ön plakaya faturalı + 2 × M4 · kapalı bölümde 6 mm EŞİK (et suyunu tutar, akışı sürekli yapar) · alt ağız 48 × 44 · 3 bayonet TIRNAĞI 8 × 4 × 3,2 (tüple tek parça) · plakaya geçme 1,3 + O-ring"))
    ...
    for ad, z0, z1 in (("arka", -150.5, -133.0), ("orta", -7.0, 7.0), ("on", 133.0, 150.5)):
        ekle("orumcek_" + ad, orumcek(z0, z1), "pom", Kg, bom=("Rotor göbeği · 2 kollu", 3, "DENEME: PETG / PA12 baskı · ÜRETİM: POM-C ya da kaynaklı 304", "2 KISA kol: lamaları r 40'ta 20° eğik tutar · kare delik 8,3") if ad == "arka" else None)
    for i, a in enumerate((0.0, 180.0)):
        ekle("cubuk_%d" % i, yerine(lama(LAMA_G, LAMA_K, -146.0, 146.0), a), "celik", Kg, bom=("Sıyırıcı lama 15 × 3 × 292", 2, "AISI 304 lama · kenarları kırık", "çentiklere sıkı geçer · duvara 19 mm · kanada 22 mm (ikisi de ≥ 1,5 d: küp sıkışmaz) · köprüyü bozar") if i == 0 else None)
    ...
    ekle("on_kovan", kovan, "pom", Kg, bom=("Ön kovan", 1, "POM / baskı", "içeriden takılır · plakada döner · mil içinden kayar · teğet pim deliği · KAŞAR KABIYLA ORTAK"))
    ...
    for s in (1, -1): kapak = kapak.union(kut(min(s * 63.5, s * 66.5), max(s * 63.5, s * 66.5), Y_UST - 6, Y_UST + 0.1, -100, 100)).cut(sily(s * 30.0, 0.0, 11.0, Y_UST - 1, Y_UST + 5))
...
GRUP = {"helezon": dict(pivot=(0, CY * MM, 0), eksen="z", aci=lambda t: -1.3 * min(t, T_DOK) / T_DOK),
...
def glb_yaz(yol, parcalar, dokular):
    ...
    while off[0] % 4: blob.append(b"\x00"); off[0] += 1
    bb = b"".join(blob)
    g = {"asset": {"version": "2.0", "generator": "AUTOKITCH sucuk_cad_v1"}, "scene": 0, "scenes": [{"nodes": kok}], "nodes": nodes, "meshes": meshes, "materials": mats, "accessors": accs,
...
if __name__ == "__main__":
 ...
 ("orumcek_arka", "gobek_karistirici", "örümcek ↔ göbek flanşı (1,0)"), ("orumcek_on", "on_kovan", "örümcek ↔ kovan flanşı (1,0)"),
    ...
    asm = cq.Assembly(name="SUCUK_KASETI_v2"); RENK = dict(cam=(0.75, 0.85, 0.92, 0.45), pom=(0.95, 0.95, 0.92, 1), celik=(0.75, 0.77, 0.8, 1), silikon=(0.16, 0.5, 0.95, 1), koyu=(0.15, 0.15, 0.17, 1))
    bom = []
    for p in PARCALAR:
    ...
    asm.save(os.path.join(URETIM, "SUCUK_KASETI_v2_MONTAJ.step"))
    ...
    import sucuk_akis_model_v1 as KM                                    # CAD ile HESAP aynı sayıları mı kullanıyor?
    ...
    dokular = {"ad": doku_ad("KÜP SUCUK KASETİ", "bu yönde tak  ·  140 × 325 × 360 mm  ·  %s L  ·  çıkış ÖNDE alttan" % ("%.1f" % V).replace(".", ","), ok_sol=True),
    ...
    b1 = glb_yaz(os.path.join(OUT, "sucuk_v2.glb"), kap_ag, dokular)
    print("sucuk_v2.glb · %d parca · %d ucgen · %.0f KB" % (len(kap_ag), sum(len(m.I) // 3 for _, m, _, _ in kap_ag), b1 / 1024.0))
    b2, prim, sorun, uyari = usdz_yaz([os.path.join(OUT, "sucuk_v2.usdz")], "sucuk_v2", [(a, m, mal) for a, m, mal, _ in kap_ag], dokular)
    ...
    rnd = random.Random(11); ks = Mesh(); d_k = 8.0
    for i in range(137):                                                                       # pide üstünde 137 küp (70 g · d 8)
    ...
    doz.append(("sucuk_pide_ustu", ks, "sucuk", "tabla")); dk = Mesh()
    ...
    doz.append(("sucuk_dusen", dk, "sucuk", None))
    dolgu = v4.kasar_dolgu(yd); doz.append(("sucuk_dolgu", dolgu, "sucuk_dolgu", None))
    b3 = glb_yaz(os.path.join(OUT, "sucuk_v2_dozaj.glb"), doz, dokular)
    print("sucuk_v2_dozaj.glb · %d parca · %.0f KB · toplam %.0f sn" % (len(doz), b3 / 1024.0, time.time() - t0))
