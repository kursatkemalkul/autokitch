# -*- coding: utf-8 -*-
"""AUTOKITCH - TABLALI HAT · MODULER v7 · TAM 3 GUN · ATOSA KASETI (14 Eyl 2026): ON + UST GORUNUS. Olculer mm.
Kemal: "bunun 3 gunluk yani hep yaptigimiz versiyonu" -> MODULER v1 yerlesimi aynen + v6'daki Atosa kaseti kurali.
ATOSA KASETI 108 x 250 x 360 · 4,7 L (brosur topping 6 lb = 2,72 kg, 0,58 kg/L) · dolum en cok %90.
MAKINEDE 1 GUNLUK SET = 13 kaset: on sira HARC 6 + KIYMA · arka sira KASAR 3 + KUSBASI 2 + SUCUK.
YEDEK 2 GUNLUK SET = 26 kaset, 3 yedek hazne rafinda (raf basi 10 = 5 yan yana x 2 sira; 360 boy, eski hazneler de 360'ti):
  raf 1 HARC 10 · raf 2 HARC 2 + KASAR 6 + KIYMA 2 · raf 3 KUSBASI 4 + SUCUK 2 (4 bos yer).
Geri kalan her sey MODULER v1 ile ayni: hat 4830 x 1915 x 830, tabla 1245, pide 12 cekmece · lahmacun 18 · icecek 4 · TEKNIK KESME altinda.
Kural: paftada yalniz gorunus + olcu + parca adi; aciklama mesajda.
"""
import os, math
from PIL import Image, ImageDraw, ImageFont

OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\HAT_MODULER_v7_3gun_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 3800, 2650
S = 0.62
...
# ======================= VERI =======================
HB = 1245.0                                  # tabla / bant kotu
H = HB + 670.0                               # 1915 · tum moduller ayni
MOD = [("STORE", "STORE · tam boy", 655.0), ("PRESS", "PRESS", 670.0), ("TOPPING", "TOPPING · Atosa uyarlama", 885.0),
       ("OVEN", "OVEN · özel konveyör", 1420.0), ("KESME", "KESME · altı TEKNİK", 500.0), ("PACK", "PACK", 700.0)]
X0, _x = {}, 0.0
for k, _, w in MOD:
    X0[k] = _x
    _x += w
...
RAFAD = {1: "raf 1 · HARÇ ×10", 2: "raf 2 · HARÇ ×2 + KAŞAR ×6 + KIYMA ×2", 3: "raf 3 · KUŞBAŞI ×4 + SUCUK ×2 · 4 boş yer"}
# (modul, kolon sol kenari (modulden), ust sinir, icerik alttan uste)
KOLON = [("STORE", 17.5, 1660.0, [("raf", 1), ("raf", 2), (1, "icecek"), (4, "pide"), (2, "lahm")]),
         ("PRESS", 25.0, HB - 360.0, [(9, "lahm")]),
         ("TOPPING", 17.5, HB - 381.0, [("raf", 3), (2, "pide"), (2, "lahm")]),
         ("OVEN", 72.5, HB - 400.0, [(4, "pide"), (4, "lahm")]),
         ("OVEN", 727.5, HB - 400.0, [(3, "icecek"), (2, "pide"), (1, "lahm")])]
...
GEN = {"kaset": 108.0}                      # Atosa kaseti 108 x 250 x 360 · 4,7 L
DOLUM = {"HARÇ": "3,6 kg", "KIYMA": "3,2 kg", "KUŞBAŞI": "1,45 kg", "KAŞAR": "1,47 kg", "SUCUK": "1,4 kg"}
ON_SIRA = [("HARÇ", "kaset")] * 6 + [("KIYMA", "kaset")]
AR_SIRA = [("KAŞAR", "kaset")] * 3 + [("KUŞBAŞI", "kaset")] * 2 + [("SUCUK", "kaset")]
...
def dizi(sira):
    ...
    for ad_, tip_ in sira:
        ...
        k += GEN[tip_] + 8.0
    assert k - 8.0 <= X0["TOPPING"] + W["TOPPING"] - 35.0
    return out


ONX, ARX = dizi(ON_SIRA), dizi(AR_SIRA)
PCX = X0["PRESS"] + 335.0
TX_D = ONX[2][0] + ONX[2][1] / 2
...
txt(OX, 60, "AUTOKITCH  ·  TABLALI HAT  ·  MODÜLER  v7  ·  TAM 3 GÜN  ·  ATOSA KASETİ", f38, INK)
txt(OX, 128, "ön görünüş + üst görünüş  ·  her istasyon ayrı dolap, birleşince üst/alt/ön/arka çizgiler hizalı  ·  ayak yok  ·  hat %s × %s × 830  ·  tabla / bant %s  ·  tam 3 gün stok  ·  13 Atosa kaseti + 26 yedek  ·  ölçüler mm  ·  14 Eylül 2026" % (sayi(HAT), sayi(H), sayi(HB)), f13, GRAY)
...
# TOPPING alti yan bosluk
tb0 = X0["TOPPING"] + 17.5 + 620 + 17.5
d.rectangle([fx(tb0), fy(HB - 381.0), fx(X0["TOPPING"] + W["TOPPING"] - 10), fy(40.0)], fill=(250, 250, 251), outline=BOSL, width=1)
tarali(fx(tb0) + 1, fy(HB - 381.0) + 1, fx(X0["TOPPING"] + W["TOPPING"] - 10) - 1, fy(40.0) - 1)
txt(fx((tb0 + X0["TOPPING"] + W["TOPPING"] - 10) / 2), fy(450.0), "boş %s" % sayi(X0["TOPPING"] + W["TOPPING"] - 10 - tb0), f7, GRAY, "mm")
# --- STORE ust bant
xs = X0["STORE"]
d.rectangle([fx(xs + 10), fy(H - 10), fx(xs + W["STORE"] - 10), fy(1730.0)], fill=SOFT, outline=LINE, width=1)
txt(fx(xs + W["STORE"] / 2), fy((H + 1730) / 2), "üst bant · kablo kanalı", f7, GRAY, "mm")
# --- PRESS
px0 = X0["PRESS"] + 15.0
...
txt(fx(tx0 + W["TOPPING"] / 2), fy((H + Y_KAPAK) / 2), "ÜST KAPAK 50 · ATOSA KASETİ 108 × 250 × 360 · 4,7 L · üstten takılır", f7, INK, "mm")
...
txt(fx(tx0 + W["TOPPING"] / 2), fy(Y_KAPAK - 14), "SOĞUK BÖLME +3 °C · arka sıra: KAŞAR ×3 · KUŞBAŞI ×2 · SUCUK", f7, ACC, "mm")
...
d.rectangle([fx(kx0 + 20), fy(560.0), fx(kx0 + 480), fy(40.0)], fill=BG, outline=INK, width=2)
txt(fx(kx0 + 250), fy(300.0) - 11, "SOĞUTMA GRUBU", f8, INK, "mm")
txt(fx(kx0 + 250), fy(300.0) + 11, "STORE + yedek raflar", f7, GRAY, "mm")
d.rectangle([fx(kx0 + 20), fy(HB - 140), fx(kx0 + 480), fy(600.0)], fill=BG, outline=INK, width=2)
txt(fx(kx0 + 250), fy((HB - 140 + 600) / 2) - 11, "HAT PANOSU", f8, INK, "mm")
txt(fx(kx0 + 250), fy((HB - 140 + 600) / 2) + 11, "PLC · fırın kontaktörleri", f7, GRAY, "mm")
# --- PACK
pk0 = X0["PACK"]
d.rectangle([fx(pk0 + 33), fy(HB - 100), fx(pk0 + 667), fy(40.0)], fill=BG, outline=INK, width=2)
txt(fx(pk0 + 350), fy(560.0) - 11, "ŞARJÖR · 506 blank", f8, INK, "mm")
txt(fx(pk0 + 350), fy(560.0) + 11, "alttan kaldırmalı", f7, GRAY, "mm")
...
for yy_, s_ in ((0, "0"), (40, "40"), (HB - 400, "%s çekmece üst (fırın altı)" % sayi(HB - 400)), (HB, "%s tabla / bant" % sayi(HB)),
                (Y_AGIZ, "%s ağız" % sayi(Y_AGIZ)), (1660, "1660 STORE üst çekmece"), (Y_HZ1, "%s hazne üst" % sayi(Y_HZ1)), (H, sayi(H))):
...
# STORE
drect(fx(xs + 17.5), py(-720), fx(xs + 637.5), py(-40), DOLAP, 2)
txt(fx(xs + W["STORE"] / 2), py(-400) - 11, "ÇEKMECE 620 × 680", f8, DOLAP, "mm")
txt(fx(xs + W["STORE"] / 2), py(-400) + 11, "tam boy dolap", f7, GRAY, "mm")
# PRESS
...
for sira, z0, z1, agz, zl, pz0 in ((ONX, -305.0, -55.0, -289.0, -110.0, -38.0), (ARX, -585.0, -335.0, -351.0, -530.0, -625.0)):
    for x_, w_, ad_, tip_ in sira:
        ...
        txt(fx(cx_), py(zl) - 11, ad_ if len(ad_) <= 5 else ad_[:4] + ".", f7, INK, "mm")
...
txt(fx(kx0 + 250), py(-700), "altında TEKNİK", f7, GRAY, "mm")
# PACK
...
for c, fl, a_ in ((INK, FILL, "modül dolabı (kalın çizgi = birleşim)"), (DOLAP, BG, "çekmece · adım pide 108 / lahmacun 93 / içecek 165"),
                  (INK, BG, "yedek hazne rafı 418"), (GRAY, PUC, "PU yalıtım"), (ACC, SOGUK, "soğuk hazne bölmesi"), (FIRIN, HAVA, "fırın plenum"), (RED, AGZ, "açık ağız")):
    d.rectangle([xx, ly - 11, xx + 30, ly + 11], fill=fl, outline=c, width=3)
    txt(xx + 40, ly, a_, f9, INK, "lm")
    xx += 40 + d.textlength(a_, font=f9) + 50
txt(W_PX - 150, ly + 34, "HAT %s × %s × 830 · 6 modül · pide 12 çekmece (240) · lahmacun 18 (600) · içecek 4 (210) · Atosa 13 kaset + yedek rafta 26 kaset · TEKNİK KESME altında" % (sayi(HAT), sayi(H)), f9, GRAY, "rm")

assert not os.path.exists(OUT), "moduler v7 zaten var — yeni numara ver"
im.save(OUT)
