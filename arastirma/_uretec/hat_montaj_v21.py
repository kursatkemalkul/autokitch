...
ÇIKTI: otonom/hat3d/hat_v21.glb + .usdz  ·  otonom/hat3d/durum.json (sayfa oradan okur)  ·  FULL_MAKINE/HAT_v21_YERLESIM.step (SolidWorks)
...
import topping_hesap_v4 as TH, topping_cad_v14 as TC                                      # TOPPING modülü: derinlik dizilimi ve yuva konumları ORADAN okunur
...
if __name__ == "__main__":
    ...
    print("YUVA ETIKETI (v21): %d" % len(ye))
    ...
    b1 = glb_yaz(os.path.join(OUT, "hat_v21.glb"), parcalar, dokular)
    print("hat_v21.glb · %d birim · %.0f KB" % (len(parcalar), b1 / 1024.0))
    b2, prim, sorun, _u = usdz_yaz([os.path.join(OUT, "hat_v21.usdz")], "hat_v21", parcalar, dokular)
    print("hat_v21.usdz · %.0f KB · %d prim · USD denetimi: %s" % (b2 / 1024.0, prim, "GECTI" if not sorun else "KALDI"))
    for x in sorun: print("   HATA:", x)
    os.makedirs(STEP, exist_ok=True); asm.save(os.path.join(STEP, "HAT_v21_YERLESIM.step"))
    print("HAT_v21_YERLESIM.step · SolidWorks'te montaj olarak acilir · %s" % STEP)
