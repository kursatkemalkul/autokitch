...
def kur():
    ...
    # ON KAPAK CIZILMEZ: kapali kapak kasetlerin onunu tamamen kapatiyor, ikizde hicbir sey
    # gorunmuyordu (Kemal: "kasetlerin onune bir kapak koymussun, sil, gormuyorum").
    # Kapak gercek parca, kendi STEP'inde ve BOM'da duruyor; burada ACIK kabul ediliyor.
    ...
    # SATIN ALINAN ELEKTRONIK PARCALAR KABA AGLANIR. Gercek surucu CAD'i (STP-DRV-4830)
    # DIP anahtarlarina, klemens dislerine, vida basliklarina kadar modelli; 12 tanesi
    # montajin ucgen sayisini 600 binden 2,25 milyona cikardi ve sahne acilmaz oldu.
    # Dis olculeri, kutlesi ve carpisma zarfi 0,6 mm toleransta da AYNI kaliyor —
    # fizik ve sigma kontrolu icin fark yok, yalniz suslemesi azaliyor.
    KABA = ("surucu_",)
    for p in ps:
        sh = p["wp"].val()
    ...
    for ad, x0, x1, gen in TC.YUVA:
        ...
        for p in mod.PARCALAR:
            ...
            # v12 DUZELTME — KASAR VIDASI EKSIK GORUNUYORDU.
            # Eski yontem her dilimi OCC BOOLEAN'i (sh.intersect(kutu)) ile kesiyordu ve
            # basarisiz kesimi "except: continue" ile SESSIZCE atiyordu. Kasar helezonunda
            # B ve C segmentleri boyle kayboldu: B'nin 44 diliminden 4'u, C'nin 36'sindan
            # 4'u yazildi, kanadin govdesi hic cizilmedi. Kemal ekranda gordu.
            # YENI YONTEM: boolean YOK. Parca BIR KEZ aglanir, sonra ucgenler agirlik
            # merkezlerinin z'sine gore 2,5 mm'lik kutulara bolunur. Kesim basarisiz
            # olamaz, hicbir ucgen kaybolmaz; carpisma icin dilimlenmis olma amaci ayni.
            # Kutle: segmentin gercek hacmi, dilimlere UCGEN ALANIYLA orantili dagitilir —
            # toplam kutle birebir korunur.
            # SOS KASETI: pompa_rotoru CAD'de "karistirici" grubunda (pompa kasetlerinde
            # dozu rotor verir, grup adlari sim_makine.json'da zaten ters esleniyor).
            # gr kontrolu onu disarida birakiyordu; helisel rotorun konveks kabugu da
            # tipki vida gibi masif silindire donuyordu. Artik o da dilimleniyor.
            if (p["ad"].startswith("helezon_") or p["ad"] == "pompa_rotoru")                     and not p["ad"].endswith("cekirdek"):
                DZ = 0.0025                                   # 2,5 mm — V metre cinsinden
                hac = hacim(sh)
                c = V[F].mean(axis=1)                         # her ucgenin agirlik merkezi
                a_ = V[F][:, 1] - V[F][:, 0]
                b_ = V[F][:, 2] - V[F][:, 0]
                alan = 0.5 * np.linalg.norm(np.cross(a_, b_), axis=1)
                z0_ = c[:, 2].min()
                kova = np.floor((c[:, 2] - z0_) / DZ).astype(int)
                ...
                for q in sorted(set(kova.tolist())):
                    ...
                    kul = np.unique(Fq_)
                    yeni_ix = -np.ones(len(V), dtype=np.int32); yeni_ix[kul] = np.arange(len(kul))
                    Vq = V[kul]; Fq = yeni_ix[Fq_]
                    pay = float(alan[m_].sum() / alan_top)
                    parcalar.append(dict(ad="%s__%s_d%02d" % (k, p["ad"], q), birim="KASET_" + k, grup=grup,
                                         mal=p["mal"], hacim_m3=hac * pay,
                                         kutle_kg=hac * pay * YOGUNLUK.get(p["mal"], 7900.0),
                                         kaynak="olculdu", not_="helezon kanadi dilimi (carpisma icin) · ucgen kovalama",
                                         V=Vq, F=Fq))
                    n += 1
                continue
