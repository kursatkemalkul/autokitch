...
ÇIKTI: otonom/hat3d/hat_v6.glb + .usdz  ·  otonom/hat3d/durum.json (sayfa oradan okur)  ·  FULL_MAKINE/HAT_v6_YERLESIM.step (SolidWorks)
...
import topping_hesap_v1 as TH, topping_cad_v1 as TC                                      # TOPPING modülü: derinlik dizilimi ve yuva konumları ORADAN okunur
...
KASET_CAD = {"KAŞAR KABI": ("kasar_cad_v10", "Kaşar kabı v10", "kaset3d/index.html?k=kasar_v10"), "KIYMA": ("kiyma_cad_v5", "Kıyma kaseti v5", "kaset3d/index.html?k=kiyma_v5"),
             "KUŞBAŞI": ("kusbasi_cad_v4", "Kuşbaşı kaseti v4", "kaset3d/index.html?k=kusbasi_v4"), "KÜP SUCUK": ("sucuk_cad_v3", "Küp sucuk kaseti v3", "kaset3d/index.html?k=sucuk_v3")}
...
def glb_yaz(yol, parcalar, dokular):
    ...
    while off[0] % 4: blob.append(b"\x00"); off[0] += 1
    bb = b"".join(blob)
    g = {"asset": {"version": "2.0", "generator": "AUTOKITCH hat_montaj_v6"}, "scene": 0, "scenes": [{"nodes": list(range(len(nodes)))}], "nodes": nodes, "meshes": meshes,
...
if __name__ == "__main__":
    ...
    for b in B:
        ...
        if b["durum"] == "GERCEK_MODUL":
            ...
            for p in ps:
                ...
                ton.setdefault(p["mal"], Mesh()).ekle(TC_AG(cq.Workplane(obj=sh)))
    ...
    b1 = glb_yaz(os.path.join(OUT, "hat_v6.glb"), parcalar, dokular)
    print("hat_v6.glb · %d birim · %.0f KB" % (len(parcalar), b1 / 1024.0))
    b2, prim, sorun, _u = usdz_yaz([os.path.join(OUT, "hat_v6.usdz")], "hat_v6", parcalar, dokular)
    print("hat_v6.usdz · %.0f KB · %d prim · USD denetimi: %s" % (b2 / 1024.0, prim, "GECTI" if not sorun else "KALDI"))
    for x in sorun: print("   HATA:", x)
    os.makedirs(STEP, exist_ok=True); asm.save(os.path.join(STEP, "HAT_v6_YERLESIM.step"))
    print("HAT_v6_YERLESIM.step · SolidWorks'te montaj olarak acilir · %s" % STEP)
