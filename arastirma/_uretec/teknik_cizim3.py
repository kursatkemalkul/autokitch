...
M=[("HAMUR",750),("PRES",700),("DOZAJ",900),("FIRIN",700),("KESİM+KUTU",700),("İÇECEK",650)]
...
for i in range(6): ci(a0+31+i*px(108),Y0+px(175),px(50))
for i in range(6): ci(a0+31+i*px(108),Y0+px(350),px(50))
rc(a0+8,Y0+px(430),a1-a0-16,px(1000),1.6)
for r in range(7):
    yv=Y0+px(430)+px(138)+r*px(138); ln(a0+15,yv,a1-15,yv,1)
    for i in range(6): ci(a0+31+i*px(108),yv-px(50),px(50),1)
rc(a0+8,Y0+px(1470),a1-a0-16,px(560),1.6)
for r in range(3):
    yv=Y0+px(1470)+px(186)+r*px(186)
    if r<2: ln(a0+15,yv,a1-15,yv,1)
    for i in range(6): ci(a0+31+i*px(108),yv-px(50),px(50),1)
not_((a0+a1)/2,Y0+px(418),"ılık raf · 2×6 top Ø10")
not_((a0+a1)/2,Y0+px(1458),"GN 2/1 · 7 raf ≈168 + rezerv ≈72 top")
...
ln(c0+px(430),Y0+px(1100),c0+px(430),Y0+px(2020),1.2)
rc(c0+px(60),Y0+px(1300),px(310),px(420),1.4,4)
ci(c0+px(215),Y0+px(1510),px(120),1.2)
for a in range(5): ln(c0+px(75),Y0+px(1790)+a*px(45),c0+px(365),Y0+px(1790)+a*px(45),.9)
for r in range(3):
    yk=Y0+px(1400)+r*px(270); ln(c0+px(440),yk,c1-8,yk,1)
    rc(c0+px(460),yk-px(200),px(325),px(200),1,2)
not_(cm2,Y0+px(58),"kaşar · sucuk · kavurma — hazne 3×28")
not_(cm2,Y0+px(900),"döner tabla Ø40 · pide Ø28")
not_(cm2,Y0+px(1160),"soğutma motoru · GN 1/2 küvet 32,5×26,5×20 — 9 adet")

# D FIRIN 700 (Atalay APF-40-1 sinifi; ic 40x40; 2 kat)
d0,d1=xs[3],xs[4]
for k in range(2):
    ky=Y0+px(170)+k*px(540)
    rc(d0+px(50),ky,d1-d0-px(100),px(450),2)
    rc(d0+px(105),ky+px(70),d1-d0-px(210),px(300),1.4)
    ln(d0+px(50),ky+px(450),d1-px(50),ky+px(450),2.6)
dm=(d0+d1)/2
rc(d0+px(50),Y0+px(1180),d1-d0-px(100),px(190),1.6)               # sprey nisi
...
for i in range(3): rc(d0+30+i*px(135),Y0+px(1460),px(110),px(215),1,2)
rc(d0+30+px(410),Y0+px(1460),px(130),px(215),1.4,2)
E.append(f'<line x1="{d0+30+px(475):.1f}" y1="{Y0+px(1460):.1f}" x2="{d0+30+px(475):.1f}" y2="{Y0+px(1215):.1f}" stroke="#111" stroke-width="1" stroke-dasharray="3 4"/>')
E.append(f'<line x1="{d0+30+px(475):.1f}" y1="{Y0+px(1215):.1f}" x2="{dm+px(30):.1f}" y2="{Y0+px(1215):.1f}" stroke="#111" stroke-width="1" stroke-dasharray="3 4"/>')
for k in range(2):
...
for yk in (px(300),px(540),px(725)):
    ...
    for i in range(8): rc(f0+16+i*px(68),yv-px(118),px(66),px(115),1)
yv=Y0+px(60)+px(985); ln(f0+16,yv,f1-16,yv,1)
for i in range(6): rc(f0+16+i*px(91),yv-px(312),px(90),px(310),1)
rc(f0+10,Y0+px(1120),f1-f0-20,px(900),1.6)
for r in range(3):
    yv2=Y0+px(1120)+px(225)+r*px(225); ln(f0+16,yv2,f1-16,yv2,1)
    for i in range(7): ci(f0+27+i*px(73),yv2-px(40),px(35),1)
not_((f0+f1)/2,Y0+px(48),"3 raf × 8 kola + 6 × 1L")
...
tx(X0,Y0-70,"AUTOKITCH — HAT ÖN GÖRÜNÜŞÜ v3 · doğrulanmış standart ölçüler",15,"start","bold")
tx(X0,Y0-50,"ölçüler cm · derinlik 84 (GN dolap 83,5) · kaynak: Öztiryakiler GN600/İnoksan SDN070 · Fersah PZP-400 · Atalay APF-40-1 · GN 2/1 · 30 cm kutu",10.5,"start")

W=int(X0+px(T)+110); H=int(YZ+86)
svg=(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}">'
     f'<rect width="{W}" height="{H}" fill="#ffffff"/>'+''.join(E)+'</svg>')
OUT=r"C:\Users\Kemal\Desktop\Kemal\WEBSİTE\AUTOKITCH\arastirma\hat_on_gorunus_teknik_v3.svg"
