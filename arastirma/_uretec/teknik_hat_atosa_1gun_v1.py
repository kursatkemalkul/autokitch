# -*- coding: utf-8 -*-
"""AUTOKITCH - TABLALI HAT · ATOSA 1 GUNLUK + YEDEK KASET DOLABI v1 (14 Eyl 2026): ON + UST GORUNUS. Olculer mm.
Kemal: makinenin icindeki tum hazneler 1 gunluk olsun, geri kalani baska bos yerde dolapta beklesin, boy kac olur.
ICERIDE (Atosa standart govde 1143): on sira HARC 12 · HARC 12 · KASAR 10 kg | arka sira KIYMA 12 · KUSBASI 2,7 x 2 · SUCUK 2,7 = 7 hazne
  (harc 24 kg = 1,1 gun · kiyma 3,8 · kusbasi 1,9 · kasar 2,3 · sucuk 1,9 gun).
YEDEK DOLAP (hat basi, 620 x 830 x 1335, +3 C, 3 kizakli raf, raf adimi 385): KASAR 3 · HARC 2 · KUSBASI 2 · SUCUK 3 = 10 dolu hazne
  -> ic + yedek = hedef gunler (harc 2,2 · kiyma 3,8 · kusbasi 3,8 · kasar 9,1 · sucuk 7,8 gun).
Yedek dolap hattin altina (Atosa'nin altina) konsaydi STORE kolonlari saga kayar, hat ~5680 olurdu; hat basinda ayri kolon -> hat 5298.
Geri kalan her sey HAT_ATOSA_ENTEGRE_v1 ile ayni (hazne genisligi varsayimi, Atosa yukseklik/derinlik dagilimi, STORE 28 cekmece).
Kural: paftada yalniz gorunus + olcu + parca adi; aciklama mesajda.
"""
import os, math
from PIL import Image, ImageDraw, ImageFont

OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\HAT_ATOSA_1GUN_v1_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 4000, 2800
S, S2 = 0.62, 0.80
...
# ======================= VERI =======================
H = 1970.0
ISTASYON = [("PRESS v7", 700.0), ("TOPPING · Atosa 1 günlük", 1143.0), ("OVEN v7 · özel konveyör", 1500.0), ("KESME v1", 600.0), ("PACK v5", 700.0)]
...
AB0, ATOP = 979.0, H
Y_KAPAK = 1920.0
Y_HZ0, Y_HZ1 = 1530.0, 1890.0
Y_AGIZ = 1490.0
Y_DAML = 1150.0
Y_RAY0, Y_RAY1 = 1020.0, 1050.0
...
HG = 10.0
ON_SIRA = [("HARÇ", "sos"), ("HARÇ", "sos"), ("KAŞAR", "peynir")]
AR_SIRA = [("KIYMA", "sos"), ("KUŞBAŞI", "topping"), ("KUŞBAŞI", "topping"), ("SUCUK", "topping")]
...
# --- YEDEK KASET DOLABI: hedef gunlere tamamlayan dolu hazneler (10) · raf adimi 385 = plaka 15 + hazne 360 + pay 10
YEDEK = [("raf 1", [("KAŞAR", "peynir")], [("KAŞAR", "peynir")]), ("raf 2", [("KAŞAR", "peynir")], [("HARÇ", "sos"), ("HARÇ", "sos")]),
         ("raf 3", [("KUŞBAŞI", "topping")] * 2 + [("SUCUK", "topping")] * 3, [])]
RAF_P, RAF_T, YH = 385.0, 15.0, 1335.0
assert 120.0 + len(YEDEK) * RAF_P + 60.0 == YH
PCX = X0["PRESS"] + 350.0
RAY0, RAY1 = X0["PRESS"] + 120.0, X0["TOPPING"] + AW - 40.0
TX_F = X0["TOPPING"] + AW - 180.0
TX_D = ONX[1][0] + ONX[1][1] / 2                        # dozaj ornegi: 2. HARC
...
# ======================= BASLIK =======================
txt(OX, 70, "AUTOKITCH  ·  TABLALI HAT  ·  ATOSA 1 GÜNLÜK + YEDEK KASET DOLABI  ·  v1", f38, INK)
txt(OX, 138, "ön görünüş + üst görünüş  ·  Atosa standart gövde 1143: içinde yalnız 1 günlük 7 hazne  ·  hedef günlere tamamlayan 10 dolu hazne hat başındaki yedek dolapta (+3 °C)  ·  830 derin  ·  ölçüler mm  ·  14 Eylül 2026", f13, GRAY)
...
d.rectangle([fx(0), fy(YH), fx(620), fy(0)], fill=FILL, outline=LINE, width=3)
d.rectangle([fx(0), fy(120.0), fx(620), fy(0)], fill=SOFT, outline=LINE, width=2)
...
d.rectangle([fx(620), fy(YH), fx(YW), fy(120.0)], fill=SOFT, outline=LINE, width=1)
txt(fx(310), fy(YH) - 64, "YEDEK KASET DOLABI", f13, INK, "md")
olcu_h(fx(0), fx(620), fy(YH) - 26, "620", f11, INK)
olcu_v(fx(0) - 130, fy(YH), fy(0), "1335", f11, INK, "l")
_y = 120.0
for raf_, on_l, ar_l in YEDEK:
    ...
    for ad_, tip_ in on_l:
        ...
        txt(fx(_k + w_ / 2), fy(_y + RAF_T + 170.0), KAP[tip_], f7, RED, "mm")
        _k += w_ + HG
    _y += RAF_P
txt(fx(310), fy(60.0), "arka sıra: r1 KAŞAR · r2 HARÇ × 2 · r3 boş", f7, INK, "mm")
# --- KESME plakasi
kx0 = X0["KESME"]
...
for x_, w_, ad_, tip_ in ONX:
    ...
    txt(fx(cx_), fy(1790.0), ad_, f8, INK, "mm")
    txt(fx(cx_), fy(1790.0) + 22, "ön sıra", f7, GRAY, "mm")
    txt(fx(cx_), fy(1600.0), KAP[tip_], f7, RED, "mm")
txt(fx(tx0 + AW / 2), fy(Y_KAPAK - 12.0), "SOĞUK HAZNE BÖLMESİ +3 °C  ·  ön sıra görünüyor  ·  arka sıra: KIYMA · KUŞBAŞI × 2 · SUCUK", f7, ACC, "mm")
d.rectangle([fx(tx0 + ATW), fy(Y_AGIZ), fx(tx0 + AW - ATW), fy(Y_DAML + 40.0)], fill=(250, 250, 251), outline=GRAY, width=1)
txt(fx(tx0 + AW - 290), fy(Y_AGIZ - 40.0), "TABLA BÖLMESİ 300", f8, INK, "mm")
txt(fx(tx0 + AW - 290), fy(Y_AGIZ - 40.0) + 22, "ön açık · sağ yanda ürün geçişi", f7, GRAY, "mm")
d.rectangle([fx(tx0 + ATW), fy(Y_DAML + 40.0), fx(tx0 + AW - ATW), fy(Y_DAML)], fill=SOFT, outline=LINE, width=2)
txt(fx(tx0 + 830), fy(Y_DAML + 20.0), "DAMLAMA TAVASI 40 · çekmece, günlük yıkanır", f7, INK, "mm")
...
for _i, _t in enumerate(("RAY + MOTOR PAYI 171", "x kızak · kaldırma · döndürme · kablo zinciri")):
    ...
    _yy = fy(1122.0) + _i * 22
    d.rectangle([fx(tx0 + 830) - _tw / 2 - 6, _yy - 11, fx(tx0 + 830) + _tw / 2 + 6, _yy + 11], fill=BG)
    txt(fx(tx0 + 830), _yy, _t, _f, INK, "mm")
txt(fx(tx0 + 830), fy(Y_RAY0) + 16, "TABLA RAYI · pres altından fırın ağzına", f7, ACC, "mm")
...
txt(fx(TX_D + 190), fy(1360.0), "TABLA Ø340 · dozajda kalkar, döner + kayar", f7, ACC, "lm")
txt(fx(TX_D + 190), fy(1360.0) + 20, "tabla arabası: x kızak · kaldırma · döndürme", f7, ACC, "lm")
...
_k = 8.0 + (604.0 - 5 * 95.0 - 4 * HG) / 2
for ad_, tip_ in YEDEK[2][1]:
    d.rectangle([fx(_k), py(-290.0), fx(_k + GEN[tip_]), py(-40.0)], fill=BG, outline=INK, width=2)
    txt(fx(_k + GEN[tip_] / 2), py(-165.0) - 11, ad_[:3] + ".", f7, INK, "mm")
    txt(fx(_k + GEN[tip_] / 2), py(-165.0) + 11, KAP[tip_], f7, GRAY, "mm")
    _k += GEN[tip_] + HG
drect(fx(58), py(-560.0), fx(562), py(-310.0), GRAY, 1)
txt(fx(310), py(-435.0), "raf 3 arka: boş", f7, GRAY, "mm")
txt(fx(310), py(-645.0) - 12, "raf 2: KAŞAR | HARÇ × 2", f7, INK, "mm")
txt(fx(310), py(-645.0) + 12, "raf 1: KAŞAR | KAŞAR", f7, INK, "mm")
txt(fx(310), py(40.0) + 30, "YEDEK KASET DOLABI  ·  620", f9, INK, "mm")
...
txt(fx(tx0 + 700), py(-652.0), "ARKA SIRA TAHRİKİ 100", f7, INK, "mm")
...
drect(fx(TX_F - 110), py(-690.0), fx(TX_F + 110), py(-300.0), ACC, 1)
for xc_ in (PCX, TX_F):
...
txt(W_PX - 170, ly + 34, "HAT %s × 1970 × 830  ·  YEDEK KASET DOLABI 620 × 830 × 1335 · 10 hazne  ·  TOPPING Atosa 1143 × 800 × 991 · 7 hazne (1 gün)  ·  OVEN 1500  ·  STORE %d çekmece" % (sayi(HAT), CEK), f9, GRAY, "rm")

assert not os.path.exists(OUT), "1 gunluk v1 zaten var — yeni numara ver"
