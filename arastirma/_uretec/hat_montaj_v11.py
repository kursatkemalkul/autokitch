...
ÇIKTI: otonom/hat3d/hat_v11.glb + .usdz  ·  otonom/hat3d/durum.json (sayfa oradan okur)  ·  FULL_MAKINE/HAT_v11_YERLESIM.step (SolidWorks)
...
import topping_hesap_v3 as TH, topping_cad_v5 as TC                                      # TOPPING modülü: derinlik dizilimi ve yuva konumları ORADAN okunur
...
KASET_CAD = {"KAŞAR KABI": ("kasar_cad_v11", "Kaşar kabı v10", "kaset3d/index.html?k=kasar_v11"), "KIYMA": ("kiyma_cad_v6", "Kıyma kaseti v5", "kaset3d/index.html?k=kiyma_v6"),
             "KUŞBAŞI": ("kusbasi_cad_v5", "Kuşbaşı kaseti v4", "kaset3d/index.html?k=kusbasi_v5"), "KÜP SUCUK": ("sucuk_cad_v4", "Küp sucuk kaseti v3", "kaset3d/index.html?k=sucuk_v4")}
...
def glb_yaz(yol, parcalar, dokular):
    ...
    while off[0] % 4: blob.append(b"\x00"); off[0] += 1
    bb = b"".join(blob)
    g = {"asset": {"version": "2.0", "generator": "AUTOKITCH hat_montaj_v11"}, "scene": 0, "scenes": [{"nodes": list(range(len(nodes)))}], "nodes": nodes, "meshes": meshes,
...
if __name__ == "__main__":
    ...
    b1 = glb_yaz(os.path.join(OUT, "hat_v11.glb"), parcalar, dokular)
    print("hat_v11.glb · %d birim · %.0f KB" % (len(parcalar), b1 / 1024.0))
    b2, prim, sorun, _u = usdz_yaz([os.path.join(OUT, "hat_v11.usdz")], "hat_v11", parcalar, dokular)
    print("hat_v11.usdz · %.0f KB · %d prim · USD denetimi: %s" % (b2 / 1024.0, prim, "GECTI" if not sorun else "KALDI"))
    for x in sorun: print("   HATA:", x)
    os.makedirs(STEP, exist_ok=True); asm.save(os.path.join(STEP, "HAT_v11_YERLESIM.step"))
    print("HAT_v11_YERLESIM.step · SolidWorks'te montaj olarak acilir · %s" % STEP)
