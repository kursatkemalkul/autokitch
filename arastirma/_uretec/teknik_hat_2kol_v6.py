...
OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\HAT_2KOL_v6_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 5300, 3300
S = 0.62
...
AD = {"hamur": "TAZE PİDE", "lahm": "LAHMACUN", "icecek": "İÇECEK 330 ml · 2 katlı"}
CAP = {"hamur": (20, "top"), "lahm": (35, "top"), "icecek": (180, "kutu")}
...
KOLON = [[(6, "hamur")], [(2, "hamur"), (6, "lahm")], [(6, "lahm"), (1, "icecek")]]   # B: 8 pide + 12 lahm + 1 icecek (2 katli), 3 kolon
K1_TABAN = 260.0                                            # K1 altinda B sogutma grubu tabani (0-260)
KOL_Y0 = (K1_TABAN + 62.5 - BIND, YUZ0, YUZ0)               # kolon baslangic kotlari
KOLON_AD = ("K1", "K2", "K3")
W_B = W_A + W_BC                                            # 2100
H_B = 1000.0                                                # cekmece modulu yuksekligi (ust PU 940-1000)
BAND_UST = YUZ0 + 2 * 108.0 + 6 * 93.0 - FUGA               # K2 ustu 938,5
assert BAND_UST <= H_B - 60.0
assert BAND_UST < H_B - 60.0 + 50.0
# C · TOPPING (Atosa dozaj unitesi, tabla yok): agiz · basliklar · hazne sirasi · sogutma
T_AGZ, T_BAS, T_KAS, T_SOG = (1010.0, 1120.0), (1123.0, 1257.0), (1260.0, 1620.0), (1630.0, 1968.5)
...
PZP = (1000.0, 1950.0)                                      # Fersah PZP-400 B modulunun ustunde: 640 x 800 x 950 · 170 kg
PLAKA = 1580.0                                              # alt plaka kotu = 1000 + 580 (VARSAYIM - katalogda yok)
# D · FIRIN kolonu
TEK = (123.0, 540.0)
YAG = (545.0, 790.0)
KES = (795.0, 1060.0)
...
# ======================= BASLIK =======================
txt(OX, 70, "AUTOKITCH  ·  HAT 2 KOL  ·  TEKNİK RESİM  v6  ·  5 KAPALI İSTASYON  ·  ÇEKMECE ALTTA, PRESS + TOPPING ÜSTÜNDE  ·  ATOSA TABLA YOK  ·  FIRIN ×3  ·  KUTU", f38, INK)
txt(OX, 138, "ön · üst · yan görünüş  ·  günde 80 pide + 200 lahmacun (+ pizza)  ·  2 gün stok  ·  her istasyon kendi soğutucusu / motoru / panosuyla kapalı ürün  ·  21 çekmece tek modülde  ·  harç 2 kaset (kaşar kalıbı 280)  ·  2 × Fairino FR10 koridorda  ·  QR dolabı yok  ·  tüm modüller 830 derin  ·  ölçüler mm  ·  17 Eylül 2026", f13, GRAY)
...
kabin(X_A, W_A, H_B, 1970.0, "A · PRESS")
kapak(X_A, 33.0, 667.0, PZP[0] + 3.0, PZP[1])
kesik(X_A, 40.0, 660.0, PZP[0] + 8.0, PZP[0] + 480.0, "PZP-400 gövdesi · motor + rezistans · 3,5 kW", INK, "640 × 800 × 950 · 170 kg · B üstüne oturur")
...
modul_etiketi(X_A, W_A, 0.0, 1970.0, "MODÜL A · PRESS", "700 × 830 × 970 · B üstünde")
...
txt(fx(X_A + W_B / 2), fy(H_B - 30.0), "tavan PU 60 · üstünde MODÜL A ve C oturur · 170 kg pres için profil takviye", f7, GRAY, "mm")
d.rectangle([fx(X_A + 33), fy(K1_TABAN), fx(X_A + XI + WO + 17), fy(121.5)], fill=SOFT, outline=LINE, width=2)
kesik(X_A, 70.0, 470.0, 128.0, 252.0, "SOĞUTMA GRUBU 400 × 300 × 250 · ½ HP", INK)
kesik(X_A, 490.0, 690.0, 128.0, 252.0, "B KARTI", INK)
txt(fx(X_A + 380), fy(K1_TABAN) + 14, "B teknik tabanı 140 · hava ön ızgaradan", f7, GRAY, "mm")
cx = X_A + XI
...
for urun, gw, xc in YUVA:
    ...
    satirlar(fx(X_BC + xc), fy(1450.0) - 4, urun, f7, INK, 16)
    txt(fx(X_BC + xc), fy(1450.0) + 22, ("%s × 400 × 360" % sayi(gw)) if "SUCUK" not in urun else "180 × 400 · 14 çubuk", f7, GRAY, "mm")
txt(fx(X_BC + W_BC / 2), fy(T_KAS[1]) - 14, "HAZNE SIRASI · +3 °C · önden kaset takılır · kaset tabanı kot 1260", f7, INK, "mm")
kapak(X_BC, 33.0, 1367.0, T_SOG[0], T_SOG[1])
kesik(X_BC, 60.0, 460.0, T_SOG[0] + 20.0, T_SOG[1] - 20.0, "SOĞUTMA GRUBU · hazne kabini", INK, "400 × 300 × 280 · 1/2 HP")
kesik(X_BC, 480.0, 760.0, T_SOG[0] + 20.0, T_SOG[1] - 20.0, "EVAPORATÖR + FAN", INK, "280 × 200")
kesik(X_BC, 780.0, 1100.0, T_SOG[0] + 20.0, T_SOG[1] - 20.0, "DOZAJ SÜRÜCÜLERİ", INK, "6 motor kartı · pompa kumandası")
kesik(X_BC, 1120.0, 1350.0, T_SOG[0] + 20.0, T_SOG[1] - 20.0, "C KONTROL|KARTI", INK)
txt(fx(X_BC + W_BC / 2), fy(T_SOG[0]) + 14, "C TEKNİK BANDI · 340 · hava giriş / çıkış üst ön ızgara · önden servis", f7, INK, "mm")
modul_etiketi(X_BC, W_BC, H_B, 1970.0, "MODÜL C · TOPPING (kendi soğutucusu üstte)", "1400 × 830 × 970 · B üstünde")
olcu_h(fx(X_BC), fx(X_BC + W_BC), fy(1970) - 26, sayi(W_BC), f11, INK)

# --- D · FIRIN kolonu
kabin(X_D, W_D, 0.0, 1970.0, "D · FIRIN · KESİCİ · YAĞ · ROBOT KUTUSU")
kapak(X_D, 33.0, 667.0, TEK[0], TEK[1])
kesik(X_D, 50.0, 450.0, 150.0, 500.0, "ANA PANO · PLC + HMI · sürücüler", INK, "400 × 350 × 250 · arka duvarda")
kesik(X_D, 470.0, 650.0, 150.0, 330.0, "R1 + R2|KONTROL|KUTULARI", INK)
txt(fx(X_D + 560), fy(345.0), "245 × 180 × 45 × 2", f7, GRAY, "mm")
kesik(X_D, 470.0, 650.0, 360.0, 500.0, "UPS|fırın panosu", GRAY)
txt(fx(X_D + 350), fy(TEK[1]) - 14, "ROBOT KUTUSU · PANO · 640 × 770 × 420 · önden kapak", f7, INK, "mm")
...
kesik(X_E, 420.0, 640.0, E_MEK[0] + 160.0, E_MEK[1] - 15.0, "E KONTROL|KARTI · tahrik", INK)
...
satirlar(fx(X_E + 350), fy(1330.0), "KUTU ŞARJÖRÜ|blank 400 × 760 yatay yığın · 1220 mm|= 680 (1,8 mm) – 810 (1,5 mm) kutu · 2 gün 560 · pay var", f8, INK)
...
txt(fx(X_A + W_B / 2), fy(0) + 190, "MODÜL B · ÇEKMECE MODÜLÜ · 8 pide + 12 lahmacun + 1 içecek · 3 kolon · soğutucusu K1 altında · A ve C üstüne oturur", f8, BG, "mm")
txt(fx(X_A + W_B / 2), fy(0) + 210, "2100 × 830 × 1000", f7, BG, "mm")
olcu_v(fx(0) - 44, fy(1970), fy(0), "1970", f11, INK, "l")
for yy in (120.0, K1_TABAN, H_B, T_AGZ[1], T_KAS[0], PLAKA, T_KAS[1], PZP[1], 1970.0):
...
for xj, et, yb in ((X_BC, "A | C", H_B), (X_D, "B·C | D", 0.0), (X_E, "D | E", 0.0)):
    d.rectangle([fx(xj) - 5, fy(1970) - 12, fx(xj) + 5, fy(yb) + 12], fill=BG, outline=None)
    dline((fx(xj), fy(1970) - 12), (fx(xj), fy(yb) + 12), ACC, 3, 10, 6)
...
txt(fx(X_D + 350), py(-700.0), "EGZOZ KANALI · fan · karbon filtre · altta robot kutusu + pano", f7, GRAY, "mm")
# E plan
...
txt(SX, FY_TOP - 152, "K2 kolonundan: MODÜL B çekmeceler (alt) + MODÜL C Atosa dozaj, soğutucusu üstte · robot koridorda", f9, GRAY)
...
txt(sz(-340), fy(560.0) + 12, "motorlu · ön sıra robota sunulur · arkada evaporatör plenumu", f7, GRAY, "mm")
...
txt(sz(-660.0), fy(1090.0), "AĞIZ", f7, RED, "mm")
d.rectangle([sz(-470.0), fy(T_KAS[1]), sz(-70.0), fy(T_KAS[0])], fill=BG, outline=INK, width=2)
txt(sz(-270.0), fy(1440.0) - 10, "KASET 280 × 400 × 360 (harç)", f7, INK, "mm")
txt(sz(-270.0), fy(1440.0) + 10, "hazne kabini +3 °C", f7, GRAY, "mm")
d.rectangle([sz(-360.0), fy(T_BAS[1]), sz(-180.0), fy(T_BAS[0])], fill=SOFT, outline=INK, width=1)
txt(sz(-270.0), fy(1190.0), "NOZUL", f7, INK, "mm")
drect(sz(-740.0), fy(T_KAS[1]), sz(-480.0), fy(T_BAS[0]), GRAY, 1)
txt(sz(-610.0), fy(1390.0) - 8, "POMPA · MOTOR", f7, GRAY, "mm")
txt(sz(-610.0), fy(1390.0) + 8, "haznenin arkasında", f7, GRAY, "mm")
d.rectangle([sz(-420.0), fy(1076.0), sz(-120.0), fy(1064.0)], fill=ACC, outline=ACC)
txt(sz(-270.0), fy(1045.0), "TABAN Ø300 · tepside", f7, ACC, "mm")
d.rectangle([sz(-728.5), fy(1908.5), sz(0.0), fy(T_SOG[0])], fill=SOFT, outline=LINE, width=1)
drect(sz(-700.0), fy(T_SOG[1] - 30), sz(-400.0), fy(T_SOG[0] + 20), INK, 1)
txt(sz(-550.0), fy(1800.0) - 8, "SOĞUTMA GRUBU", f7, INK, "mm")
txt(sz(-550.0), fy(1800.0) + 8, "300 derin · 280 yüksek", f7, GRAY, "mm")
d.rectangle([sz(-300.0), fy(T_SOG[1] - 40), sz(-60.0), fy(T_SOG[0] + 40)], fill=EVC, outline=BUZ, width=1)
txt(sz(-180.0), fy(1800.0), "EVAP.", f7, BUZ, "mm")
...
for w_, c_ in ((16, BG), (9, ACC)):
    d.line([(sz(RZ), fy(OMUZ)), (sz(RZ + 160), fy(1500.0)), (sz(-60.0), fy(1080.0))], fill=c_, width=w_, joint="curve")
...
for yy in (120.0, H_B, T_AGZ[1], T_KAS[0], T_KAS[1], T_SOG[0], 1970.0):
...
txt(SX2, FY_TOP - 152, "MODÜL D: robot kutusu + pano · yağ · kesici · 3 kapaklı hazne", f9, GRAY)
...
drect(sz2(-700.0), fy(500.0), sz2(-450.0), fy(150.0), INK, 1)
drect(sz2(-300.0), fy(330.0), sz2(-100.0), fy(150.0), INK, 1)
txt(sz2(-575.0), fy(325.0) - 8, "ANA PANO", f7, INK, "mm"); txt(sz2(-575.0), fy(325.0) + 8, "250 derin", f7, GRAY, "mm")
txt(sz2(-200.0), fy(240.0) - 8, "R1 · R2", f7, INK, "mm"); txt(sz2(-200.0), fy(240.0) + 8, "kutuları", f7, GRAY, "mm")
txt(sz2(-364.0), fy(515.0), "ROBOT KUTUSU · PANO · önden kapak", f7, GRAY, "mm")
...
PARCA = [
    ("MODÜL B", "ÇEKMECE modülü · 21 çekmece · soğutma grubu K1 altında · A ve C altında", "1", "2100 × 830 × 1000 · K1 6 pide · K2 2 pide + 6 lahm · K3 6 lahm + içecek"),
    ("MODÜL A", "PRESS · Fersah PZP-400 tam makine, B üstünde", "1", "700 × 830 × 970 · 170 kg → B tavanı takviyeli"),
    ("MODÜL C", "TOPPING · Atosa dozaj · tabla yok · soğutucu + pompa sürücüleri üst bandında", "1", "1400 × 830 × 970 · B üstünde"),
    ("MODÜL D", "FIRIN kolonu · teknik + yağ + kesici + 3 hazne", "1", "700 × 830 × 1970"),
    ("MODÜL E", "KUTU kolonu · katlayıcı + vakum + şarjör", "1", "700 × 830 × 1970"),
    ("ROBOT", "Fairino FR10 · 1400 mm · 10 kg · ±0,05", "2", "kaide Ø200 × 820 · koridorda x 1150 / 2800"),
    ("ROBOT UCU", "pençe ×2 · tepsi eli · takım değiştirici", "2 set", "dock'lar aktarma rafının üstünde"),
    ("AKTARMA", "tepsi rafı · 6 tepsi Ø340 · altında çöp 60 L", "1", "koridor · x 1900–2300"),
    ("ÇEKMECE", "soğutmalı çekmece 620 × 680 · motorlu", "21", "8 pide · 12 lahm · 1 içecek 2 katlı (180 kutu + tatlı 14)"),
    ("SOĞUTMA", "soğutma grubu 1/2 HP 400 × 300 × 250–280", "2", "1 × B tabanı (K1 altı) · 1 × C üst bandı"),
    ...
    ("KUTU", "katlayıcı (kalıp + plunger + vakum) + şarjör 1220", "1", "blank 400 × 760 · 680–810 kutu"),
    ("KONTROL", "ana pano PLC + HMI · R1/R2 kontrol kutuları 245 × 180 × 45 · UPS", "1", "D altı robot kutusu (123–540)"),
]
kol = (0, 130, 700, 800)
th = 30
...
txt(W_PX - 170, ly, "HAT %s × 1970 × 830  ·  5 kapalı istasyon (B altta, A + C üstünde)  ·  2 gün  ·  21 çekmece (8 pide · 12 lahmacun · 1 içecek 2 katlı)  ·  Atosa dozaj tablasız: 2 harç 280 + kıyma + kuşbaşı + kaşar 280 + sucuk dilimleyici  ·  FIRIN 3 göz  ·  2 × FR10  ·  QR dolabı yok" % sayi(HAT), f9, GRAY, "rm")
