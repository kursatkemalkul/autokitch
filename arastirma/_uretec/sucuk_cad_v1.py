# -*- coding: utf-8 -*-
"""AUTOKITCH · KÜP SUCUK KASETİ v1 — (22 Eyl 2026) · kusbasi_cad_v2'den türetildi (yap_sucuk_v1.py)
...
URETIM = os.path.join(KOK, "arastirma", "3_TOPPING", "sucuk_kaseti_v1")
...
def kap():
    ...
    ekle("cikis_tupu", tup, "cam", bom=("Çıkış tüpü + eşik", 1, "PC / PETG şeffaf · baskı ya da torna + freze", "iç Ø72 · ön plakaya faturalı + 2 × M4 · kapalı bölümde 6 mm EŞİK (et suyunu tutar, akışı sürekli yapar) · alt ağız 48 × 44 · 3 bayonet pimi"))
...
if __name__ == "__main__":
 ...
 ("orumcek_arka", "gobek_karistirici", "örümcek ↔ göbek flanşı (1,0)"), ("orumcek_on", "on_kovan", "örümcek ↔ kovan flanşı (1,0)"),
    ...
    asm = cq.Assembly(name="SUCUK_KASETI_v1"); RENK = dict(cam=(0.75, 0.85, 0.92, 0.45), pom=(0.95, 0.95, 0.92, 1), celik=(0.75, 0.77, 0.8, 1), silikon=(0.16, 0.5, 0.95, 1), koyu=(0.15, 0.15, 0.17, 1))
    bom = []
    for p in PARCALAR:
    ...
    asm.save(os.path.join(URETIM, "SUCUK_KASETI_v1_MONTAJ.step"))
    ...
    b1 = glb_yaz(os.path.join(OUT, "sucuk_v1.glb"), kap_ag, dokular)
    print("sucuk_v1.glb · %d parca · %d ucgen · %.0f KB" % (len(kap_ag), sum(len(m.I) // 3 for _, m, _, _ in kap_ag), b1 / 1024.0))
    b2, prim, sorun, uyari = usdz_yaz([os.path.join(OUT, "sucuk_v1.usdz")], "sucuk_v1", [(a, m, mal) for a, m, mal, _ in kap_ag], dokular)
    ...
    b3 = glb_yaz(os.path.join(OUT, "sucuk_v1_dozaj.glb"), doz, dokular)
    print("sucuk_v1_dozaj.glb · %d parca · %.0f KB · toplam %.0f sn" % (len(doz), b3 / 1024.0, time.time() - t0))
