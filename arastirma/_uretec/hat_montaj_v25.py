...
ÇIKTI: otonom/hat3d/hat_v25.glb + .usdz  ·  otonom/hat3d/durum.json (sayfa oradan okur)  ·  FULL_MAKINE/HAT_v25_YERLESIM.step (SolidWorks)
...
import topping_hesap_v5 as TH, topping_cad_v19 as TC                                      # TOPPING modülü: derinlik dizilimi ve yuva konumları ORADAN okunur
KASET_Z = (TH.Z_KASET[1], TH.Z_KASET[0])                                                                     # (-80, -405): kulp artık kabinin içinde (Kemal: "niye bu kadar önde")
...
birim("TOPPING_MODUL", "TOPPING modülü: kabin + yalıtım + 6 yuva + 12 tahrik + soğutma + pano", "C", "GERCEK_MODUL",
      (X_BC, X_BC + W_BC), (H_B, H_B + TC.Y), (-DZ, 0.0), "sac", "topping_cad_v1.py", "")
...
def TC_AG(wp, kaba=False):
    ...
    if kaba:
        ...
        return _K.ag(cq.Workplane(obj=sh), 2.0, 1.2)
...
if __name__ == "__main__":
    ...
    for b in B:
        ...
        if b["durum"] == "GERCEK_MODUL":
            ...
            ps = [p for p in TC.PARCALAR if not p["ad"].startswith("_bom") and p["ad"] not in ("on_kapak", "kapak_contasi")]
            ton = {}
            for p in ps:
                sh = p["wp"].val().translate(cq.Vector(b["x"][0], b["y"][0], 0.0))
                # v25 · SITE MODELI TELEFONDA ACILACAK, BOY SINIRI VAR.
                # Olculdu: 12 gercek surucu + 12 gercek motor CAD'i modul_C.glb'yi
                # 13,4 MB'den 34,7 MB'ye cikardi — mobil veride acilmaz.
                # SURUCULER elektrik bandinda, dis sacin arkasinda: disaridan hicbir
                # zaman gorunmuyorlar. Onlar GERCEK OLCULU ZARF kutusu olarak ciziliyor
                # (kutu STEP'in kendi sinir kutusundan aliniyor, uydurma degil).
                # MOTOR ve REDUKTOR kuru bolmede gozukuyor, GERCEK CAD kaliyor, yalniz
                # kaba agleniyor. Isaac ikizinde hepsi tam detayiyla duruyor.
                _ad = p["ad"]
                if _ad.startswith("surucu_"):
                    _b = sh.BoundingBox()
                    _z = cq.Workplane("XY").box(_b.xlen, _b.ylen, _b.zlen, centered=False)                            .translate((_b.xmin, _b.ymin, _b.zmin)).val()
                    ton.setdefault((p["mal"], grup_modul(_ad)), Mesh()).ekle(TC_AG(cq.Workplane(obj=_z)))
                else:
                    _kaba = _ad.startswith(("motor_", "reduktor_", "x_motoru"))
                    ton.setdefault((p["mal"], grup_modul(_ad)), Mesh()).ekle(TC_AG(cq.Workplane(obj=sh), _kaba))
    ...
    b1 = glb_yaz(os.path.join(OUT, "hat_v25.glb"), parcalar, dokular)
    print("hat_v25.glb · %d birim · %.0f KB" % (len(parcalar), b1 / 1024.0))
    b2, prim, sorun, _u = usdz_yaz([os.path.join(OUT, "hat_v25.usdz")], "hat_v25", parcalar, dokular)
    print("hat_v25.usdz · %.0f KB · %d prim · USD denetimi: %s" % (b2 / 1024.0, prim, "GECTI" if not sorun else "KALDI"))
    for x in sorun: print("   HATA:", x)
    os.makedirs(STEP, exist_ok=True); asm.save(os.path.join(STEP, "HAT_v25_YERLESIM.step"))
    print("HAT_v25_YERLESIM.step · SolidWorks'te montaj olarak acilir · %s" % STEP)
