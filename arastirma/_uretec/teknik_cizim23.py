...
ln(a0+8,Y0+px(1120),am-px(35),Y0+px(1120),1.6)                    # kuru sarf ayirici
...
not_((a0+am)/2,Y0+px(1150),"BOŞ — büyüme payı")
...
# HAT USTU SARF BANDI (pres..kesim) + firin ustu karbon filtre
bx0=xs[1]+2; bx1=xs[5]-2
rc(bx0,Y0-px(295),bx1-bx0,px(285),1.8,4)
ln(bx0+10,Y0-px(150),bx1-10,Y0-px(150),1)
for i in range(9): rc(bx0+px(60)+i*px(290),Y0-px(272),px(230),px(105),1,2)   # kutu desteleri / koliler
for i in range(11): rc(bx0+px(45)+i*px(245),Y0-px(132),px(200),px(112),1,2)
fu0,fu1=xs[3],xs[4]
rc(fu0+px(60),Y0-px(290),fu1-fu0-px(120),px(275),1.6,3)                      # filtre unitesi
ci((fu0+fu1)/2,Y0-px(180),px(70),1.2)
ln((fu0+fu1)/2-px(50),Y0-px(230),(fu0+fu1)/2+px(50),Y0-px(130),.9); ln((fu0+fu1)/2-px(50),Y0-px(130),(fu0+fu1)/2+px(50),Y0-px(230),.9)
E.append(f'<path d="M {(fu0+fu1)/2:.1f} {Y0-px(295):.1f} l -7 12 h 14 z" fill="#111"/>')
not_((bx0+fu0)/2,Y0-px(310),"HAT ÜSTÜ SARF BANDI 275×84×30 ≈ 0,7 m³ — kutu desteleri · saçet/peçete · haftalık içecek kolileri")
not_((fu0+fu1)/2,Y0-px(115),"fırın üstü: karbon filtreli koku çıkışı + fan")
ox(X0+px(T)+28,Y0-px(295),Y0-px(10),"30")

# OLCULER (cm)
for i,(ad,w) in enumerate(M):
    oy(xs[i],xs[i+1],Y0-112,str(round(w/10)))
...
tx(X0,Y0-155,"AUTOKITCH — HAT v23 · 415 × 197 × 84 + HAT ÜSTÜ SARF BANDI",15,"start","bold")
tx(X0,Y0-133,"ölçüler cm · hat üstü bant: haftalık dünya ↔ 3 günlük makine tamponu (insan bölgesi — robot gitmez; ayrı hafif modül, sahada üste cıvatalanır) · fırın üstü karbon filtre · TAZE 160=2G · DONMUŞ 80=1G",10.5,"start")

W=int(X0+px(T)+110); H=int(YT2+px(1860))
svg=(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}">'
     f'<rect width="{W}" height="{H}" fill="#ffffff"/>'+''.join(E)+'</svg>')
OUT=r"C:\Users\Kemal\Desktop\Kemal\WEBSİTE\AUTOKITCH\arastirma\hat_on_gorunus_teknik_v23.svg"
