# -*- coding: utf-8 -*-
"""AUTOKITCH · KAŞAR KABI v6 — ÜRETİM MODELİ (21 Eyl 2026) · CadQuery (OpenCascade) ile GERÇEK KATILAR
...
ÇIKTI: otonom/kaset3d/kasar_v6.glb/.usdz + kasar_v6_dozaj.glb  ·  arastirma/3_TOPPING/kasar_kabi_v6/{step,stl}/ + MONTAJ.step + BOM.csv
"""
...
URETIM = os.path.join(KOK, "arastirma", "3_TOPPING", "kasar_kabi_v6")
...
def kap():
    ...
    SEG = [("helezon_A", -151.0, KANAT[1][2], [(KANAT[0][0], KANAT[0][2], KANAT[1][2], 0.0)], "hatve 22 · 5 tur"),
    ...
    for k_, (ad, zA, zB, kanatlar, acik) in enumerate(SEG):
        ...
        g = silz(0, CY, R_MIL, zA + (0 if ilk else BOYUN), (233.0 if son else zB - BOYUN))
...
def glb_yaz(yol, parcalar, dokular):
    ...
    while off[0] % 4: blob.append(b"\x00"); off[0] += 1
    bb = b"".join(blob)
    g = {"asset": {"version": "2.0", "generator": "AUTOKITCH kasar_cad_v6"}, "scene": 0, "scenes": [{"nodes": kok}], "nodes": nodes, "meshes": meshes, "materials": mats, "accessors": accs,
...
if __name__ == "__main__":
 ...
 ("orumcek_arka", "gobek_karistirici", "örümcek ↔ göbek flanşı (1,0)"), ("orumcek_on", "on_kovan", "örümcek ↔ kovan flanşı (1,0)"),
    ...
    asm = cq.Assembly(name="KASAR_KABI_v6"); RENK = dict(cam=(0.75, 0.85, 0.92, 0.45), pom=(0.95, 0.95, 0.92, 1), celik=(0.75, 0.77, 0.8, 1), silikon=(0.16, 0.5, 0.95, 1), koyu=(0.15, 0.15, 0.17, 1))
    bom = []
    for p in PARCALAR:
    ...
    asm.save(os.path.join(URETIM, "KASAR_KABI_v6_MONTAJ.step"))
    ...
    b1 = glb_yaz(os.path.join(OUT, "kasar_v6.glb"), kap_ag, dokular)
    print("kasar_v6.glb · %d parca · %d ucgen · %.0f KB" % (len(kap_ag), sum(len(m.I) // 3 for _, m, _, _ in kap_ag), b1 / 1024.0))
    b2, prim, sorun, uyari = usdz_yaz([os.path.join(OUT, "kasar_v6.usdz")], "kasar_v6", [(a, m, mal) for a, m, mal, _ in kap_ag], dokular)
    ...
    b3 = glb_yaz(os.path.join(OUT, "kasar_v6_dozaj.glb"), doz, dokular)
    print("kasar_v6_dozaj.glb · %d parca · %.0f KB · toplam %.0f sn" % (len(doz), b3 / 1024.0, time.time() - t0))
