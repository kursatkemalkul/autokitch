...
v3 (ANA MAKİNE v48, Kemal "v2 kabul"): TOPPING v24 teknesi 2500'de bittiği için giriş ön odasındaki TOPPING CEBİ KALKTI (kabuk ve
  giriş duvarı düz). Fırın üstüne HAVALI RAF (3 mm, 5 mm takozlu): üstünde pizza kutusu yedeği (55 kutu) + TOPPING kompresörü
...
UST_RAF_Y = (YG1 + 5.0, YG1 + 8.0)                              # 1478–1481 · fırın üstünde havalı raf (5 mm takoz + 3 mm plaka)
...
def adaptor(nema23=None):
    ...
    for i, (x, z) in enumerate(((X_F0 + 30.0, -30.0), (XC_TP, -30.0), (X_F1 - 30.0, -30.0), (X_F0 + 30.0, -415.0), (XC_TP, -415.0), (X_F1 - 30.0, -415.0))):
...
BIRIMLER = [
    ...
    ("F_UST_RAF", "Fırın üstü havalı raf (bizim) · 3 mm, 5 mm takozlu · üstünde pizza kutusu yedeği 55 + TOPPING kompresörü"),
...
def glb_yaz(yol, parcalar):
    ...
    while off[0] % 4: blob.append(b"\x00"); off[0] += 1
    bb = b"".join(blob)
    g = {"asset": {"version": "2.0", "generator": "AUTOKITCH firin_tp10_cad_v3"}, "scene": 0, "scenes": [{"nodes": list(range(len(nodes)))}], "nodes": nodes,
...
if __name__ == "__main__":
    ...
    yol = os.path.join(KOK, "otonom", "hat3d", "firin_tp10_v3.glb")
    ...
    b2, prim, sorun, _u = K3.usdz_yaz([yol.replace(".glb", ".usdz")], "firin_tp10_v3", parca, _dk)
    print("firin_tp10_v3.glb %.0f KB · usdz %.0f KB · %d prim · USD: %s · %.0f sn" % (b1 / 1024.0, b2 / 1024.0, prim, "GEÇTİ" if not sorun else sorun, time.time() - t0))
