# -*- coding: utf-8 -*-
"""AUTOKITCH - BANTLI HAT TEKNIK RESIM v3 (13 Eyl 2026): ON GORUNUS (montaj hali) + UST GORUNUS (bant kotu plan kesiti).
...
OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\HAT_BANTLI_v3_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 3800, 2800
S, S2 = 0.62, 0.80
...
# ======================= BASLIK =======================
txt(OX, 70, "AUTOKITCH  ·  BANTLI HAT  ·  TEKNİK RESİM  v3", f38, INK)
txt(OX, 138, "ön görünüş (montaj hâli) + üst görünüş (bant kotu plan kesiti)  ·  4 ürünlük özel konveyör fırın 830 derinliğe sığdırıldı  ·  ölçüler mm  ·  13 Eylül 2026", f13, GRAY)
...
yalitim(fx(OX0), fy(FY1), fx(OX0 + 2100.0), fy(FY1 - ISO))
yalitim(fx(OX0), fy(FY0 + ISO), fx(OX0 + 2100.0), fy(FY0))
yalitim(fx(OX0), fy(FY1), fx(OX0 + DUVAR_X), fy(FY0))
yalitim(fx(OX0 + 2100.0 - DUVAR_X), fy(FY1), fx(OX0 + 2100.0), fy(FY0))
d.rectangle([fx(OX0 + DUVAR_X), fy(IC_Y1), fx(OX0 + 2100.0 - DUVAR_X), fy(IC_Y0)], fill=BG, outline=FIRIN, width=2)
...
d.line([(fx(OX0 - 10), fy(Y_BANT)), (fx(OX0 + 2110.0), fy(Y_BANT))], fill=FIRIN, width=5)
for k in range(4):
...
txt(fx(HZ0 + 700.0), fy(1075.0), "PİŞİRME HAZNESİ 1400  ·  aynı anda 4 ürün  ·  bant 450, 5,8 mm/s (pide 240 s)", f8, FIRIN, "mm")
txt(fx(HZ0 - 150.0), fy(1420.0), "TÜNEL", f7, FIRIN, "mm"); txt(fx(HZ0 - 150.0), fy(1180.0), "300", f7, FIRIN, "mm")
txt(fx(HZ1 + 150.0), fy(1420.0), "TÜNEL", f7, FIRIN, "mm"); txt(fx(HZ1 + 150.0), fy(1180.0), "300", f7, FIRIN, "mm")
txt(fx(OX0 + 1050.0), fy(FY1 - 25.0), "taşyünü 50 · fan ve rezistanslar arka plenumda (bkz. üst görünüş)", f7, GRAY, "mm")
d.rectangle([fx(OX0), fy(FY0), fx(OX0 + 2100.0), fy(960.0)], fill=BG, outline=GRAY, width=1)
txt(fx(OX0 + 1050.0), fy(980.0), "havalandırma boşluğu 40", f7, GRAY, "mm")
d.rectangle([fx(OX0 + 33), fy(1968.0), fx(OX0 + 2067), fy(1660.0)], fill=BG, outline=LINE, width=2)
d.rectangle([fx(HZ1 - 120.0), fy(1660.0), fx(HZ1), fy(FY1)], fill=SOFT, outline=LINE, width=1)
txt(fx(OX0 + 1050.0), fy(1814.0), "EGZOZ DAVLUMBAZI · fan · yağ + karbon filtre", f9, INK, "mm")
olcu_h(fx(OX0), fx(TUN_IN0), fy(FY1) - 26, "50", f7, GRAY)
olcu_h(fx(TUN_IN0), fx(HZ0), fy(FY1) - 26, "300", f8, INK)
olcu_h(fx(HZ0), fx(HZ1), fy(FY1) - 26, "HAZNE 1400", f9, FIRIN)
olcu_h(fx(HZ1), fx(TUN_OUT1), fy(FY1) - 26, "300", f8, INK)
olcu_h(fx(TUN_OUT1), fx(OX0 + 2100.0), fy(FY1) - 26, "50", f7, GRAY)
olcu_v(fx(TUN_IN0 + 100.0), fy(FY1), fy(FY0), "gövde 620", f8, INK, "r")
# --- KESME
...
txt(W_PX - 170, ly, "HAT %s × 1970 × 830  ·  OVEN 2100 × 830 × 620 gövde  ·  hazne 1400 = 4 ürün  ·  tavan 60/saat  ·  STORE %d çekmece" % (sayi(HAT), CEK), f9, GRAY, "rm")

assert not os.path.exists(OUT), "v3 zaten var — yeni numara ver"
