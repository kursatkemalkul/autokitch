# -*- coding: utf-8 -*-
# DÜKKAN PLANI v7 (7 Eyl 2026) — Kemal: krokiyi 90° döndür, sokak cephesi AŞAĞI baksın, düzen aynı; TEK FİNAL RESİM (plan + sokak cephesi).
...
SW,SD=420.0,264.0
WALL=6.0; COR=90.0; LINE_D=84.0; FZ=84.0
YW0,YW1=FZ,FZ+WALL; YC0,YC1=YW1,YW1+COR; YL0,YL1=YC1,SD            # ön zon 0–84 · ince duvar 84–90 · robot 90–180 · hat 180–264
...
SRV=(222.0,292.0); SRV_Y=(0.0,84.0)                                # SERVICE 70×84: önü cephe (y 0), arkası ince duvara (y 84)
DOL=(296.0,420.0); DOL_Y=(0.0,52.0)                                # dolap 124×52: önü cephe, arkası y 52; arkasındaki 32 robot alanına açık
...
tx(30,32,'AUTOKITCH — DÜKKAN v7 · PLAN + SOKAK CEPHESİ · düz cephe, ön zon = SERVICE derinliği 84 · iç %d × %d = %s'%(SW,SD,m2(IC)),13,'start','bold')
tx(30,50,'SERVICE arkası ince duvara, önü cephe hattı; tezgah ve teslim dolabı aynı hatta, kademe yok · dolap arkasındaki 32 robot alanına açık · ekran + kod SERVICE sırtında · hat sağa yaslı · ray 300 · tezgah 30 + eleman 54 (dar, bkz. not)',8.5,'start','','#444')
...
rc(X(DOL[0]),Y(YC0),K*(SW-DOL[0]),K*(YC0-DOL_Y[1]),.6,0,'#7fb3d5',None,'#eef6fb')             # dolap arkası 32+6: robot alanı
rc(X(SRV[1]),Y(YC0),K*(DOL[0]-SRV[1]),K*YC0,.5,0,'#999',None,'#ddd')                          # SERVICE–dolap arası dolu 4
# hat
xx=0.0
for nm,w in M:
...
tx(X(135),Y(YW0-5)+3,'ROBOT KAPISI 70 (kilitli)',4.6,'middle','','#c0392b')
tx(X(111),Y(YW0+3)+2,'İNCE DUVAR 6 · x 0–296 (SERVICE sırtı dayalı)',5.2,'middle','bold','#7a5c2e')
# SERVICE (arkası koridora, kapısı tezgah zonuna, sırtı sokağa)
rc(X(SRV[0]),Y(SRV_Y[1]),K*(SRV[1]-SRV[0]),K*84,1.2,2,'#111',None,'#e4e6ea'); tx(X(257),Y(50)+3,'7 SERVICE',6.2,'middle','bold'); tx(X(257),Y(38)+3,'70 × 84 · arkası ince duvara',5,'middle','','#555')
ln(X(SRV[0]),Y(14),X(SRV[0]),Y(64),1.4,'#fff'); ln(X(SRV[0]),Y(14),X(SRV[0])-K*40,Y(14),1); tx(X(SRV[0])-4,Y(72)+3,'kapı (tezgah zonuna)',4.6,'end','','#555')
# tezgah + eleman
rc(X(TEZ[0]),Y(30),K*(TEZ[1]-TEZ[0]),K*30,1.2,2,'#111',None,'#f3e6d3'); tx(X(150),Y(15)+3,'TEZGAH 140 × 30 — sokağa bakar',6,'middle','bold')
ci(X(150),Y(57),K*18,1.2,'#111',None,'#fff'); tx(X(150),Y(57)+3,'eleman',5.4,'middle',''); tx(X(150),Y(35)+3,'eleman şeridi 54 (dar)',4.6,'middle','','#c0392b')
# teslim dolabı (arkası koridora)
rc(X(DOL[0]),Y(DOL_Y[1]),K*(DOL[1]-DOL[0]),K*52,1.4,1,'#111',None,'#dfe1e4'); tx(X(358),Y(30)+3,'TESLİM DOLABI 124 × 52',6.2,'middle','bold'); tx(X(358),Y(16)+3,'12 dolap · önü cephe',5,'middle','','#555')
ln(X(DOL[0]),Y(DOL_Y[1]),X(DOL[1]),Y(DOL_Y[1]),1.6,'#1d7a4f'); tx(X(358),Y(DOL_Y[1]+6)+3,'robot klapeleri (arka yüz) — arkası 32 + 6 robot alanı, ray eksenine 83',5,'middle','','#1d7a4f')
# cephe açıklıkları
FW=Y(0)
...
dim(X(EKR[0]),FW+50,X(EKR[1]),FW+50,'45',5.4,'#666'); dim(X(KOD[0]),FW+50,X(KOD[1]),FW+50,'20',5.4,'#666'); dim(X(DOL[0]),FW+50,X(DOL[1]),FW+50,'124',5.4,'#666')
...
xr=X(SW)+30
dim(xr,Y(SD),xr,Y(YL0),'hat 84'); dim(xr,Y(YL0),xr,Y(YC0),'robot 90'); dim(xr,Y(YC0),xr,Y(DOL_Y[1]),'38'); dim(xr,Y(DOL_Y[1]),xr,Y(0),'dolap 52'); dim(X(SW)+52,Y(SD),X(SW)+52,Y(0),'%d'%SD,7)
xl=X(0)-30
dim(xl,Y(SD),xl,Y(YC0),'174'); dim(xl,Y(YC0),xl,Y(YW0),'6'); dim(xl,Y(YW0),xl,Y(30),'eleman 54'); dim(xl,Y(30),xl,Y(0),'tezgah 30')
...
tx(X(-30),FZ_(300)-12,'SOKAK CEPHESİ (önden) · duvar boyu 300 · düz',9.5,'start','bold')
...
rows=[('hat kabinleri',LINE_D*SW/1e4),('robot koridoru',COR*SW/1e4),('ince duvar',WALL*DOL[0]/1e4),('tezgah zonu (tezgah 30 + eleman 54)',FZ*TZX/1e4),('SERVICE',70*84/1e4),('teslim dolabı + arkası (robot)',124*84/1e4)]
yy=by+40
...
tx(bx+12,yy+20,'%d × %d, kademesiz düz cephe'%(SW,SD),6,'start','','#555')
yy=para(bx+12,yy+40,'ERİŞİM (mesafe en yakın ray noktasından, kol 142 = CRX-20iA/L): dolap sol dibi 119 ✓, sağ dibi 132 ✓ (dolap arkası 32 + duvar 6 = 38 robot alanı) · TOPPING kap dibi çatalla 113 ✓ · STORE sol çekmece 111 ✓ · PACK sağ 111 ✓. TM20 (130) sağ dibe yetişmez.',62,6,'#333')
yy=para(bx+12,yy+2,'SERVICE önü cephe, arkası ince duvara; kapısı tezgah zonuna. Ekran + kod SERVICE sırtında, cephe hattında. Dolap cephede, arkasındaki 32 + duvar boşluğu 6 robot alanına açık (ince duvar dolap arkasında yok).',62,6,'#333')
para(bx+12,yy+2,'v5 13,4 → v7 11,1 m². NOT: tezgah arkasında eleman şeridi 54 cm — dar; rahat çalışma için ön zon 100 (tezgah 30 + 70) = 420 × 280 = 11,8 m².',62,6,'#c0392b','bold')
tx(W-30,H-8,'AUTOKITCH · arastirma/FULL_MAKINE/dukkan_plani_v7 · 7 Eyl 2026',6.5,'end','','#666')
svg='<svg xmlns="http://www.w3.org/2000/svg" width="%d" height="%d" viewBox="0 0 %d %d">'%(W,H,W,H)+chr(10).join(o)+'</svg>'
xml.dom.minidom.parseString(svg.encode('utf-8'))
out=r"C:\Users\Kemal\Desktop\Kemal\WEBSİTE\AUTOKITCH\arastirma\FULL_MAKINE\dukkan_plani_v7.svg"; io.open(out,'w',encoding='utf-8').write(svg); print('ok', IC)
