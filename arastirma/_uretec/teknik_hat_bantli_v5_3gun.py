# -*- coding: utf-8 -*-
"""AUTOKITCH - BANTLI HAT TEKNIK RESIM v5 · TAM 3 GUN (14 Eyl 2026): ON GORUNUS (montaj hali) + UST GORUNUS (bant kotu plan kesiti). Olculer mm.
Kemal (hat secenekleri karsilastirmasi): "diger seceneklerde ne kadar varsa bunda da o kadar olmali" (icecek yer yok olamaz).
v4'ten FARKI: 14 Eyl kurali (her sey TAM 3 gun) + cekmece v2 (conta alin 33: pide 108 · lahmacun 35'lik 93 · icecek 165).
  bant alti STORE 5 kolon (tavan 940 · 1140 · 900 · 900 · 900): K1 7 pide · K2 5 pide + 4 lahm · K3 7 lahm · K4 7 lahm · K5 4 icecek = 34 cekmece
  (pide 240 · lahmacun 600 / 630 yer · icecek 210 / 224 yer); vakum paket cekmeceleri kalkti.
  kaset 12,5 L, gunluk set 6 (KASAR · SUCUK · HARC 2 · KIYMA · KUSBASI): dozaj 6 (1. gun) + depo 6 (2. gun) + YEDEK KASET KULESI 6 (3. gun).
  kule bant altina sigmaz (34 cekmece + 2 raf 4230 mm > kolonlar 3943 mm) -> hat basinda 180 genis tam boy modul, 6 raf x 300.
  hat 4500 -> 4680. PACK sarjor 506 = 1,8 gun (3 gun 840 blank; karar Kemal'de).
Kural: paftada yalniz gorunus + olcu + parca adi; aciklama mesajda.
"""
import os, math
from PIL import Image, ImageDraw, ImageFont

OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\HAT_BANTLI_v5_3gun_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 3800, 2800
S, S2 = 0.62, 0.80
...
# ======================= VERI =======================
H = 1970.0
ISTASYON = [("KASET v1", 180.0), ("PRESS v7", 700.0), ("TOPPING-BANT v2", 1000.0), ("OVEN v7 · özel konveyör", 1500.0), ("KESME v1", 600.0), ("PACK v5", 700.0)]
X0, _x = {}, 0.0
...
# ======================= BASLIK =======================
txt(OX, 70, "AUTOKITCH  ·  BANTLI HAT  ·  TEKNİK RESİM  v5  ·  TAM 3 GÜN", f38, INK)
txt(OX, 138, "ön görünüş (montaj hâli) + üst görünüş (bant kotu plan kesiti)  ·  4 ürünlük özel konveyör fırın, tünelsiz: ürün topping bandından doğrudan hazneye  ·  830 derin  ·  tam 3 gün stok  ·  ölçüler mm  ·  14 Eylül 2026", f13, GRAY)
...
HHK = {"hamur": 75.0, "lahm": 60.0, "icecek": 132.0}
ADK = {"hamur": "TAZE PİDE", "lahm": "LAHMACUN", "icecek": "İÇECEK"}
CAPK = {"hamur": (20, "top"), "lahm": (35, "top"), "icecek": (56, "kutu")}
KOLON = [(940.0, [(7, "hamur")]), (1140.0, [(5, "hamur"), (4, "lahm")]), (900.0, [(7, "lahm")]), (900.0, [(7, "lahm")]), (900.0, [(4, "icecek")])]
for top_, g_ in KOLON:
    assert sum(n * (HHK[t] + 2 * BIND + FUGA) for n, t in g_) - FUGA <= top_ - (CELL0 - BIND), ("kolon tasiyor", top_)
cx = X0["PRESS"] + XI
...
olcu_h(fx(X0["PRESS"]), fx(TEK0X), fy(0) + 80, "STORE  %s  ·  %d çekmece  ·  bant altında" % (sayi(TEK0X - X0["PRESS"]), CEK), f11, INK)
# --- YEDEK KASET KULESI (3. gun seti, 6 raf x 300)
kk0 = X0["KASET"]
for i_, ad_k in enumerate(("HARÇ", "HARÇ", "KIYMA", "KUŞBAŞI", "KAŞAR", "SUCUK")):
    y0_ = 150.0 + i_ * 300.0
    d.rectangle([fx(kk0 + 12), fy(y0_ + 12), fx(kk0 + 168), fy(y0_)], fill=SOFT, outline=LINE, width=1)
    d.rectangle([fx(kk0 + 20), fy(y0_ + 252), fx(kk0 + 160), fy(y0_ + 12)], fill=KAS, outline=INK, width=2)
    txt(fx(kk0 + 90), fy(y0_ + 132) - 10, ad_k, f7, INK, "mm")
    txt(fx(kk0 + 90), fy(y0_ + 132) + 10, "3. gün", f7, GRAY, "mm")
...
# --- YEDEK KASET KULESI
d.rectangle([fx(X0["KASET"] + 20), py(-695.5), fx(X0["KASET"] + 160), py(-15.5)], fill=KAS, outline=INK, width=2)
txt(fx(X0["KASET"] + 90), py(-380.0), "KASET", f7, INK, "mm")
txt(fx(X0["KASET"] + 90), py(-350.0), "× 6", f7, GRAY, "mm")
# --- PRESS
...
txt(W_PX - 170, ly + 34, "HAT %s × 1970 × 830  ·  tam 3 gün  ·  STORE %d çekmece (12 pide · 18 lahmacun · 4 içecek)  ·  kaset 18: dozaj 6 + depo 6 + kule 6  ·  OVEN hazne 1400 = 4 ürün" % (sayi(HAT), CEK), f9, GRAY, "rm")

assert not os.path.exists(OUT), "v5 zaten var — yeni numara ver"
