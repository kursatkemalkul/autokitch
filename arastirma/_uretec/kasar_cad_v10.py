# -*- coding: utf-8 -*-
"""KAPAK YOK (Kemal, 22 Eyl: "kapak olmasın, TOPPING'de de yok") → gövde ağzında İÇE 8 mm bükme dudak · ROTOR KAYNAKLI (lama-kol kaynak yakaları) · ön kovanda SAVURMA DİSKİ. Üreteç: yap_kapaksiz_v1.py · önceki sürüm: kasar_cad_v9.py
...
ÇIKTI: otonom/kaset3d/kasar_v10.glb/.usdz + kasar_v10_dozaj.glb  ·  arastirma/3_TOPPING/kasar_kabi_v10/{step,stl}/ + MONTAJ.step + BOM.csv
"""
...
URETIM = os.path.join(KOK, "arastirma", "3_TOPPING", "kasar_kabi_v10")
...
if __name__ == "__main__":
 ...
 ("orumcek_arka", "gobek_karistirici", "örümcek ↔ göbek flanşı (1,0)"), ("orumcek_on", "on_kovan", "örümcek ↔ kovan flanşı (1,0)"),
    ...
    b1 = glb_yaz(os.path.join(OUT, "kasar_v10.glb"), kap_ag, dokular)
    print("kasar_v10.glb · %d parca · %d ucgen · %.0f KB" % (len(kap_ag), sum(len(m.I) // 3 for _, m, _, _ in kap_ag), b1 / 1024.0))
    b2, prim, sorun, uyari = usdz_yaz([os.path.join(OUT, "kasar_v10.usdz")], "kasar_v10", [(a, m, mal) for a, m, mal, _ in kap_ag], dokular)
    ...
    b3 = glb_yaz(os.path.join(OUT, "kasar_v10_dozaj.glb"), doz, dokular)
    print("kasar_v10_dozaj.glb · %d parca · %.0f KB · toplam %.0f sn" % (len(doz), b3 / 1024.0, time.time() - t0))
