...
if __name__ == "__main__":
    ...
    b2, prim, sorun, _u = usdz_yaz([os.path.join(OUT, "hat_v46.usdz")], "hat_v46", _usd + E_USDZ, dokular)
    print("hat_v46.usdz · %.0f KB · %d prim · USD denetimi: %s" % (b2 / 1024.0, prim, "GECTI" if not sorun else "KALDI"))
    ...
    with io.open(os.path.join(OUT, "durum.json"), "w", encoding="utf-8") as f:
        json.dump(dict(hat=dict(w=HAT_W, h=H_MAK, d=DZ, pafta="HAT_ATOSA_TABLALI v12 · v46 (animasyon duzeltmesi, geometri v45 ile ayni) · K = kesme_cad_v1 (gercek) · 1 tam animasyon · alt kisim v3 · B = store_cad_v5 · kompresor K tabani arkasinda · TOPPING v2 UNO'lu (topping_uno_cad_v5) · E = kutu_cad_v3 · alt taban 123 · taban hizasi · surec 1168"), sayac=sayac, animasyon=dict(sure=T_J, adim=ADIM_HAT), modul=[dict(kod=k, ad=a, x=[x, x + w], y=list(MODUL_Y[k])) for k, a, x, w in MODUL],
