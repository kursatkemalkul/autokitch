...
M=[("HAMUR",600),("PRES",700),("DOZAJ",650),("FIRIN",650),("KESİM+KUTU",550),("İÇECEK",550)]
...
for r in range(29):                                               # sol: katlanmis sarjor (derinlikte 2 kolon)
    rc(e0+6,Y0+px(680)+r*px(45),px(320),px(45),1.05)
ln(e0+px(360),Y0+px(680),e0+px(360),Y0+px(2020),1.2)              # bolme duvari
for tb in (px(1270),px(1780)):                                    # sag: 2 yatay raf, karton dikine
    ln(e0+px(368),Y0+tb,e1-6,Y0+tb,1)
    for i in range(4): ln(e0+px(385)+i*px(40),Y0+tb-px(450),e0+px(385)+i*px(40),Y0+tb,.8)
not_(em,Y0+px(68),"piston + bıçak yıldızı Ø30")
not_(em,Y0+px(655),"kesim tablası Ø30")
not_(em,Y0+px(2012),"şarjör 29×2 derin ≈58 · açık dikine ≈300")
...
ln(f0+16,Y0+px(1615),f1-16,Y0+px(1615),1.2)
for r in range(8): rc((f0+f1)/2-px(160),Y0+px(1630)+r*px(45),px(320),px(42),1.05)
not_((f0+f1)/2,Y0+px(48),"3 raf × 6 kola + 5 × 1L")
not_((f0+f1)/2,Y0+px(1108),"paketli 2 raf · altta yedek kutu ≈20")
...
tx(X0,Y0-70,"AUTOKITCH — HAT ÖN GÖRÜNÜŞÜ v5 · OPTİMUM GENİŞLİK",15,"start","bold")
tx(X0,Y0-50,"ölçüler cm · derinlik 84 · GN tepsi derinlemesine · hazne 2+1 · kutu şarjörü derinlemesine 2 kolon · karton dikine · pres CP-330 seçilirse toplam 360",10.5,"start")

W=int(X0+px(T)+110); H=int(YZ+86)
svg=(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}">'
     f'<rect width="{W}" height="{H}" fill="#ffffff"/>'+''.join(E)+'</svg>')
OUT=r"C:\Users\Kemal\Desktop\Kemal\WEBSİTE\AUTOKITCH\arastirma\hat_on_gorunus_teknik_v5.svg"
