...
M=[("SOĞUK DEPO — HAMUR + İÇECEK + TATLI",1400),("PRES",700),("DOZAJ",700),("FIRIN",650),("KESİM+KUTU",700)]
...
rc(a0+px(60),Y0+px(370),am-px(35)-a0-px(60),px(1440),1)           # donmus ic — KOMPLE KOLON (12 raf)
for kz in range(20):
    ln(a0+px(60),Y0+px(420)+kz*px(70),a0+px(85),Y0+px(420)+kz*px(70),.8)
    ln(am-px(85),Y0+px(420)+kz*px(70),am-px(60),Y0+px(420)+kz*px(70),.8)
for r in range(12):                                               # 12 raf = 240 (tamami donmus)
    ty=Y0+px(485)+r*px(119)
...
not_((a0+a1)/2,Y0+px(62),"soğutma grupları ×2 — standart üst bölme")
not_((a0+am)/2,Y0+px(330),"KOMPLE −18° · 12 raf = 240 (3G, TAMAMI DONMUŞ)")
not_((a0+am)/2,Y0+px(356),"+ et/peynir/kavurma porsiyon küvetleri (adres paylaşımlı)")

# A2 sag yari: ILIK + TAZE + iade
a2,a3=am,xs[1]
ln(a2+12,Y0+px(310),a3-15,Y0+px(310),1)
ln(a2+px(35),Y0+px(315),a3-15,Y0+px(315),1.4)
ln(a2+px(35),Y0+px(1010),a3-15,Y0+px(1010),1.4)
rc(a2+px(40),Y0+px(360),a3-px(60)-a2-px(40),px(650),1)
for kz in range(9):
    ln(a2+px(60),Y0+px(405)+kz*px(70),a2+px(85),Y0+px(405)+kz*px(70),.8)
    ln(a3-px(85),Y0+px(405)+kz*px(70),a3-px(60),Y0+px(405)+kz*px(70),.8)
for r in range(5):
    ty=Y0+px(485)+r*px(126)
...
not_((a2+a3)/2,Y0+px(340),"ÇÖZÜLME TAMPONU +3° · 5 raf ≈100 (gece+öğlen indirme)")
...
rc(d0+px(40),Y0+px(20),d1-d0-px(80),px(115),1.4,3)
ci((d0+d1)/2,Y0+px(77),px(42),1.1)
ln((d0+d1)/2-px(30),Y0+px(47),(d0+d1)/2+px(30),Y0+px(107),.8); ln((d0+d1)/2-px(30),Y0+px(107),(d0+d1)/2+px(30),Y0+px(47),.8)
not_((d0+d1)/2,Y0+px(155),"üstte: karbon filtre + fan (koku çıkışı arkaya) · APF-40-1 · iç 40×40 · taş taban")
...
# KOLON2 ALTI: ICECEK + TATLI (yatik kanallar, uctan gorunus — 3 gunluk ~122 icecek + ~20 tatli)
for k4 in range(4):
    cy4=Y0+px(1035)+k4*px(122)
    rc(a2+px(30),cy4,a3-px(62)-a2,px(112),1.4,3)
    for i in range(7): rc(a2+px(40)+i*px(78),cy4+px(5),px(64),px(102),1)
rc(a2+px(30),Y0+px(1535),a3-px(62)-a2,px(295),1.6,3)
for i in range(5): rc(a2+px(52)+i*px(104),Y0+px(1552),px(84),px(262),1)
not_((a2+a3)/2,Y0+px(1026),"İÇECEK+TATLI — 4 çekmece × 7 kanal (kutu 24 · tatlı 3 · yedek 1) — HAFTALIK")
not_((a2+a3)/2,Y0+px(1528),"1L çekmecesi — 5 kanal × 8 (haftalık 25 + yedek)")
...
not_((a0+a1)/2,YT2+px(45),"tek kasa 2 KAPI: sol donmuş · sağ taze+içecek+tatlı — kapı yayları")
...
tx(cm2+px(300),ry-14,"ray ~3,3 m · 7. eksen — kesik yay: kol erişimi (~130 cm)",9.5,"start","","#555")
ln(X0-30,YT2+px(1740),X0+px(3450),YT2+px(1740),2.4)
ln(X0-30,YT2+px(1762),X0+px(3450),YT2+px(1762),1)
rc(X0+px(3450)+2,YT2+px(1740),px(700)-4,px(840),2.2,5)            # SERVIS IST. karsi sirada
for _sr in (280,560): ln(X0+px(3470),YT2+px(1740)+px(_sr),X0+px(4130),YT2+px(1740)+px(_sr),.8)
tx(X0+px(3800),YT2+px(1740)+px(430),"SERVİS İST.",10.5,"middle","bold")
not_(X0+px(3800),YT2+px(1740)+px(700),"insan tarafı — dolum + temizlik")
oy(X0+px(3450),X0+px(4150),YT2+px(1740)+px(920),"70")
tx(X0+px(1700),YT2+px(1815),"KAPALI PANEL — CAM YOK, mutfak GİZLİ · tek açıklık: QR teslim + kiosk",10,"middle","","#555")
ox(X0+px(T)+28,YT2,YT2+px(840),"84")
ox(X0+px(T)+28,YT2+px(840),YT2+px(1740),"90 koridor")

# SERVIS ISTASYONU — ayri duvarda (makineden bagimsiz)
sv=X0+px(T)+px(260); sw=px(700)
rc(sv,Y0,sw,px(HG),2.2,5)
rc(sv+12,YT,12,px(HA),1.4); rc(sv+sw-24,YT,12,px(HA),1.4)
ln(sv+8,Y0+px(300),sv+sw-8,Y0+px(300),1.4)
rc(sv+px(40),Y0+px(60),px(200),px(160),1.4,3)                       # mini UPS
tx(sv+px(140),Y0+px(155),"UPS",10,"middle","bold")
rc(sv+px(300),Y0+px(90),px(320),px(110),1.4,20)                     # yangin tupu (yatik)
ci(sv+px(300)+px(20),Y0+px(145),px(28),1.1)
not_(sv+sw/2,Y0+px(272),"TEKNİK: mini UPS · yangın tüpü · priz")
ln(sv+8,Y0+px(780),sv+sw-8,Y0+px(780),1)                            # kutu raf 1/2
ln(sv+8,Y0+px(1260),sv+sw-8,Y0+px(1260),1.4)
for rrr in (330,810):
    for i in range(6): ln(sv+px(80)+i*px(95),Y0+px(rrr),sv+px(80)+i*px(95),Y0+px(rrr)+px(400),.8)
not_(sv+sw/2,Y0+px(322),"AMBALAJ: dikine kutu 2 raf × 160 = 320")
rc(sv+px(60),Y0+px(1320),sw-px(120),px(360),1.4,4)                  # temizlik kilitli
ci(sv+px(120),Y0+px(1360),px(16),1.1)
for i in range(4): rc(sv+px(110)+i*px(130),Y0+px(1450),px(90),px(190),1,3)
not_(sv+sw/2,Y0+px(1305),"TEMİZLİK — kilitli (kimyasal gıdadan ayrı) · mop kapı içinde")
rc(sv+px(40),Y0+px(1720),sw-px(80),px(100),1.4,3)                   # poset cekmecesi
ln(sv+sw/2-20,Y0+px(1770),sv+sw/2+20,Y0+px(1770),2)
not_(sv+sw/2,Y0+px(1708),"poşet + çöp poşedi çekmecesi")
oy(sv,sv+sw,Y0-24,"70")
tx(sv+sw/2,YZ+24,"SERVİS İSTASYONU",11.5,"middle","bold")
tx(sv+sw/2,YZ+42,"(hattın KARŞISINDA — koridorun öbür yakası, kesimin karşısı)",9.5,"middle","","#555")
...
tx(X0,Y0-70,"AUTOKITCH — HAT v36 · 3 GÜNLÜK DÜZ HAT · SERVİS İSTASYONU KARŞI SIRADA · KAPALI MUTFAK (cam yok)",15,"start","bold")
tx(X0,Y0-32,"HESAP: 80 pide × %50 katılım = 40 içecek/gün × 7 = 280/hafta → kutu 255 (kola 112 · ayran 70 · su 34 · fanta 22 · sprite 17 → kanal: 10·7·3·2·2 + 1 yedek) + 1L 25 · tatlı %8 → 45/hafta",10.5,"start","","#555")
tx(X0,Y0-50,"ölçüler cm · 72s STRESİ YOK: her şey DONMUŞ gelir — SOL komple −18 (240 top + porsiyon küvetleri) · SAĞ: çözülme tamponu 100 + içecek/tatlı 4×7 + 1L · fırıncı 3 günde bir · et/peynir/kavurma + içecek HAFTALIK",10.5,"start")


# ================= YAN GORUNUS =================
sv2=X0+px(T)+px(1300)
tx(sv2+px(900),Y0-44,"YAN GÖRÜNÜŞ (sağdan)",13,"middle","bold")
ln(sv2-14,Y0-20,sv2-14,YZ+2,4)                                    # dukkan duvari
rc(sv2+2,Y0,px(840)-4,px(HG),2.2,5)                               # kabin kesiti
rc(sv2+14,YT,12,px(HA),1.4); rc(sv2+px(840)-26,YT,12,px(HA),1.4)
for r in range(6): ln(sv2+px(70),Y0+px(340)+r*px(240),sv2+px(770),Y0+px(340)+r*px(240),.7)
not_(sv2+px(420),Y0+px(200),"kabin kesiti — raflar temsilî")
tx(sv2+px(420),YZ+24,"KABİN (sırtı duvara)",11.5,"middle","bold")
k0=sv2+px(840); k1=k0+px(950)
ln(sv2-14,YZ,k1+px(760),YZ,2)                                     # zemin
rc(k0+px(380),YZ-px(150),px(190),px(150),1.6,2)                   # ray+araba yandan
ln(k0+px(475),YZ-px(150),k0+px(475),YZ-px(1250),1.8)              # kol govdesi
ci(k0+px(475),YZ-px(1250),px(60),1.4)                             # omuz eklemi
ln(k0+px(475),YZ-px(1250),k0+px(790),YZ-px(1500),1.8)             # ust kol
not_(k0+px(475),YZ-px(60),"ray + araba")
tx(k0+px(475),YZ+24,"ROBOT KORİDORU",11.5,"middle","bold")
rc(k1+2,Y0,px(840)-4,px(HG),2.2,5)                                # SERVIS IST. kesiti (karsi sira)
rc(k1+14,YT,12,px(HA),1.4); rc(k1+px(840)-26,YT,12,px(HA),1.4)
for r in range(4): ln(k1+px(70),Y0+px(340)+r*px(400),k1+px(770),Y0+px(340)+r*px(400),.7)
not_(k1+px(420),Y0+px(200),"servis ist. kesiti")
tx(k1+px(420),YZ+24,"SERVİS İST. (karşı sıra)",11.5,"middle","bold")
tx(k1+px(880),Y0+px(480),"servis olmayan bölümlerde:",9.5,"start","","#555")
tx(k1+px(880),Y0+px(560),"KAPALI PANEL (cam yok)",9.5,"start","bold")
tx(k1+px(880),Y0+px(700),"müşteri tarafı →",9.5,"start","","#555")
oy(sv2,k0,YZ+56,"84"); oy(k0,k1,YZ+56,"95"); oy(k1,k1+px(840),YZ+56,"84")
ox(sv2-40,Y0,YZ,"197")
tx(sv2+px(1330),YZ+92,"kesit servis istasyonu hizasından — toplam derinlik 84+95+84 = 263 · tek açıklık: QR teslim (hat ucunda)",9.5,"middle","","#555")

W=int(X0+px(T)+px(4550)); H=int(YT2+px(2760))
svg=(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}">'
     f'<rect width="{W}" height="{H}" fill="#ffffff"/>'+''.join(E)+'</svg>')
OUT=r"C:\Users\Kemal\Desktop\Kemal\WEBSİTE\AUTOKITCH\arastirma\hat_teknik_3gun_v36.svg"
