# -*- coding: utf-8 -*-
"""AUTOKITCH - BASE HAT TEKNIK RESIM v7 (13 Eyl 2026): ON + UST (PLAN KESITI) + YAN (STORE KESITI). Olculer mm.

v6'dan FARKI (Kemal, 13 Eyl: "830 indir hepsini"):
  TUM ISTASYONLAR 830 DERIN (Oztiryakiler 79K4.12NMV.10 dik dolap: 1344 x 830 x 2000).
  PRESS v6   : Fersah PZP-400 (800 derin) 30 mm one -> z -788..+12 ; arka 0,5 · on ince fuga lamasina (z 14) 2
               on cerceve dikmesi pres bolmesinde ince lamaya doner, bas-ac mandali presin kaidesinin ustune cikar
  PACK v4    : sarjor + kalip + kutu + plunger + dikey tahrik ekseni 30 mm one
               blank z -785,5..-25,5 (on 3,5 pay) · kutu z -410..-90 · tahrik ekseni z -755
  OVEN v5    : arka duvar panosu 30 mm one (planda gorunmez) · firin govdesi ve buhar kanali ayni yerde
  TOPPING v5 : 12 Eyl 830 plani (kaset z -695,5..-15,5). ARKA SOKET-MOTOR PAKETI VE EVAPORATOR SIGMIYOR -> ACIK MADDE
  STORE v5   : degismedi (zaten 830)
  HAT 4885 x 1970 x 830.
KAYNAK: teknik_hat_base_v6.py basligi + 13 Eyl derinlik analizi (sw_press5 / sw_oven4 / sw_pack3 / sw_topping4 koordinatlari).
Kural: paftada yalniz gorunus + olcu + parca adi; aciklama mesajda.
"""
import os, math
from PIL import Image, ImageDraw, ImageFont

OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\HAT_BASE_v7_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 4300, 2800
S = 0.62
...
ISTASYON = [("STORE v5", WS, DZ), ("PRESS v6", 700.0, DZ), ("TOPPING v5", 700.0, DZ),
            ("OVEN v5", 700.0, DZ), ("PACK v4", 700.0, DZ)]
HAT = sum(w for _, w, _ in ISTASYON)                   # 4885

# cephe: (tip k=kapak / a=acik agiz, x0, x1, y0, y1, ad, kaset yuvasi, etiket kotu) — derinlik degisikligi cepheyi degistirmez
CEPHE = {
    ...
    "TOPPING v5": [("k", 33.0, 667.0, 210.0, 489.0, "", "alt2", None),
...
# ======================= BASLIK =======================
txt(OX, 70, "AUTOKITCH  ·  BASE HAT  ·  TEKNİK RESİM  v7", f38, INK)
txt(OX, 138, "ön · üst · yan görünüş  ·  tüm istasyonlar 830 derin  ·  ölçüler mm  ·  13 Eylül 2026", f13, GRAY)
...
txt(W_PX - 170, ly, "HAT %s × 1970 × 830  ·  STORE 32 çekmece  ·  TOPPING 14 yuva  ·  OVEN 3 hazne" % sayi(HAT), f9, GRAY, "rm")

assert not os.path.exists(OUT), "v7 zaten var — yeni numara ver"
os.makedirs(os.path.dirname(OUT), exist_ok=True)
im.save(OUT)
print("yazildi:", OUT, "· hat %s mm · derinlik %s" % (sayi(HAT), sayi(DZ)))
