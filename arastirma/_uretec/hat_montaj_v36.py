...
ÇIKTI: otonom/hat3d/hat_v36.glb + .usdz  ·  otonom/hat3d/durum.json (sayfa oradan okur)  ·  FULL_MAKINE/HAT_v36_YERLESIM.step (SolidWorks)
...
if __name__ == "__main__":
    ...
    b1 = glb_yaz(os.path.join(OUT, "hat_v36.glb"), parcalar, dokular)
    print("hat_v36.glb · %d birim · %.0f KB" % (len(parcalar), b1 / 1024.0))
    b2, prim, sorun, _u = usdz_yaz([os.path.join(OUT, "hat_v36.usdz")], "hat_v36", parcalar, dokular)
    print("hat_v36.usdz · %.0f KB · %d prim · USD denetimi: %s" % (b2 / 1024.0, prim, "GECTI" if not sorun else "KALDI"))
    for x in sorun: print("   HATA:", x)
    os.makedirs(STEP, exist_ok=True); asm.save(os.path.join(STEP, "HAT_v36_YERLESIM.step"))
    print("HAT_v36_YERLESIM.step · SolidWorks'te montaj olarak acilir · %s" % STEP)
    ...
    with io.open(os.path.join(OUT, "durum.json"), "w", encoding="utf-8") as f:
        json.dump(dict(hat=dict(w=HAT_W, h=H_MAK, d=DZ, pafta="HAT_ATOSA_TABLALI_v7 · v36 · B = store_cad_v1 · taban hizasi · surec 1168"), sayac=sayac, modul=[dict(kod=k, ad=a, x=[x, x + w], y=list(MODUL_Y[k])) for k, a, x, w in MODUL],
