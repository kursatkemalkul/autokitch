# -*- coding: utf-8 -*-
"""AUTOKITCH - HAT v11 (ilk cizime sadik: cekmeceler istasyon altlarinda, onde YER RAYI + TEK ROBOT) (16 Eyl 2026): iki robot, bant yok, 3 gozlu kaviteli firin (2 lahmacun alt alta + 1 pide).
...
OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\HAT_ROBOT_v11_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 5200, 2960
...
R1X, R2X = 760.0, 3900.0                  # arabanin gosterildigi yer / 2. robot yeri
OMUZ = 1400.0
ERISIM = 1900.0
...
def on_gorunus(OX, ZY, S):
    ...
    for ad, w in MOD:
        ...
        txt(px(x0 + w / 2), pz(460.0), ad, f11, INK, "mm")
    ...
    # --- robotlar
    # --- ray yalniz cizgi olarak (gercek yeri yan gorunuste)
    dline((px(RAY0), pz(170.0)), (px(RAY1), pz(170.0)), ACC, 3, 18, 10)
    txt(px((RAY0 + RAY1) / 2), pz(90.0), "YER RAYI · STROK 4985  (hattın önünde — yan görünüşe bak)", f9, ACC, "mm")
    ok(px(R1X + 500), pz(300.0), px(RAY1 - 60), pz(300.0), ACC, 3)
    ok(px(R1X - 100), pz(300.0), px(RAY0 + 60), pz(300.0), ACC, 3)
    # robot silueti (onde durdugu icin yalniz kontur)
    cx = R1X
    d.ellipse([px(cx) - 22, pz(OMUZ) - 22, px(cx) + 22, pz(OMUZ) + 22], fill=ACC, outline=INK, width=2)
    hedef = (X0["PRESS"] + W["PRESS"] / 2, HB)
    mx = (cx + hedef[0]) / 2
    d.line([(px(cx), pz(OMUZ)), (px(mx), pz(OMUZ + 420))], fill=ACC, width=9)
    d.line([(px(mx), pz(OMUZ + 420)), (px(hedef[0]), pz(hedef[1] + 110))], fill=ACC, width=9)
    d.line([(px(hedef[0]), pz(hedef[1] + 110)), (px(hedef[0]), pz(hedef[1] + 30))], fill=ACC, width=6)
    txt(px(cx), pz(1985.0), "ROBOT · FR5 (922) · ray üstünde", f11, ACC, "mm")
    dcircle(px(cx), pz(OMUZ), 922 * S, ERIS, 2)
    d.ellipse([px(R2X) - 20, pz(OMUZ) - 20, px(R2X) + 20, pz(OMUZ) + 20], fill=BG, outline=GRAY, width=2)
    txt(px(R2X), pz(1985.0), "2. ROBOT YERİ (sonradan)", f9, GRAY, "mm")
...
def yan_gorunus(OX, ZY, S):
    """kesit: makine 830 derin + koridor 800 + yer rayi + robot"""
    def py_(y):
        return OX + y * S          # y: makine on yuzunden koridora dogru (0 = on yuz)

    def pz(z):
        return ZY - z * S

    # makine kesiti (arkada)
    d.rectangle([py_(-DER), pz(H_MAK), py_(0), pz(0)], fill=FILL, outline=LINE, width=2)
    txt(py_(-DER / 2), pz(950.0), "MAKİNE", f11, INK, "mm")
    d.rectangle([py_(-DER + 40), pz(BANT1), py_(-40), pz(BANT0)], fill=SOGUK, outline=DOLAP, width=1)
    txt(py_(-DER / 2), pz((BANT0 + BANT1) / 2), "ÇEKMECE", f8, DOLAP, "mm")
    # zemin
    d.line([(py_(-DER) - 40, pz(0)), (py_(900.0), pz(0))], fill=INK, width=3)
    # ray + araba + kaide + robot
    d.rectangle([py_(330.0), pz(150.0), py_(650.0), pz(0)], fill=ACC, outline=INK, width=2)
    txt(py_(490.0), pz(75.0), "RAY", f8, BG, "mm")
    d.rectangle([py_(300.0), pz(360.0), py_(680.0), pz(150.0)], fill=SOFT, outline=INK, width=2)
    txt(py_(490.0), pz(255.0), "ARABA", f8, GRAY, "mm")
    d.rectangle([py_(400.0), pz(OMUZ - 60), py_(580.0), pz(360.0)], fill=SOFT, outline=INK, width=2)
    txt(py_(490.0) + 90, pz(880.0), "KAİDE 1040", f8, GRAY, "lm")
    d.ellipse([py_(490.0) - 20, pz(OMUZ) - 20, py_(490.0) + 20, pz(OMUZ) + 20], fill=ACC, outline=INK, width=2)
    d.line([(py_(490.0), pz(OMUZ)), (py_(200.0), pz(OMUZ + 330))], fill=ACC, width=9)
    d.line([(py_(200.0), pz(OMUZ + 330)), (py_(-120.0), pz(1200.0))], fill=ACC, width=9)
    dcircle(py_(490.0), pz(OMUZ), 922 * S, ERIS, 2)
    olcu_h(py_(-DER), py_(0), pz(0) + 46, "830")
    olcu_h(py_(0), py_(800.0), pz(0) + 100, "KORİDOR 800")
    olcu_v(py_(900.0) + 40, pz(H_MAK), pz(0), "1915")
    olcu_v(py_(-DER) - 60, pz(OMUZ), pz(0), "1400", yon="l")
    txt(py_(-DER) - 60, pz(H_MAK) + 60, "YAN GÖRÜNÜŞ", f16, ACC, "la")
    ok(py_(-DER - 520), pz(1300.0), py_(-DER - 60), pz(1300.0), INK, 3)
    txt(py_(-DER - 560), pz(1300.0), "MÜŞTERİ", f9, INK, "rm")


# ======================= SAYFA =======================
txt(210, 92, "AUTOKITCH · HAT v11 · YER RAYI + TEK ROBOT · 3 GÖZLÜ FIRIN", f34, INK)
txt(210, 152, "ön görünüş + üst görünüş  ·  bant yok  ·  2 günlük stok  ·  2. robot için yer ayrıldı  ·  ölçüler mm  ·  16 Eylül 2026", f13, GRAY)

on_gorunus(620, 1560, 0.52)
yan_gorunus(4400, 1560, 0.52)
ust_gorunus(620, 2380, 0.42)
