...
import topping_hesap_v6 as TH, topping_cad_v24 as TC                                      # v48: tekne 2500'de biter                                      # TOPPING modülü: derinlik dizilimi ve yuva konumları ORADAN okunur
...
birim("TOPPING_MODUL", "TOPPING v2 (UNO'lu): 4 UNO çekirdeği (sos · harç · kıyma · kuşbaşı) + kaşar ve küp sucuk kaseti + hava tesisatı · tabla mekanizması v1'den", "C", "GERCEK_MODUL",
      (X_BC, X_BC + W_BC), (H_B, H_B + TC.Y), (-DZ, 0.0), "sac", "topping_uno_cad_v8.py + topping_cad_v24.py", "hat/topping_v2.html")
birim("HAVA_KOMPRESOR", "Kompresör JUN-AIR OF302-15B (yağsız, 15 L, 25 kg) · FIRIN ÜSTÜNDE (havalandırmalı raf 1516, ortam sınırı 40 °C, davlumbaz bölmesinin ön yarısı, kutu yedeğinin yanı — TOPPING'in tek istisnası) · Ø10 ana hat fırın üstünden TOPPING sağ dış sac rakoruna (y 2000 / z −415) → teknik cep → kuru bölme → şartlandırıcı · K'ye dal", "D", "GERCEK_HAVA",
      (3600.0, 3980.0), (1516.0, 2026.0), (-420.0, -40.0), "sac", "topping_uno_cad_v8.py", "hat/topping_v2.html")
# ---- v42 · TOPPING v2 parçaları ----
import importlib.util as _ilu
_sp = _ilu.spec_from_file_location("TU8", os.path.join(os.path.dirname(os.path.abspath(__file__)), "topping_uno_cad_v8.py"))   # v50: fitil boşluğu   # v49: katalog yay/pim, haç 7,0   # v48: kaset yuvaları + yalıtım bloğu + fitil   # v45: kama yarık + dönen gruplar
...
import firin_tp10_cad_v4 as FT                                                          # v49: fırın v4 (havalandırmalı raf: 40 mm takoz + ışınım kalkanı)
import itici_cad_v2 as IT                                                              # v49: aktarma iticisi + destek plakası (dünya koordinatı)
...
KOMP_KAY = (-510.0, 976.0, 0.0)               # v48: kompresör (TU x 3410–3790 · y 520–1030 · z −40…−420) → fırın üstü raf: dünya x 3600–3980 · y 1481–1991 · z aynı
...
birim("D_TABAN_KABIN", "F taban dolabı 123–956 (fırın gövdesi bant 1166 için 956'ya iner — istasyon tabanı 1060 kuralının F istisnası, Kemal 26 Eyl) · önde bulaşık + temizlik + pizza kutusu yedeği · arkada deterjan + robot kontrol + ana pano + UPS", "D", "KUTU", (X_D, X_D + W_D), (Y_ALT, FT.YG0), (-DZ, 0.0), "kabin", "v44 alt kısım v3 · v48 üstü 956")
...
for kod_, ad_, (a_, b_), (y0_, y1_), (z0_, z1_), kat_, kay_ in (
        ...
        ("D_TEMIZLIK", "Temizlik · 2 × 5 L bidon (deterjan / dezenfektan) + üstte bez · eldiven · poşet · önde", (510.0, 656.0), (130.0, 950.0), (-210.0, -20.0), False, "v48: üstü 950 · B'nin K4 nişinden taşındı · bidon 130 × 190 × 290 VARSAYIM"),
        ...
        ("D_PIZZA_YEDEK", "Pizza kutusu yedeği · dolapta 505 kutu düz (804 × 404 × 808 + taban 15) + fırın üstünde 55 = 560 · şarjör 567 + 560 = 1127 = 4 gün (kural 5.5) · önde", (666.0, 1470.0), (130.0, 953.0), (-424.0, -20.0), False, "kutu 1,6 mm (kutu_cad_v3) · 1,8 mm olursa 517"),
...
birim("D_PIZZA_YEDEK_UST", "Pizza kutusu yedeği · fırın üstü rafta 55 kutu düz (804 × 404 × 88) · dolaptaki 505 ile 560 = 2 gün", "D", "KUTU", (X_D + 20.0, X_D + 824.0), (1516.0, 1604.0), (-424.0, -20.0), "kutu", "v48 · kural 5.5 · v49 raf 1516")
...
for _k, _a in IT.BIRIMLER:
    ...
    birim(_k, _a, "C", "GERCEK_ITICI", (min(q.xmin for q in _bb), max(q.xmax for q in _bb)), (min(q.ymin for q in _bb), max(q.ymax for q in _bb)),
          (min(q.zmin for q in _bb), max(q.zmax for q in _bb)), "sac", "itici_cad_v2.py", "hat/oven.html#itici")
...
def yolculuk(parcalar):
    ...
    ADIM.append((T_F0, "FIRIN", "TP10 kesitli fırın, gövdesi 1500: bant gövde dışına çıkmaz, ısıtılan 1316 mm, aynı anda 4 ürün. Çıkışta K bandı üstündeki 20° çit ürünü hat eksenine (−170) geri alır. Animasyonda HIZLANDIRILMIŞ: gerçekte 3,5 dk, burada 10 s."))
...
if __name__ == "__main__":
    ...
    for k_ in ("A_KABIN", "TOPPING_MODUL"):
    ...
    print("ANA MONTAJ ANIMASYONU (v50): tek tam dongu %.0f sn · %d kanal · %d adim · %d eksenli doner dugum" % (T_J, len(ANIM_HAT), len(ADIM_HAT), len(OZEL_T)))
    ...
    b1 = glb_yaz(os.path.join(OUT, "hat_v50.glb"), [x for x in parcalar if x[0] not in HARIC_T], dokular, ozel=OZEL + OZEL_T, liste=ANIM_HAT)   # v46: donenler kendi ekseninde
    ...
    print("hat_v50.glb · %d dugum · %.0f KB · animasyon %.0f sn" % (len(parcalar), b1 / 1024.0, T_J))
    _usd = [x for x in parcalar if not x[0].startswith("URUN__")]                         # ürün düğümleri yalnız animasyonda (USDZ durağan)
    b2, prim, sorun, _u = usdz_yaz([os.path.join(OUT, "hat_v50.usdz")], "hat_v50", _usd + E_USDZ, dokular)
    print("hat_v50.usdz · %.0f KB · %d prim · USD denetimi: %s" % (b2 / 1024.0, prim, "GECTI" if not sorun else "KALDI"))
    ...
    with io.open(os.path.join(OUT, "durum.json"), "w", encoding="utf-8") as f:
        json.dump(dict(hat=dict(w=HAT_W, h=H_MAK, d=DZ, pafta="HAT v50 (26 Eyl gece) · AKTARMA ITICISI itici_cad_v2 (SMC MY1B16-250 capraz 24,9°, pivotlu cubuk, sabit pimle kalkar, destek plakasi) · F = TP10 kesitli 1500 firin v4 (havalandirmali raf) · kompresor firin ustunde · kutu yedegi 505 + 55 · TOPPING v24 + v2 v8 (katalog yay/pim, hac 7,0, fitil boslugu) · K = kesme_cad_v1 · 1 tam animasyon · B = store_cad_v5 · E = kutu_cad_v3 · alt taban 123 · surec 1168", firin_denetim=FIRIN_OZET, itici_denetim=ITICI_OZET), sayac=sayac, animasyon=dict(sure=T_J, adim=ADIM_HAT), modul=[dict(kod=k, ad=a, x=[x, x + w], y=list(MODUL_Y[k])) for k, a, x, w in MODUL],
