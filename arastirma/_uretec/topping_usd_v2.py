# -*- coding: utf-8 -*-
"""TOPPING MODULU -> ISAAC SIM FIZIKLI USD (2. asama) — v2
...
CIKTI = os.path.join(GIR, "TOPPING_fizik_v2.usd")
...
def main():
    ...
    def eklem_kur(e):
        ...
        if e["tip"] == "prizmatik":
            ...
            tip_s = "linear"
            hedef_hiz = 0.0
            maxkuv = 2000.0                                             # N — GT3 kayis + NEMA23 siniri
        else:
            j = UsdPhysics.RevoluteJoint.Define(stage, yol)
            j.CreateAxisAttr(e["eksen"])
            tip_s = "angular"
            hedef_hiz = 0.0
            maxkuv = 6.0                                                # N·m — tasarim torku (hesaptan)
