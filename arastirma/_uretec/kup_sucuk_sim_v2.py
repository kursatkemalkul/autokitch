# -*- coding: utf-8 -*-
"""KÜP SUCUK DOZAJ SIMULASYONU — gercek kuplerle, tam makinede
...
# ---- HELEZON: SDF CARPISMA (v1'de 0 g cikmisti — kusku dogru cikarsa sebebi buydu)
# Helezon kanadi KONKAV. Konveks ayristirma helis bosluklarini DOLDURUP vidayi masif
# silindire cevirebilir; o zaman kupler kanadin arasina hic giremez ve hicbir sey akmaz.
# PhysX 5'in SDF (isaretli uzaklik alani) carpismasi tam bu is icin: donen konkav rijit
# cisim + cok sayida kucuk parca. Kaset ici parcalara SDF veriyoruz.
sdf_n = 0
for g in ("HELEZON_KUP_SUCUK", "KARISTIRICI_KUP_SUCUK"):
    gp = st.GetPrimAtPath(MY_ON + "/" + g)
    if not gp.IsValid(): print("  grup yok:", g); continue
    for c in gp.GetChildren():
        mc = UsdPhysics.MeshCollisionAPI(c)
        if not mc: continue
        mc.CreateApproximationAttr("sdf")
        sdf = PhysxSchema.PhysxSDFMeshCollisionAPI.Apply(c.GetPrim())
        sdf.CreateSdfResolutionAttr(256)          # 8 mm kup icin ~0,3 mm coznurluk
        sdf.CreateSdfMarginAttr(0.0005)
        sdf_n += 1
print("SDF carpismaya cevrilen parca: %d" % sdf_n)
...
if c > 5:
    print("   KOPRULEME         : YOK — akis surdu")
else:
    print("   AKIS YOK          : ya kopruleme var ya da SIM KURULUMU hatali.")
    print("                       Once helezonun SDF carpismasi dogru mu bakilacak; 0 g'i")
    print("                       tasarim bulgusu diye YAZMA.")
print("\nBITTI")
app.close()
