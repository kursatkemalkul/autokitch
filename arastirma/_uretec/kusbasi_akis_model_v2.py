# -*- coding: utf-8 -*-
"""AUTOKITCH · KUŞBAŞI AKIŞ MODELİ v2 (22 Eyl 2026) · ORTAK GÖVDE: rotor KISA kollu (lama duvara 19 mm, kanada 22 mm) · küp sucuk modeli bu dosyayı taban alır
(v1) — kaşar modelinin (kasar_akis_model_v2) kuşbaşı karşılığı.
...
G = dict(RT=36.0, C=2.0, R_MIL=8.0, R_KOK=14.0, T=4.0, P0=40.0, P1=50.48, L=304.0, W_IC=134.0,
         AGIZ=(48.0, 44.0), ESIK=6.0, TIKAC=12.0, ROTOR_BOSLUK=22.3, LAMA_DUVAR=19.3)
G["D"] = 2 * (G["RT"] - G["C"])
...
def degerlendir(d):
    ...
    return dict(d=d, m_kup=kup(d), n_kup=PORS / kup(d),
                ...
                lama=(G["LAMA_DUVAR"], k["aralik_buyuk"], G["LAMA_DUVAR"] <= k["aralik_kucuk"] or G["LAMA_DUVAR"] >= k["aralik_buyuk"]))     # ya küp giremesin ya rahat geçsin


def kapasite(z_oran, fi=None):
    fi = FI_S if fi is None else fi          # varsayılanı ÇAĞRI anında oku: sucuk modeli bu modülün kopyasında FI_S'yi değiştiriyor
...
def janssen(L=None, mu=None, K=0.5):
...
URUN = "KUSBASI"; JSON_ADI = "kusbasi_model.json"
...
def calistir():
    ...
    print("  d   kup g  adet/pide | bogaz>=4d   | R on / arka (CEMA 1,75·2,5·4,5) | hatve boslugu>=3d | agiz>=4d  | rotor-kanat>=1,5d | lama-duvar>=1,5d")
    S["tane"] = []
    for d in D_ARALIK:
    ...
    cik = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "otonom", "kaset3d", JSON_ADI)
