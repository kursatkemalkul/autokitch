# -*- coding: utf-8 -*-
"""TOPPING DIJITAL IKIZI — TAM SAHNE v2 (modul + GERCEK tepsi + pide · ROBOT YOK)

C:\\isaacsim\\python.bat topping_ikiz_v1.py

ICERIK
  · TOPPING_fizik_v10.usd        makinenin kendisi (534 parca, 14 eklem, gercek kutleler)
...
MAKINE = os.path.join(FIZ, "TOPPING_fizik_v10.usd")
CIKTI = os.path.join(FIZ, "TOPPING_IKIZ_v10.usd")
...
def puk(yol, r, kal, kg, renk, x, y_alt, z):
    ...
    xf.AddTranslateOp().Set(Gf.Vec3d(x, -z, y_alt))      # CAD (x,y,z) -> USD (x,-z,y)
    m = UsdGeom.Mesh.Define(stage, yol + "/ag")
    P, F, n = [], [], 64
    for s_ in (0, 1):
        z_ = kal if s_ else 0.0                 # Z-yukari: kalinlik Z'de
        for i in range(n):
            a = 2 * math.pi * i / n
            P.append(Gf.Vec3f(r * math.cos(a), r * math.sin(a), z_))
    for i in range(n):
        j = (i + 1) % n
        F += [[i, j, n + j], [i, n + j, n + i]]
...
# ---- ROBOT: v9'da YOK.
# Kemal: "robotu kaldir ordan suan robotla bir isimiz yok." UR10e sahneden cikarildi;
# tepsiyi getirip goturme isi simdilik modellenmiyor, TOPPING modulune odaklaniyoruz.
# (Geri istenirse topping_ikiz_v8.py'deki blok aynen geri konur.)
