...
ÇIKTI: otonom/hat3d/hat_v31.glb + .usdz  ·  otonom/hat3d/durum.json (sayfa oradan okur)  ·  FULL_MAKINE/HAT_v31_YERLESIM.step (SolidWorks)
...
if __name__ == "__main__":
    ...
    b1 = glb_yaz(os.path.join(OUT, "hat_v31.glb"), parcalar, dokular)
    print("hat_v31.glb · %d birim · %.0f KB" % (len(parcalar), b1 / 1024.0))
    b2, prim, sorun, _u = usdz_yaz([os.path.join(OUT, "hat_v31.usdz")], "hat_v31", parcalar, dokular)
    print("hat_v31.usdz · %.0f KB · %d prim · USD denetimi: %s" % (b2 / 1024.0, prim, "GECTI" if not sorun else "KALDI"))
    for x in sorun: print("   HATA:", x)
    os.makedirs(STEP, exist_ok=True); asm.save(os.path.join(STEP, "HAT_v31_YERLESIM.step"))
    print("HAT_v31_YERLESIM.step · SolidWorks'te montaj olarak acilir · %s" % STEP)
