...
KLİKLİ bayonet (tırnak + tümsek + cep + O-ring yayı) · gövde contası · muylu O-ringleri · tüp O-ringi + M4 insert. Önceki sürüm: kasar_cad_v8.py
...
ÇIKTI: otonom/kaset3d/kasar_v9.glb/.usdz + kasar_v9_dozaj.glb  ·  arastirma/3_TOPPING/kasar_kabi_v9/{step,stl}/ + MONTAJ.step + BOM.csv
"""
...
URETIM = os.path.join(KOK, "arastirma", "3_TOPPING", "kasar_kabi_v9")
...
if __name__ == "__main__":
 ...
 ("orumcek_arka", "gobek_karistirici", "örümcek ↔ göbek flanşı (1,0)"), ("orumcek_on", "on_kovan", "örümcek ↔ kovan flanşı (1,0)"),
    ...
    asm = cq.Assembly(name="KASAR_KABI_v9"); RENK = dict(cam=(0.75, 0.85, 0.92, 0.45), pom=(0.95, 0.95, 0.92, 1), celik=(0.75, 0.77, 0.8, 1), silikon=(0.16, 0.5, 0.95, 1), koyu=(0.15, 0.15, 0.17, 1))
    bom = []
    for p in PARCALAR:
    ...
    asm.save(os.path.join(URETIM, "KASAR_KABI_v9_MONTAJ.step"))
    ...
    b1 = glb_yaz(os.path.join(OUT, "kasar_v9.glb"), kap_ag, dokular)
    print("kasar_v9.glb · %d parca · %d ucgen · %.0f KB" % (len(kap_ag), sum(len(m.I) // 3 for _, m, _, _ in kap_ag), b1 / 1024.0))
    b2, prim, sorun, uyari = usdz_yaz([os.path.join(OUT, "kasar_v9.usdz")], "kasar_v9", [(a, m, mal) for a, m, mal, _ in kap_ag], dokular)
    ...
    b3 = glb_yaz(os.path.join(OUT, "kasar_v9_dozaj.glb"), doz, dokular)
    print("kasar_v9_dozaj.glb · %d parca · %.0f KB · toplam %.0f sn" % (len(doz), b3 / 1024.0, time.time() - t0))
