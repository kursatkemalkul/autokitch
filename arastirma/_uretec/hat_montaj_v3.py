...
ÇIKTI: otonom/hat3d/hat_v3.glb + .usdz  ·  otonom/hat3d/durum.json (sayfa oradan okur)  ·  FULL_MAKINE/HAT_v3_YERLESIM.step (SolidWorks)
...
def mal_ad(b, ton=None):
    ...
    if ton:
        ad += "__" + ton
    MALZEME.setdefault(ad, dict(MALZEME[ton or b["mal"]]))
    return ad
...
def glb_yaz(yol, parcalar, dokular):
    ...
    while off[0] % 4: blob.append(b"\x00"); off[0] += 1
    bb = b"".join(blob)
    g = {"asset": {"version": "2.0", "generator": "AUTOKITCH hat_montaj_v3"}, "scene": 0, "scenes": [{"nodes": list(range(len(nodes)))}], "nodes": nodes, "meshes": meshes,
...
if __name__ == "__main__":
    ...
    b1 = glb_yaz(os.path.join(OUT, "hat_v3.glb"), parcalar, dokular)
    print("hat_v3.glb · %d birim · %.0f KB" % (len(parcalar), b1 / 1024.0))
    b2, prim, sorun, _u = usdz_yaz([os.path.join(OUT, "hat_v3.usdz")], "hat_v3", parcalar, dokular)
    print("hat_v3.usdz · %.0f KB · %d prim · USD denetimi: %s" % (b2 / 1024.0, prim, "GECTI" if not sorun else "KALDI"))
    for x in sorun: print("   HATA:", x)
    os.makedirs(STEP, exist_ok=True); asm.save(os.path.join(STEP, "HAT_v3_YERLESIM.step"))
    print("HAT_v3_YERLESIM.step · SolidWorks'te montaj olarak acilir · %s" % STEP)
