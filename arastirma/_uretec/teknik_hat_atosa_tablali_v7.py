...
X_F, W_F, X_K, W_K, X_E, W_E = 2500.0, 1500.0, 4000.0, 600.0, 4600.0, 700.0
HAT = X_E + W_E                                              # 5300
...
def ciz_F(P, taban_parcalar):
    ...
    d.rectangle([fx(X_F + 50), fy(g1 - 50.0), fx(X_F + 50 + HAZNE), fy(g0 + 50.0)], fill=SICAK, outline=TURUNCU, width=2)
    txt(fx(X_F + W_F / 2), fy(g1 - 25.0), "taşyünü 50 · fan ve rezistanslar arka plenumda", f7, GRAY, "mm")
    txt(fx(X_F + W_F / 2), fy(P + 190.0), "ÜST PLENUM · hava parmakları × 7", f7, TURUNCU, "mm")
    txt(fx(X_F + W_F / 2), fy(P - 150.0), "ALT PLENUM · hava parmakları × 7", f7, TURUNCU, "mm")
    txt(fx(X_F + W_F / 2), fy(g0 + 72.0), "PİŞİRME HAZNESİ %s · bant 450 · aynı anda 4 ürün (adım %s) · pide / pizza / lahmacun · ayarlı sıcaklık + bant hızı" % (sayi(HAZNE), sayi(ADIM)), f7, TURUNCU, "mm")
    for i in range(7):
        xx = X_F + 50.0 + 100.0 + i * 200.0
        for yy0, yy1 in ((P + 60.0, P + 130.0), (P - 100.0, P - 30.0)):
            d.rectangle([fx(xx - 40), fy(yy1), fx(xx + 40), fy(yy0)], fill=BG, outline=TURUNCU, width=1)
    d.line([(fx(X_F - 40), fy(P)), (fx(X_F + W_F + 30), fy(P))], fill=TURUNCU, width=4)
    for i in range(4):
        xx = X_F + 50.0 + ADIM / 2 + i * ADIM
        d.ellipse([fx(xx - 150), fy(P + 28.0), fx(xx + 150), fy(P + 2.0)], fill=URUN, outline=(180, 140, 70), width=1)
    ...
    modul_etiketi(X_F, W_F, "MODÜL F · KONVEYÖR FIRIN", "1500 × 830 × 2030 · gövde %s–%s · bant kotu %s" % (sayi(g0), sayi(g1), sayi(P)))
...
def ciz_E(P):
    ...
    yig = m1 - m0 - 40.0
    satirlar(fx(X_E + 350), fy((m0 + m1) / 2), "KUTU ŞARJÖRÜ · alttan kaldırmalı|blank 400 × 760 yatay yığın · %s mm|= %d (1,8 mm) – %d (1,5 mm) kutu|4 GÜNLÜK YEDEK K MODÜLÜNDE" % (sayi(yig), int(yig / 1.8), int(yig / 1.5)), f8, INK)
...
def tablali():
    ...
    baslik("AUTOKITCH  ·  ATOSA TABLALI HAT  ·  TEKNİK RESİM  v7 · HD  ·  TOPPING = topping_cad_v22 · MONTAJ = hat_montaj_v34 · STROK 1987  ·  TABLA ARABASI AÇICI → DOZAJ → FIRIN  ·  HAT 5300 × 2030 × 830",
           "ön · üst · yan görünüş  ·  günde 80 pide + 200 lahmacun (+ pizza)  ·  2 gün stok  ·  her istasyon kapalı ürün  ·  ürün tepsisiz: hamur topu çalışma diskinde KONİLİ AÇICI ile açılır, dozajlanır, bıçak burunlu bant fırına çeker → kesme plakası → kutu  ·  robot yalnız hamur + kutu + içecek taşır  ·  TEK Fairino FR5 yer rayında · omuz 970 · ray ekseni hat yüzünden 360  ·  DÜKKÂNDA YALNIZ LAVABO: bulaşık makinesi, temizlik malzemesi ve 4 günlük kutu + içecek yedeği hattın içinde  ·  tabla kotları VARSAYIM (Atosa cevabı yok)  ·  ölçüler mm  ·  24 Eylül 2026")
    # A
    ...
    olcu_h(fx(X_A), fx(X_A + W_A), fy(H_MAK) - 26, sayi(W_A), f11, INK)
    ciz_B("TEMİZLİK MALZEMESİ|385 × 305 · deterjan · bez · eldiven · poşet")
    # C
    ...
    lejant()
    yol = KLASOR + r"\HAT_ATOSA_TABLALI_v7_HD.png"
    im.save(yol, dpi=(int(200 * K_HD), int(200 * K_HD))); print("yazildi:", yol, "· hat", sayi(HAT), "· bilek", round(D1), round(D2))


if __name__ == "__main__":
    hangi = sys.argv[1] if len(sys.argv) > 1 else "ikisi"
    if hangi in ("bant", "ikisi"):
        bantli()
    if hangi in ("tabla", "ikisi"):
        tablali()
