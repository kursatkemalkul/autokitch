# -*- coding: utf-8 -*-
"""AUTOKITCH - BASE · STORE + TOPPING · ON GORUNUS + PLAN KESITI + STOK/KUTLE TABLOSU (12 Eyl 2026)
...
OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\STORE_TOPPING_v1_on_plan_tablo.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 3000, 2760
S = 0.62
...
CAP = {"donmus":20,"hamur":20,"lahm":30,"icecek":63,"1L":42}
...
KG = {"donmus":(18.21, 6.00), "hamur":(17.39, 6.00), "lahm":(17.38, 4.90), "icecek":(21.58, 22.36), "1L":(26.64, 44.10)}
...
def on_store(ox, oy):
    ...
    txt((x0+x1)/2,y1-46,"STORE v5  ·  2740",f16,ACC,"md")
...
def on_topping(ox, oy):
    ...
    txt((x0+x1)/2,y1-46,"TOPPING v4  ·  700",f16,ACC,"md")
...
def plan_store(ox, oy):
    ...
    def P(x, z): return (ox + x*S, oy + (z+820)*S)       # z=-820 arka (ustte), z=+40 on (altta)
    x0,y0 = P(0,-820); x1,y1 = P(WS,40)
    d.rectangle([x0,y0,x1,y1],fill=FILL,outline=LINE,width=3)
    # PU yan + arka
    for (a,b) in ((1.5,61.5),(WS-61.5,WS-1.5)): d.rectangle([P(a,-818.5),P(b,-16)],fill=PUC,outline=GRAY,width=1)
    d.rectangle([P(61.5,-818.5),P(WS-61.5,-758.5)],fill=PUC,outline=GRAY,width=1)
    txt(*P(WS/2,-790),"PU 60 · arka",f7,GRAY,"mm")
    cx = XI
    for ki in range(4):
        if ki:
            bw = ST["bolme"][ki-1]; d.rectangle([P(cx-bw,-757.5),P(cx,-16)],fill=(INK if bw>=60 else SOFT),outline=LINE,width=1)
        # cekmece kutusu 620 x 700 (z -700..0) + on yuz 40
        d.rectangle([P(cx+16,-700),P(cx+WO-16,0)],fill=(255,255,255),outline=DOLAP,width=2)
        d.rectangle([P(cx-15,0),P(cx+WO+15,40)],fill=(255,255,255),outline=DOLAP,width=2)
        # raylar
        for rx in (cx+1, cx+WO-15): d.rectangle([P(rx,-660),P(rx+14,0)],fill=SOFT,outline=GRAY,width=1)
        # arka plenum: evaporator + fan + motor
        d.rectangle([P(cx+190,-753),P(cx+450,-708)],fill=(220,235,255),outline=BUZ,width=1); txt(*P(cx+320,-731),"EVAP",f7,BUZ,"mm")
        d.rectangle([P(cx+17,-745),P(cx+67,-709)],fill=(255,230,230),outline=RED,width=1); txt(*P(cx+42,-727),"M",f7,RED,"mm")
        txt(*P(cx+WO/2,-350),"K%d" % (ki+1),f11,GRAY,"mm")
        txt(*P(cx+WO/2,-120),"620 x 700",f7,GRAY,"mm")
        cx += WO + (ST["bolme"][ki] if ki<3 else 0)
    txt(*P(WS/2,-735+30),"ARKA PLENUM 57,5 · evaporator · fan · cekmece motoru",f7,GRAY,"mm")
    olcu(x0-40,y0,y1,"860",INK,f11,dik=True)
    txt(*P(WS/2, 40+70),"STORE v5 · PLAN KESITI (+3 kolon, cekmece seviyesi)",f11,ACC,"mm")
    return x1

def plan_topping(ox, oy):
    def P(x, z): return (ox + x*S, oy + (z+820)*S)
    x0,y0 = P(0,-820); x1,y1 = P(700,40)
    d.rectangle([x0,y0,x1,y1],fill=FILL,outline=LINE,width=3)
    for (a,b) in ((1.5,61.5),(638.5,698.5)): d.rectangle([P(a,-818.5),P(b,-16)],fill=PUC,outline=GRAY,width=1)
    d.rectangle([P(61.5,-818.5),P(638.5,-758.5)],fill=PUC,outline=GRAY,width=1)
    # 2 kaset (kat) 140 x 680 (z -700..-20)
    for xc in (270, 430):
        d.rectangle([P(xc-70,-700),P(xc+70,-20)],fill=(255,255,255),outline=INK,width=2)
        txt(*P(xc,-360),"KASET",f7,INK,"mm"); txt(*P(xc,-330),"140 x 680",f7,GRAY,"mm")
        d.rectangle([P(xc-35,-797),P(xc+35,-701)],fill=(255,230,230),outline=RED,width=1); txt(*P(xc,-749),"SOKET+M",f7,RED,"mm")
    ...
    txt(*P(350,40+70),"TOPPING v4 · PLAN KESITI (kat seviyesi, 2 dozaj kaseti)",f11,ACC,"mm")
    return x1
...
txt(W_PX-240,ly+17,"kabin 1970 x 840 · plint 120 · cekmece 620 x 700 · alin 33 · kaset 140 x 240 x 680",f8,GRAY,"rm")
