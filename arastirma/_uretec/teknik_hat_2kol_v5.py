...
OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\HAT_2KOL_v5_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 5300, 3300
S = 0.62
...
KOLON = [[(7, "hamur")], [(1, "hamur"), (6, "lahm")], [(6, "lahm")]]   # B: 20 hamur cekmecesi, 3 kolon (press + topping altinda)
KOLON_AD = ("K1", "K2", "K3")
W_B = W_A + W_BC                                            # 2100
H_B = 1000.0                                                # cekmece modulu yuksekligi (ust PU 940-1000)
BAND_UST = YUZ0 + 7 * (75.0 + 2 * BIND + FUGA) - FUGA        # K1 ustu 923,5
MOTOR = (1000.0, 1300.0)                                    # dolap motor ve diger (yalniz topping altinda)
assert BAND_UST < H_B - 60.0 + 50.0
# C · TOPPING (Atosa dozaj unitesi, tabla yok): agiz · basliklar · hazne sirasi · sogutma
T_AGZ, T_BAS, T_KAS, T_SOG = (1310.0, 1420.0), (1423.0, 1557.0), (1560.0, 1920.0), (1925.0, 1968.5)
...
# ======================= BASLIK =======================
txt(OX, 70, "AUTOKITCH  ·  HAT 2 KOL  ·  TEKNİK RESİM  v5  ·  5 LEGO MODÜL  ·  ÇEKMECE MODÜLÜ ALTTA, PRESS + TOPPING ÜSTÜNDE  ·  ATOSA DOZAJ TABLA YOK  ·  FIRIN ×3  ·  KUTU KATLAYAN", f38, INK)
txt(OX, 138, "ön · üst · yan görünüş  ·  günde 80 pide + 200 lahmacun (+ pizza)  ·  2 gün stok  ·  20 hamur çekmecesi tek modülde press + topping altında · 2 içecek çekmecesi kutu altında  ·  harç 2 kaset (kaşar kalıbı 280)  ·  2 × Fairino FR10 koridorda  ·  QR dolabı yok  ·  tüm modüller 830 derin  ·  ölçüler mm  ·  17 Eylül 2026", f13, GRAY)
...
kapak(X_BC, 33.0, 1367.0, MOTOR[0] + 3.0, MOTOR[1])
kesik(X_BC, 60.0, 460.0, MOTOR[0] + 15.0, MOTOR[1] - 15.0, "SOĞUTMA GRUBU 1 · çekmeceler", INK, "400 × 300 × 280 · 1/2 HP")
kesik(X_BC, 480.0, 880.0, MOTOR[0] + 15.0, MOTOR[1] - 15.0, "SOĞUTMA GRUBU 2 · hazne kabini", INK, "400 × 300 × 280 · 1/2 HP")
kesik(X_BC, 900.0, 1140.0, MOTOR[0] + 15.0, MOTOR[1] - 15.0, "HAZNE EVAPORATÖRÜ", INK, "240 × 200 + fan")
kesik(X_BC, 1160.0, 1350.0, MOTOR[0] + 15.0, MOTOR[1] - 15.0, "ÇEKMECE|SÜRÜCÜLERİ", INK, "20 motor kartı")
txt(fx(X_BC + W_BC / 2), fy(MOTOR[1]) - 14, "DOLAP MOTOR VE DİĞER · 300 · önden servis · hava giriş ön / çıkış arka", f7, INK, "mm")
...
for urun, gw, xc in YUVA:
    ...
    satirlar(fx(X_BC + xc), fy(1750.0) - 4, urun, f7, INK, 16)
    txt(fx(X_BC + xc), fy(1750.0) + 22, ("%s × 400 × 360" % sayi(gw)) if "SUCUK" not in urun else "180 × 400 · 14 çubuk", f7, GRAY, "mm")
txt(fx(X_BC + W_BC / 2), fy(T_KAS[1]) - 14, "HAZNE SIRASI · +3 °C · önden kaset takılır · kaset tabanı kot 1560", f7, INK, "mm")
kapak(X_BC, 33.0, 1367.0, T_SOG[0], T_SOG[1], "soğuk hava kanalı 45")
modul_etiketi(X_BC, W_BC, H_B, 1970.0, "MODÜL C · MOTOR BANDI + TOPPING", "1400 × 830 × 970 · B üstünde")
...
txt(fx(X_A + W_B / 2), fy(0) + 190, "MODÜL B · ÇEKMECE MODÜLÜ · 20 hamur çekmecesi · 3 kolon · A ve C bunun üstüne oturur", f8, BG, "mm")
txt(fx(X_A + W_B / 2), fy(0) + 210, "2100 × 830 × 1000", f7, BG, "mm")
olcu_v(fx(0) - 44, fy(1970), fy(0), "1970", f11, INK, "l")
for yy in (120.0, H_B, MOTOR[1], T_AGZ[1], T_KAS[0], PLAKA, T_KAS[1], PZP[1], 1970.0):
...
txt(fx(X_A + 350), py(-660.0), "altta: K1 · TAZE PİDE × 7 (B)", f7, DOLAP, "mm")
# B/C plan
...
txt(SX, FY_TOP - 152, "K2 kolonundan: MODÜL B çekmeceler (alt) + MODÜL C motor bandı ve Atosa dozaj (üst) · robot koridorda", f9, GRAY)
...
txt(sz(-340), fy(560.0) - 10, "ÇEKMECE 680 · K2: PİDE × 1 + LAHMACUN × 6", f8, DOLAP, "mm")
...
d.rectangle([sz(-728.5), fy(MOTOR[1]), sz(0.0), fy(MOTOR[0] + 1.5)], fill=SOFT, outline=LINE, width=1)
drect(sz(-700.0), fy(MOTOR[1] - 15), sz(-400.0), fy(MOTOR[0] + 15), INK, 1)
txt(sz(-550.0), fy(1150.0) - 8, "SOĞUTMA GRUBU", f7, INK, "mm")
txt(sz(-550.0), fy(1150.0) + 8, "300 derin · 280 yüksek", f7, GRAY, "mm")
txt(sz(-200.0), fy(1150.0), "hava giriş ön", f7, GRAY, "mm")
...
txt(sz(-660.0), fy(1390.0), "AĞIZ", f7, RED, "mm")
d.rectangle([sz(-470.0), fy(T_KAS[1]), sz(-70.0), fy(T_KAS[0])], fill=BG, outline=INK, width=2)
txt(sz(-270.0), fy(1740.0) - 10, "KASET 280 × 400 × 360 (harç)", f7, INK, "mm")
txt(sz(-270.0), fy(1740.0) + 10, "hazne kabini +3 °C", f7, GRAY, "mm")
d.rectangle([sz(-360.0), fy(T_BAS[1]), sz(-180.0), fy(T_BAS[0])], fill=SOFT, outline=INK, width=1)
txt(sz(-270.0), fy(1490.0), "NOZUL", f7, INK, "mm")
drect(sz(-740.0), fy(T_KAS[1]), sz(-480.0), fy(T_BAS[0]), GRAY, 1)
txt(sz(-610.0), fy(1690.0) - 8, "POMPA · MOTOR", f7, GRAY, "mm")
txt(sz(-610.0), fy(1690.0) + 8, "haznenin arkasında", f7, GRAY, "mm")
d.rectangle([sz(-420.0), fy(1376.0), sz(-120.0), fy(1364.0)], fill=ACC, outline=ACC)
txt(sz(-270.0), fy(1345.0), "TABAN Ø300 · tepside", f7, ACC, "mm")
d.rectangle([sz(-728.5), fy(1968.5), sz(0.0), fy(T_SOG[0])], fill=EVC, outline=BUZ, width=1)
txt(sz(-364), fy(1947.0), "soğuk hava kanalı", f7, BUZ, "mm")
...
for w_, c_ in ((16, BG), (9, ACC)):
    d.line([(sz(RZ), fy(OMUZ)), (sz(RZ + 140), fy(1620.0)), (sz(-60.0), fy(1380.0))], fill=c_, width=w_, joint="curve")
...
for yy in (120.0, H_B, MOTOR[1], T_AGZ[1], T_KAS[0], T_KAS[1], 1970.0):
...
PARCA = [
    ("MODÜL B", "ÇEKMECE modülü · 20 hamur çekmecesi · 3 kolon · A ve C altında", "1", "2100 × 830 × 1000 · K1 7 pide · K2 1 pide + 6 lahm · K3 6 lahm"),
    ("MODÜL A", "PRESS · Fersah PZP-400 tam makine, B üstünde", "1", "700 × 830 × 970 · 170 kg → B tavanı takviyeli"),
    ("MODÜL C", "MOTOR BANDI (2 soğutma grubu + hazne evaporatörü + sürücüler) + Atosa dozaj · tabla yok", "1", "1400 × 830 × 970 · B üstünde"),
    ...
    ("SOĞUTMA", "soğutma grubu 1/2 HP 400 × 300 × 280", "2", "C motor bandı · evaporatörler B arkası + bant içi"),
...
txt(W_PX - 170, ly, "HAT %s × 1970 × 830  ·  5 modül (B altta, A + C üstünde)  ·  2 gün  ·  22 çekmece (8 pide · 12 lahmacun · 2 içecek + tatlı)  ·  Atosa dozaj tablasız: 2 harç 280 + kıyma + kuşbaşı + kaşar 280 + sucuk dilimleyici  ·  FIRIN 3 göz  ·  2 × FR10  ·  QR dolabı yok" % sayi(HAT), f9, GRAY, "rm")
