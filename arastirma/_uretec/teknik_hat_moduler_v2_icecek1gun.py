# -*- coding: utf-8 -*-
"""AUTOKITCH - TABLALI HAT · MODULER v2 · ICECEK 1 GUN (14 Eyl 2026): ON + UST GORUNUS. Olculer mm.
Kemal: "bir gunluk icecek olsa ne olur?" -> icecek 70 kutu = 2 cekmece (56'lik); hamur ve yedek hazne seti 3 gun (degismedi).
MODULLER (moduler v1 ile ayni dizilim, uc uca hizali, ayak yok): STORE tam boy 655 | PRESS 670 | TOPPING 885 | OVEN 1420 | KESME 500 (alti TEKNIK) | PACK 700.
TAM ARAMA: tabla 1245 -> 1163 (yuvarlak 1165), yukseklik 1915 -> 1835, hat 4830 (degismedi).
  (topping 1310 duzeni: hat 4600, tabla 1337, yukseklik 2007 - Atosa yaninda olu ust alan)
DIZILIM (alttan uste): STORE raf1 · raf2 · 2 pide · 6 lahm | PRESS 7 pide | TOPPING raf3 · 3 pide | OVEN-A 1 icecek · 6 lahm | OVEN-B 1 icecek · 6 lahm.
Kural: paftada yalniz gorunus + olcu + parca adi; aciklama mesajda.
"""
import os, math
from PIL import Image, ImageDraw, ImageFont

OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\HAT_MODULER_v2_icecek1gun_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 3800, 2650
S = 0.62
...
KOLON = [("STORE", 17.5, 1660.0, [("raf", 1), ("raf", 2), (2, "pide"), (6, "lahm")]),
         ("PRESS", 25.0, HB - 360.0, [(7, "pide")]),
         ("TOPPING", 17.5, HB - 381.0, [("raf", 3), (3, "pide")]),
         ("OVEN", 72.5, HB - 400.0, [(1, "icecek"), (6, "lahm")]),
         ("OVEN", 727.5, HB - 400.0, [(1, "icecek"), (6, "lahm")])]
...
assert say == {"pide": 12, "lahm": 18, "icecek": 2}, say
...
txt(OX, 60, "AUTOKITCH  ·  TABLALI HAT  ·  MODÜLER  v2  ·  İÇECEK 1 GÜN", f38, INK)
txt(OX, 128, "ön görünüş + üst görünüş  ·  her istasyon ayrı dolap, birleşince üst/alt/ön/arka çizgiler hizalı  ·  ayak yok  ·  hat %s × %s × 830  ·  tabla / bant %s  ·  içecek 1 gün · hamur + yedek hazne 3 gün  ·  ölçüler mm  ·  14 Eylül 2026" % (sayi(HAT), sayi(H), sayi(HB)), f13, GRAY)
...
txt(fx(pk0 + 350), fy(560.0) - 11, "ŞARJÖR · ~%d blank" % int((HB - 140) / 2.13), f8, INK, "mm")
...
txt(W_PX - 150, ly + 34, "HAT %s × %s × 830 · 6 modül · pide 12 çekmece (240) · lahmacun 18 (600) · içecek 2 (70 · 1 gün) · yedek hazne rafı 3 · TEKNİK KESME altında" % (sayi(HAT), sayi(H)), f9, GRAY, "rm")

assert not os.path.exists(OUT), "moduler v2 zaten var — yeni numara ver"
im.save(OUT)
