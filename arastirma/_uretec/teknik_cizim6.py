...
for r in range(20):
    for kx in (e0+8, e0+8+px(320)+4): rc(kx,Y0+px(700)+r*px(45),px(320),px(45),1.05)
rc(e0+8,Y0+px(1660),px(320)*2+4,px(300),1.4)
for i in range(7): ln(e0+16,Y0+px(1690)+i*px(38),e0+196,Y0+px(1690)+i*px(38),.8)
not_(em,Y0+px(68),"piston + bıçak yıldızı Ø30")
not_(em,Y0+px(660),"kesim tablası Ø30")
not_(em,Y0+px(1628),"katlanmış: 2 cephe × 2 derin × 20 sıra = 80")
not_(em,Y0+px(1995),"açık yatay deste ≈ 65 · koli dükkân rafında")
...
tx(X0,Y0-70,"AUTOKITCH — HAT ÖN GÖRÜNÜŞÜ v6 · 80 KUTULUK ŞARJÖR",15,"start","bold")
tx(X0,Y0-50,"ölçüler cm · ORTAK derinlik 84 (belirleyen: GN dolap 83,5 + pres 80) · kutu şarjörü 2 cephe × 2 derin · pres CP-330 seçilirse 375",10.5,"start")

W=int(X0+px(T)+110); H=int(YZ+86)
svg=(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}">'
     f'<rect width="{W}" height="{H}" fill="#ffffff"/>'+''.join(E)+'</svg>')
OUT=r"C:\Users\Kemal\Desktop\Kemal\WEBSİTE\AUTOKITCH\arastirma\hat_on_gorunus_teknik_v6.svg"
