# -*- coding: utf-8 -*-
"""TOPPING MODULU -> ISAAC SIM FIZIKLI USD (2. asama) — v7 · Z-YUKARI

EKSEN DUZELTMESI (Kemal: "urunlerin yonu yanlis, yukari bakiyor"): bizim CAD Y-YUKARI
calisiyor, Isaac Sim / ROS / robot varliklari ise Z-YUKARI. Kutuphaneden gelen UR10e
Z-yukari geldigi icin makineyle uyumsuzdu. Artik USD yazarken donusum yapiliyor:
    CAD (x, y, z)  ->  USD (x, -z, y)          [X etrafinda +90 derece]
Yani CAD'in Y'si (yukari) USD'nin Z'si olur, CAD'in Z'si (derinlik) USD'nin -Y'si.
Eklem eksenleri de cevriliyor: tabla CAD-Y -> USD-Z · kaset milleri CAD-Z -> USD-Y.
...
_T0 = time.time()
def sure(m): print("   [%6.1f sn] %s" % (time.time() - _T0, m), flush=True)
import numpy as np
...
CIKTI = os.path.join(GIR, "TOPPING_fizik_v14.usd")

# ton -> (renk, metalik, puruz, OPAKLIK). Sitedeki paletle ayni; kaset gövdeleri SEFFAF.
MALZEME = {
    "cam":     ((0.70, 0.85, 0.95), 0.00, 0.10, 0.22),   # PC/PETG seffaf — kasetin govdesi
    "sac":     ((0.74, 0.77, 0.80), 0.85, 0.32, 1.00),
    "celik":   ((0.66, 0.69, 0.73), 0.80, 0.35, 1.00),
    "koyu":    ((0.22, 0.24, 0.27), 0.60, 0.45, 1.00),
    "pom":     ((0.90, 0.90, 0.88), 0.00, 0.55, 1.00),
    "silikon": ((0.95, 0.45, 0.35), 0.00, 0.70, 1.00),
    "pu":      ((0.93, 0.90, 0.78), 0.00, 0.85, 1.00),
    "bakir":   ((0.72, 0.45, 0.20), 0.90, 0.30, 1.00),
    "kart":    ((0.10, 0.45, 0.25), 0.10, 0.60, 1.00),
    "motor":   ((0.30, 0.32, 0.36), 0.70, 0.40, 1.00),
}
_MAT = {}


def malzeme(stage, ton):
    """her ton icin bir UsdPreviewSurface — seffaflik ve metaliklik dahil"""
    if ton in _MAT: return _MAT[ton]
    renk, met, ruf, opak = MALZEME.get(ton, ((0.6, 0.6, 0.6), 0.2, 0.5, 1.0))
    yol = "/World/Malzemeler/" + ton
    mat = UsdShade.Material.Define(stage, yol)
    sh = UsdShade.Shader.Define(stage, yol + "/yuzey")
    sh.CreateIdAttr("UsdPreviewSurface")
    sh.CreateInput("diffuseColor", Sdf.ValueTypeNames.Color3f).Set(Gf.Vec3f(*renk))
    sh.CreateInput("metallic", Sdf.ValueTypeNames.Float).Set(met)
    sh.CreateInput("roughness", Sdf.ValueTypeNames.Float).Set(ruf)
    sh.CreateInput("opacity", Sdf.ValueTypeNames.Float).Set(opak)
    if opak < 1.0:
...
RENK = {  # eski duz renk tablosu (yedek)
...
def main():
    ...
    P = M["parca"]
    print("okundu(kontrol): %d parca · %d eklem" % (len(P), len(M["eklem"])))
    sure("veri okundu")

    stage = Usd.Stage.CreateNew(CIKTI)
    UsdGeom.SetStageUpAxis(stage, UsdGeom.Tokens.z)      # Isaac/ROS standardi
    ...
    s_ = 6.0
    gp.CreatePointsAttr([Gf.Vec3f(-s_, -s_, 0), Gf.Vec3f(s_, -s_, 0), Gf.Vec3f(s_, s_, 0), Gf.Vec3f(-s_, s_, 0)])
    ...
    # Kemal: "zemini de kaldir". GORUNTUDEN kaldiriliyor ama CARPISMA yuzeyi kaliyor —
    # dusen bir parca sonsuza kadar dusmesin, olcum yapilabilsin diye. Gorunmez bir
    # dosemedir; sahnede gozukmez, fizikte durur.
    UsdGeom.Imageable(gp.GetPrim()).CreateVisibilityAttr("invisible")
    ...
    # v14 · EKLEMSIZ GRUP SERBEST CISIM OLMASIN.
    # topping_cad_v21 ile ACICI, KONI_ON, KONI_ARKA, BANT, BANT_BURUN, BANT_TAHRIK
    # gruplari geldi. Bunlarin site animasyonunda hareketi var ama FIZIK ikizinde
    # henuz eklem tanimi yok. "SABIT degilse rijit cisim" kurali onlari serbest
    # birakir ve yercekiminde DUSERLER. Eklemi olmayan grup STATIK sayilir.
    EKLEMLI = {e["grup"] for e in M["eklem"]}
    eklemsiz = sorted(g for g in gruplar if g != "SABIT" and g not in EKLEMLI)
    if eklemsiz:
        print("   EKLEMI OLMAYAN GRUPLAR STATIK yazildi (ikizde henuz kinematigi yok): %s" % ", ".join(eklemsiz))
    ...
    for g, idx in sorted(gruplar.items()):
        ...
        hareketli = (g != "SABIT") and (g in EKLEMLI)
        ...
        for i in idx:
            ...
            V = Z["V%d" % i]; F = Z["F%d" % i]
            V = np.stack([V[:, 0], -V[:, 2], V[:, 1]], axis=1)      # Y-yukari -> Z-yukari
            ...
            mesh.CreateDisplayColorAttr([Gf.Vec3f(*MALZEME.get(p["mal"], ((0.6, 0.6, 0.6),))[0])])
            UsdShade.MaterialBindingAPI(mesh.GetPrim()).Bind(malzeme(stage, p["mal"]))
    ...
    def eklem_kur(e):
        ...
        # EKSEN CEVIRME: CAD X->USD X · CAD Y->USD Z · CAD Z->USD -Y
        EKS = {"X": "X", "Y": "Z", "Z": "Y"}
        if e["tip"] == "prizmatik":
            j = UsdPhysics.PrismaticJoint.Define(stage, yol)
            j.CreateAxisAttr(EKS[e["eksen"]])
        ...
        pv = [float(pv[0]), -float(pv[2]), float(pv[1])]            # Y-yukari -> Z-yukari
        j.CreateLocalPos0Attr(Gf.Vec3f(*pv))
        j.CreateLocalPos1Attr(Gf.Vec3f(*pv))
    ...
    sure("sahne kuruldu, kaydediliyor")
