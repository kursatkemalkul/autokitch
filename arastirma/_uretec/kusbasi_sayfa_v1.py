# -*- coding: utf-8 -*-
"""otonom/kaset3d/akis_kusbasi.html + kusbasi_dagilim.png — SAYILAR kusbasi_model.json'dan (= kusbasi_akis_model_v1 çıktısı) okunur, elle yazılmaz."""
import io, json, math, os, random
import matplotlib; matplotlib.use("Agg")
import matplotlib.pyplot as plt
import kusbasi_akis_model_v1 as KM
...
S = json.load(io.open(os.path.join(K3, "kusbasi_model.json"), encoding="utf-8")); G = S["giris"]["G"]
...
for ax, d in zip(axs, (10.0, 12.0, 15.0)):
    ...
    for x, z in zip(xs, zs): ax.add_patch(plt.Rectangle((x - d / 2, z - d / 2), d, d, color="#8c2b28"))
...
plt.tight_layout(); plt.savefig(os.path.join(K3, "kusbasi_dagilim.png"), dpi=110, facecolor="#000"); plt.close()
...
<h2>1 · Ürün</h2>
<div class="kart"><div><b>145 g</b>pide başına · 20 pide/gün</div><div><b>5,8 kg</b>2 günlük = __LITRE__ L</div><div><b>__RHO__ g/mL</b>dökme yoğunluk <span class="v">VARSAYIM</span><br>1 cup çiğ küp et 170–198 g</div><div><b>μ __MU__</b>duvar sürtünmesi <span class="v">VARSAYIM</span><br>kas dokusu 0,29–0,36</div></div>
...
<img src="kusbasi_dagilim.png" alt="pide üstünde küpler">
...
rp = {"__LITRE__": tr(G["KG2"] / G["RHO"], 2) if False else tr(S["giris"]["KG2"] / S["giris"]["RHO"], 2), "__RHO__": tr(S["giris"]["RHO"], 2), "__MU__": tr(S["giris"]["MU"], 2),
...
io.open(os.path.join(K3, "akis_kusbasi.html"), "w", encoding="utf-8").write(H)
print("akis_kusbasi.html + kusbasi_dagilim.png · doluluk 0,60: %s g/tur · %s tur · %s dev/dk" % (tr(D0["g_tur"]), tr(D0["tur"], 2), tr(D0["rpm"])))
