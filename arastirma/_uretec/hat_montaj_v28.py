...
ÇIKTI: otonom/hat3d/hat_v28.glb + .usdz  ·  otonom/hat3d/durum.json (sayfa oradan okur)  ·  FULL_MAKINE/HAT_v28_YERLESIM.step (SolidWorks)
...
birim("ACICI_KAFA", "Konili döner açıcı kafası", "A", "KUTU", (X_A + 200.0, X_A + 500.0), (1200.0, 1420.0), (-320.0, -20.0), "katalog",
      "VARSAYIM ZARF: iki ters yönlü konili kafa, kendi redüktörlü motorlarıyla. Ölçü tedarikçiden gelince gerçekleşecek")
birim("ACICI_KOLON", "Açıcı kolonu + iniş ekseni", "A", "KUTU", (X_A + 300.0, X_A + 400.0), (1060.0, 1460.0), (-400.0, -330.0), "kutu",
      "kafayı taşır ve indirir; TOPPING'in rayı bunun altından geçer")
birim("ACICI_TABLA_KILIDI", "Tabla kilidi pimi", "A", "KUTU", (X_A + 330.0, X_A + 370.0), (1100.0, 1160.0), (-260.0, -220.0), "kutu",
...
if __name__ == "__main__":
    ...
    b1 = glb_yaz(os.path.join(OUT, "hat_v28.glb"), parcalar, dokular)
    print("hat_v28.glb · %d birim · %.0f KB" % (len(parcalar), b1 / 1024.0))
    b2, prim, sorun, _u = usdz_yaz([os.path.join(OUT, "hat_v28.usdz")], "hat_v28", parcalar, dokular)
    print("hat_v28.usdz · %.0f KB · %d prim · USD denetimi: %s" % (b2 / 1024.0, prim, "GECTI" if not sorun else "KALDI"))
    for x in sorun: print("   HATA:", x)
    os.makedirs(STEP, exist_ok=True); asm.save(os.path.join(STEP, "HAT_v28_YERLESIM.step"))
    print("HAT_v28_YERLESIM.step · SolidWorks'te montaj olarak acilir · %s" % STEP)
