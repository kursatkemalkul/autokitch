...
rc(a0+px(60),Y0+px(370),am-px(35)-a0-px(60),px(950),1)            # donmus ic (6 raf)
ln(a0+8,Y0+px(1360),am-px(35),Y0+px(1360),1.6)                    # kuru sarf ayirici
...
for r in range(6):                                                # 6 raf = plan 120 TAM
...
not_((a0+am)/2,Y0+px(345),"DONMUŞ −18° · 6 raf × 20 = 120 (plan TAM)")
not_((a0+am)/2,Y0+px(1385),"KURU SARF (izolesiz bölme) — saçet · peçete · yedek teneke")
...
not_((a2+a3)/2,Y0+px(638),"TAZE +3 °C · plan ≈120 — çukur hendeği 1 cm = pençe dalış payı")
...
for cy in (1515,1675):
    ...
    rc(a2+px(188),Y0+px(cy)+px(152),px(120),px(12),.9,2)          # 24V cekmece surucusu
    for i in range(5):
        rc(a2+px(50)+i*px(82),Y0+px(cy)+px(18),px(66),px(115),1)
for g in range(3): ln(a2+px(58)+4*px(82),Y0+px(1563)+g*px(28),a2+px(108)+4*px(82),Y0+px(1563)+g*px(28),.8)
tx(a2+px(50)+4*px(82)+px(33),Y0+px(1620),"T",11,"middle","bold")
rc(a2+px(468),Y0+px(1515),px(100),px(315),1.6,3)                  # 1L dik cekmece
rc(a2+px(476),Y0+px(1528),px(84),px(295),1)
not_((a2+a3)/2,Y0+px(1478),"İÇECEK: motorlu çekmece yalnız ÖN SIRA açığa çıkana dek açılır (~12 cm) — kol ÜSTTEN alır")
...
tx(X0,Y0-70,"AUTOKITCH — HAT v18 · SOĞUK DEPO 2 KAPI · 415 × 197 × 84 cm",15,"start","bold")
tx(X0,Y0-50,"ölçüler cm · MANTIKLI SAYILAR: ılık 8 · taze 120 · donmuş 120 (ısıtmalı pres sayesinde ılık küçük) · içecek 122 (su 0,33 kutu) · tatlı 20 · sol alt: kuru sarf · ayak 12",10.5,"start")

W=int(X0+px(T)+110); H=int(YT2+px(1860))
svg=(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}">'
     f'<rect width="{W}" height="{H}" fill="#ffffff"/>'+''.join(E)+'</svg>')
OUT=r"C:\Users\Kemal\Desktop\Kemal\WEBSİTE\AUTOKITCH\arastirma\hat_on_gorunus_teknik_v18.svg"
