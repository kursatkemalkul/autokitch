# -*- coding: utf-8 -*-
"""TP10 SÜRÜMÜ v1 (26 Eyl 2026) · AYRI SÜRÜM — ana makine v47 aynen durur. F = Sveba Dahlen TP10 (gerçek katalog fırını) +
...
# ---------------------------------------------------------------- v45 · 1 TAM ANİMASYON: ÜRÜN YOLCULUĞU (TP10 SÜRÜMÜ: yolculuk_tp10_v1) ----------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    print("ANA MONTAJ ANIMASYONU (TP10 SURUMU v1): tek tam dongu %.0f sn · %d kanal · %d adim · %d eksenli doner dugum" % (T_J, len(ANIM_HAT), len(ADIM_HAT), len(OZEL_T)))
    ...
    b1 = glb_yaz(os.path.join(OUT, "hat_tp10_v1.glb"), [x for x in parcalar if x[0] not in HARIC_T], dokular, ozel=OZEL + OZEL_T, liste=ANIM_HAT)   # v46: donenler kendi ekseninde
    ...
    print("hat_tp10_v1.glb · %d dugum · %.0f KB · animasyon %.0f sn" % (len(parcalar), b1 / 1024.0, T_J))
    _usd = [x for x in parcalar if not x[0].startswith("URUN__")]                         # ürün düğümleri yalnız animasyonda (USDZ durağan)
    b2, prim, sorun, _u = usdz_yaz([os.path.join(OUT, "hat_tp10_v1.usdz")], "hat_tp10_v1", _usd + E_USDZ, dokular)
    print("hat_tp10_v1.usdz · %.0f KB · %d prim · USD denetimi: %s" % (b2 / 1024.0, prim, "GECTI" if not sorun else "KALDI"))
    ...
    with io.open(os.path.join(OUT, "durum_tp10_v1.json"), "w", encoding="utf-8") as f:
        json.dump(dict(hat=dict(w=HAT_W, h=H_MAK, d=DZ, pafta="TP10 SURUMU v1 (ayri) · FIRIN_TP10 v2 · ana makine v47 + F = Sveba Dahlen TP10 + giris bandi + 2 cit · K bant girisi + itici motoru uyarlandi · K = kesme_cad_v1 (gercek) · 1 tam animasyon · alt kisim v3 · B = store_cad_v5 · kompresor K tabani arkasinda · TOPPING v2 UNO'lu (topping_uno_cad_v5) · E = kutu_cad_v3 · alt taban 123 · taban hizasi · surec 1168"), sayac=sayac, animasyon=dict(sure=T_J, adim=ADIM_HAT), modul=[dict(kod=k, ad=a, x=[x, x + w], y=list(MODUL_Y[k])) for k, a, x, w in MODUL],
    ...
    print("durum_tp10_v1.json yazildi · GERCEK %d / %d birim (%%%.0f) · toplam %.0f sn" % (g_, len(B), 100.0 * g_ / len(B), time.time() - t0))
