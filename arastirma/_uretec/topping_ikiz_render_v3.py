...
# KAMERA: rep.create.camera'nin pose'u bu surumde tutmadi (kare hep ayni acidan cikti).
# Duz bir USD kamerasi kurup render urunune ONU veriyoruz — konumu her karede biz yaziyoruz.
# Calisma bolgesi: modul on yuzu z=0 · tabla ekseni z=-170 · tepsi y=105 · kaset agzi y=160 mm.
KAM_YOL = "/World/KAMERA"
_kam = UsdGeom.Camera.Define(st, KAM_YOL)
_kam.CreateFocalLengthAttr(24.0)
_kam.CreateClippingRangeAttr(Gf.Vec2f(0.02, 50.0))
_kt = UsdGeom.Xformable(_kam.GetPrim())
_op_t = _kt.AddTranslateOp()
_op_r = _kt.AddRotateXYZOp()


def kamera_koy(hx):
    """arabayi takip et: 0,55 m onunde, 0,30 m yukarida, asagi-iceri bakar"""
    _op_t.Set(Gf.Vec3d(hx + 0.28, 0.42, 0.62))
    _op_r.Set(Gf.Vec3f(-22.0, 28.0, 0.0))


kamera_koy(0.9)
urun = rep.create.render_product(KAM_YOL, (1280, 720))
