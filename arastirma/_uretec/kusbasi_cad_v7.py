...
AUTOKITCH · KUŞBAŞI KASETİ v2 — ÜRETİM MODELİ (22 Eyl 2026) · ORTAK GÖVDE · kiyma_cad_v3'ten türetildi (yap_kusbasi_v7.py)
...
ÇIKTI: otonom/kaset3d/kusbasi_v7.glb/.usdz + kusbasi_v7_dozaj.glb · arastirma/3_TOPPING/kusbasi_kaseti_v7/
...
URETIM = os.path.join(KOK, "arastirma", "3_TOPPING", "kusbasi_kaseti_v7")
...
TUP_Z1 = 246.5
BORU_D, BORU_ET, BORU_ALT, BORU_GECIS = 52.0, 3.0, -10.0, 22.0      # yuvarlak çıkış borusu: iç çap · et · alt uç · tüpten boruya konik geçiş boyu
...
if __name__ == "__main__":
 ...
 ("orumcek_arka", "gobek_karistirici", "örümcek ↔ göbek flanşı (1,0)"), ("orumcek_on", "on_kovan", "örümcek ↔ kovan flanşı (1,0)"),
    ...
    b1 = glb_yaz(os.path.join(OUT, "kusbasi_v7.glb"), kap_ag, dokular)
    print("kusbasi_v7.glb · %d parca · %d ucgen · %.0f KB" % (len(kap_ag), sum(len(m.I) // 3 for _, m, _, _ in kap_ag), b1 / 1024.0))
    b2, prim, sorun, uyari = usdz_yaz([os.path.join(OUT, "kusbasi_v7.usdz")], "kusbasi_v7", [(a, m, mal) for a, m, mal, _ in kap_ag], dokular)
    ...
    b3 = glb_yaz(os.path.join(OUT, "kusbasi_v7_dozaj.glb"), doz, dokular)
    print("kusbasi_v7_dozaj.glb · %d parca · %.0f KB · toplam %.0f sn" % (len(doz), b3 / 1024.0, time.time() - t0))
