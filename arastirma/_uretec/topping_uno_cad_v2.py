# -*- coding: utf-8 -*-
"""TOPPING v2 (UNO'lu) · 3B MODEL · topping_uno_cad_v2 · 25 Eyl 2026
...
_yt = kut(90, W - 90, YAL_TABAN[0], YAL_TABAN[1], Z_KAPAK[1], Z_BOLME[1])
CEP = {}
for _ad, _cx, *_r in UNO:
    if _ad in BICAK:
        CEP[_ad] = (_cx - 30.0, _cx + BICAK[_ad]["boy"] + 12.0, YAL_TABAN[0] - 1, 1275.0, ZT + 28.0, ZT - 28.0)
        _yt = _yt.cut(kut(*CEP[_ad]))
ekle("yalitim_tabani", _yt, "pu", "Ö", not_="mevcut yalıtım tabanı 1183–1317 · sos ve harç bıçakları için alttan cep")
for _ad, (x0_, x1_, y0_, y1_, z0_, z1_) in CEP.items():
    ast = kut(x0_ - 1.5, x1_ + 1.5, YAL_TABAN[0], y1_ + 1.5, z0_ + 1.5, z1_ - 1.5).cut(kut(x0_, x1_, YAL_TABAN[0] - 1, y1_, z0_, z1_))
    ekle("bicak_cebi_astari_" + _ad.lower(), ast, "saydam_celik", "V", not_="bıçak cebi: 1 mm paslanmaz astar, yalıtım içinde · ölçü VARSAYIM")
...
for i, (ad, cx, hw, hust, d_sil, agiz, doz) in enumerate(UNO):
    ...
    if agiz == "yassi":
        ...
        # bıçak: üstte Ø36 boru ağzı (cx), altta x yönünde L × yarık düz ağız (cx … cx+L); iki yanı yassı sac, kıvrımlı sırt
        dis = loft_y(cember_y(cx, BICAK_UST, ZT, 18.0), dikdort_y(cx + L_ / 2.0, BICAK_ALT, ZT, L_ + 4.0, g_ + 3.0))
        ic = loft_y(cember_y(cx, BICAK_UST + 0.5, ZT, 16.5), dikdort_y(cx + L_ / 2.0, BICAK_ALT - 0.5, ZT, L_, g_))
        ekle("%s_kaplama_bicagi" % k, dis.cut(ic), "paslanmaz", "B+V",
             not_="Beldos icing nozzle tipi düz bıçak · yarıçap boyunca %.0f mm · yarık %.0f mm · ağzı %.0f (pideden %.0f) · ölçü VARSAYIM" % (L_, g_, BICAK_ALT, BICAK_ALT - PIDE_UST))
        ekle("%s_bicak_baglanti_kelepcesi" % k, sily(cx, ZT, 25.0, BICAK_UST, BICAK_UST + 12.0).cut(sily(cx, ZT, 18.2, BICAK_UST - 1, BICAK_UST + 13.0)), "paslanmaz", "V",
             not_="bıçak UNO ağzına 1,5\" TC ile takılır (aletsiz sökülür)")
        ekle("%s_bicak_kulak" % k, kut(cx + L_ - 6.0, cx + L_ + 2.0, BICAK_ALT + 10.0, BICAK_ALT + 40.0, ZT - 1.0, ZT + 1.0), "paslanmaz", "V",
             not_="bıçak ucu sabitleme kulağı")
    else:
...
for ad in ("SOS", "HARC"):
    b_ = bb("%s_kaplama_bicagi" % ad.lower())
    kontrol("%s bıçağı yarıçap boyunca (x %.0f…%.0f, boy %.0f)" % (ad, b_.xmin, b_.xmax, b_.xmax - b_.xmin), b_.xmax - b_.xmin >= 125.0)
    kontrol("%s bıçak ağzı pideden %.0f mm yukarıda" % (ad, b_.ymin - PIDE_UST), 3.0 <= b_.ymin - PIDE_UST <= 10.0)
    y_ = [p for p in P if p["ad"] == "yalitim_tabani"][0]["sh"]
    kontrol("%s bıçağı yalıtıma girmiyor" % ad, y_.intersect([p for p in P if p["ad"] == "%s_kaplama_bicagi" % ad.lower()][0]["sh"]).Volume() < 1.0)
b_ = bb("harc_kaplama_bicagi"); k_ = bb("kiyma__valf_topuzu")
kontrol("harç bıçağı kıyma istasyonuna değmiyor (%.0f < %.0f)" % (b_.xmax, k_.xmin), b_.xmax < k_.xmin)
...
def glb_yaz(yol):
    ...
    while off[0] % 4: blob.append(b"\x00"); off[0] += 1
    bb_ = b"".join(blob)
    gl = {"asset": {"version": "2.0", "generator": "AUTOKITCH topping_uno_cad_v2"}, "scene": 0, "scenes": [{"nodes": [0]}], "nodes": nodes, "meshes": meshes,
...
if __name__ == "__main__":
    ...
    assert not kal, kal
    glb_yaz(os.path.join(OUT, "topping_uno_v2.glb"))
    with open(os.path.join(OUT, "topping_uno_v2.json"), "w", encoding="utf-8") as f:
        json.dump(dict(surum="topping_uno_cad_v2 · %s" % time.strftime("%d.%m.%Y %H:%M"),
