...
ÇIKTI: otonom/hat3d/hat_v43.glb + .usdz  ·  otonom/hat3d/durum.json (sayfa oradan okur)  ·  (v38: SolidWorks STEP'i YAZILMAZ — Kemal 25 Eyl)
...
birim("TOPPING_MODUL", "TOPPING v2 (UNO'lu): 4 UNO çekirdeği (sos · harç · kıyma · kuşbaşı) + kaşar ve küp sucuk kaseti + hava tesisatı · tabla mekanizması v1'den", "C", "GERCEK_MODUL",
      (X_BC, X_BC + W_BC), (H_B, H_B + TC.Y), (-DZ, 0.0), "sac", "topping_uno_cad_v4.py + topping_cad_v22.py", "hat/topping_v2.html")
birim("HAVA_KOMPRESOR", "Kompresör JUN-AIR OF302-15B (yağsız, 15 L) + Ø10 ana hat · K tabanı → F tabanı arkası → TOPPING", "K", "GERCEK_HAVA",
      (X_BC + 1800.0, X_BC + 3900.0), (123.0, 1100.0), (-DZ, 0.0), "sac", "topping_uno_cad_v4.py", "hat/topping_v2.html")
# ---- v42 · TOPPING v2 parçaları ----
import importlib.util as _ilu
_sp = _ilu.spec_from_file_location("TU3", os.path.join(os.path.dirname(os.path.abspath(__file__)), "topping_uno_cad_v4.py"))
...
def glb_yaz(yol, parcalar, dokular, ozel=None, anim=True):
...
if __name__ == "__main__":
    ...
    print("B CEKMECE MODULU (store_cad_v4 · alt taban %.0f · 3 elemanli ray): %d birim · %d parca · %d cekmece" % (Y_ALT, len(_bs), sum(b.get("parca", 0) for b in _bs), len(SC.CEK)))
    ...
    b1 = glb_yaz(os.path.join(OUT, "hat_v43.glb"), parcalar, dokular, anim=False)   # v40: animasyonsuz (istasyon modellerinde oynar)
    ...
    print("hat_v43.glb · %d birim · %.0f KB" % (len(parcalar), b1 / 1024.0))
    b2, prim, sorun, _u = usdz_yaz([os.path.join(OUT, "hat_v43.usdz")], "hat_v43", parcalar + E_USDZ, dokular)
    print("hat_v43.usdz · %.0f KB · %d prim · USD denetimi: %s" % (b2 / 1024.0, prim, "GECTI" if not sorun else "KALDI"))
    ...
    with io.open(os.path.join(OUT, "durum.json"), "w", encoding="utf-8") as f:
        json.dump(dict(hat=dict(w=HAT_W, h=H_MAK, d=DZ, pafta="HAT_ATOSA_TABLALI_v7 (+ E 830: pafta v8 bekliyor) · v43 · TOPPING v2 UNO'lu (topping_uno_cad_v4 · 3 mm raf) · kompresor K tabaninda · E = kutu_cad_v3 · B = store_cad_v4 · alt taban 123 · taban hizasi · surec 1168"), sayac=sayac, modul=[dict(kod=k, ad=a, x=[x, x + w], y=list(MODUL_Y[k])) for k, a, x, w in MODUL],
