# -*- coding: utf-8 -*-
"""KAPAK YOK (Kemal, 22 Eyl: "kapak olmasın, TOPPING'de de yok") → gövde ağzında İÇE 8 mm bükme dudak · ROTOR KAYNAKLI (lama-kol kaynak yakaları) · ön kovanda SAVURMA DİSKİ. Üreteç: yap_kapaksiz_v1.py · önceki sürüm: sucuk_cad_v2.py
...
AUTOKITCH · KÜP SUCUK KASETİ v1 — (22 Eyl 2026) · kusbasi_cad_v2'den türetildi (yap_sucuk_v4.py)
...
ÇIKTI: otonom/kaset3d/sucuk_v4.glb/.usdz + sucuk_v4_dozaj.glb · arastirma/3_TOPPING/sucuk_kaseti_v4/
SURUM: kare agiz YUVARLAK BORU oldu (ic D 42, kopruleme kurali D >= 3 x en buyuk parca) ve
...
URETIM = os.path.join(KOK, "arastirma", "3_TOPPING", "sucuk_kaseti_v4")
...
TUP_Z1 = 246.5
BORU_D, BORU_ET, BORU_ALT = 42.0, 3.0, -100.0      # yuvarlak çıkış borusu: iç çap · et · alt uç (kaset tabanının altında)
...
if __name__ == "__main__":
 ...
 ("orumcek_arka", "gobek_karistirici", "örümcek ↔ göbek flanşı (1,0)"), ("orumcek_on", "on_kovan", "örümcek ↔ kovan flanşı (1,0)"),
    ...
    asm = cq.Assembly(name="SUCUK_KASETI_v3"); RENK = dict(cam=(0.75, 0.85, 0.92, 0.45), pom=(0.95, 0.95, 0.92, 1), celik=(0.75, 0.77, 0.8, 1), silikon=(0.16, 0.5, 0.95, 1), koyu=(0.15, 0.15, 0.17, 1))
    bom = []
    for p in PARCALAR:
    ...
    asm.save(os.path.join(URETIM, "SUCUK_KASETI_v3_MONTAJ.step"))
    ...
    b1 = glb_yaz(os.path.join(OUT, "sucuk_v4.glb"), kap_ag, dokular)
    print("sucuk_v4.glb · %d parca · %d ucgen · %.0f KB" % (len(kap_ag), sum(len(m.I) // 3 for _, m, _, _ in kap_ag), b1 / 1024.0))
    b2, prim, sorun, uyari = usdz_yaz([os.path.join(OUT, "sucuk_v4.usdz")], "sucuk_v4", [(a, m, mal) for a, m, mal, _ in kap_ag], dokular)
    ...
    b3 = glb_yaz(os.path.join(OUT, "sucuk_v4_dozaj.glb"), doz, dokular)
    print("sucuk_v4_dozaj.glb · %d parca · %.0f KB · toplam %.0f sn" % (len(doz), b3 / 1024.0, time.time() - t0))
