# -*- coding: utf-8 -*-
"""PIDE & LAHMACUN MODULE - CONCEPT FRONT VIEW v4 (14 Sep 2026). v4: Kemal - dozaj ve firin aktarimi anlatilmaz (sizin dolum istasyonunuza gecer / firina otomatik gecer), derinlik 800 = Auto Pizza Artisan, pisme sureleri yazilmaz, secenekler tercih sirasi. v3: Kemal - varsayim yok, 80 pide + 200 lahmacun EN YUKSEK hedef (x1,4 kalkti), kilolar buna gore (kasar 20x130 + 20x90 = 4,4 kg/gun), her hazne hedef sure + en az 1 tam gun.
 Olculer mm. Ingilizce (Yindu'ya gidecek).
...
OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\ATOSA_MODUL_v4_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
...
HEDEF = [("Lahmacun paste", "110 g × 200 lahmacun", 200 * 110 / 1000, 2, 1.00),
         ("Minced-meat pide filling", "160 g × 20 pide", 20 * 160 / 1000, 3, 1.00),
         ("Diced meat (kuşbaşı)", "145 g × 20 pide", 20 * 145 / 1000, 3, 0.58),
         ("Grated kaşar cheese", "130 g × 20 + 90 g × 20 pide", (20 * 130 + 20 * 90) / 1000, 7, 0.40),
         ("Sucuk (sliced)", "70 g × 20 pide", 20 * 70 / 1000, 7, 0.59)]
for ad_, _, kg, gun, rho in HEDEF:
    print("%-26s %4.1f kg/day x %d days = %5.1f kg  ~%5.1f L" % (ad_, kg, gun, kg * gun, kg * gun / rho))

# ---------------- BASLIK ----------------
txt(110, 28, "PIDE & LAHMACUN MODULE  ·  CONCEPT FRONT VIEW  v4", f44, INK)
...
txt(fx(350), fy(1745), "complete module: by Yindu", f16, BLUE, "mm")
txt(fx(350), fy(1705), "topping only: by customer", f16, RED, "mm")
...
HOP = [("LAHMACUN", "PASTE", "44 kg", "2 d (min 1)", 240), ("MINCED", "MEAT", "9.6 kg", "3 d (min 1)", 175), ("DICED", "MEAT", "8.7 kg", "3 d (min 1)", 175),
       ("KAŞAR", "CHEESE", "30.8 kg", "7 d (min 1)", 190), ("SUCUK", "SLICED", "9.8 kg", "7 d (min 1)", 170)]
x = 710.0
CX = []
...
txt(fx(CX[0] + 200), fy(1250), "your topping (dosing) station", f14, BLUE, "lm")
...
txt(fx(2450), fy(1470), "at least 60 products / hour", f18, ORG, "mm")
txt(fx(2450), fy(1110), "baking chamber ≈ 1400  ·  belt ≤ 450 wide  ·  depth 800 (same as Auto Pizza Artisan)", f16, ORG, "mm")
...
ok(fx(CX[0] + 180), fy(1390), fx(1690), fy(1320), ORG, 4)
txt(fx(1330), fy(1300), "automatic transfer to oven", f14, ORG, "mm")
adim(fx(1370), fy(1420), 4, ORG)
adim(fx(1800), fy(1430), 5, ORG)
adim(fx(3300), fy(1340), 6, INK)
...
txt(fx(0), top - 66, "PREFERRED: complete module  ·  press + topping + conveyor oven", fb20, BLUE, "lm")
txt(fx(1200), top - 32, "IF NOT POSSIBLE: topping machine only", fb20, RED, "mm")
olcu_h(fx(0), fx(3200), fy(0) + 40, "3200  ·  depth 800 for all modules (same as Auto Pizza Artisan)", f20)
...
AD = ["A dough ball is placed on the tray under the press.",
      ...
      "The tray moves to your topping (dosing) station.",
      "After topping, the product goes into the conveyor oven automatically.",
...
for i, s in enumerate(AD):
    c = GRN if i == 0 else ORG if i in (3, 4) else INK if i == 5 else RED
...
ty += 55
for k, v in (("Product", "round base Ø300 mm · lahmacun + pide"), ("Max. daily target", "80 pide + 200 lahmacun = 280 / day"),
             ("Beyond this", "more capacity would be a plus"), ("Design output", "60 products / hour at peak")):
    txt(TX0, ty, k, fb18, INK, "la")
    txt(TX0 + 230, ty, v, f18, INK, "la")
    d.line([(TX0, ty + 32), (TX1, ty + 32)], fill=(220, 220, 226), width=1)
    ty += 42
ty += 20
txt(TX0, ty, "HOPPERS · PLANNED USE AT MAX. TARGET", fb22, RED, "la")
txt(TX0, ty + 32, "target = ideal duration without refilling · minimum for every hopper: 1 full day", f16, GRAY, "la")
ty += 72
for hx, h in ((TX0, "ingredient"), (TX0 + 395, "per day"), (TX0 + 505, "target"), (TX0 + 590, "capacity"), (TX0 + 700, "≈ L")):
...
txt((TX0 + TX1) / 2, ty + 20, "Minimum 1 full day per hopper · more hoppers per ingredient are fine.", fb18, RED, "mm")
ty += 68
txt(TX0, ty, "OVEN · POWER · CONTROL", fb22, ORG, "la")
ty += 42
for k, v in (("Oven", "electric conveyor · 4 in chamber"), ("", "at least 60 products / hour"),
             ("Size", "belt ≤ 450 · depth 800 (= Auto Pizza Artisan)"), ("Power", "400 V 3-phase 50 Hz per site"),
             ("Control", "PLC")):
    txt(TX0, ty, k, fb18, INK, "la")
