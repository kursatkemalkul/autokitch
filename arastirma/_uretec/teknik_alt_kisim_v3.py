# -*- coding: utf-8 -*-
"""HAT · ALT KISIM YERLEŞİMİ v3 (Kemal: soğukta her şey 2 gün · 4 güne kadar fazlası dışarıda soğutmasız içecek · kaşar + sucuk 4 gün saklanır · kompresör eklenir)
v2 — ÖN GÖRÜNÜŞ · ÜST GÖRÜNÜŞ · 4 YAN KESİT · STOK TABLOSU · 25 Eyl 2026
...
CIKTI = os.path.join(KOK, "arastirma", "FULL_MAKINE", "ALT_KISIM_v3_teknik.png")
...
HATVE = {"pide": 108.0, "lah": 93.0, "ic": 159.0, "tat": 104.0}          # tatlı kabı Ø95 × 60 (V) → HH 71
PANEL = {k: v - 3.0 for k, v in HATVE.items()}
AD = {"pide": "pide · 20 top", "lah": "lahmacun · 36", "ic": "içecek · 48 kutu", "tat": "tatlı · 30 (5 şerit × 6)"}
KOL = [("K1", 62.5, 620.0, YUZ0, [("pide", 5), ("lah", 2), ("tat", 1)]),
       ("K2", 717.5, 620.0, YUZ0, [("pide", 3), ("lah", 5)]),
       ("K3", 1372.5, 620.0, YUZ0, [("lah", 5), ("ic", 2)]),
       ("K4", 2027.5, 400.0, 671.5, [("ic", 2)])]
GN = (438.5, 668.5)                                                          # kaşar + sucuk deposu (kapaklı, +3 °C)
...
KOMP = dict(x=(4040.0, 4420.0), y=(130.0, 640.0), z=(-425.0, -805.0))       # JUN-AIR OF302-15B 380 × 380 × 510 · 25 kg
YAG = dict(x=(4040.0, 4440.0), y=(650.0, 840.0), z=(-425.0, -665.0))
KKART = dict(x=(4040.0, 4260.0), y=(850.0, 1050.0), z=(-425.0, -615.0))
...
d.text((60, 24), "HAT · ALT KISIM YERLEŞİMİ v3 — soğukta 2 gün · dışarıda 4 güne kadar · kaşar + sucuk 4 gün · kompresör dahil", font=fB, fill=INK)
...
kutu(X(2027.5), Y(GN[1]), X(2427.5), Y(GN[0]), fill=(222, 238, 250), renk=MAVI, w=2)
orta("KAŞAR + SUCUK DEPOSU|kapaklı · +3 °C · 2 gün|GN 1/1-100 + GN 1/2-100", X(2227.5), Y(620), fXS, MAVI)
...
orta("İÇECEK YEDEĞİ|5 koli × 24 = 120 kutu", X(4240), Y(712), fS, TUR)
kutu(X(4040), Y(1045), X(4560), Y(ICY["y"][1] + 10), fill=(250, 250, 250), renk=GRI, w=1); orta("boş raf|520 × 390 × 290", X(4300), Y(960), fXS, GRI)
orta("arkada (kesit 4): KOMPRESÖR 130–640 · yağ 650–840 · K kartı 850–1050", X(4300), Y(Y_ALT) + 8, fXS, INK)
# E alt kısmı
...
kutu(PX(ICY["x"][0]), PZz(ICY["z"][1]), PX(ICY["x"][1]), PZz(ICY["z"][0]), fill=(255, 244, 230), renk=TUR, w=2); orta("İÇECEK YEDEĞİ|soğutmasız", PX(4240), PZz(-160), fXS, TUR)
kutu(PX(KOMP["x"][0]), PZz(KOMP["z"][1]), PX(KOMP["x"][1]), PZz(KOMP["z"][0]), fill=KOYU, renk=INK, w=2); orta("KOMPRESÖR 380 × 380|üstünde yağ + K kartı", PX(4230), PZz(-630), fXS, (255, 255, 255))
kesik((PX(4230), PZz(-812)), (PX(2400), PZz(-812)), MAVI, 3, 10, 6); d.text((PX(2560), PZz(-812) - 18), "hava hattı Ø10 → F arkası → TOPPING", font=fXS, fill=MAVI)
...
S3 = 0.44
KES = [("KESİT 1 · B / K3 (içecek tek kat)", 60), ("KESİT 2 · B / K4 (depo + içecek)", 700), ("KESİT 3 · F (pizza · robot · pano)", 1340), ("KESİT 4 · K (içecek · kompresör)", 1980)]
OYK = 2060
...
kutu(z(-586), YK(GN[1] - 10), z(-56), YK(GN[0] + 5), fill=(222, 238, 250), renk=MAVI, w=2); orta("kaşar + sucuk deposu", (z(-586) + z(-56)) / 2, YK(560), fXS, MAVI)
y = 671.5
for tip in ("ic", "ic"):
    kutu(z(-736), YK(y + PANEL[tip] - 12), z(-57), YK(y + 15), fill=SOG, renk=DOL, w=1)
    kutu(z(-40), YK(y + PANEL[tip]), z(0), YK(y), fill=(255, 255, 255), renk=DOL, w=1); y += HATVE[tip]
d.text((z(-826), YK(1000) + 4), "üst 989,5 · ara sac Secop ↔ PLC", font=fXS, fill=GRI)
...
kutu(z(KOMP["z"][1]), YK(KOMP["y"][1]), z(KOMP["z"][0]), YK(KOMP["y"][0]), fill=KOYU, renk=INK, w=2); orta("KOMP-|RESÖR|25 kg", (z(-805) + z(-425)) / 2, YK(420), fXS, (255, 255, 255))
kutu(z(YAG["z"][1]), YK(YAG["y"][1]), z(YAG["z"][0]), YK(YAG["y"][0]), fill=(250, 250, 250), renk=INK, w=1); orta("yağ", (z(-665) + z(-425)) / 2, YK(760), fXS)
kutu(z(KKART["z"][1]), YK(KKART["y"][1]), z(KKART["z"][0]), YK(KKART["y"][0]), fill=(250, 250, 250), renk=INK, w=1); orta("K kartı", (z(-615) + z(-425)) / 2, YK(965), fXS)
d.text((z(-826), YK(H_B) + 4), "arka sacta ızgara (kompresör havası)", font=fXS, fill=GRI)
...
SAT = [("ürün", "günlük", "gerekli", "yer · adet", "kapasite"),
       ...
       ("tatlı (soğuk)", "5,5", "4 gün 22", "B · K1 · 5 şerit × 6", "30 · yeter"),
       ("içecek (soğutmasız)", "", "4 gün 277", "K tabanı önde · 5 koli × 24", "160 + 120 = 280 · yeter"),
       ("pizza kutusu", "280", "2 gün 560", "E şarjörü", "567 · yeter"),
       ("pizza kutusu", "", "4 gün 1120", "F tabanı önde · düz yığın", "567 + 553 = 1120 · yeter"),
       ("kaşar / sucuk", "", "4 gün", "kaset 2 gün + K4 deposu 2 gün", "8,8+8,8 · 2,8+2,8 kg"),
       ("kompresör", "", "", "K tabanı arkada (JUN-AIR)", "hat → TOPPING")]
CX = [0, 150, 222, 330, 650]
...
d.text((TX, TY + 36 + len(SAT) * 30 + 8), "K'nin üst bölmesi boşaldı (içecek yedeği alta indi) · (V): robot kutusu, ana pano, koli, tatlı kabı, bidon ölçüleri", font=fXS, fill=GRI)
...
ic = 2 * 48 + 2 * 32
print("PNG", CIKTI)
print("çekmece: pide %d (=%d top) · lahmacun %d (=%d) · içecek %d kutu · tatlı 30 · kaşar/sucuk deposu K4" % (cek["pide"], cek["pide"] * 20, cek["lah"], cek["lah"] * 36, ic))
