# -*- coding: utf-8 -*-
"""AUTOKITCH · F FIRIN (AYRI SÜRÜM) · Sveba Dahlen TP10 KESİTİ, GÖVDESİ F MODÜLÜNE (1500) UZATILMIŞ · 3B MODEL + HATTA UYARLAMA v2 (26 Eyl 2026)
...
CEP_AGIZ = (X_F0, 1058.0, 1137.0, -465.0, -3.0)                 # kabuğun giriş yüzündeki açıklık (y · z)
CEP_DUVAR = ((2563.0, 2587.0, 1058.0, 1094.0, -418.0, -3.0),    # giriş duvarı alt bloğunda: tekne + 3 kiriş
             (2563.0, 2582.0, 1058.0, 1114.0, -400.0, -387.0),  # X motoru kaidesi (pahlı)
             (2563.0, 2572.0, 1101.0, 1126.0, -83.0, -62.0))    # X ekseni uç tamponu
...
def firin(ayak=False, plaka=False):
    ...
    kab = kab.cut(kut(x0 - 1.0, x0 + 2.0, CEP_AGIZ[1], CEP_AGIZ[2], CEP_AGIZ[3], CEP_AGIZ[4]))  # TOPPING mekanizma teknesi + X tahriki cebi ağzı
...
def adaptor(nema23=None):
    ...
    # ---- 3.4 F ARKA SACI (istasyon = kapalı ürün) ----
...
def glb_yaz(yol, parcalar):
    ...
    while off[0] % 4: blob.append(b"\x00"); off[0] += 1
    bb = b"".join(blob)
    g = {"asset": {"version": "2.0", "generator": "AUTOKITCH firin_tp10_cad_v2"}, "scene": 0, "scenes": [{"nodes": list(range(len(nodes)))}], "nodes": nodes,
...
if __name__ == "__main__":
    ...
    yol = os.path.join(KOK, "otonom", "hat3d", "firin_tp10_v2.glb")
    b1 = glb_yaz(yol, parca)
    _dk = {"ad": K3.doku_ad("TP10 KESİTİ · 1500", "uzatılmış fırın · AUTOKITCH v2"), "montaj": K3.doku_ad("TP10-UZUN", "ayrı sürüm v2", ok_sol=False)}
    b2, prim, sorun, _u = K3.usdz_yaz([yol.replace(".glb", ".usdz")], "firin_tp10_v2", parca, _dk)
    print("firin_tp10_v2.glb %.0f KB · usdz %.0f KB · %d prim · USD: %s · %.0f sn" % (b1 / 1024.0, b2 / 1024.0, prim, "GEÇTİ" if not sorun else sorun, time.time() - t0))
