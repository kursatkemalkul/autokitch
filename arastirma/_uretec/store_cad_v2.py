# -*- coding: utf-8 -*-
"""AUTOKITCH · B ÇEKMECE MODÜLÜ — ÜRETİM MODELİ v2 (24 Eyl 2026)
...
STANDART ÜRÜNLER (üretici datasheet ölçüleriyle modellendi; resmi STEP henüz indirilmedi):
  ...
  ray     Accuride DZ3832-0070 · 700 · tam çekilir · 45,7 × 12,7 · 45–50 kg
...
def kasa():
    ...
    for i, (ax, az) in enumerate([(60.0, -110.0), (W_B - 60.0, -110.0), (60.0, -760.0), (W_B - 60.0, -760.0), (W_B / 2.0, -110.0), (W_B / 2.0, -760.0)]):
        ekle("ayak_%d" % i, sily(ax, az, 20.0, 0.0, 8.0).union(sily(ax, az, 6.0, 8.0, Y_PLINT)), "celik", B,
             bom=("Ayarlı ayak Elesa+Ganter LV.A-SST · M12", 6, "paslanmaz AISI 304 · taban Ø40", "elesa-ganter.com LV.A-SST") if i == 0 else None)
    W_ = W_B
    ...
    ekle("taban_pu", kut(29.0, W_ - 29.0, Y_PLINT + 1.5, Y_TABAN - 1.0, -DZ + 1.5, -1.5).cut(kut(VENT[0] - 20, VENT[1] + 20, Y_PLINT, Y_TABAN, VENT[2] - 20, VENT[3] + 20)), "pu", B)
    ...
    cx0, cy0, cz1 = kx0 + 25.0, Y_TABAN + 4.0, Z_CER0 - 10.0
...
def denetim(ozet, tarama=True):
    print("B CEKMECE MODULU v2 · %d parca · %d cekmece" % (len(PARCALAR), len(CEK)))
...
ALT_KURAL = [
    ...
    (r"_ray_dis_sag$", "Accuride DZ3832-0070"), (r"_ray_ic_(sol|sag)$", "Accuride DZ3832-0070 (iç profil)"), (r"_reed_acik$", "Littelfuse 59135"),
    ...
    (r"^(yan_pu_(sol|sag)|arka_pu_37\.5|taban_pu|bolme_\d_pu|k1_teknik_raf_pu_38|k4_ara_pu)$", "PU köpük gövde"),
]
...
if __name__ == "__main__":
    oz = modul(); denetim(oz)
    print("BOM:", bom_yaz(os.path.join(KOK, "arastirma", "1_STORE_v5")))
