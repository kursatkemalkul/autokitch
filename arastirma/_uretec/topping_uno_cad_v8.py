...
def glb_yaz(yol):
    ...
    while off[0] % 4: blob.append(b"\x00"); off[0] += 1
    bb_ = b"".join(blob)
    gl = {"asset": {"version": "2.0", "generator": "AUTOKITCH topping_uno_cad_v8"}, "scene": 0, "scenes": [{"nodes": [0]}], "nodes": nodes, "meshes": meshes,
...
if __name__ == "__main__":
    ...
    assert not kal, kal
    glb_yaz(os.path.join(OUT, "topping_uno_v8.glb"))
    with open(os.path.join(OUT, "topping_uno_v8.json"), "w", encoding="utf-8") as f:
        json.dump(dict(surum="topping_uno_cad_v8 · %s" % time.strftime("%d.%m.%Y %H:%M"),
