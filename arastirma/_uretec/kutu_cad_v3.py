...
def glb_yaz(yol, adim=1.0 / 15.0):
    ...
    while off[0] % 4: blob.append(b"\x00"); off[0] += 1
    bb = b"".join(blob)
    gl = {"asset": {"version": "2.0", "generator": "AUTOKITCH kutu_cad_v3"}, "scene": 0, "scenes": [{"nodes": kok}], "nodes": nodes, "meshes": meshes,
...
ALT_KURAL = [r"^asansor_vidasi_alt_ucu$", r"^ayak_[1-3]$", r"^kilavuz_(sag|arka)_uhmw$", r"^kalip_ayagi_[1-3]$", r"^kalip_taragi_on_z_[1-3]$",
...
if __name__ == "__main__":
    ...
    if "bom" in arg or "hepsi" in arg:
        bom_yaz(os.path.join(KOK, "arastirma", "5_PACK_kutu_v3"))   # v3: eski SolidWorks 5_PACK_v3 klasoruyle karismasin
