# -*- coding: utf-8 -*-
"""AUTOKITCH - TABLALI HAT · MODULER v4 · HER SEY 1 GUN · ALCAK (14 Eyl 2026): ON + UST GORUNUS. Olculer mm.
Kemal (v3 duzeltmesi): sarjor de 1 gunluk (427 degil) · sogutma grubu + hat panosu SOLA · cekmeceler yataya · yukseklik iyice dussun · bos alan kalmasin.
1 GUN: pide 80 = 4 cekmece (20'lik) · lahmacun 200 = 6 cekmece (35'lik) · icecek 70 = KESME altinda 2 dar cekmece (5 kanal x 8 = 40'lik) · kutu 280 blank.
YERLESIM: TEKNIK 400 (sol, tam boy) | PRESS 670 | TOPPING 885 | OVEN 1420 | KESME 500 | PACK 700 = 4575.
  TEKNIK PRESS altina sigmaz: 640 x 340 bolmeye sogutma grubu (~450 x 330, varsayim) + pano (~400 x 600, varsayim) birlikte girmez.
KOT: sarjor 280 x 2,13 = 596 -> sarjor ustu = tabla - 100 >= 636 -> tabla 740 (baglayici kisit), yukseklik 1410 (v3: 1720).
  cekmece (alttan uste): PRESS 3 lahm 279/340 · TOPPING 3 lahm 279/319 · OVEN-A 2 pide 216/300 · OVEN-B 2 pide 216/300 · KESME 2 dar icecek 330/540.
Kural: paftada yalniz gorunus + olcu + parca adi; aciklama mesajda.
"""
import os, math
from PIL import Image, ImageDraw, ImageFont

OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\HAT_MODULER_v4_hersey1gun_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 3800, 2420
S = 0.62
...
# ======================= VERI =======================
HB = 740.0                                  # tabla / bant kotu
H = HB + 670.0                               # 1410 · tum moduller ayni
assert 280 * 2.13 <= HB - 140.0, "sarjor 1 gun sigmiyor"
MOD = [("TEKNİK", "TEKNİK", 400.0), ("PRESS", "PRESS", 670.0), ("TOPPING", "TOPPING · Atosa uyarlama", 885.0),
...
KOLON = [("PRESS", 25.0, HB - 360.0, [(3, "lahm")]),
         ("TOPPING", 17.5, HB - 381.0, [(3, "lahm")]),
         ("OVEN", 72.5, HB - 400.0, [(2, "pide")]),
         ("OVEN", 727.5, HB - 400.0, [(2, "pide")])]
...
txt(OX, 60, "AUTOKITCH  ·  TABLALI HAT  ·  MODÜLER  v4  ·  HER ŞEY 1 GÜN  ·  ALÇAK", f38, INK)
txt(OX, 128, "ön görünüş + üst görünüş  ·  her istasyon ayrı dolap, birleşince üst/alt/ön/arka çizgiler hizalı  ·  ayak yok  ·  hat %s × %s × 830  ·  tabla / bant %s  ·  her şey 1 gün  ·  TEKNİK solda  ·  ölçüler mm  ·  14 Eylül 2026" % (sayi(HAT), sayi(H), sayi(HB)), f13, GRAY)
...
txt(fx((tb0 + X0["TOPPING"] + W["TOPPING"] - 10) / 2), fy((40.0 + HB - 381.0) / 2), "boş %s" % sayi(X0["TOPPING"] + W["TOPPING"] - 10 - tb0), f7, GRAY, "mm")
# --- PRESS
px0 = X0["PRESS"] + 15.0
...
# --- TEKNİK modülü (sol, tam boy)
tk0, tw0 = X0["TEKNİK"], W["TEKNİK"]
d.rectangle([fx(tk0 + 20), fy(520.0), fx(tk0 + tw0 - 20), fy(40.0)], fill=BG, outline=INK, width=2)
txt(fx(tk0 + tw0 / 2), fy(280.0) - 11, "SOĞUTMA GRUBU", f8, INK, "mm")
txt(fx(tk0 + tw0 / 2), fy(280.0) + 11, "çekmeceler", f7, GRAY, "mm")
d.rectangle([fx(tk0 + 20), fy(H - 40.0), fx(tk0 + tw0 - 20), fy(560.0)], fill=BG, outline=INK, width=2)
txt(fx(tk0 + tw0 / 2), fy((H - 40.0 + 560.0) / 2) - 11, "HAT PANOSU", f8, INK, "mm")
txt(fx(tk0 + tw0 / 2), fy((H - 40.0 + 560.0) / 2) + 11, "PLC · fırın kontaktörleri", f7, GRAY, "mm")
# --- PACK
pk0 = X0["PACK"]
...
# TEKNİK
d.rectangle([fx(tk0 + 30), py(-760), fx(tk0 + tw0 - 30), py(10)], fill=BG, outline=INK, width=2)
txt(fx(tk0 + tw0 / 2), py(-375) - 12, "HAT PANOSU", f8, INK, "mm")
txt(fx(tk0 + tw0 / 2), py(-375) + 12, "altında SOĞUTMA GRUBU", f7, GRAY, "mm")
# PRESS
...
txt(W_PX - 150, ly + 34, "HAT %s × %s × 830 · 6 modül · pide 4 çekmece (80) · lahmacun 6 (200) · içecek 2 dar çekmece (70) · şarjör 280 blank · TEKNİK solda" % (sayi(HAT), sayi(H)), f9, GRAY, "rm")

assert not os.path.exists(OUT), "moduler v4 zaten var — yeni numara ver"
im.save(OUT)
