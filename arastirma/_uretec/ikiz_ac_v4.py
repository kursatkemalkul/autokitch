# -*- coding: utf-8 -*-
r"""DIJITAL IKIZ + KUMANDA PANELI  (v4)

C:\isaacsim\python.bat ikiz_ac_v4.py

v3'ten farki: ROBOT YOK, ZEMIN GORUNMEZ, konsol UTF-8, panel fontu acikca NVIDIA Sans.
...
# TURKCE KARAKTER: Windows konsolu varsayilan olarak cp857/cp1254 — UTF-8 yazinca
# "kaÅŸarlÄ±" gibi cikiyordu. Konsolu ve akisi UTF-8'e aliyoruz.
try:
    import ctypes
    ctypes.windll.kernel32.SetConsoleOutputCP(65001)
except Exception:
    pass
for _ak in (sys.stdout, sys.stderr):
    try: _ak.reconfigure(encoding="utf-8", errors="replace")
    except Exception: pass

_T0 = time.time()
def sure(s_):
    print("[%6.1f sn] %s" % (time.time() - _T0, s_), flush=True)
...
sure("sahne açılıyor...")
omni.usd.get_context().open_stage(os.path.join(FIZ, "TOPPING_IKIZ_v9.usd"))
...
sure("fizik kuruluyor (çarpışma gövdeleri pişiyor, ilk açılışta uzun sürer)...")
world.reset()
sure("fizik hazır")
...
# omni.ui'nin varsayilan fontu yerine NVIDIA Sans'i acikca veriyoruz: cmap'i kontrol
# edildi, ç Ç ğ Ğ ı İ ö Ö ş Ş ü Ü harflerinin HEPSI var.
FONT = "C:/isaacsim/kit/resources/fonts/NVIDIASans_Rg.ttf"
STIL = {"Label": {"font": FONT}, "Button.Label": {"font": FONT}, "Button": {"font": FONT}}
pencere = ui.Window("AUTOKITCH · TOPPING", width=330, height=470)
pencere.frame.set_style(STIL)
sw_surekli = None
with pencere.frame:
    with ui.VStack(spacing=8, height=0):
        ui.Spacer(height=4)
        ui.Label("ÜRÜN SEÇ", height=22, style={"font_size": 18, "color": 0xFFD0D0D0, "font": FONT})
        for _ad, _ in RECETE:
        ...
        ui.Spacer(height=8)
        l_durum = ui.Label("hazır", height=22, style={"font_size": 16, "color": 0xFF7FD4A0, "font": FONT})
