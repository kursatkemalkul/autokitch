...
ÇIKTI: otonom/hat3d/hat_v4.glb + .usdz  ·  otonom/hat3d/durum.json (sayfa oradan okur)  ·  FULL_MAKINE/HAT_v4_YERLESIM.step (SolidWorks)
...
KASET_CAD = {"KAŞAR KABI": ("kasar_cad_v10", "Kaşar kabı v10", "kaset3d/index.html?k=kasar_v10"), "KIYMA": ("kiyma_cad_v5", "Kıyma kaseti v5", "kaset3d/index.html?k=kiyma_v5"),
             "KUŞBAŞI": ("kusbasi_cad_v4", "Kuşbaşı kaseti v4", "kaset3d/index.html?k=kusbasi_v4"), "SUCUK": ("sucuk_cad_v3", "Küp sucuk kaseti v3", "kaset3d/index.html?k=sucuk_v3")}
...
ATLA = ("saplama_", "somun_", "pul_", "vida_", "setuskur", "yayli_pim", "oring_", "insert_", "kilit_pimi", "conta_")
# Kasetin GORUNEN her parcasi modele girer; yalniz kucuk baglanti elemanlari atlanir (dosya sismesin).
# Makine gorunumunde govde seffaf oldugu icin helezon ve rotor ICERIDE gorunur — kaset "beyaz kutu" gibi durmaz.
def alinir(ad):
    return not ad.startswith(ATLA)


def kaset_parcalari(modul):
    """kasetin GÖRÜNEN parçaları: gövde, plakalar, kulp, tüp, yatak kapağı + İÇİ (helezon, rotor, mil, kovan, topuz).
    Yalnız küçük bağlantı elemanları (saplama, somun, pul, vida, O-ring, conta, pim) alınmaz — onlar kasetin kendi sayfasında."""
...
def mal_ad(b, ton=None):
    ...
    if ton:
        ad += "__" + ton
    m = dict(MALZEME[ton or b["mal"]])
    if ton == "cam":
        # Makine/istasyon gorunumunde kasetin PC govdesi KATI cizilir: seffaf birakinca kaset hayalet gibi duruyor
        # ve imlecle secmek zorlasiyor (Kemal, 22 Eyl). Sayfadaki "Icini gor" dugmesi alfayi tekrar dusurur.
        m["renk"] = (0.90, 0.92, 0.94, 1.0); m["ruf"] = 0.35; m.pop("saydam", None)
    MALZEME.setdefault(ad, m)
    return ad
...
def glb_yaz(yol, parcalar, dokular):
    ...
    while off[0] % 4: blob.append(b"\x00"); off[0] += 1
    bb = b"".join(blob)
    g = {"asset": {"version": "2.0", "generator": "AUTOKITCH hat_montaj_v4"}, "scene": 0, "scenes": [{"nodes": list(range(len(nodes)))}], "nodes": nodes, "meshes": meshes,
...
if __name__ == "__main__":
    ...
    for b in B:
        ...
        if b["durum"] == "GERCEK":
            ...
            for p in ps:
                ...
                ton.setdefault(p["mal"], Mesh()).ekle(V.ag(cq.Workplane(obj=sh), 0.5, 0.9))
    ...
    b1 = glb_yaz(os.path.join(OUT, "hat_v4.glb"), parcalar, dokular)
    print("hat_v4.glb · %d birim · %.0f KB" % (len(parcalar), b1 / 1024.0))
    b2, prim, sorun, _u = usdz_yaz([os.path.join(OUT, "hat_v4.usdz")], "hat_v4", parcalar, dokular)
    print("hat_v4.usdz · %.0f KB · %d prim · USD denetimi: %s" % (b2 / 1024.0, prim, "GECTI" if not sorun else "KALDI"))
    for x in sorun: print("   HATA:", x)
    os.makedirs(STEP, exist_ok=True); asm.save(os.path.join(STEP, "HAT_v4_YERLESIM.step"))
    print("HAT_v4_YERLESIM.step · SolidWorks'te montaj olarak acilir · %s" % STEP)
