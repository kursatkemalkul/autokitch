...
for x in (1.446, 1.455, 1.464, 1.480, 1.496, 1.504):
    for z in (-0.470, -0.400, -0.330, -0.260):
        h = q.raycast_closest([x, 0.400, z], [0.0, -1.0, 0.0], 0.5)
        if h and h.get("hit"):
...
for z in (-0.500, -0.470, -0.440, -0.410, -0.380, -0.350, -0.320, -0.290, -0.260, -0.230):
        h = q.raycast_closest([1.472, 0.352, z], [0.0, -1.0, 0.0], 0.09)
        print("   z %.3f -> %s" % (z, (("y %.4f  %s" % (h["position"][1], str(h["rigidBody"]).split('/')[-1])) if h and h.get("hit") else "BOSLUK (kanat arasi)")))
