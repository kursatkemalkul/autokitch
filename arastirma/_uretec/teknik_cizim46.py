# -*- coding: utf-8 -*-
"""HAT v46 — TUM ISTASYONLAR SON VERSIYON (5 Eyl 2026): STORE v4 (+ v5 oneri: −18 kaset kati ustte, klapeli, 4 kap sol modul) ·
...
X0,Y0=90,150
HG,HA=1850,120
M=[("1 · STORE — soğuk depo (hamur + içecek + tatlı + donmuş kap ×4)",1400),("2 · PRESS",700),("3 · TOPPING",700),("4 · OVEN",650),("5 · PACK",700)]
...
# TOPPING v25 kabi (on gorunus): 14x24 + altta 2 kizak 3x2 + L raf cizgisi
def kap25(x,y,nm,alt='',bos=False,c='#111',fill='#f3efe4',fs=6.2,rail=True):
    col='#999' if bos else c; dsh='4,3' if bos else None
    rc(x,y,px(140),px(240),1.2 if bos else 1.3,2,col,dsh,'#f7f6f2' if bos else fill)
    if not bos:
        rc(x+px(6),y+px(40),px(128),px(140),0,0,'none',None,'#e9dfa8')                         # malzeme
        ln(x+px(6),y+px(200),x+px(35),y+px(234),.8,col); ln(x+px(134),y+px(200),x+px(105),y+px(234),.8,col)   # 55° eğimler
        ci(x+px(70),y+px(205),px(35),1.1,'#1d7a4f',None,'#fff'); ci(x+px(70),y+px(120),px(45),.8,'#6b4fa8','3,2')  # helezon Ø70 · tarak Ø90
...
for i,(_ad,_w) in enumerate(M):
    ...
    if i!=2: rc(_x+14,YT,12,px(HA),1.4); rc(_x+px(_w)-26,YT,12,px(HA),1.4)   # TOPPING: ayak yerine plint
...
# ================= 4 OVEN (tepsiyle · tank + pompa) =================
d0,d1=xs[3],xs[4]; dm=(d0+d1)/2; WD=d1-d0-px(40)
for k in range(2):
    ky=Y0+px(170)+k*px(540)
    rc(d0+px(25),ky,d1-d0-px(50),px(450),2)
    rc(dm-px(200),ky+px(70),px(400),px(300),1.4)
    ln(d0+px(25),ky+px(450),d1-px(25),ky+px(450),2.6)
    rc(dm-px(195),ky+px(330),px(390),px(35),1,1,'#777',None,'#eee')
    tray(dm,ky+px(312),kulp=True)
    tx(dm+px(230),ky+px(62),'iç 40',4.6,'middle','','#777')
rc(d0+px(40),Y0+px(20),d1-d0-px(80),px(115),1.4,3); ci(dm,Y0+px(77),px(42),1.1)
notew(dm,Y0+px(154),"karbon filtre + fan · iç 40×40 · taş taban · 2 kavite",WD,5.8)
notew(dm,Y0+px(212),"tepsi taşa konur · kilit açılır · kapak kapanır · pişince kilitlenir",WD,5.6)
nk(dm,Y0+px(648),"② tepsi Ø32 + kulp 12 = 44 > kavite 40 — AÇIK",6.6)
nk(dm,Y0+px(668),"öneri: kulp ÖNE dönük + kavite derinliği 50 (84 içinde)",6.2)
nk(dm,Y0+px(686),"ya da kulp 6 cm",6.2)
notew(dm,Y0+px(1170),'⑩ SADEYAĞ ÜSTTE + MİNİ POMPA — püskürtme 1-2 bar ister, cazibe damlatır',WD,5.6,'#9a6b1f','middle','bold')
rc(d0+px(25),Y0+px(1200),d1-d0-px(50),px(120),1.6)
rc(d0+px(45),Y0+px(1215),px(230),px(90),1.4,3,'#c9a227',None,'#fff8e0'); tx(d0+px(160),Y0+px(1252),'TANK 4 L',7,'middle','bold','#8a6a3a'); tx(d0+px(160),Y0+px(1278),'45 °C ısıtıcı',6,'middle','','#8a6a3a')
rc(d0+px(300),Y0+px(1215),px(130),px(90),1.2,3); tx(d0+px(365),Y0+px(1252),'POMPA',6.5,'middle','bold'); tx(d0+px(365),Y0+px(1278),'12 V · 8 W',6,'middle','','#555')
rc(d0+px(450),Y0+px(1215),px(120),px(90),1,2); tx(d0+px(510),Y0+px(1265),'vana',6.5,'middle','','#555')
rc(dm-px(22),Y0+px(1320),px(44),px(24),1.3); ln(dm,Y0+px(1344),dm,Y0+px(1365),1.4)
rc(d0+px(25),Y0+px(1345),d1-d0-px(50),px(140),1.4)
ln(dm,Y0+px(1365),dm-px(100),Y0+px(1445),1,'#111','4 4'); ln(dm,Y0+px(1365),dm+px(100),Y0+px(1445),1,'#111','4 4')
tray(dm,Y0+px(1457),kulp=True)
notew(dm,Y0+px(1500),'sprey nişi 14 — tepsi 2 sn geçer, yağ sıcak pidede erir',WD,5.6)
rc(d0+18,Y0+px(1520),d1-d0-36,px(320),1.4,3)
for i in range(3): rc(d0+px(60)+i*px(180),Y0+px(1585),px(150),px(230),1.2,2); tx(d0+px(135)+i*px(180),Y0+px(1710),'teneke',6.5)
notew(dm,Y0+px(1548),'SADEYAĞ STOĞU — 3 teneke ≈ 4 ay · kapaklı bölme, çekmece yok',WD,5.6)
...
for xk,nm in ((200,'KAŞAR A'),(360,'SUCUK küp')):
    ...
    for dx_ in (30,110): ln(c0+px(xk+dx_),YT2+px(110),c0+px(xk+dx_),YT2+px(770),.7,'#555','3,2')      # kızak cepleri (çatal buraya girer)
    ln(c0+px(xk+70),YT2+px(120),c0+px(xk+70),YT2+px(760),1.4,'#1d7a4f')
    ci(c0+px(xk+70),YT2+px(760),px(22),1.4,'#1d7a4f',None,'#fff'); ci(c0+px(xk+70),YT2+px(760),px(270),1,'#1d7a4f','5,3')
...
notew(cm2,YT2+px(1050),'kat 1 planı · kap 68 arka duvara dayalı, ara mil yok · ağız ön uçta y 76, merkez x 27/43 · süpürme R 27 = tepsi 16 + spiral 11 · kesikli = kızak cepleri (çatal) · 10+68+2+4 = 84',c1-c0-20,6.5)
# OVEN ust
rc(d0+px(25),YT2+px(120),d1-d0-px(50),px(620),1.6)
rd(dm-px(200),YT2+px(230),px(400),px(400))
ci(dm,YT2+px(430),px(160),1.2,'#1a49b8','4,3'); rc(dm-px(15),YT2+px(590),px(30),px(120),1,1,'#1a49b8'); tx(dm+px(60),YT2+px(700),'kulp öne (öneri)',5,'start','','#1a49b8')
notew(dm,YN,"taş 40×40 · tepsi Ø32 + kulp 12 = 44 > 40 — ② açık: kulp öne + kavite derinliği 50 (84 içinde) ya da kulp 6",d1-d0-20,7)
# PACK ust
...
notew(cm2+px(230),ry-20,"uçlar: TEPSİ eli + PENÇE + ÇATAL (uç değiştirici PRESS kabininin altında) · kap değişimleri gece, haftada 9-10 + STORE→ALT 4",X0+px(T)-cm2-px(260),8.6,"#555","start")
...
rc(kx-10,ky-18,KW,px(1800),1.4,6,'#b3452b',None,'#fff8f5')
tx(kx,ky,"KONTROL — istasyonlar arası uyum (5 Eyl 2026, HAT v46)",12,"start","bold","#b3452b")
K=[
 ...
 ("② OVEN kavite 40×40: tepsi Ø32 + kulp 12 = 44 → kapak kapanmaz — AÇIK","→ öneri: kulp ÖNE dönük (robot kürek gibi sürer) + kavite derinliği 50 → 84 derinliğe sığar, kabin büyümez · alternatif kulp 6 cm — KARAR","#b3452b"),
 ...
 ("⑦ KOL YÜKÜ YENİDEN AÇIK: kap boş ≈ 4,6 kg (PE 6 mm 2,9 + helezon 1,2 + tarak 0,5) + çatal 2,0 → kıyma 15,2 kg · sucuk 15,0 · kaşar 13,8","→ 12,5 kg kobot YETMEZ → 16–20 kg sınıfı + menzil ≥ 130 (CRX-20iA/L 142 · H2017 170 · TM20 130; UR16e 90 ✗) ya da kıyma/sucuk 2 günlük dolum (6 kg) — KARAR","#b3452b"),
 ("⑧ ✓ TOPPING v25: TEK kap 14×68×24 (PE 6 mm, 17,5 L, simetrik, 16 kap tek kalıp) · 3 kat × 2 · kap ARKAYA DAYALI (ara mil yok)","→ altta 2 kızak + kabinde 2 L raf; robot ÇATALLA önden alır (gir 50 → kaldır 0,5 → çek 70); ALT: soğutma dipte 20 + 2 sıra raf; evaporatör sol kanal","#1d7a4f"),
 ...
 ("⑩ OVEN sadeyağ: tank ÜSTTE + 12 V pompa 8 W (püskürtme basınç ister) — AÇIK","→ cazibe yalnız damlatır; çekmeceler kalktı → teneke stoğu ×3 (4 ay)","#9a6b1f"),
 ("⑪ 197 ✓ · 415 ✓ · derinlik 84 HER KABİN ✓ · TOPPING 3×(27+14) + 74 = 197 (kat 27 = kap 24 + kızak 2 + raf 0,2 + pay 0,8; ALT 74 = 20 + 27 + 27)","→ TOPPING derinlik 10 + 68 + 2 + 4 = 84 ✓ · STORE kaset katı 8 + 68 + 2 + 6 = 84 ✓ · TOPPING'de ayak yerine plint 12 (soğutma ızgarası)","#1d7a4f"),
 ("⑬ ÇATAL ucu: 2 lama 16×12 mm × 50 cm + sırt plakası ≈ 2 kg · dizi: gir 50 → kaldır 0,5 → çek 70 (5° yatık taşıma) · 18 çift L raf (TOPPING 6 + ALT 8 + STORE 4)","→ haftalık: robot 9–10 kap değişimi (kaşar 4–5 · kıyma 2 · kuşbaşı 2 · sucuk 1) + 4 STORE→ALT taşıma (gece) · eleman haftada 1 (5 kaşar + 1 sucuk + 2+2 donmuş, 10 boş alır) · uç değiştirici PRESS alt yuvaları","#1d7a4f"),
 ("⑫ Pide Ø30 → tepsi Ø32 zinciri: PRESS plaka Ø29 ✓ · PACK bıçak Ø28 ✓ · OVEN 44 (②) · robot tepsi eli v1 (Ø34) → v2 GEREK","→ harçlar KAVRULMUŞ/SOTE vakumlu (çiğ olmaz) · v25 açıkları: tırnak-cep hizası (pim + kamera) · −18 raf buzlanması · klape contaları · soğutma grubu ≤ 20 boy","#9a6b1f"),
]
yy=ky+22
KWc=KW-24
for a_,b_,c_ in K:
...
tx(X0,Y0-94,"AUTOKITCH — HAT v46 · TÜM İSTASYONLAR SON VERSİYON (5 Eyl 2026) — STORE v4 (+ v5 öneri: −18 kaset katı) · PRESS v8 · TOPPING v25 (kap 14×68×24 · çatal · L raf · kap arkaya dayalı) · OVEN tank+pompa · PACK 116",15,"start","bold")
tx(X0,Y0-72,"Robot: tek kol, 16–20 kg sınıfı (en ağır yük: kıyma kabı 8,6 + kap 4,6 + çatal 2,0 = 15,2 kg — ⑦ açık) · uç değiştirici: TEPSİ eli (pide press'ten kutuya kadar tepside, fırına tepsiyle) + PENÇE (hamur · kutu · içecek) + ÇATAL (kap, forklift gibi önden). Mavi = tepsi Ø32 (pide Ø30). Kırmızı = KONTROL bulgusu.",10,"start","","#333")
tx(X0,Y0-54,"Ölçüler cm. HER KABİN 70/65/140 × 197 × 84 (gövde 185 + ayak 12; TOPPING'de ayak yerine plint 12 = soğutma ızgarası). Açık: ② fırın kavitesi 44 · ⑤ Fersah Ø30 taban · ⑦ kol yükü 15,2 kg + menzil · ⑩ yağ pompa · ⑫ tepsi Ø32 zinciri (robot uç v2) · ① STORE v5 onayı · TOPPING prototipleri (spiral 11, kaşar akışı, çatal-cep hizası)",10,"start","","#333")

W=int(X0+px(T)+px(2200)); H=int(YT2+px(1860))
...
OUT=r"C:\Users\Kemal\Desktop\Kemal\WEBSİTE\AUTOKITCH\arastirma\FULL_MAKINE\hat_on_gorunus_teknik_v46.svg"
