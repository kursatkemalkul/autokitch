...
for r in range(10):                                               # plan: 6 raf dolu, 4 raf BOS
...
not_((a0+am)/2,Y0+px(345),"DONMUŞ −18° · plan ≈120 (6 raf) — alttaki 4 raf BOŞ")
not_((a0+am)/2,Y0+px(1560),"boş kızaklar — büyüme payı")
...
rc(a2+px(38),Y0+px(1518),px(432),px(292),1.4,4)                   # motorlu servis kapagi cercevesi
ln(a2+px(45),Y0+px(1665),a2+px(455),Y0+px(1665),1)                # kutu raflari ara cizgi
for i in range(4): rc(a2+px(50)+i*px(82),Y0+px(1538),px(66),px(115),1)   # ust sira: 4 dik kutu
rc(a2+px(50)+4*px(82),Y0+px(1538),px(66),px(115),1,2)             # tatli gozu (dik istif)
for g in range(3): ln(a2+px(58)+4*px(82),Y0+px(1568)+g*px(28),a2+px(108)+4*px(82),Y0+px(1568)+g*px(28),.8)
for i in range(5): rc(a2+px(50)+i*px(82),Y0+px(1678),px(66),px(115),1)   # alt sira: 5 dik kutu
rc(a2+px(468),Y0+px(1515),px(100),px(320),1.2,2)                  # 1L dik serit
rc(a2+px(473),Y0+px(1524),px(90),px(305),1)
not_((a2+a3)/2,Y0+px(1531),"İÇECEK — motorlu servis kapağı ardında · DİK kutu + yaylı itici · 9 kanal ≈ 110")
not_((a2+a3)/2,Y0+px(1830),"kavrama ÖN-ARKA yönde: 4 cm çıkıklık = arka parmak payı · 5. göz TATLI · 1L önü açık, sarma")
...
tx(X0,Y0-70,"AUTOKITCH — HAT v15 · SOĞUK DEPO 2 KAPI · 415 × 197 × 84 cm",15,"start","bold")
tx(X0,Y0-50,"ölçüler cm · HER ŞEY 3 GÜNLÜK: 240 top (120 donmuş + 120 taze) · 122 içecek (kola 48 · ayran 30 · fanta/sprite 18 · su 15 · 1L 11) · tatlı 20 — fazla hacim BOŞ · ayak 12",10.5,"start")

W=int(X0+px(T)+110); H=int(YT2+px(1860))
svg=(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}">'
     f'<rect width="{W}" height="{H}" fill="#ffffff"/>'+''.join(E)+'</svg>')
OUT=r"C:\Users\Kemal\Desktop\Kemal\WEBSİTE\AUTOKITCH\arastirma\hat_on_gorunus_teknik_v15.svg"
