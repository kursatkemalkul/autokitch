# -*- coding: utf-8 -*-
# STORE v8 — DÜZELTİLMİŞ: teknik bölme 250 (kompresör 200 + 50 pay), ayırıcı 42
#   alt buzluk (−18) · üst dolap (+3) · teknik bölme (soğutma + pano) ÜSTTE · ayaklar duruyor
#   A: kabuk aynı (teknik bölme 300)      → 16 çekmece
#   B: teknik bölme 250 + ayırıcı 42      → 17 çekmece, v1 kapasitesi tam
...
tx(30, 38, 'AUTOKITCH — 1 · STORE v8 (9 Eyl 2026) — MİNİMUM AÇIKLIK · TEKNİK BÖLME HESABI · İKİ ALTERNATİF — ölçüler mm', 14, 'start', 'bold', '#111')
...
rc(30, 84, 560, 350, 1, 6, '#bbb'); tx(42, 102, 'A · MİNİMUM AÇIKLIK — içerik yüksekliğinden', 8.6, 'start', 'bold')
...
# ---- ALTERNATİF A
rc(30, 450, 690, 565, 1, 6, '#bbb'); tx(42, 470, 'B · ALTERNATİF A — kabuk aynı (teknik bölme 300)', 8.6, 'start', 'bold')
...
cephe(120, 1000, 'A · 16 çekmece + 1 klape · ayırıcı 690', None, 300.0, 60.0, (690.0, 766.0), (690.0, 766.0),
      A_sol_ust, A_sol_alt, A_sag_ust, A_sag_alt, BLU)

# ---- ALTERNATİF B
rc(740, 450, 690, 565, 1, 6, '#bbb'); tx(752, 470, 'C · ALTERNATİF B — teknik bölme 250, ayırıcı 42', 8.6, 'start', 'bold')
B_sol_alt = [('KLAPE', 197.5, 284.0), ('donmuş', 514.5, 88.0)]
B_sol_ust = [('1L', 682.0, 300.0)] + dizi('içecek', 1015.0, 4, 132.0)
B_sag_alt = dizi('donmuş', 197.5, 3, 88.0)
B_sag_ust = dizi('hamur', 682.0, 8, 88.0)
cephe(830, 1000, 'B · 17 çekmece + 1 klape · ayırıcı 625 (düz hat)', None, 250.0, 60.0, (625.0, 667.0), (625.0, 667.0),
      B_sol_ust, B_sol_alt, B_sag_ust, B_sag_alt, GRN)

# ============================== D · KARŞILAŞTIRMA
rc(600, 84, 830, 350, 1, 6, '#bbb'); tx(612, 102, 'D · KARŞILAŞTIRMA', 8.6, 'start', 'bold')
...
rows2 = [('içecek', '4 · 280 kutu', '3 · 210', '3 · 210', '4 · 280'),
         ...
         ('ayırıcı bant', '690 · 76', '690 · 76', '690 · 76', '625 · 42'),
         ('teknik bölme', '300', '300', '300', '250'),
...
yy = para(616, yy+8, 'ALTERNATİF B kabuğu değiştirmiyor: dış ölçü 1400 × 1970 × 860 aynı, ayaklar aynı, soğutma ve pano yine üstte. Tek değişen teknik bölme 300 → 250 ve ayırıcı 76 → 42.', 176, 6.2, '#333')
yy = para(616, yy+4, 'B ile v1 kapasitesine BİREBİR dönülüyor: 17 çekmece · 280 kutu · 42 şişe · 8 hamur + 4 donmuş = 240 top · 4 kaset kabı. Ayırıcı bant iki modülde de 625 hizasında, tek düz panel.', 176, 6.2, GRN, 'bold')
...
yy = para(616, yy+4, 'TEKNIK BOLME 250 NEDEN: hermetik kompresor DIK calisir (yag karteri), yassi tipi yoktur. En alcak sinif Embraco NEK yaklasik 200 mm. Ustune titresim takozu ve boru payi 50 -> 250. Kondenserler bolmeye yatay sigmaz, arka duvara DIK monte edilir, fanlar da dik.', 176, 6.2, AMB)
yy = para(616, yy+4, 'UST SINIR 260 mm: bundan yuksegi sol modulun ust hucresini 990 altina dusurur ve bir icecek cekmecesi gider. Ayirici bandi 42 (PU 40) - iki soguk hucre arasi fark sadece 21 K, kacak yaklasik 12 W.', 176, 6.2, '#333')
tx(W-30, H-14, 'AUTOKITCH · arastirma/1_STORE/ist1_store_detay_v8 · 9 Eyl 2026', 7, 'end', '', GRY)

svg = '<svg xmlns="http://www.w3.org/2000/svg" width="%d" height="%d" viewBox="0 0 %d %d">\n<rect x="0" y="0" width="%d" height="%d" fill="#fff"/>\n' % (W, H, W, H, W, H) + '\n'.join(o) + '\n</svg>'
xml.dom.minidom.parseString(svg.encode('utf-8'))
out = r"C:\Users\Kemal\Desktop\Kemal\WEBSİTE\AUTOKITCH\arastirma\1_STORE\ist1_store_detay_v8.svg"
