# -*- coding: utf-8 -*-
"""TOPPING v2 (UNO'lu) · 3B MODEL · topping_uno_cad_v3 · 25 Eyl 2026
...
for _ad, _cx, *_r in UNO:
    if _ad in BICAK:
        CEP[_ad] = (_cx - 110.0, _cx + BICAK[_ad]["boy"] + 20.0, YAL_TABAN[0] - 1, 1296.0, ZT + 45.0, ZT - 45.0)
...
for i, (ad, cx, hw, hust, d_sil, agiz, doz) in enumerate(UNO):
    ...
    if agiz == "yassi":
        ...
        bor = silx(yb, ZT, 18.0, xb0, xb1).cut(silx(yb, ZT, 16.5, xb0 - 1, xb1 + 1)).cut(kut(xb0 + 4.0, xb1 - 4.0, yb - 20.0, yb - 14.0, ZT - g_ / 2.0, ZT + g_ / 2.0))
        ...
        ekle("%s_spreader_hortum_kovani" % k, sily(hx, ZT + 34.0, 7.0, 1296.0, YAL_TABAN[1] + 3.0).cut(sily(hx, ZT + 34.0, 4.5, 1295.0, YAL_TABAN[1] + 4.0)), "paslanmaz", "V",
             not_="hortum yalıtımdan kovanla geçer")
    else:
...
for ad in ("SOS", "HARC"):
    ...
    for pp in [p for p in P if p["ad"].startswith("%s_spreader_" % ad.lower()) and "hortum" not in p["ad"]]:
        kontrol("%s yalıtıma girmiyor" % pp["ad"], y_.intersect(pp["sh"]).Volume() < 1.0)
...
def glb_yaz(yol):
    ...
    while off[0] % 4: blob.append(b"\x00"); off[0] += 1
    bb_ = b"".join(blob)
    gl = {"asset": {"version": "2.0", "generator": "AUTOKITCH topping_uno_cad_v3"}, "scene": 0, "scenes": [{"nodes": [0]}], "nodes": nodes, "meshes": meshes,
...
if __name__ == "__main__":
    ...
    assert not kal, kal
    glb_yaz(os.path.join(OUT, "topping_uno_v3.glb"))
    with open(os.path.join(OUT, "topping_uno_v3.json"), "w", encoding="utf-8") as f:
        json.dump(dict(surum="topping_uno_cad_v3 · %s" % time.strftime("%d.%m.%Y %H:%M"),
