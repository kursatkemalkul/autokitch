...
rc(a0+px(60),Y0+px(370),am-px(35)-a0-px(60),px(540),1)            # donmus ic (4 raf, ust bolme)
rc(a0+8,Y0+px(950),am-px(35)-a0-8,px(50),1.6)                     # YATAY IZOLELI AYIRICI (alti +3)
for kz in range(7):
...
not_((a0+am)/2,Y0+px(345),"ÜST BÖLME DONMUŞ −18° · 4 raf = 80 (1 gün) — altı yatay izoleyle +3°")
for k5 in range(5):
    cy5=Y0+px(1035)+k5*px(140)
    rc(a0+px(35),cy5,am-px(70)-a0,px(128),1.4,3)
    for i in range(5): rc(a0+px(50)+i*px(82),cy5+px(8),px(66),px(112),1)
rc((a0+am)/2-px(60),Y0+px(1023),px(120),px(9),.9,2)
rc(a0+px(35),Y0+px(1745),am-px(70)-a0,px(95),1.4,3)
for i in range(4): rc(a0+px(70)+i*px(105),Y0+px(1752),px(80),px(80),1,2)
not_((a0+am)/2,Y0+px(1015),"HAFTALIK İÇECEK — 5 çekmece × 5 kanal × 11 ≈ 275 · SKU satışa göre")
not_((a0+am)/2,Y0+px(1738),"tatlı çekmecesi ≈48 (haftalık)")

# A2 sag yari: ILIK + TAZE + iade
a2,a3=am,xs[1]
ln(a2+12,Y0+px(310),a3-15,Y0+px(310),1)
ln(a2+px(35),Y0+px(315),a3-15,Y0+px(315),1.4)
ln(a2+px(35),Y0+px(1460),a3-15,Y0+px(1460),1.4)
...
# SAG ALT: genis 1L cekmecesi (haftalik)
rc(a2+px(40),Y0+px(1480),a3-px(60)-a2-px(40),px(345),1.6,3)
for i in range(3): rc(a2+px(90)+i*px(160),Y0+px(1500),px(90),px(305),1)
not_((a2+a3)/2,Y0+px(1472),"1L çekmecesi — 3 kanal × 8 = 24 (haftalık)")
...
tx(X0,Y0-70,"AUTOKITCH — HAT v26 · TEK DOLAPTA HER ŞEY (2 kapı) + SERVİS İSTASYONU 70",15,"start","bold")
tx(X0,Y0-50,"ölçüler cm · TEK KAPI fiziken olmaz (140 kanat açılamaz) → 2 KAPILI kombi: SOL üst donmuş 80 (1G) + altta HAFTALIK içecek 275 + tatlı 48 · SAĞ taze 160 (2G) + 1L ×24 · ambient koli bölmesi KALKTI",10.5,"start")

W=int(X0+px(T)+px(1030)); H=int(YT2+px(1860))
svg=(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}">'
     f'<rect width="{W}" height="{H}" fill="#ffffff"/>'+''.join(E)+'</svg>')
OUT=r"C:\Users\Kemal\Desktop\Kemal\WEBSİTE\AUTOKITCH\arastirma\hat_on_gorunus_teknik_v26.svg"
