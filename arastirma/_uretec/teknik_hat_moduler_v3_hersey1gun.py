# -*- coding: utf-8 -*-
"""AUTOKITCH - TABLALI HAT · MODULER v3 · HER SEY 1 GUN (14 Eyl 2026): ON + UST GORUNUS. Olculer mm.
Kemal: "her seyin bir gunluk oldugu versiyon; uc uca, duzgun; boy ne kadar kuculur?"
1 GUN: pide 80 = 4 cekmece · lahmacun 200 = 6 cekmece (35'lik) · icecek 70 = 2 cekmece · malzeme = yalniz Atosa'daki 1 gunluk set (yedek hazne yok) · kutu 1 gun.
MODULLER (uc uca hizali, ayak yok): PRESS 670 | TOPPING 885 | OVEN 1420 | KESME 500 (alti TEKNIK) | PACK 700 -> hat 4175 (tam boy STORE modulu gerekmez).
KOT: depo tek basina tabla 772 ile sigar; TEKNIK (sogutma grubu + pano) KESME altinda en az ~0,40 m2 -> 460 x (Hb - 180) >= 0,40 -> tabla 1050, yukseklik 1720.
DIZILIM (alttan uste): PRESS 4 pide · 2 lahm | TOPPING 4 lahm | OVEN-A bos | OVEN-B 2 icecek.
Kural: paftada yalniz gorunus + olcu + parca adi; aciklama mesajda.
"""
import os, math
from PIL import Image, ImageDraw, ImageFont

OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\HAT_MODULER_v3_hersey1gun_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 3800, 2650
S = 0.62
...
KOLON = [("PRESS", 25.0, HB - 360.0, [(4, "pide"), (2, "lahm")]),
         ("TOPPING", 17.5, HB - 381.0, [(4, "lahm")]),
         ("OVEN", 72.5, HB - 400.0, []),
         ("OVEN", 727.5, HB - 400.0, [(2, "icecek")])]
...
assert say == {"pide": 4, "lahm": 6, "icecek": 2}, say
...
txt(OX, 60, "AUTOKITCH  ·  TABLALI HAT  ·  MODÜLER  v3  ·  HER ŞEY 1 GÜN", f38, INK)
txt(OX, 128, "ön görünüş + üst görünüş  ·  her istasyon ayrı dolap, birleşince üst/alt/ön/arka çizgiler hizalı  ·  ayak yok  ·  hat %s × %s × 830  ·  tabla / bant %s  ·  her şey 1 gün  ·  ölçüler mm  ·  14 Eylül 2026" % (sayi(HAT), sayi(H), sayi(HB)), f13, GRAY)
...
txt(fx(kx0 + 250), fy(300.0) + 11, "çekmeceler · 1 gün", f7, GRAY, "mm")
...
txt(W_PX - 150, ly + 34, "HAT %s × %s × 830 · 5 modül · pide 4 çekmece (80) · lahmacun 6 (200) · içecek 2 (70) · yedek hazne yok · TEKNİK KESME altında" % (sayi(HAT), sayi(H)), f9, GRAY, "rm")

assert not os.path.exists(OUT), "moduler v3 zaten var — yeni numara ver"
im.save(OUT)
