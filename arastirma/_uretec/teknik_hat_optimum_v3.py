# -*- coding: utf-8 -*-
"""AUTOKITCH - TABLALI HAT · OPTIMUM v3 (14 Eyl 2026): ON + UST GORUNUS. Olculer mm.
Kemal: "komple yeniden dusun, en optimize sekli ve yerlesimi hesapla". KURALLAR AYNEN: tam 3 gun stok; Atosa'da 1 gunluk set (7 hazne);
  2 gunluk dolu hazne seti (14 hazne) sogutmali dolapta; 60 urun/saat; 830 derin; firinin onu arkasi bos yok.
HESAP: hat alti kullanilir yukseklik 120..1005 = 885 (tabla/bant 1405, firin alti 1105, Atosa alti 1084, PU 60).
  ihtiyac: 36 cekmece x 111 + 3 raf x 385 = 5151 -> 5151 / 885 = 5,8 -> 6 kolon (K1: 2 raf + 1 bos cekmece · K2: 1 raf + 4 cekmece · K3-K6: 8 cekmece)
  6 kolon = 35 + 6 x 620 + 5 x 35 = 3930 + PACK sarjoru 700 + 30 = 4660 = hat boyu.
  ust sira: PRES 670 + Atosa 885 + firin 1420 + KESME 500 + PACK 700 = 4175 -> 485 bos kalir -> TEKNIK buraya (485 x 1010, K1 ustu).
  sonuc: ayri yedek dolap kolonu gerekmez; hat 4830 -> 4660. Yedek raflar kizakli, insan ceker (robot degil).
Kural: paftada yalniz gorunus + olcu + parca adi; aciklama mesajda.
"""
import os, math
from PIL import Image, ImageDraw, ImageFont

OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\HAT_OPTIMUM_v3_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 4000, 2800
S, S2 = 0.62, 0.80
...
# ======================= BASLIK =======================
txt(OX, 70, "AUTOKITCH  ·  TABLALI HAT  ·  OPTİMUM  ·  v3", f38, INK)
txt(OX, 138, "ön görünüş + üst görünüş  ·  hat 4660 × 2075 × 830  ·  tam 3 gün stok  ·  Atosa 1 günlük set (7 hazne)  ·  2 günlük yedek set (14 hazne) hat altı raflarda  ·  TEKNİK hat başında  ·  ölçüler mm  ·  14 Eylül 2026", f13, GRAY)
...
# --- STORE (alt kat): 6 kolon · kullanilir yukseklik 120..1005 · cekmece adimi 111 · yedek hazne rafi adimi 385 (insan ceker)
XI, WO, BOL, STORE_TOP = 35.0, 620.0, 35.0, 1005.0
ADK = {"hamur": "TAZE PİDE", "lahm": "LAHMACUN", "icecek": "İÇECEK"}
CAPK = {"hamur": "%d top", "lahm": "%d top", "icecek": "%d kutu"}
ADET = {"hamur": 20, "lahm": 30, "icecek": 52.5}
ARKA = ["arka: HARÇ ×2", "arka: KAŞAR", "arka: KUŞ. ×4 · SUC. ×2"]
KOLON = [[("raf", 0), ("raf", 1), (1, "bos")], [("raf", 2), (4, "hamur")], [(8, "hamur")], [(8, "lahm")], [(8, "lahm")], [(4, "icecek"), (4, "lahm")]]
cx, CEK = XI, 0
for ki, kol in enumerate(KOLON):
    if ki:
        d.rectangle([fx(cx - BOL), fy(STORE_TOP), fx(cx), fy(CELL0 - BIND)], fill=SOFT, outline=LINE, width=1)
    d.rectangle([fx(cx), fy(STORE_TOP + 60.0), fx(cx + WO), fy(STORE_TOP)], fill=PUC, outline=GRAY, width=1)
    y = CELL0 - BIND
    for it in kol:
        if it[0] == "raf":
            raf_, on_l, ar_l = YEDEK[it[1]]
            d.rectangle([fx(cx + 8), fy(y + RAF_T), fx(cx + WO - 8), fy(y)], fill=(200, 200, 206))
            _k = cx + 8.0 + (604.0 - sum(GEN[t] for _, t in on_l) - HG * (len(on_l) - 1)) / 2
            for i_, (ad_, tip_) in enumerate(on_l):
                w_ = GEN[tip_]
                d.rectangle([fx(_k), fy(y + RAF_T + 360.0), fx(_k + w_), fy(y + RAF_T)], fill=BG, outline=INK, width=2)
                txt(fx(_k + w_ / 2), fy(y + RAF_T + 270.0), ad_, f8, INK, "mm")
                txt(fx(_k + w_ / 2), fy(y + RAF_T + 205.0), DOLUM[ad_], f7, RED, "mm")
                if i_ == 0:
                    txt(fx(_k + w_ / 2), fy(y + RAF_T + 110.0), ARKA[it[1]], f7, GRAY, "mm")
                _k += w_ + HG
            y += RAF_P
            continue
        n, tip = it
        ...
        ybas = y
        for _ in range(n):
            d.rectangle([fx(cx + 8), fy(y + h), fx(cx + WO - 8), fy(y)], fill=BG, outline=(BOSL if tip == "bos" else DOLAP), width=1)
            y += h + FUGA
        ym = (fy(ybas) + fy(y - FUGA)) / 2
        if tip == "bos":
            txt(fx(cx + WO / 2), ym, "boş çekmece", f7, GRAY, "mm")
            continue
        CEK += n
        ikinci = (CAPK[tip] % (n * ADET[tip])) + " · +3 °C"
    ...
    ust = y - FUGA
    assert ust <= STORE_TOP + 0.1, "kolon %d tasiyor %.1f" % (ki + 1, ust)
    if STORE_TOP - ust > 20:
        d.rectangle([fx(cx + 8), fy(STORE_TOP), fx(cx + WO - 8), fy(ust)], fill=(250, 250, 251), outline=BOSL, width=1)
        tarali(fx(cx + 8) + 1, fy(STORE_TOP) + 1, fx(cx + WO - 8) - 1, fy(ust) - 1)
    txt(fx(cx + WO / 2), fy(CELL0 - BIND) + 12, "K%d · üst %s" % (ki + 1, sayi(ust)), f7, GRAY, "mm")
    cx += WO + BOL
TEK0X = cx - BOL
assert TEK0X <= X0["PACK"] - 30.0, "STORE PACK altina tasiyor"
olcu_h(fx(XI), fx(XI + WO), fy(0) + 44, "620", f8, GRAY)
olcu_h(fx(0), fx(TEK0X), fy(0) + 80, "STORE  %s  ·  %d çekmece + 3 yedek hazne rafı  ·  tabla hattı altında" % (sayi(TEK0X), CEK), f11, INK)
...
d.rectangle([fx(25), fy(H - 30.0), fx(YW - 25), fy(1640.0)], fill=BG, outline=INK, width=2)
txt(fx(YW / 2), fy(1850.0) - 12, "HAT PANOSU", f8, INK, "mm")
txt(fx(YW / 2), fy(1850.0) + 12, "PLC · fırın kontaktörleri", f7, GRAY, "mm")
d.rectangle([fx(25), fy(1600.0), fx(YW - 25), fy(STORE_TOP + 95.0)], fill=BG, outline=INK, width=2)
txt(fx(YW / 2), fy(1340.0) - 12, "SOĞUTMA GRUBU", f8, INK, "mm")
txt(fx(YW / 2), fy(1340.0) + 12, "STORE + yedek raflar", f7, GRAY, "mm")
...
txt(W_PX - 170, ly + 34, "HAT %s × 2075 × 830  ·  TEKNİK 485  ·  TOPPING Atosa uyarlama 885 · 7 hazne (1 gün)  ·  OVEN 1420 · 4 ürün  ·  STORE %d çekmece + 3 yedek raf (14 hazne, 2 gün)" % (sayi(HAT), CEK), f9, GRAY, "rm")

assert not os.path.exists(OUT), "optimum v3 zaten var — yeni numara ver"
