...
OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\HAT_2KOL_v8_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 6200, 3400
S = 0.62
...
CELL0 = 182.5
YUZ0 = CELL0 - BIND
HH = {"hamur": 75.0, "lahm": 60.0, "depo": 241.0}
AD = {"hamur": "TAZE PİDE", "lahm": "LAHMACUN", "depo": "KAŞAR + SUCUK DEPOSU · kapaklı"}
CAP = {"hamur": (20, "top"), "lahm": (35, "top"), "depo": (0, "26,4 kg kaşar + 8,4 kg sucuk · 6 gün")}
...
KOLON = [[(6, "hamur")], [(2, "hamur"), (6, "lahm")], [(6, "lahm"), (1, "depo")]]   # B: 8 pide + 12 lahm + kasar/sucuk deposu, 3 kolon
K1_TABAN = 240.0                                            # K1 altinda B sogutma grubu tabani (0-240, plint yerine)
...
PLAKA = 1640.0                                              # alt plaka kotu = 1060 + 580 (VARSAYIM - katalogda yok)
# D · FIRIN kolonu
TEK = (123.0, 540.0)
YAG = (545.0, 790.0)
KES = (795.0, 1060.0)
FIR = [(1065.0, 1380.0, "FIRIN HAZNESİ 1|LAHMACUN · 120 s"), (1385.0, 1700.0, "FIRIN HAZNESİ 2|LAHMACUN · 120 s"),
       (1705.0, 2028.5, "FIRIN HAZNESİ 3|PİDE · PİZZA · 240 s · ayrı sıcaklık")]
# E · KUTU kolonu
E_MEK = (123.0, 420.0)
E_AGZ = (430.0, 680.0)
E_SAR = (690.0, 2028.5)
...
# ======================= BASLIK =======================
txt(OX, 70, "AUTOKITCH  ·  HAT 2 KOL  ·  TEKNİK RESİM  v8  ·  5 KAPALI İSTASYON  ·  KAŞAR + SUCUK 6 GÜN B'DE KAPAKLI  ·  HAT 3500 × 2030", f38, INK)
txt(OX, 138, "ön · üst · yan görünüş  ·  günde 80 pide + 200 lahmacun (+ pizza)  ·  2 gün stok  ·  her istasyon kapalı ürün  ·  20 contalı motorlu çekmece + kaşar/sucuk 6 günlük kapaklı bölme tek modülde  ·  içecek + QR dolabı + servis: SERVİS_TESLİM paftası  ·  hat 2030 (B 1060 + pres 950 + pay)  ·  harç 2 kaset (kaşar kalıbı 280)  ·  2 × Fairino FR10 koridorda  ·  QR dolabı yok  ·  tüm modüller 830 derin  ·  ölçüler mm  ·  17 Eylül 2026", f13, GRAY)
...
d.rectangle([fx(X_A + 33), fy(K1_TABAN), fx(X_A + XI + WO + 17), fy(20.0)], fill=SOFT, outline=LINE, width=2)
kesik(X_A, 70.0, 470.0, 30.0, 230.0, "SOĞUTMA GRUBU 400 × 300 × 220 · ⅓ HP", INK, "hava ön ızgaradan")
kesik(X_A, 490.0, 690.0, 30.0, 230.0, "B KARTI|24 V güç", INK)
txt(fx(X_A + 380), fy(K1_TABAN) + 14, "B teknik tabanı 0–240 (plint yerine)", f7, GRAY, "mm")
cx = X_A + XI
for ki, gruplar in enumerate(KOLON):
    ...
    for adet, tip in gruplar:
        ...
        ybas = y
        for _ in range(adet):
            d.rectangle([fx(cx + 8), fy(y + h), fx(cx + WO - 8), fy(y)], fill=BG, outline=DOLAP, width=(2 if tip == "depo" else 1))
            if tip == "depo":
                for hy in (y + 40.0, y + h - 40.0):
                    d.rectangle([fx(cx + 10), fy(hy + 12.0), fx(cx + 26), fy(hy - 12.0)], fill=DOLAP)
                d.rectangle([fx(cx + WO - 60), fy(y + h / 2 + 30.0), fx(cx + WO - 48), fy(y + h / 2 - 30.0)], fill=INK)
                drect(fx(cx + 60), fy(y + h - 30.0), fx(cx + WO - 90), fy(y + 30.0), GRAY, 1)
                txt(fx(cx + WO / 2 - 15), fy(y + h - 45.0), "üst raf: kaşar 26,4 kg (vakum paketler) · alt raf: sucuk 14 çubuk × 3", f7, GRAY, "mm")
            y += h + FUGA
        cap, br = CAP[tip]
...
modul_etiketi(X_BC, W_BC, H_B, H_MAK, "MODÜL C · TOPPING (kendi soğutucusu üstte)", "1400 × 830 × 970 · B üstünde")
...
txt(fx(X_A + W_B / 2), fy(0) + 190, "MODÜL B · ÇEKMECE MODÜLÜ · 8 pide + 12 lahmacun + kaşar/sucuk 6 günlük kapaklı bölme · 3 kolon · soğutucusu K1 altında · A ve C üstüne oturur", f8, BG, "mm")
txt(fx(X_A + W_B / 2), fy(0) + 210, "2100 × 830 × 1060", f7, BG, "mm")
...
txt(SX1, FY_TOP - 152, "pres kolonu: K1 (soğutma tabanı) + PZP-400", f9, GRAY)
d.rectangle([z1(-790), fy(K1_TABAN), z1(40), fy(0)], fill=SOFT, outline=LINE, width=2)
drect(z1(-700.0), fy(230.0), z1(-400.0), fy(30.0), INK, 1)
txt(z1(-550.0), fy(130.0) - 8, "SOĞUTMA GRUBU", f7, INK, "mm"); txt(z1(-550.0), fy(130.0) + 8, "300 derin · 220 yüksek", f7, GRAY, "mm")
txt(z1(-200.0), fy(130.0), "hava ön ızgara", f7, GRAY, "mm")
...
drect(z1(-760.0), fy(PZP[0] + 480.0), z1(20.0), fy(PZP[0] + 10.0), GRAY, 1)
txt(z1(-370.0), fy(PZP[0] + 240.0) - 8, "PZP-400 gövdesi", f7, INK, "mm"); txt(z1(-370.0), fy(PZP[0] + 240.0) + 8, "800 derin · motor + rezistans", f7, GRAY, "mm")
...
d.rectangle([z1(-10.0), fy(PLAKA + 220.0), z1(40.0), fy(PLAKA + 10.0)], fill=AGZ, outline=RED, width=2)
...
txt(sz(-270.0), fy(T_AGZ[0] + 35.0), "TABAN Ø300 · tepside", f7, ACC, "mm")
...
txt(sz2(-705.0), fy(1540.0), "EGZOZ", f7, GRAY, "mm")
txt(sz2(40) + 14, fy(1540.0), "MOTORLU KAPAK", f8, RED, "lm")
...
PARCA = [
    ("MODÜL B", "ÇEKMECE modülü · 20 çekmece + kapaklı depo · soğutma grubu K1 altında", "1", "2100 × 830 × 1060 · K1 6 pide · K2 2 pide + 6 lahm · K3 6 lahm + kaşar/sucuk deposu"),
    ...
    ("DEPO", "kapaklı soğuk bölme 620 × 680 × 240 · 2 raf · elle açılır", "1", "kaşar 26,4 kg + sucuk 8,4 kg · 6 gün · K3 üstü"),
    ("SOĞUTMA", "B: ⅓ HP 400 × 300 × 220 (1,0 m³) · C: ⅕ HP 300 × 250 × 220 (0,19 m³)", "2", "B tabanı K1 altı · C üst bandı"),
...
txt(W_PX - 170, ly, "HAT %s × 2030 × 830  ·  5 kapalı istasyon (B altta, A + C üstünde)  ·  2 gün  ·  20 çekmece (8 pide · 12 lahmacun) + kaşar/sucuk 6 gün kapaklı  ·  Atosa dozaj tablasız: 2 harç 280 + kıyma + kuşbaşı + kaşar 280 + sucuk dilimleyici  ·  FIRIN 3 göz  ·  2 × FR10  ·  QR dolabı yok" % sayi(HAT), f9, GRAY, "rm")
