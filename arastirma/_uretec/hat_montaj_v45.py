...
birim("TOPPING_MODUL", "TOPPING v2 (UNO'lu): 4 UNO çekirdeği (sos · harç · kıyma · kuşbaşı) + kaşar ve küp sucuk kaseti + hava tesisatı · tabla mekanizması v1'den", "C", "GERCEK_MODUL",
      (X_BC, X_BC + W_BC), (H_B, H_B + TC.Y), (-DZ, 0.0), "sac", "topping_uno_cad_v5.py + topping_cad_v22.py", "hat/topping_v2.html")
...
def yolculuk(parcalar):
    ...
    TOPZ.git(5.6, 6.4, ZT); TOPY.git(6.4, 6.9, H_B + 108.0 + TOP_R * TOP_K + 0.5, "ss")
    ...
    # top
    kanal("URUN__top", lambda t: (TOPX(t) * MM, (TOPY(t) - (TOP_R * TOP_K) * ACMA(t) * 0.95) * MM, TOPZ(t) * MM))
    ...
    # TOPPING düğümleri
    P_T = (OX + TC.XC_TABLA, 0.0, ZT); A_K = (OX + TC.XC_TABLA, OY + 116.0, ZT); ya = math.radians(17.82)
    ...
    for ad in sorted({a_ for a_, _m, _x in parcalar if a_.startswith("TOPPING_MODUL__") and a_.count("__") == 2}):
        ...
        if g == "ARABA":
            ...
            kanal(ad, lambda t: pivotlu(P_T, qy(TH(t)), (X(t) - TC.XC_TABLA, 0.0, 0.0)))
            kanal(ad, lambda t: qy(TH(t)), "rotation")
        elif g == "ACICI":
            kanal(ad, lambda t: (0.0, 60.0 * KAFA(t) * MM, 0.0))
        elif g in ("KONI_ON", "KONI_ARKA"):
            yon = 1.0 if g == "KONI_ON" else -1.0; ax = (0.0, math.sin(ya), yon * math.cos(ya))
            kanal(ad, lambda t, ax=ax, yon=yon: pivotlu(A_K, qax(ax, yon * KONI(t)), (0.0, 60.0 * KAFA(t), 0.0)))
            kanal(ad, lambda t, ax=ax, yon=yon: qax(ax, yon * KONI(t)), "rotation")
        elif g in ROL:
            P_, r_ = ROL[g]
            kanal(ad, lambda t, P_=P_, r_=r_: pivotlu(P_, qax((0, 0, 1), -math.degrees(BANT(t) / r_))))
            kanal(ad, lambda t, P_=P_, r_=r_: qax((0, 0, 1), -math.degrees(BANT(t) / r_)), "rotation")
        elif g.startswith("PISTON_") and g[7:] in PIS:
            k = g[7:]; kanal(ad, lambda t, k=k: (0.0, 0.0, -PIS[k](t) * MM))
        elif g.startswith("VALF_") and g[5:] in VAL:
            k = g[5:]; P_ = (OX + TU.GRUP[g][0], TU.GRUP[g][1], TU.GRUP[g][2])
            kanal(ad, lambda t, k=k, P_=P_: pivotlu(P_, qax((1, 0, 0), -90.0 * VAL[k](t))))
            kanal(ad, lambda t, k=k: qax((1, 0, 0), -90.0 * VAL[k](t)), "rotation")
        elif g.startswith(("HELEZON_", "KARISTIRICI_")):
            k = g.split("_", 1)[1]; Z_ = HEL[k] if g.startswith("HELEZON_") else KAR[k]; P_ = (OX + TU.GRUP[g][0], TU.GRUP[g][1], 0.0)
            kanal(ad, lambda t, Z_=Z_, P_=P_: pivotlu(P_, qax((0, 0, 1), -Z_(t))))
            kanal(ad, lambda t, Z_=Z_: qax((0, 0, 1), -Z_(t)), "rotation")
    # B çekmecesi + robot
    ...
    for o in OZEL:
        g = o["ad"].split("__", 1)[1]
        if g == "KOL": kanal(o["ad"], lambda t: KC.quat("z", KC.kol_beta(te(t))), "rotation")
        elif g == "PARMAK": kanal(o["ad"], lambda t: KC.quat("z", KC.parmak_psi(te(t))), "rotation")
        elif g == "B_ROOT":
            T0 = o["T"]
    ...
    return A, [dict(t=round(a[0], 2), ad=a[1], not_=a[2]) for a in ADIM], float(T_J)
...
if __name__ == "__main__":
    ...
    with io.open(os.path.join(OUT, "durum.json"), "w", encoding="utf-8") as f:
        json.dump(dict(hat=dict(w=HAT_W, h=H_MAK, d=DZ, pafta="HAT_ATOSA_TABLALI v12 · v45 · K = kesme_cad_v1 (gercek) · 1 tam animasyon · alt kisim v3 · B = store_cad_v5 · kompresor K tabani arkasinda · TOPPING v2 UNO'lu (topping_uno_cad_v5) · E = kutu_cad_v3 · alt taban 123 · taban hizasi · surec 1168"), sayac=sayac, animasyon=dict(sure=T_J, adim=ADIM_HAT), modul=[dict(kod=k, ad=a, x=[x, x + w], y=list(MODUL_Y[k])) for k, a, x, w in MODUL],
