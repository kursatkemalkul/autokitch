...
NE ÇÖZÜLDÜ
  · KASET ÖNE TAŞIYORDU: kulp z = 0'ın 48 mm önüne çıkıyordu, kabin kapanmıyordu. Kaset 80 mm içeri alındı,
    önüne yalıtımlı KAPAK kondu; kulp artık kabinin içinde (topping_hesap_v2.py · derinlik dizilimi).
...
ÇIKTI: arastirma/3_TOPPING/topping_modul_v2/ (parça başına STEP + STL, MONTAJ.step, BOM.csv) · otonom/hat3d/ (GLB/USDZ)
...
URETIM = os.path.join(KOK, "arastirma", "3_TOPPING", "topping_modul_v2")
from kaset_3d_v3 import Mesh, MM, MALZEME, doku_ad, usdz_yaz, OUT
import topping_hesap_v2 as H
...
def modul():
    ...
    for ad, x0, x1, y0, y1 in PUP:
        ...
        ekle(ad, w_, "pu", bom=("PU yalıtım paneli 60 mm", 4, "poliüretan 60 · λ 0,022 W/mK", "soğuk hücreyi sarar; ısı kaybı hesabı topping_hesap_v2") if ad == "pu_taban" else None)
...
if __name__ == "__main__":
    ...
    print("TOPPING MODULU v2 · %d parca (%d cizili + %d yalniz listede) · %.0f sn" % (len(PARCALAR), len(gercek), len(PARCALAR) - len(gercek), time.time() - t0))
