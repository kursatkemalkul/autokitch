...
M=[("SOĞUK DEPO — HAMUR + İÇECEK + TATLI",2000),("PRES",700),("DOZAJ",700),("FIRIN",650),("KESİM+KUTU",700)]
...
ci(a0+px(500),Y0+px(140),px(80),1.2); ci(a0+px(1500),Y0+px(140),px(80),1.2)
ln(a0+px(460),Y0+px(100),a0+px(540),Y0+px(180),.9); ln(a0+px(460),Y0+px(180),a0+px(540),Y0+px(100),.9)
ln(a0+px(1460),Y0+px(100),a0+px(1540),Y0+px(180),.9); ln(a0+px(1460),Y0+px(180),a0+px(1540),Y0+px(100),.9)
...
not_((a0+a1)/2,Y0+px(58),"TEK PARÇA 3 kapılı kombine (standart) · soğutma ×2: −18° kolon + tüm +3° hacim")
...
# 3. KOLON: ICECEK + TATLI (ayni +3 hacim, ince ayirici)
ln(am2,Y0+px(310),am2,YT-6,1.2)
g0,g1=am2,a1
for yk in (540,780,1010):
    yv=Y0+px(yk); ln(g0+12,yv,g1-15,yv,1)
    for i in range(6): rc(g0+14+i*px(92),yv-px(118),px(66),px(115),1)
yv=Y0+px(1310); ln(g0+12,yv,g1-15,yv,1)
for i in range(5): rc(g0+16+i*px(112),yv-px(305),px(90),px(300),1)
for r in range(2):
    yv2=Y0+px(1530)+r*px(220); ln(g0+12,yv2+px(60),g1-15,yv2+px(60),1)
    for i in range(6): ci(g0+30+i*px(92),yv2,px(35),1)
not_((g0+g1)/2,Y0+px(345),"İÇECEK — kanal/SKU satışa göre (BEYİN planogramı)")
not_((g0+g1)/2,Y0+px(1345),"kutu 3 raf × 6 kanal + 1L × 5 · derinlemesine ≈ 3 gün")
not_((g0+g1)/2,Y0+px(1800),"paketli tatlı 2 raf")
...
ln(am,YT2,am,YT2+px(840),1.4); ln(am2,YT2,am2,YT2+px(840),1.2)
...
not_((a0+a1)/2,YT2+px(45),"tek kasa 3 kapı: donmuş · taze · içecek+tatlı — kapı yayları")
...
# SOGUK DEPO 3. kolon ustten: kanallar
for i in range(7): ln(am2+px(20)+i*px(90),YT2+px(60),am2+px(20)+i*px(90),YT2+px(760),1)
for i in range(6):
    for j in range(3): ci(am2+px(65)+i*px(90),YT2+px(150)+j*px(250),px(30),.9)
...
tx(X0,Y0-70,"AUTOKITCH — HAT v14 · SOĞUK DEPO BİRLEŞİK · 475 × 197 × 84 cm",15,"start","bold")
tx(X0,Y0-50,"ölçüler cm · 3 GÜNLÜK TÜM SOĞUK STOK TEK DOLAPTA: 200 donmuş + 160 taze + içecek + tatlı · motor 4→2 grup · kanal/SKU satışa göre · ayak 12",10.5,"start")

W=int(X0+px(T)+110); H=int(YT2+px(1860))
svg=(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}" width="{W}" height="{H}">'
     f'<rect width="{W}" height="{H}" fill="#ffffff"/>'+''.join(E)+'</svg>')
OUT=r"C:\Users\Kemal\Desktop\Kemal\WEBSİTE\AUTOKITCH\arastirma\hat_on_gorunus_teknik_v14.svg"
