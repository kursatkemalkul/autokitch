...
def tablali():
    ...
    baslik("AUTOKITCH  ·  ATOSA TABLALI HAT  ·  TEKNİK RESİM  v13  ·  MONTAJ = hat_montaj_v47  ·  TOPPING = topping_uno_cad_v5 (4 UNO + kaşar/sucuk kaseti · kama yarık) + topping_cad_v23 tabla · K = kesme_cad_v1 · E = kutu_cad_v3 · B = store_cad_v5 (tam kaplama)  ·  ALT TABAN 123 · SÜREÇ 1168  ·  HAT 5430 × 2030 × 830",
           "ön · üst · yan görünüş  ·  günde 80 pide + 200 lahmacun (+ pizza)  ·  2 gün stok  ·  her istasyon kapalı ürün  ·  bütün gövdeler yerden 123'te başlar, istasyon tabanları 1060  ·  ürün tepsisiz: hamur topu çalışma diskinde KONİLİ AÇICI ile açılır, dozajlanır, bıçak burunlu bant fırına çeker → K bandı (sprey + kesme aynı kafada) → kutu  ·  TOPPING: sos · harç · kıyma · kuşbaşı = UNO çekirdeği (Beldos valf + ürün silindiri + Ø32 hava silindiri, bizim hazne) · sos + harç spreader · 3 mm taşıyıcı raf · kaşar + küp sucuk = bizim kaset · hava: kompresör K tabanında  ·  TEK Fairino FR5 yer rayında · omuz 970 · ray ekseni hat yüzünden 360  ·  ölçüler mm  ·  26 Eylül 2026")
    ...
    for ad, a0, a1, ust, hz, yassi in UNO9:
        ...
        d.rectangle([X(c - 91), fy(1380.0), X(c - 41), fy(1336.0)], fill=BG, outline=ACC, width=2)
        ...
        uc = 1262.0 if yassi else 1216.0
        d.rectangle([X(c - 18), fy(1340.0), X(c + 18), fy(uc)], fill=BG, outline=INK, width=2)
        if yassi:                                                  # v10 · spreader (topping_uno_cad_v4)
            d.rectangle([X(c - 25), fy(1274.0), X(c + 25), fy(1262.0)], fill=BG, outline=INK, width=1)
            d.rectangle([X(c - 18), fy(1262.0), X(c + 18), fy(1240.0)], fill=BG, outline=INK, width=2)
            d.rectangle([X(c - 54), fy(1294.0), X(c - 26), fy(1240.0)], fill=BG, outline=ACC, width=2)
            d.polygon([(X(c - 20), fy(1242.0)), (X(c + 123), fy(1242.0)), (X(c + 123), fy(1214.0))], fill=BG, outline=INK)
            d.rectangle([X(c - 8), fy(1218.0), X(c + 125), fy(1182.0)], fill=BG, outline=INK, width=2)
    ...
    txt(X(1150.0), fy(1200.0), "ağızlar tabla ekseninde (z −170) · sos + harç SPREADER: dağıtıcı boru tabla yarıçapı boyunca 133, altı 1182 · yuvarlak uç 1216 · pide üstü 1176", f7, INK, "mm")
    d.rectangle([X(90.0), fy(1320.0), X(1710.0), fy(1317.0)], fill=INK)
    txt(X(1250.0), fy(1300.0), "3 mm TAŞIYICI RAF 1317–1320 · AISI 304 · 154 kg · sehim 3,4 · büküm 40 (altı yalıtımsız)", f7, INK, "mm")
    drect(X(150.0), fy(1660.0), X(458.0), fy(1600.0), GRAY, 1); txt(X(304.0), fy(1630.0), "valf adası (arkada)", f7, GRAY, "mm")
    drect(X(1650.0), fy(1420.0), X(1700.0), fy(1250.0), GRAY, 1); txt(X(1675.0), fy(1236.0), "şartlandırıcı", f7, GRAY, "mm")
    ...
    modul_etiketi(X_C, W_C, "MODÜL C · TOPPING v2 (topping_uno_cad_v4) · 4 UNO + 2 bizim kaset · havalı", "1800 × 830 × 970 · B üstünde · disk %s" % sayi(P))
    olcu_h(fx(X_C), fx(X_C + W_C), fy(H_MAK) - 26, sayi(W_C), f11, INK)
    # ---- F K E
    ciz_F([])
    ...
    txt(fx(1600.0), py(-630.0) - 14, "UNO: valf + ürün silindiri soğuk hücrede (z −346…−559) · mil arka yalıtımdan kovanla geçer (−560…−635) · Ø32 hava silindiri kuru bölmede (−672…−811)", f7, GRAY, "mm")
    ...
    parca_listesi([
        ...
        ("MODÜL C", "TOPPING v2 (topping_uno_cad_v5 · sos + harç yayıcısında KAMA YARIK: sos 2,0→3,2 · harç 6,0→9,6) · 4 UNO çekirdeği (Beldos valf + ürün silindiri Ø52/Ø70 + Ø32 hava silindiri · bizim hazne: sos 15 L V · harç 45 L · kıyma 8 L · kuşbaşı 8 L) · sos + harç spreader + havalı kesme valfi · kaşar kaseti 280 + küp sucuk kaseti 140 · 3 mm taşıyıcı raf · valf adası 12 × 5/2 + şartlandırıcı kuru bölmede", "1", "1800 × 830 × 970 · disk üstü 1168 · tavan sos+harç 1968 · UYARI: tekne sağa 85, kasnak 35 mm taşıyor"),
        ("HAVA", "JUN-AIR OF302-15B yağsız kompresör · 15 L · 43 L/dk @ 7 bar · K tabanı ARKADA (130–640), üstünde tereyağı tankı + pano · K kesicisi de bu hattan · Ø10 ana hat F tabanı arkasından TOPPING'e", "1", "380 × 380 × 510 · 25 kg · arka sacta ızgara"),
        ...
        ("BANDA AKTARMA", "BIÇAK BURUNLU · bant Ø20 burun silindirine dolanır, üst yüzü diskin 2 mm altı (1166) · pide boşluğu kendi gövdesiyle köprüler", "1", "itici ve köprü YOK · Ø60 kauçuk tahrik silindiri + PTFE kaplı cam elyaf bant · v13: bant motoru ARKADA, tahrik silindiriyle eş eksenli · ön sac + mil uçları ön yüzün içinde · çıkış yarığı 294 × 44"),
        ("MODÜL F", "KONVEYÖR FIRIN · özel · elektrikli · hazne 1400 · aynı anda 4 ürün · taban dolabı: önde bulaşık + temizlik + pizza kutusu yedeği · arkada deterjan + robot kontrol + ana pano + UPS", "1", "1500 × 830 × 2030 · gövde 1060–1486 · bant 1166 · AÇIK: bant altı pay 106 (alt ısıtma)"),
    ...
    lejant()
    yol = KLASOR + r"\HAT_ATOSA_TABLALI_v13_HD.png"
    im.save(yol, dpi=(int(200 * K_HD), int(200 * K_HD))); print("yazildi:", yol, "· hat", sayi(HAT), "· bilek", round(D1), round(D2))
    return yol
...
if __name__ == "__main__":
    ...
    k.save(KLASOR + r"\HAT_ATOSA_TABLALI_v13_EKRAN.png", optimize=True); print("yazildi EKRAN", k.size)
