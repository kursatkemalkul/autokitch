# -*- coding: utf-8 -*-
"""TP10 SÜRÜMÜ v2 (26 Eyl 2026) · AYRI SÜRÜM — ana makine v47 aynen durur. F = TP10 kesitli fırın, gövdesi 1500'e uzatılmış
...
birim("D_TABAN_KABIN", "F taban dolabı 123–956 (TP10 SÜRÜMÜ: fırın gövdesi bant 1166 için 956'ya iner · istasyon tabanı kuralı 1060'tan 104 aşağı) · önde bulaşık + temizlik + pizza kutusu yedeği · arkada deterjan + robot kontrol + ana pano + UPS", "D", "KUTU", (X_D, X_D + W_D), (Y_ALT, FT.YG0), (-DZ, 0.0), "kabin", "v44 alt kısım v3 · TP10 sürümünde üstü 956")
...
for kod_, ad_, (a_, b_), (y0_, y1_), (z0_, z1_), kat_, kay_ in (
        ...
        ("D_TEMIZLIK", "Temizlik · 2 × 5 L bidon (deterjan / dezenfektan) + üstte bez · eldiven · poşet · önde", (510.0, 656.0), (130.0, 950.0), (-210.0, -20.0), False, "TP10 sürümü: üstü 950 · B'nin K4 nişinden taşındı · bidon 130 × 190 × 290 VARSAYIM"),
        ...
        ("D_PIZZA_YEDEK", "Pizza kutusu yedeği · TP10 SÜRÜMÜ: 505 kutu düz (804 × 404 × 808 + taban 15) · şarjör 567 + 505 = 1072 = 3,8 gün — 4 gün için 48 kutu eksik (KARAR)", (666.0, 1470.0), (130.0, 953.0), (-424.0, -20.0), False, "kutu 1,6 mm (kutu_cad_v3) · 1,8 mm olursa 517"),
...
# ---------------------------------------------------------------- v45 · 1 TAM ANİMASYON: ÜRÜN YOLCULUĞU (TP10 SÜRÜMÜ v2: yolculuk_tp10_v2) ----------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    for b in B:
        ...
        if b["durum"] == "GERCEK_MODUL":
            ...
            ps = [p for p in TC.PARCALAR if v1_kalir(p["ad"]) and p["ad"] not in KAPAK and p["ad"] not in AKTARMA_TP10]   # TP10 SÜRÜMÜ: aktarma bandı yok
            ton = {}
            for p in ps:
            ...
            for (t_, g_), m_ in sorted(ton.items()):
                parcalar.append((dugum_ad(b["kod"], t_, g_), m_, mal_ad(b, t_)))
            b["parca"] = len(ps)
        elif b["durum"] == "GERCEK_FIRIN":                                                   # TP10 SÜRÜMÜ
    ...
    _yeni = [b for b in B if b["modul"] in ("D", "K", "E") and not b["kod"].endswith("_KABIN") and b["durum"] not in ("GERCEK_KUTU", "GERCEK_KESME", "GERCEK_FIRIN")]   # TP10 v2: fırın parçaları gerçek katıyla taranır · v45: K kendi taramasını yapar (kesme_cad_v1) · v38: E kendi taramasini yapiyor (kutu_cad_v2: makine 201 an + kutu 46 an + pizza 25 an = 0)
    _cak = []
    ...
    print("ANA MONTAJ ANIMASYONU (TP10 SURUMU v2): tek tam dongu %.0f sn · %d kanal · %d adim · %d eksenli doner dugum" % (T_J, len(ANIM_HAT), len(ADIM_HAT), len(OZEL_T)))
    ...
    b1 = glb_yaz(os.path.join(OUT, "hat_tp10_v2.glb"), [x for x in parcalar if x[0] not in HARIC_T], dokular, ozel=OZEL + OZEL_T, liste=ANIM_HAT)   # v46: donenler kendi ekseninde
    ...
    print("hat_tp10_v2.glb · %d dugum · %.0f KB · animasyon %.0f sn" % (len(parcalar), b1 / 1024.0, T_J))
    _usd = [x for x in parcalar if not x[0].startswith("URUN__")]                         # ürün düğümleri yalnız animasyonda (USDZ durağan)
    b2, prim, sorun, _u = usdz_yaz([os.path.join(OUT, "hat_tp10_v2.usdz")], "hat_tp10_v2", _usd + E_USDZ, dokular)
    print("hat_tp10_v2.usdz · %.0f KB · %d prim · USD denetimi: %s" % (b2 / 1024.0, prim, "GECTI" if not sorun else "KALDI"))
    for x in sorun: print("   HATA:", x)
    # ---- her modul icin AYRI model: istasyon sayfasi kendi modulunu BUYUK gosterir ----
    MODUL_DOSYA = {}
    for mk in ("D",):                                                              # TP10 SÜRÜMÜ: yalnız F (modul_* ana dosyalarına dokunmaz)
        hrf = "R" if mk == "-" else mk
    ...
    with io.open(os.path.join(OUT, "durum_tp10_v2.json"), "w", encoding="utf-8") as f:
        json.dump(dict(hat=dict(w=HAT_W, h=H_MAK, d=DZ, pafta="TP10 SURUMU v2 (ayri) · FIRIN_TP10 v3 · ana makine v47 + F = TP10 kesitli firin 1500 (bant govde disina cikmaz, isitilan 1316, 4 urun) · giris bandi on odada · K bandinda giris citi · K parcalari degismez · K = kesme_cad_v1 (gercek) · 1 tam animasyon · alt kisim v3 · B = store_cad_v5 · kompresor K tabani arkasinda · TOPPING v2 UNO'lu (topping_uno_cad_v5) · E = kutu_cad_v3 · alt taban 123 · taban hizasi · surec 1168"), sayac=sayac, animasyon=dict(sure=T_J, adim=ADIM_HAT), modul=[dict(kod=k, ad=a, x=[x, x + w], y=list(MODUL_Y[k])) for k, a, x, w in MODUL],
                       dosya=MODUL_DOSYA, tp10_denetim=TP10_OZET, birim=[dict(kod=b["kod"], ad=b["ad"], modul=b["modul"], durum=b["durum"], olcu=b["olcu"], x=list(b["x"]), y=list(b["y"]), z=list(b["z"]), mal=mal_ad(b),
    ...
    print("durum_tp10_v2.json yazildi · GERCEK %d / %d birim (%%%.0f) · toplam %.0f sn" % (g_, len(B), 100.0 * g_ / len(B), time.time() - t0))
