...
ÇIKTI: otonom/hat3d/hat_v44.glb + .usdz  ·  otonom/hat3d/durum.json (sayfa oradan okur)  ·  (v38: SolidWorks STEP'i YAZILMAZ — Kemal 25 Eyl)
...
if __name__ == "__main__":
    ...
    b1 = glb_yaz(os.path.join(OUT, "hat_v44.glb"), parcalar, dokular, anim=False)   # v40: animasyonsuz (istasyon modellerinde oynar)
    ...
    with io.open(os.path.join(OUT, "durum.json"), "w", encoding="utf-8") as f:
        json.dump(dict(hat=dict(w=HAT_W, h=H_MAK, d=DZ, pafta="HAT_ATOSA_TABLALI_v7 (+ E 830: pafta v8 bekliyor) · v44 · alt kisim v3 · B = store_cad_v5 (tam kaplama kapaklar) · kompresor K tabani arkasinda · TOPPING v2 UNO'lu (topping_uno_cad_v4) · E = kutu_cad_v3 · alt taban 123 · taban hizasi · surec 1168"), sayac=sayac, modul=[dict(kod=k, ad=a, x=[x, x + w], y=list(MODUL_Y[k])) for k, a, x, w in MODUL],
