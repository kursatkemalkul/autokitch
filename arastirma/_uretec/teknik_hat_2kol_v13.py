...
OUT = r"C:\Users\Kemal\Desktop\Kemal\WEBSITE\AUTOKITCH\arastirma\FULL_MAKINE\HAT_2KOL_v13_teknik.png".replace("WEBSITE", "WEBS\u0130TE")
W_PX, H_PX = 6600, 3400
S = 0.62
...
YAG = (245.0, 490.0)
KES = (495.0, 760.0)
FIR = [(765.0, 1080.0, "FIRIN HAZNESİ 1|LAHMACUN · 120 s"), (1085.0, 1400.0, "FIRIN HAZNESİ 2|LAHMACUN · 120 s"),
       (1405.0, 1728.0, "FIRIN HAZNESİ 3|PİDE · PİZZA · 240 s · ayrı sıcaklık")]
# E · KUTU kolonu
E_MEK = (123.0, 420.0)
E_AGZ = (430.0, 680.0)
E_SAR = (690.0, 2028.5)
...
# ======================= BASLIK =======================
txt(OX, 70, "AUTOKITCH  ·  HAT 2 KOL  ·  TEKNİK RESİM  v13  ·  TEPSİ SOKETLİ (ÇATAL YOK)  ·  TOPPING 1800  ·  İÇECEK + TATLI K3 ÜSTÜNDE  ·  DEPO K4'TE  ·  HAT 3900 × 2030", f38, INK)
...
etiket(fx(K4X + K4W / 2), fy(1030.0), "TEPSİ NİŞİ · 6 tepsi · raf 50 · y 690–995", f7, RED, RED)
for i in range(6):
    yy = 702.0 + i * 50.0
...
# --- D · FIRIN kolonu
kabin(X_D, W_D, 0.0, H_MAK, "D · PANO (üstte) · FIRIN · KESİCİ · YAĞ")
kapak(X_D, 33.0, 667.0, TEK[0], TEK[1])
kapak(X_D, 33.0, 667.0, 123.0, 242.0, "SERVİS BOŞLUĞU|boş · kablo / temizlik erişimi")
kesik(X_D, 50.0, 450.0, TEK[0] + 20.0, TEK[0] + 270.0, "ANA PANO · PLC · ana şalter", INK, "ekran yok · taşınır tablet (Wi-Fi)")
kesik(X_D, 470.0, 650.0, TEK[0] + 20.0, TEK[0] + 130.0, "ROBOT KONTROL", INK, "245 × 180 × 45")
kesik(X_D, 470.0, 650.0, TEK[0] + 150.0, TEK[0] + 270.0, "UPS", GRAY, "fırın panosu")
txt(fx(X_D + 350), fy(TEK[0] + 9.0), "pano bölmesi · tabanı ısı yalıtımlı + fan · önden kapak", f7, GRAY, "mm")
...
txt(fx(X_A + W_B / 2), fy(0) + 190, "MODÜL B · ÇEKMECE MODÜLÜ · 8 pide + 12 lahmacun + içecek/tatlı 2 katlı çekmece (K3 üstü) · K4: soğutma grubu + kaşar/sucuk deposu + 6 tepsi · K1 üstü: B kartı + sürücüler · A ve C üstüne oturur", f8, BG, "mm")
...
txt(fx(X_D + 350), py(-700.0), "EGZOZ KANALI · fan · karbon filtre · üstte pano bölmesi", f7, GRAY, "mm")
# E plan
...
txt(SX2, FY_TOP - 152, "pano bölmesi (üstte) · 3 kapaklı hazne · kesici · yağ", f9, GRAY)
govde(SX2, 0.0, H_MAK, plint=True)
d.rectangle([sz2(-728.5), fy(TEK[1]), sz2(0.0), fy(TEK[0])], fill=SOFT, outline=LINE, width=1)
drect(sz2(-700.0), fy(TEK[0] + 270.0), sz2(-450.0), fy(TEK[0] + 20.0), INK, 1)
drect(sz2(-300.0), fy(TEK[0] + 130.0), sz2(-100.0), fy(TEK[0] + 20.0), INK, 1)
txt(sz2(-575.0), fy(TEK[0] + 145.0) - 8, "ANA PANO", f7, INK, "mm"); txt(sz2(-575.0), fy(TEK[0] + 145.0) + 8, "250 derin", f7, GRAY, "mm")
txt(sz2(-200.0), fy(TEK[0] + 75.0) - 8, "ROBOT", f7, INK, "mm"); txt(sz2(-200.0), fy(TEK[0] + 75.0) + 8, "kontrol", f7, GRAY, "mm")
txt(sz2(-364.0), fy(TEK[0] + 285.0), "PANO BÖLMESİ · ısı yalıtımlı taban", f7, GRAY, "mm")
txt(sz2(-364.0), fy(183.0), "servis boşluğu", f7, GRAY, "mm")
d.rectangle([sz2(-728.5), fy(YAG[1]), sz2(-400.0), fy(YAG[0])], fill=BG, outline=LINE, width=1)
txt(sz2(-564.0), fy((YAG[0] + YAG[1]) / 2) - 8, "YAĞ KARTUŞU × 2", f7, INK, "mm"); txt(sz2(-564.0), fy((YAG[0] + YAG[1]) / 2) + 8, "160 × 160 × 180 · 45 °C", f7, GRAY, "mm")
d.rectangle([sz2(-400.0), fy(YAG[1] - 35), sz2(40.0), fy(YAG[0] + 35)], fill=AGZ, outline=RED, width=2)
txt(sz2(-180.0), fy((YAG[0] + YAG[1]) / 2), "SPREY AĞZI", f7, RED, "mm")
...
txt(sz2(-705.0), fy(1240.0), "EGZOZ", f7, GRAY, "mm")
txt(sz2(40) + 14, fy(1240.0), "MOTORLU KAPAK", f8, RED, "lm")
olcu_h(sz2(-620), sz2(-20), fy(H_MAK) - 60, "hazne 600", f8, INK)
olcu_h(sz2(-790), sz2(40), fy(0) + 44, "830", f11, INK)
for yy in (120.0, YAG[0], YAG[1], KES[1], FIR[0][1], FIR[1][1], FIR[2][1], H_MAK):
...
PARCA = [
    ...
    ("TEPSİ", "Ø340 · sap 150 · dişi soket · ayaklı", "6", "K4 nişi 690–995 · 6 dolaşımda · yedek 2 adet serviste"),
    ...
    ("FIRIN", "yıldız kesici Ø300 + piston · yağ kartuşu 4 L × 2 + sprey", "1", "kesim 495–760 · yağ 245–490"),
    ("KUTU", "katlayıcı (kalıp + plunger + vakum) + şarjör 1280", "1", "blank 400 × 760 · 710–850 kutu"),
    ("KONTROL", "ana pano PLC · robot kontrol kutusu 245 × 180 × 45 · UPS · taşınır tablet (makinede ekran yok)", "1", "D üstü pano bölmesi (1733–2030) · ısı yalıtımlı taban + fan"),
]
kol = (0, 130, 700, 800)
th = 30
